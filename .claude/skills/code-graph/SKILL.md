...
quench has a pre-built symbol and call graph in `.codegraph/` (CodeGraph v1.5.0, ~800 ms to
...
`ccg` lives at `~/github.com/dripsmvcp/cplusplus-cuda-graph/ccg`:
...
`gemv_q6k_q8_1` is generated by token pasting (`QUENCH_DP4A_QUANT_TYPES(QUENCH_DEFINE_GEMV_DP4A)`,
...
**2. "No callers found" ≠ dead.** Measured on this repo: 99% of the 423 `__global__` kernels have
...
*The residue after that check is worth reading, though: a kernel whose only occurrences are its
declaration in a header and its definition is genuinely unused. Name no candidate as dead until
the grep confirms it.*
...
quote it** — dependent counts shift whenever keys move between headers (the dispatch-relevant
keys live in `src/core/dispatch_policy.h`, not `src/runtime/config.h`). A number in a skill ages
exactly like a number in a doc:
...
Implicit destructor calls are recovered by `ccg enrich` (85% of quench's 71 destructors have a
