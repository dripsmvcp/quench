...
description: Use when building quench, running its test suite, checking CI status, or debugging build/test failures — "make build", "run the tests", "test-gpu", "verify-fast", GTEST_FILTER, Docker/CUDA toolchain, dependency bumps, "CI is red/blocked", determinism or perplexity checks. Do NOT use for benchmarking/profiling (benchmark-cuda) or output-quality batteries (check-degeneration).
---
...
1. **The host has NO CUDA toolkit by design.** All build/run/test happens inside Docker (`make build` → image `quench:test`, CUDA 13.3 on Ubuntu 26.04 / GCC 15.2, C++23 for host AND device — `CMAKE_CUDA_STANDARD 23` needs the shim at `CMakeLists.txt:10`). Never apt-install toolchains on the host.
...
5. **Dependency pins are single-sourced in `cmake/quench-deps.cmake`** (current: CUTLASS v4.5.2, GTest v1.17.0, nlohmann/json v3.12.0, httplib v0.48.0). `make build` extracts them and injects Docker `--build-arg`s via `scripts/dep_build_args.sh` — bump ONLY that one file; never re-pin in the Dockerfile or CMakeLists.
...
7. **`main`'s CI status is stale by default — read the PR run, not the branch.** A squash merge performed by AUTO-MERGE does not start a workflow run: it is attributed to `GITHUB_TOKEN` (auto-merge.yml arms it with that token) and GitHub does not trigger further runs from that token. A merge a human clicks does fire. Coverage is not lost (the PR run builds exactly the tree that gets squashed), but the branch's reported state can be arbitrarily old — stale warnings on that page read as live bugs. Refresh it with `gh workflow run CI --ref main` (`workflow_dispatch`), and when judging whether something is fixed on `main`, check the PR run of the fixing commit or the file itself.
...
| Vision goldens | `make test-vision` | `QUENCH_VISION_GOLDEN_DUMP=1` to regenerate |
| Pre-push gate | `make verify-fast` | build + filtered tests + perf gate + peak-VRAM gate + graphs-ON/OFF gate + smoke (the VRAM gate adds its own short model load; skip with `QUENCH_VERIFY_SKIP_VRAM=1`) |
...
docker run --rm --gpus all -v $HOME/models:/models \
  -e QUENCH_TEST_MODEL=/models/Qwen3-8B-Q8_0.gguf \
  quench:test quench-tests --gtest_filter="DegenerationTest.*"
```

Test binaries in the image: `quench-tests` (full GPU), `quench-tests-unit` (CPU), plus split binaries `test-core test-text test-compute test-attention test-quant test-kv test-moe-gdn test-e2e test-gdn`. Gotcha: `test_ssm.cpp` tests live in **`test-moe-gdn`**, not a binary of their own.
...
**`verify-fast` does not build — it measures whatever `quench:test` already holds.** On a
host without cmake it re-execs into that image with `QUENCH_VERIFY_SKIP_BUILD=1` and prints
...
**`make test-unit` is NOT the CI lane.** It runs `quench-tests-unit` (~37 tests); CI runs `ctest -L unit` → **`test-core`** (550+) + test-text + an e2e subset. A new CPU test belongs in `test-core`, and the honest no-GPU check is `docker run --rm quench:test test-core` (no `--gpus`). Green in `quench-tests-unit` says nothing about CI.

Tool binaries in the image: `quench-server`, `quench-cli`, `quench-bench`, and `quench-quantize` (offline BF16/FP16 → NVFP4 conversion, experimental — see `quant-formats`). A new tool needs BOTH a `cp` in the builder stage and a `COPY --from=builder` line in the Dockerfile, or it silently isn't in the image.

## Determinism & quality caveats

- `--set runtime.deterministic=true` gives full temp=0 reproducibility (covers MoE routing atomics + top-k sampling races + implies `deterministic_gemm`). Default OFF — costs throughput. The engine promotes it process-wide.
...
- Perplexity: `quench-cli --perplexity <textfile>` (teacher-forced, chunk-aware).
...
- `std::sort/find/max_element` etc. "not declared": libstdc++ 15 no longer includes `<algorithm>` transitively — add the missing include, don't downgrade the toolchain.
- FetchContent mismatch vs Docker deps-clone → see hard rule 5 (bump `cmake/quench-deps.cmake` only).
- Out-of-space: Docker images are large; prune old `quench:*` images first.
