...
theory** — the pinned roofline run (`make roofline-pin`, ref in
`tools/roofline/history/BASELINE`). Two readings that change decisions: the dense NVFP4
...
| Pointer advance without `sizeof(T)` | See known-issues (FP8 FMHA S_tile pointer advance). |
