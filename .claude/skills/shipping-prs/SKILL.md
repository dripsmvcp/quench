---
name: shipping-prs
description: Use when opening, merging, or releasing a PR for quench — branching off main, `gh pr create`, enabling auto-merge, cutting a tagged release (version bump + CHANGELOG + tag). Symptoms — "PR stuck BLOCKED", "my last commit didn't land on main", "which check is required", "how do I cut a release", "auto-merge merged too early". Do NOT use for build/test mechanics (building-and-testing) or perf measurement / baseline refresh (benchmark-cuda).
---

# Shipping PRs & Releases — quench

## Hard rules (violations cost recovery PRs or lost work)

1. **Always branch off `main`; `gh pr create --base main`. NEVER stack PRs.** Squash-merge + stacking causes recovery-PR cascades. Branch from fresh `main` every time. Prefer fewer, **batched** PRs over one-per-fix.
...
## The auto-merge race (it can silently drop your last commit — read it)
...
- **The failure mode can be nastier than a lost commit: it can publish wrong data.** When a follow-up commit corrects claims a PR already carries and the correction loses the race, `main` documents figures that have been disproved. Nothing is red — CI passes, the PR merges, the branch looks done — and it takes a second PR to fix. **The verify step above is not optional bookkeeping** — when the late commit changes *claims* (numbers, docs, a caveat), losing it means shipping something you know to be false. Grep `main` for the corrected value, don't just check that the merge happened.
...
- **`Build` shows green but is not registering as satisfied** → the required-check name ≠ `Build` (hard rule 4). This is the quench-specific gotcha and the usual culprit, but confirm it's actually the only required check.
...
Single source of truth for the version is **`CMakeLists.txt`** `project(quench … VERSION X.Y.Z)`. A release is its own PR:
...
3. Merge that PR (squash) as usual, then tag the merged commit on `main`: `git tag vX.Y.Z <sha> && git push origin vX.Y.Z`. Tags are `vX.Y.Z`. `scripts/check-release.sh` gates release-touching PRs in CI.
...
- Releasing but only bumped one of {CMakeLists VERSION, CHANGELOG} → bump both.
