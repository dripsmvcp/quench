...
description: Use when verifying that a model in the quench inference engine produces coherent output without repetition loops, token-stuck states, or state corruption across turns. Triggers on "degenerates", "check degeneration", "repetition loop", "own own own", "stuck token", "multi-turn regression", "does it still work", and after enabling CUDA graphs / changing forward pass / MoE routing / KV cache / GDN state.
---

# Degeneration Check — quench Inference Engine

Models in quench regress in specific, recurring ways. Run the battery whenever you touch the forward pass, graph capture, MoE routing, KV cache, or GDN state.

## Known failure modes (what we are guarding against)
...
| Token-0 garbage (`!`, `!!!` from the first token) | `!!!!…` immediately, sometimes recovers | Silent VRAM-alloc failure in a decode fallback (MXFP4→FP16 on GDN hybrids); check init logs for failed reserves |
| Answer in `reasoning_content`, `content` empty (server only) | API "empty response", CLI fine | Thinking-state ≠ rendered prompt tail (closed `<think></think>` template block) — see `server-api` reconcile bullet |
...
The deepest battery — probes a **running quench-server** through the OpenAI API,
...
# quench-server --host 0.0.0.0 --model /models/<MODEL>
...
QUENCH_TEST_MODEL=/models/<MODEL>.gguf \
  ./build/quench-tests --gtest_filter="DegenerationTest.*"
```

Source: `tests/test_degeneration.cpp` — uses the same quench C API the production engine uses, default model `Qwen3-8B-Q8_0.gguf`.
...
```bash
docker run --rm --gpus all -v $PWD/models:/models quench:test \
...
```bash
# graphs off
./build/quench-cli --set runtime.cuda_graphs=never \
...
**Byte-level A/Bs: do NOT diff CLI stdout** — `quench-cli` interleaves log lines with generated text and stripping is error-prone. Run quench-server and diff the JSON `content` field instead. Qwen3.6-**27B** is byte-deterministic at temp=0 (graphs on/off produce identical output) — a good graph-parity model; 35B is NOT (see below).
...
**Perplexity corpus:** when you fall back to PPL, use
...
**Quant-file caveat:** greedy behavior on logit-tie prompts is quant-file-specific — the local Qwen3-4B (unsloth) GGUF degenerates on synthetic list prompts even unchunked (that's model-intrinsic, NOT an engine bug). For byte-level A/B prefer NLL/perplexity comparison over exact-output equality (the ChunkedPrefill tests compare NLL for this reason). Qwen3.6-35B is non-deterministic even at temp=0 — greedy-token A/B is INVALID there; use perplexity or `degen_suite.py --skip-deterministic`.
