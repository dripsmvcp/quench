# quench — Project Instructions

From-scratch C++23/CUDA LLM inference engine for **dense llama-family GGUF models** targeting **exactly one chip: NVIDIA Blackwell `sm_120a`** (RTX 5090 / GB202, 32 GB GDDR7, 1792 GB/s, native FP4 tensor cores). No portability layer. ~100k LOC (src/ + include/). See [`docs/architecture.md`](docs/architecture.md) (canonical narrative) and [`docs/sm120.md`](docs/sm120.md).
...
| quench-server / OpenAI+Anthropic HTTP API | skill **server-api** |
...
Canonical references: `docs/architecture.md` (narrative), `docs/sm120.md` (hardware), `docs/MEMORY_ARCHITECTURE.md` (memory subsystem: tiers, allocators, invariants I1-I7), `AGENTS.md` (subagent roles + guardrails).

## Two facts about WDDM/WSL2 hosts that mislead rather than fail

- **A successful `cudaMalloc` proves nothing about free VRAM.** WDDM oversubscribes
  into host memory and returns `cudaSuccess` — a 28 GiB allocation can succeed with
  22.6 GiB reported free. Measure *bandwidth* to tell resident from spilled: ~1530 vs
  ~237 GB/s on a 5090. That 6.5x cliff silently turns a resident model into a spilled
  one, so "0 MiB free" is a correctness problem, not a tight fit.
...
- Runtime config is `RuntimeConfig` in `src/runtime/config.h` (`quench.conf` +
  `--config` + `--set`). The env vars seeded are `QUENCH_DETERMINISTIC`, `QUENCH_FMHA_FA2`,
  and the three trace knobs backed by config keys (`QUENCH_SPEC_TRACE`, `QUENCH_JUMP_TRACE`,
  `QUENCH_PPL_DUMP` → `diagnostics.spec_trace` / `.jump_trace` / `.ppl_dump`);
  don't reintroduce ad-hoc env reads.
- Internal errors throw and are translated to `QuenchError` at the
  `src/api/quench_api.cpp` boundary — intentional, don't convert them to status returns.
...
- **No FP4 cuBLASLt kernels on sm_120** → CUTLASS is the primary GEMM path; FP8
  ...
  `cmake/quench-deps.cmake` — bump only that file.
...
Verify the model still produces coherent output (no repetition loops / token-stuck / state corruption) after touching the forward pass, KV cache, speculation, or CUDA-graph capture. Skill **check-degeneration**.
