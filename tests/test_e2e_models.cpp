...
static const char* primary_model() { return std::getenv(quench_test::kEnvModel); }

static const char* gdn_model() { return std::getenv(quench_test::kEnvModelGdn); }

static const char* gemma4_model() { return std::getenv(quench_test::kEnvModelGemma4); }
...
protected:
    void SetUp() override {
        path_ = primary_model();
        if (!path_)
            GTEST_SKIP() << "Set QUENCH_TEST_MODEL to run";

        ASSERT_EQ(quench_model_load(path_, QUENCH_FORMAT_GGUF, &model_), QUENCH_SUCCESS);
        ...
        ASSERT_EQ(quench_context_create(model_, &cfg, &ctx_), QUENCH_SUCCESS);
    }
...
TEST_F(PrimaryModelTest, GenerateCoherentOutput) {
    ...
    char output[4096];
    size_t len = 0;
    ASSERT_EQ(quench_generate(ctx_, "The capital of France is", &params, output, sizeof(output), &len),
              QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
...
TEST_F(PrimaryModelTest, MultiTurnConversation) {
    ...
    char out1[4096];
    size_t len1 = 0;
    ASSERT_EQ(quench_generate(ctx_, "Say hello.", &params, out1, sizeof(out1), &len1), QUENCH_SUCCESS);
    EXPECT_GT(len1, 0u);
    ...
    char out2[4096];
    size_t len2 = 0;
    ASSERT_EQ(quench_generate(ctx_, "What is 2+2? Answer with just the number.", &params, out2, sizeof(out2),
                           &len2),
              QUENCH_SUCCESS);
    EXPECT_GT(len2, 0u);
...
TEST_F(PrimaryModelTest, TokenizeRoundtrip) {
    ...
    int32_t tokens[256];
    int n_tokens = 0;
    ASSERT_EQ(quench_tokenize(model_, text, tokens, &n_tokens, 256), QUENCH_SUCCESS);
    EXPECT_GT(n_tokens, 3);

    char buf[1024];
    ASSERT_EQ(quench_detokenize(model_, tokens, n_tokens, buf, sizeof(buf)), QUENCH_SUCCESS);
...
TEST_F(PrimaryModelTest, PrefillThenDecodeMultipleTokens) {
    int32_t tokens[128];
    int n_tokens = 0;
    ASSERT_EQ(quench_tokenize(model_, "The meaning of life is", tokens, &n_tokens, 128), QUENCH_SUCCESS);
    ASSERT_GT(n_tokens, 0);

    ASSERT_EQ(quench_prefill(ctx_, tokens, n_tokens), QUENCH_SUCCESS);
    ...
    for (int i = 0; i < 16; i++) {
        int32_t tok = 0;
        QuenchError err = quench_decode_step(ctx_, &params, &tok);
...
protected:
    void SetUp() override {
        path_ = gdn_model();
        if (!path_)
            GTEST_SKIP() << "Set QUENCH_TEST_MODEL_GDN to run";
...
TEST_F(GDNModelTest, GenerateCoherentOutput) {
    ...
    ASSERT_EQ(quench_generate(ctx_, "Write a short paragraph about the planet Jupiter.", &params, output,
                           sizeof(output), &len),
              QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
    ...
    // but very few unique words. This catches a GDN recurrent-state collapse
    // that the length-only check above misses.
...
TEST_F(GDNModelTest, MultiTurnGDNState) {
    ...
    char out2[4096];
    size_t len2 = 0;
    ASSERT_EQ(quench_generate(ctx_, "What is 1+1? Answer with just the number.", &params, out2, sizeof(out2),
                           &len2),
              QUENCH_SUCCESS);
    EXPECT_GT(len2, 0u);
...
//
// Primary regression test — the Gemma-4 forward pass is easy to break
// subtly. This test locks in
...
protected:
    void SetUp() override {
        path_ = gemma4_model();
        if (!path_)
            GTEST_SKIP() << "Set QUENCH_TEST_MODEL_GEMMA4 to run";
...
TEST_F(Gemma4ModelTest, AnswersCapitalOfFrance) {
    ...
    char output[4096];
    size_t len = 0;
    ASSERT_EQ(quench_generate(ctx_, "What is the capital of France?", &params, output, sizeof(output), &len),
              QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
...
TEST_F(Gemma4ModelTest, PrefillFusesSwaLayers_Hd512StaysCublas) {
    ...
    // slower, so fusing them would regress. So a short prefill must
    ...
    quench::attention_cublas_prefill_reset_count();

    int32_t tokens[128];
    int n_tokens = 0;
    ASSERT_EQ(quench_tokenize(model_, "What is the capital of France?", tokens, &n_tokens, 128), QUENCH_SUCCESS);
    ASSERT_GT(n_tokens, 0);
    ASSERT_EQ(quench_prefill(ctx_, tokens, n_tokens), QUENCH_SUCCESS);

    uint64_t cublas_calls = quench::attention_cublas_prefill_call_count();
...
TEST_F(Gemma4ModelTest, NoRepetitionDegeneration) {
    ...
    char output[8192];
    size_t len = 0;
    ASSERT_EQ(quench_generate(ctx_, "Name three European capitals.", &params, output, sizeof(output), &len),
              QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
...
// ---------------------------------------------------------------------------
// regression: decode-graph launch-topology re-derivation on ctx growth.
//
...
TEST(DecodeGraphCtxGrowthTest, LongPromptAfterShortRequestStaysAlive) {
    ...
    ASSERT_EQ(quench_context_create(model, &cfg, &ctx), QUENCH_SUCCESS);
    ...
    params.max_tokens = 16;
    ASSERT_EQ(quench_generate(ctx, "The capital of France is", &params, output, sizeof(output), &len),
              QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
    ...
    ASSERT_EQ(quench_generate(ctx, prompt.c_str(), &params, output, sizeof(output), &len), QUENCH_SUCCESS);
    EXPECT_GT(len, 0u);
    ...
    params.max_tokens = 16;
    ASSERT_EQ(quench_generate(ctx, "The capital of Italy is", &params, output, sizeof(output), &len),
              QUENCH_SUCCESS);
