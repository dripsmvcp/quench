// The persisted library-reserve measurement.
//
...
std::string tmp_path(const char* name) {
    auto p = std::filesystem::temp_directory_path() / "quench_lrc_test" / name;
...
TEST(LibraryReserveCache, KeyIsStableAndSeparatesWhatTheChargeVariesWith) {
    ...
    // Each of these selects a different execution path or library stack —
    // measured to be the thing the charge actually varies with.
...
TEST(LibraryReserveCache, AnUnwritablePathFailsWithoutThrowing) {
    ...
    EXPECT_FALSE(library_reserve_cache_store("/proc/quench_lrc_should_fail/x", key_a(), 1));
}
