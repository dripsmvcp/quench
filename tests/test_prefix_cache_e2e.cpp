// Prefix-cache E2E equivalence.6).
//
...
//
// Engine seam (verified in src/api/quench_api.cpp + src/runtime/engine*.cpp):
...
//   blocks). The manual token-level path (quench_prefill_with_params +
//   quench_decode_step) never registers hashes, AND quench_context_reset()
//   deliberately evicts ALL cached blocks. So the cache engages ONLY across
//   two consecutive quench_generate() calls on the SAME context with NO reset
//   between them: call 1 finishes and caches its prefix; call 2 hits it.
//   We therefore drive quench_generate twice and rely on greedy determinism.
//
// Skipped without QUENCH_TEST_MODEL (no GPU model in CI).
...
const char* model_path() { return std::getenv(quench_test::kEnvModel); }
...
protected:
    void SetUp() override {
        ...
        if (!path)
            GTEST_SKIP() << "Set QUENCH_TEST_MODEL to run prefix-cache E2E equivalence";
        path_ = path;
        fmt_ = is_safetensors_dir(path_) ? QUENCH_FORMAT_SAFETENSORS : QUENCH_FORMAT_GGUF;
        ASSERT_EQ(quench_model_load(path, fmt_, &model_), QUENCH_SUCCESS);
    }
    ...
    // Greedy generation through quench_generate (the path that reaches
    ...
    std::string gen(const char* prompt, int max_tokens) {
        ...
        size_t out_len = 0;
        EXPECT_EQ(quench_generate(ctx_, prompt, &p, out.data(), out.size(), &out_len), QUENCH_SUCCESS);
        out.resize(out_len);
        return out;
    }

    std::string path_;
    QuenchModelFormat fmt_ = QUENCH_FORMAT_GGUF;
...
// 0. CONTROL: with caching OFF, two consecutive greedy quench_generate calls on
...
// =============================================================================
// Why the equality gate matters: a prefix-cache HIT can diverge
// from the fresh prefill under greedy decoding with neither
// the cache nor FP rounding at fault: were GPUBatchPool::upload_into_pool to skip
// the block-table re-upload based on a (count + first-block-ID) proxy, a
// prefix hit defeats that proxy (same first reused block, same count, different
// middle after reuse+eviction, e.g. [0,1,4,3,2]) — the first eager decode step
// then runs on the PREVIOUS request's table, writing the new token's KV into the
// wrong physical block while the async graph reads garbage at the self-token
// position. batch.cpp therefore content-compares the table. These tests
// are the feature's ship gate ("the test IS the
