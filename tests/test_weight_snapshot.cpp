...
using quench::make_weight_key;
using quench::parse_meminfo_available;
using quench::Tensor;
using quench::QType;
using quench::WeightUploadLog;
...
TEST(WeightKeyTest, CanonicalFormats) {
    ...
    EXPECT_STREQ(make_weight_key(buf, sizeof(buf), "q_norm", quench::kMtpKeyLayer), "mtp.q_norm");
}
...
TEST(WeightSnapshotArmSlotTest, ArmTakeDisarm) {
    ...
    auto* fake = reinterpret_cast<quench::WeightSnapshot*>(0x1234);

    EXPECT_EQ(quench::weight_snapshot_take_armed(), nullptr);
    quench::weight_snapshot_arm(fake);
    EXPECT_EQ(quench::weight_snapshot_take_armed(), fake);
    EXPECT_EQ(quench::weight_snapshot_take_armed(), nullptr);  // consumed

    quench::weight_snapshot_arm(fake);
    quench::weight_snapshot_disarm(fake);
    EXPECT_EQ(quench::weight_snapshot_take_armed(), nullptr);

    // Disarm of a non-armed pointer leaves an armed one alone.
    quench::weight_snapshot_arm(fake);
    quench::weight_snapshot_disarm(reinterpret_cast<quench::WeightSnapshot*>(0x5678));
    EXPECT_EQ(quench::weight_snapshot_take_armed(), fake);
}
