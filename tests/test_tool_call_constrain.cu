...
// Constrained tool calling: the TOOL_CALL schema FSM, envelope literals
...
// ---------------------------------------------------------------------------
// TOOL_CALL enforcement: envelope literals forced, "name" before
...
// nested model) is now enforceable ( stage 2): the nested defs are hoisted
...
// parallel_tool_calls: in strict optional mode with allow_parallel, the
...
// Llama3 forced tool call: `<function=NAME>{args}</function>` — the body
...
// ---------------------------------------------------------------------------
// Qwen-Coder XML tool calls ( follow-up): the body is
//   <function=NAME>\n<parameter=KEY>\nVALUE\n</parameter>\n...</function>
