...
//
// WHY: a constraint quench cannot compile must never be dropped with the
// request answered anyway — HTTP 200, free-form text, nothing in the reply
// telling the caller its guarantee was not applied. These assert the 400
// contract on the CPU, in CI, where the real handler never runs.
//
...
TEST(ConstraintValidation, EnforceablePatternsAreAccepted) {
    ...
    // Edge anchors are redundant, not unsupported — they must NOT 400.
    EXPECT_TRUE(accepts(regex_body("^[0-9]{3}$")));
    EXPECT_TRUE(accepts(regex_body("^(yes|no)$")));
    // Non-capturing groups compile.
...
TEST(ConstraintValidation, ResponsesDialectSchemaReachesValidation) {
    ...
    const json converted = quench_server::responses::responses_to_openai_body(rsp);
...
TEST(ConstraintValidation, ResponsesDialectGoodSchemaStillPasses) {
    ...
    EXPECT_TRUE(accepts(quench_server::responses::responses_to_openai_body(rsp)));
}

// ---------------------------------------------------------------------------
// Content parts. A part this server cannot read must not fall through the
// parsing chain in silence: `video_url` (quench has no video path) falling
// through produces a 200 answering a prompt the model never saw. The caller
// cannot tell that reply apart from one that actually used its input.
// ---------------------------------------------------------------------------
