...
(`make build` → image `quench:test`, CUDA 13.3). See the `building-and-testing`
...
```bash
docker run --rm --gpus all -v $HOME/models:/models quench:test \
...
- **Stage 1 — pre-commit (GPU), local.** `scripts/pre-commit.hook` runs
  ...
  and skips with `QUENCH_VERIFY_SKIP_VRAM=1`.)
...
- **Stage 3 — server (GPU), local, opt-in.** `make test-server`
  (`scripts/test_server.sh`) boots a real `quench-server` against a live model and
  ...
  `exercise_all_endpoints.py` (no 5xx), `test_server_robustness.py` ( bad
  ...
  `test_server_0token_battery.py` ( no empty-completion wedge). `make
  coverage` runs the same set ungated to measure `tools/quench-server/` line coverage.
...
Correctness tests compare against an **independent** reference, never quench-vs-quench
...
3. **Committed golden tensors** generated by a `tests/refs/` script from a pinned
   ...
   `tests/generate_tokenizer_golden.py`, `QUENCH_VISION_GOLDEN_DUMP=1` for vision).
...
- Add the source to the right binary in `CMakeLists.txt` (under the matching
  `quench_add_test_module(...)`; server-transform tests go under the
  `QUENCH_BUILD_SERVER` block of `test-core`).
...
- If a test exposes a real engine bug, **file it** (a GitHub issue) with a
...
QUENCH_VISION_GOLDEN_DUMP=1 make test-vision
```
