// process_diag ↔ RuntimeConfig coupling.
//
// process_diag is a process-wide snapshot of the flags that leaf kernels read
// when they have no RuntimeConfig to hand. Until process_diag_install()
...
RuntimeConfig make_non_default() {
    ...
    c.diagnostics.dump_hidden_dir = "/var/tmp/quench-hidden";
    c.diagnostics.graph_dump_dir = "/var/tmp/quench-graphs";
...
TEST(ProcessDiag, InstallTransfersEveryMirroredField) {
    ...
    EXPECT_EQ(std::string(process_diag_dump_hidden_dir()), "/var/tmp/quench-hidden");
    ASSERT_NE(process_diag_graph_dump_dir(), nullptr);
    EXPECT_EQ(std::string(process_diag_graph_dump_dir()), "/var/tmp/quench-graphs");
...
// --------------------------------------------------------------------------
//
// `log_set_level` is the only writer of g_log_level; if nothing called it,
// the level would stay pinned at INFO and every QUENCH_LOG_DEBUG site in the
// engine would be unreachable — a debug facility that cannot be switched on.
// So a config key is applied here, in the one function that runs from both
// tool mains AND Engine::init. These tests pin both halves: the parse and
// the transfer.
