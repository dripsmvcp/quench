...
TEST_F(GemmCaptureSm120, Correctness_NarrowN_2048x32x2560) {
    // The GDN-projection shape: rejecting it (N=32 < BN=128) makes it fall
    // through to cuBLASLt → status-14 under capture → the whole decode graph
    // aborts. The partial-N tile must be masked correctly.
