// CPU unit tests for the server streaming text pipeline.
// Targets the pure holdback + UTF-8-boundary logic in
// tools/quench-server/stream_pipeline.h, which is the single source of truth used
// by all three streaming handlers in handlers.cpp.
//
// The bug this guards: with max_stop_len == 0 (no stop sequences) naive holdback
// math `pending.size() - max_stop_len + 1` evaluates to size + 1; fed to
// flush_text it reads ONE byte past pending_text — emitting the std::string
// '\0' terminator into every SSE content delta and silently disabling
// cross-token stop matching. Mock-server contract tests cannot see any of this.
//
...
using quench::stream::holdback_decision;
using quench::stream::utf8_complete_len;
...
// ---------------------------------------------------------------------------
// The NUL overrun guard: max_stop_len == 0 must flush exactly the buffer, no +1.
// ---------------------------------------------------------------------------

TEST(Holdback, NoStopSequencesFlushesWholeBufferNoOverrun) {
    // No stop sequences -> max_stop_len 0. The buffer "4 "
    // must flush all 2 bytes and NOTHING more. Buggy holdback
    // math returns 3 (= 2 - 0 + 1), reading the '\0' terminator.
