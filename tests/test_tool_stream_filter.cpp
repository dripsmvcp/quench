// =============================================================================
// Unit tests for tools/quench-server/tool_stream_filter.h + the streaming tag
// scanner/body parser in tool_call.cpp (scan_tool_tag, parse_stream_tool_body).
//
// WHY THIS EXISTS: streaming tool-call state machines duplicated inline
// per handler tend to recognize only
// ChatML <tool_call> and Llama3 <function=. Real agents stream (stream:true),
// and then on Gemma-4 raw <|tool_call> markers leak as visible text while
// Qwen3.6 XML bodies are silently swallowed by a catch(...). The machinery
// is the shared StreamToolCallFilter; these tests pin its contract across
...
// (content, calls) spelled out — no quench-vs-quench.
// =============================================================================
...
using quench::ChatTemplateFamily;
using quench::server::StreamToolCallFilter;
...
TEST(ToolStreamFilterLlama3, Chunked) {
    std::string input = "Looking it up. <function=lookup>{\"q\": \"quench\"}</function>";
