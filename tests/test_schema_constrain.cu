...
// forced_text probe. The any-JSON constrainer + preamble gate live
...
//: a backslash inside an object KEY was accepted and silently dropped
...
TEST(SchemaConstrainTest, BackslashInKeyRejected) {
    ...
    EXPECT_FALSE(key[6]) << "'\\' must be masked inside a key";
...
//: an unbounded integer/number must not run digits forever — once the digit
...
// ---------------------------------------------------------------------------
// minItems / maxItems — the degeneration guard: a budget-force-closed
...
// ---------------------------------------------------------------------------
// $ref / $defs (issue) — pydantic/zod emit $defs+$ref for EVERY nested
...
// ===========================================================================
// Vocab-mismatch guard (a SIGBUS class): SafeTensors models have
...
// constrainer classifies only the TOKENIZER vocab. Mishandle the gap and the
// host validation loop reads token_texts_[i] out of bounds (SIGBUS in
// quench-server on the first json_mode request), and the mask kernels read the
// category/allow device buffers out of bounds. Contract under test: apply_mask with a
...
// ===========================================================================
// Raw-control-char guard: JSON forbids unescaped U+0000–
// U+001F inside strings; an FSM that accepts "any content char" there lets
// multi-char tokens whose FIRST char passes the category mask (`"<newline>`
// opens a string and smuggles a raw newline in one step) produce output that
// json.loads() rejects ("Invalid control character"). Reachable in real
...
// ---------------------------------------------------------------------------
// Jump-ahead: forced_text — the characters every schema-legal
