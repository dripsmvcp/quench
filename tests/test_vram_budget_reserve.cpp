// KV-pool sizing vs weight-cache demand (the Ornith-35B Q4_K_M case).
//
...
// aware) routes them to the FP16 cache. A KV backstop that
// sizes the pool to available-minus-heuristic eats all post-weight
// VRAM: phases 1/3 build 0 cache tensors, and decode falls back to
...
TEST(VramBudgetReserve, PlannerDemandKeepsKvPoolFromEatingWeightCacheRoom) {
    ...
    // Isolate this test from the library charge (which floors reserve_bytes at
    // what cuBLAS/CUTLASS claim on the first forward). That charge has its own
    // test below; folding it in here would make this one assert the sum of two
    // unrelated policies.
    ...
    EXPECT_LE(kv_bytes, free_vram - 3 * GiB)
        << "KV pool ate the weight-cache room (heuristic-only backstop)";
}

// When KV is cheap (hybrid — few attention layers), the
// auto floor must cover the full advertised context plus the StreamingLLM
// headroom, not stop at the flat 16384-token cap. With a flat-capped floor a
// max_seq_len=17408 hybrid gets a pool a 16k prompt fills to 94%, tripping
...
TEST(VramBudgetReserve, BeneficialSourcesKeepFullKvPool) {
    ...
    //
    // The threshold is RELATIVE by design. An absolute `kv_bytes >= 4 GiB`
    // would silently encode a specific safety-reserve size — and a reserve
    // that plans KV down to a level the VRAMAllocator refuses to allocate
    // against (it requires free >= bytes + 5% of total for anything >=16 MiB)
    // yields a plan that cannot be executed: the caches it starves fail
    // mid-build. An absolute 4 GiB is also unreachable in this synthetic
    // 8 GiB scenario. The property under test is that the planner-driven
    // weight-cache reservation must not fire, so express it relative to what
    // is actually distributable — "KV keeps most of it".
...
// --- quench.conf [vram] knobs (kv_fraction / reserve_floor_pct) ---
//
...
TEST(VramBudgetReserve, ReserveFloorPctScalesReserve) {
    ...
    // library charge wins on any real card — which would make the pct floor
    // unobservable here.
...
// phase 0b registers — a naive inline elems/16 heuristic omits them and
// is not an upper bound under SfAtom padding.
...
size_t sf_entry(int n, int k_packed) {
    ...
    size_t sz = quench::cutlass_nvfp4_sf_size(n, k_packed * 2);
    return (sz + kSfAlign - 1) / kSfAlign * kSfAlign;
}

size_t sf_group(int ne, int n, int k_packed) {
    constexpr size_t kSfAlign = 256;
    size_t sz = static_cast<size_t>(ne) * quench::cutlass_nvfp4_sf_size(n, k_packed * 2);
    return (sz + kSfAlign - 1) / kSfAlign * kSfAlign;
}
...
TEST(VramBudgetReserve, PreallocCoversDemandAndFreesKv) {
    ...
    // The floors are stated by the PLAN, not conditional on bytes some
    // caller physically pre-held. They are
...
// The floor that added, and the test it should have come with. Its
// absence is why three tests in this file encoded the pre- arithmetic and
...
TEST(VramBudgetReserve, ReserveIsFlooredAtTheLibraryCharge) {
    ...
    // bytes the first forward is about to take (the failure: budget
...
// the pool takes the PLAN's number, so the live pass being more generous
...
TEST(VramBudgetReserve, PlanNeverExceedsTheBudgetItWasGiven) {
    ...
    // asserting. Mutation-verified: without the impossible case, removing the
    // fit check inside plan_memory() leaves this test green.
    EXPECT_GE(refused, 1) << "no case exercised the refusal path";
}

// A divergence that is easy to mistake for a bug, pinned so nobody "fixes"
...
// answers to different questions. The pool takes the plan's.
TEST(VramBudgetReserve, LivePassMayExceedThePlanAtSmallContexts) {
    ...
    EXPECT_GT(live.kv_max_blocks, plan.plan.kv.blocks)
        << "if this ever stops holding, the two passes have converged at small "
           "contexts and the divergence note above needs revisiting";
}
...
//: the budget and the VRAMAllocator must agree on the headroom. Mode 2
...
// kv_block_bytes_per_layer: the single source for KV-size estimates.
// A naive estimate that multiplies by raw dtype_size() gets this wrong:
// dtype_size() returns 0 for NVFP4/MXFP4_KV (zeroing the KV headroom) and
// 1 byte/elem for INT4 (2x the packed size), and counts no scale overhead.
TEST(VramBudgetReserve, KvBlockBytesPerLayerIsPackingAndScaleAware) {
    ...
    // returns 0 here, which is exactly the failure mode.
