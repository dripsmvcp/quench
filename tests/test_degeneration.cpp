...
//
// These tests require a real model and GPU. Skip via QUENCH_TEST_MODEL env var
// or run with: quench-tests --gtest_filter="DegenerationTest.*"
...
// ship as dirs). Without this the battery hard-coded QUENCH_FORMAT_GGUF and could
// not load any NVFP4 model — leaving the priority quant with zero degeneration
// coverage (see the NVFP4 prefill crash, which shipped undetected).
static QuenchModelFormat detect_format(const char* p) {
    std::string s = p ? p : "";
    return (s.size() >= 5 && s.substr(s.size() - 5) == ".gguf") ? QUENCH_FORMAT_GGUF
                                                                : QUENCH_FORMAT_SAFETENSORS;
}
...
// Helper: generate text via quench API and return as string
static std::string generate(QuenchModel model, QuenchContext ctx, const std::string& prompt, int max_tokens,
...
protected:
    ...
    void SetUp() override {
        ...
        ASSERT_EQ(quench_model_load(get_model_path(), detect_format(get_model_path()), &model_), QUENCH_SUCCESS);
...
//
// Determinism requires CUBLAS_WORKSPACE_CONFIG=:4096:8 in the test process
...
TEST_F(DegenerationTest, GreedyDeterminism) {
    auto gen_greedy = [&](const std::string& prompt) {
        ...
        char buf[2048];
        size_t len = 0;
        quench_generate(ctx_, prompt.c_str(), &p, buf, sizeof(buf), &len);
