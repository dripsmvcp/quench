...
// (,,,,, — every one found by a symptom, none
...
// NOT ASCII-only: an ASCII-only generator would bless a token classifier that
// drops non-ASCII — the failure mode where constrained German output silently
// loses every umlaut.
//
// Worth being precise about what this generator does and does not cover: these
// tests drive the FSM, and the FSM is not the fragile part — it compares
// through `unsigned char` and accepts umlauts. The fragile layer is one layer
...
// what actually fails if that pre-filter mishandles non-ASCII). So this covers
// UTF-8 through the grammar; it does not cover the mask. Both are needed.
...
// This is the direction broke (valid nested docs were REJECTED:
...
// (a) is the shape accepted (`,"bare-string"` then `]]` while the FSM
...
// ===========================================================================
// P4b — Trailing commas. A comma before a closer is the single most
// common way a model writes almost-JSON, and an FSM allows it whenever, after a
// value, `,` returns to the same state an OPENER produces — that state
// legally accepts the closer, because an empty `[]` / `{}` is valid. Then `[1,]`
// and `{"a":1,}` pass the mask and the reply does not parse, breaking exactly
// the contract json_object exists to keep.
//
// Generative, because this shape can hide three levels
...
TEST(JsonConstrainPropertyTest, RejectsMinimalTrailingComma) {
    ...
    // The empty forms stay legal — rejecting trailing commas must not ban `[]` / `{}`.
...
// exact failure mode of (close popped its own frame but resumed in the
// grandparent's state).
// ===========================================================================
...
// --- probe: does the number FSM reject a second decimal point? --------
...
TEST(JsonConstrainFsm, RejectsSecondDecimalPointInNumber) {
    ...
    EXPECT_FALSE(d.sim_token_valid(".5"))
        << "FSM accepted a SECOND decimal point — this is the output shape";
...
TEST(JsonConstrainFsm, NumberGrammarMatchesRfc8259) {
    ...
    const Case cases[] = {
        ...
        {"1e-5", true},
        {"-2.5e-3", true},
        {"3.5.5", false},  // second decimal point — the shape
...
// --- part 2: force-close near the token budget ------------------------
...
//: raw control characters must never reach a string. The grammar has
...
// ---------------------------------------------------------------------------
//: constrained output lost every non-ASCII character — "Die Bären hören"
...
TEST(TokenCategory, NonAsciiCountsAsStringContent) {
    ...
    // What must STAY rejected — accepting raw bytes must not open the door too wide.
