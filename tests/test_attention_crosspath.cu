// Attention cross-path agreement.
//
...
// FA2 (f16 QK), and flash_attention_blackwell. The regression shipped
...
protected:
    void SetUp() override {
        ...
        // production config (f16acc on, which only meets the wider
        // f16-class envelope, not the 1e-2 pairwise assert.
        saved_f16acc_ = quench::process_diag_fa2_f16acc();
        saved_pv_f16acc_ = quench::process_diag_fa2_pv_f16acc();
        quench::process_diag_set_fa2_f16acc(false);
        quench::process_diag_set_fa2_pv_f16acc(false);
    }
    void TearDown() override {
        quench::process_diag_set_fa2_f16acc(saved_f16acc_);
        quench::process_diag_set_fa2_pv_f16acc(saved_pv_f16acc_);
    ...
    // the f32-score-chain paths (cuBLAS FP32-S, FA2-f16) and
    // the WMMA paths (fmha_sm120, blackwell; measured ~4e-4 max_rel)
    ...
    // at short rows where no averaging happens — the mechanism) are
    ...
    void run_config(int cfg_idx, const char* name, int Sq, int Skv, int NH, int NKV, int HD, bool causal,
        ...
        {  // 5. FA2 register-resident, f16 QK (the short-prefill production path,)
        ...
        // Held to 1e-2 vs fp64 since the race fix (measured ~4e-4).
        ...
        // the executor's short-prefill replacement must be
...
//
// Measured envelopes (all numbers = max_rel vs fp64 ref, denom max(1,|ref|)):
//   mild (amp=2, score sigma ~0.6):  fp8-QK 0.58-0.71 (worst at SHORT rows —
//     e4m3 score noise with no averaging, the mechanism). f32-chain:
//     < 1e-2.
//   sharp (amp=8, near-one-hot softmax): fp8 ~2.3 (76% of elements past
//     5e-2). f32-chain still < 1e-2.
// The WMMA paths measure ~4e-4 on all four configs and are held strict at
// 1e-2. That strictness guards a known failure mode: an in-place
// float→half S/P-tile compaction that clobbers neighbor rows' float scores
// (half row r aliases the bytes of float row r/2; padding rows racing
// ahead zero-fill valid rows — worst at short seq).
// Production survives the characterized paths because real activations are
// correlated and long-context averaging dilutes per-row flips — but
// SHORT prefill must stay on the f32 chain, and %13-style
...
TEST_F(AttentionCrossPathTest, MildLongGQA) {
    run_config(0, "mild_long_gqa", 192, 192, 4, 2, 128, true, 0, 0.0f, 2.0f, /*mild=*/true,
               quench_refs::mild_long_gqa_spot_idx, quench_refs::mild_long_gqa_spot_val, 64);
}

TEST_F(AttentionCrossPathTest, MildShortGQA32_8) {
    run_config(1, "mild_short_gqa32", 24, 24, 32, 8, 128, true, 0, 0.0f, 2.0f, /*mild=*/true,
               quench_refs::mild_short_gqa32_spot_idx, quench_refs::mild_short_gqa32_spot_val, 64);
}
...
TEST_F(AttentionCrossPathTest, MildSlidingWindowSoftcap) {
    run_config(2, "mild_sliding_softcap", 160, 160, 4, 4, 128, true, 64, 50.0f, 2.0f, /*mild=*/true,
               quench_refs::mild_sliding_softcap_spot_idx, quench_refs::mild_sliding_softcap_spot_val, 64);
}
...
TEST_F(AttentionCrossPathTest, SharpLongGQA_F32ChainStrict) {
    run_config(3, "sharp_long_gqa", 192, 192, 4, 2, 128, true, 0, 0.0f, 8.0f, /*mild=*/false,
               quench_refs::sharp_long_gqa_spot_idx, quench_refs::sharp_long_gqa_spot_val, 64);
}
...
// ---- seqlen edge cases: the base crosspath matrix
// bottoms out at Sq=24. These cover the degenerate single-/two-token
...
TEST_F(AttentionCrossPathTest, SingleTokenPrefill) {
    ...
    // degenerate empty-row case none of the base configs reach.
...
// ---- head-dim edges: the parity matrix above is
...
// HD=64/256 kernel just declines and is skipped). HD=256 is the Gemma-3
