// The pre-cudaDeviceReset hook registry must not be silently empty.
//
...
// their `if (!ptr)` guards re-arm. Before the hooks were listed by hand
...
TEST(CudaStaticReset, HookRegistryIsPopulated) {
    EXPECT_GT(quench::cuda_static_reset_hook_count(), 0)
