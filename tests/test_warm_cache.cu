...
//
// Requires a real model on disk: QUENCH_TEST_MODEL or /models/Qwen3-8B-Q8_0.gguf.
...
constexpr const char* kCacheDir = "/tmp/quench-warm-cache-test";
...
struct Cycle {
    ...
    bool up(const char* path) {
        ...
        quench::RuntimeConfig rc;
        rc.warm_cache.enabled = true;
        rc.warm_cache.dir = kCacheDir;
        quench::set_pending_runtime_config(rc);
        QuenchConfig config = quench_config_default();
        config.max_seq_len = 1024;
        config.max_batch_size = 1;
        return quench_context_create(model, &config, &ctx) == QUENCH_SUCCESS;
    }
    ...
    bool generate_greedy() {
        ...
        char buf[2048] = {};
        size_t n = 0;
        if (quench_generate(ctx, "Name three colors.", &params, buf, sizeof(buf), &n) != QUENCH_SUCCESS)
            return false;
        output.assign(buf, n);
        return n > 0;
    }
...
TEST(WarmCacheTest, ColdLoadWritesCacheWarmBootTokenIdentical) {
    SKIP_IF_NO_MODEL();
    const std::string cache = quench::weight_cache_path_for(get_model_path(), kCacheDir);
