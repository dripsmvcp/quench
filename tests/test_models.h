// Central model-env registry for the test suite.
//
// Model-dependent tests are gated on a small set of QUENCH_TEST_MODEL* env vars.
...
// therefore read:
//
//   const std::string path = quench_test::env_path(quench_test::kEnvModel);
//   if (path.empty())
//       GTEST_SKIP() << "Set " << quench_test::kEnvModel << " to run ...";
//
// or, for the GGUF suites that fall back to the Docker bind-mount and then
// skip-on-missing-file:
//
//   const std::string path = quench_test::env_path_or(quench_test::kEnvModel,
//                                                   "/models/Qwen3-8B-Q8_0.gguf");
//
// The hardcoded /models/... fallbacks are NOT a defect: they
...
#ifndef QUENCH_TESTS_TEST_MODELS_H
#define QUENCH_TESTS_TEST_MODELS_H
...
inline constexpr const char* kEnvModel = "QUENCH_TEST_MODEL";

// Generic GGUF model for loader/tensor-kind coverage.
inline constexpr const char* kEnvGguf = "QUENCH_TEST_GGUF";

// Tokenizer-compat golden (paired with kEnvModel).
inline constexpr const char* kEnvGolden = "QUENCH_TEST_GOLDEN";

// Architecture-specific overrides for the multi-model E2E suite.
inline constexpr const char* kEnvModelGdn = "QUENCH_TEST_MODEL_GDN";
inline constexpr const char* kEnvModelGemma4 = "QUENCH_TEST_MODEL_GEMMA4";
...
inline constexpr const char* kEnvModelQwen4b = "QUENCH_TEST_MODEL_QWEN4B";
inline constexpr const char* kEnvModelLlama = "QUENCH_TEST_MODEL_LLAMA";

// MoE/hybrid model for the deterministic-mode E2E gate.
inline constexpr const char* kEnvMoeModel = "QUENCH_TEST_MOE_MODEL";

// Vision mmproj GGUFs for the SigLIP / gemma4v golden tests.
inline constexpr const char* kEnvMmproj = "QUENCH_TEST_MMPROJ";
inline constexpr const char* kEnvMmprojGemma4 = "QUENCH_TEST_MMPROJ_GEMMA4";
...
inline constexpr const char* kEnvModelDeepSeek = "QUENCH_TEST_MODEL_DEEPSEEK";
...
#endif  // QUENCH_TESTS_TEST_MODELS_H
