...
// (positionally blind since integration), Phi-4 RoPE-NeoX, the FA2
// short-prefill regression, and the gemma mode-2 NaN collapse
// (/) — all of which shipped while per-kernel parity suites were
// green.
//
// Lock lifecycle (tests/refs/e2e_greedy_locks.h):
//   1. Generate candidates: QUENCH_LOCK_PRINT=1 prints ready-to-paste lock
...
protected:
    void SetUp() override {
        ...
        if (!path)
            GTEST_SKIP() << "Set QUENCH_TEST_MODEL to run greedy locks";
        path_ = path;

        QuenchModelFormat fmt = is_safetensors_dir(path_) ? QUENCH_FORMAT_SAFETENSORS : QUENCH_FORMAT_GGUF;
        ASSERT_EQ(quench_model_load(path, fmt, &model_), QUENCH_SUCCESS);
    ...
    std::vector<int32_t> generate_greedy(const char* prompt, int n_gen) {
        ...
        int n_prompt = 0;
        EXPECT_EQ(quench_tokenize(model_, prompt, prompt_tokens.data(), &n_prompt, 512), QUENCH_SUCCESS);
        prompt_tokens.resize(n_prompt);

        EXPECT_EQ(quench_context_reset(ctx_), QUENCH_SUCCESS);
        EXPECT_EQ(quench_prefill_with_params(ctx_, prompt_tokens.data(), n_prompt, &params), QUENCH_SUCCESS);

        std::vector<int32_t> out;
        for (int i = 0; i < n_gen; i++) {
            int32_t tok = -1;
            if (quench_decode_step(ctx_, &params, &tok) != QUENCH_SUCCESS || tok < 0)
                break;
            out.push_back(tok);
        }
        return out;
    ...
    std::string detok(const std::vector<int32_t>& toks) {
        ...
        if (quench_detokenize(model_, toks.data(), (int)toks.size(), buf.data(), buf.size()) != QUENCH_SUCCESS)
...
// Candidate prompts for NEW locks (QUENCH_LOCK_PRINT=1). Raw completion
...
TEST_F(GreedyLockTest, FrozenSequences) {
    ...
    const bool print_mode = std::getenv("QUENCH_LOCK_PRINT") != nullptr;
    ...
    if (print_mode) {
        ...
        GTEST_SKIP() << "QUENCH_LOCK_PRINT mode: candidates printed, nothing asserted";
    }

    int n_locks = 0;
    for (const auto& lock : quench_refs::kGreedyLocks) {
        ...
        EXPECT_EQ(run1, want) << "GREEDY LOCK BROKEN for " << base << "\n  prompt: " << lock.prompt
                              ...
                              << "\nIf this change is intentional (quality-neutral kernel rework), "
                                 "regenerate with QUENCH_LOCK_PRINT=1, re-verify externally "
                                 "(tests/refs/e2e_greedy_locks.h header comment), and say so in the PR.";
    }

    if (n_locks == 0)
        GTEST_SKIP() << "no greedy locks recorded for model '" << base
                     << "' — generate with QUENCH_LOCK_PRINT=1 and add to tests/refs/e2e_greedy_locks.h";
}
