...
// inside an open <think> block. Regression cover for — Qwen3.5-4B's
...
using quench::server::reconcile_thinking_with_prompt_tail;
...
TEST(ThinkingReconcile, ClosedBlockDefaultsOff) {
    //: heuristic defaulted ON, template rendered a pre-closed block.
...
// test_sse_stream_utils.cpp (issue, which also uses extract_reasoning as
