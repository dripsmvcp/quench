...
# Needs a running quench-server on :8080; fires 4 concurrent chat requests.
set -e
...
for i in 1 2 3 4; do
    curl -s "$BASE" \
        ...
        -d "{\"model\":\"quench\",\"messages\":[{\"role\":\"user\",\"content\":\"Count from $i to $((i+5))\"}],\"max_tokens\":32,\"temperature\":0}" \
        -o "/tmp/quench_concurrent_$i.json" &
done
...
failures=0
for i in 1 2 3 4; do
    f="/tmp/quench_concurrent_$i.json"
    if [ ! -s "$f" ]; then
