// =============================================================================
// Unit tests for quench-server CLI flag parsing (parse_server_args) — closes the
// "--vram-budget flag parse and the server-limit flags are untested" gap from
// issue. The planner logic behind --vram-budget is well covered elsewhere
...
ServerArgs parse(std::vector<std::string> args) {
    ...
    argv_storage.emplace_back("quench-server");
    for (auto& a : args)
