...
// A system message must not cost the image its tokens.
//
// Keying the image block on "message index 0 is a user message" means any
// request that opens with a system prompt — the normal shape for a pipeline —
// silently produces a text-only prompt. The picture is still decoded and
// encoded, and the model answers fluently that it cannot see an image, which
