// Paged decode attention quant variants vs fp64 reference
// (the decode twin of the cross-path test).
//
...
// when its KV mode is active, yet the only other numeric oracle is
// test_paged_attention.cu, whose quant tests (INT4) build their CPU reference
// FROM THE DEQUANTIZED values — i.e. quench-vs-quench on the quant grid (a
// tautology: it cannot see a quant kernel that is wrong, only one that is
// inconsistent with its own dequant). And those use benign sin/cos fills.
//
// This test avoids both problems:
...
//     the periodic-%13 / size_t-underflow vacuity.
...
//     rows where quant score noise has no averaging to hide behind,
...
// measured and frozen below with ~50% margin. NVFP4's
// 1e-1 single-op class is a floor expectation, not the envelope — paged KV
// quant on short uncorrelated rows is looser, exactly what this suite
// characterizes.
...
// score-noise has no averaging; 1024 is the long-context dilution case.
// ===========================================================================
...
// ===========================================================================
// Measured characterization envelopes — MEASURED on RTX 5090
// (sm_120a). All numbers are max_rel vs the
...
//   MXFP4-KV   0.0801    0.0362    0.0251    0.0163    0.12  (MEASURED)
//
// Findings (real):
//   * Monotone improvement with kv_len — confirms the mechanism in the
...
//     short-row band is the same weak spot the cross-path prefill suite
//     characterizes.
