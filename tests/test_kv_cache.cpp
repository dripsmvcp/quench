...
// evict a live sequence. Every lru_order_ entry is a live sequence and quench has
...
TEST(KVCacheManagerTest, PersistedCacheFingerprintGate) {
    ...
    const std::string path = "/tmp/quench_prefix_cache_fp_gate.bin";
...
TEST(KVCacheManagerTest, EvictMiddleBlocksKeepsSinksAndWindow) {
    ...
    // (=> 4 window blocks) plus ONE extra boundary block (: the decode
    ...
    EXPECT_EQ(bt_after[15], bt_before[15]);            // boundary block survives
...
// StreamingLLM middle-block eviction must retain the
// block containing the decode kernels' window start. The paged decode
// kernels compute window_start_block = floor((ctx_len - window) /
// block_size); an eviction that retains ceil-aligned from the sequence tail
// (total_blocks - ceil(window / block_size)) disagrees by one for
// non-block-aligned ctx_len — the kernels' first window block then holds a -1
// sentinel, and phys_block = -1 produces an out-of-bounds KV read (an
