...
//
// WHY THIS EXISTS:
//   tests/test_mmvq.cu compares quench's MMVQ kernel against quench's
//   dp4a kernel (a quench-vs-quench tautology). All of GGUF serving
//   (Q8_0 / Q6_K / Q4_K models) consumes these dequant + GEMV kernels, so
//   they need an independent anchor. This file is that anchor:
...
//   from quench's headers; the dequant math is reconstructed and justified in
//   comments against ggml-common.h / dequantize_row_q*), and compares quench's
//   GPU kernels against it.
//
// INDEPENDENCE (no tautologies):
...
//     quench's quantizer (which would make a quantize->dequant round-trip and
//     prove nothing).
//   * The reference dequant is a slow fp64 host loop, never a call into quench.
...
// quench; used to fill raw quant bytes and to pick scale halfs.
// -----------------------------------------------------------------------------
struct Lcg {
    uint32_t s;
...
// Layouts mirror quench's headers (= the format), arithmetic is reconstructed.
// -----------------------------------------------------------------------------
...
//
// TYPED_TEST over the GGUF block formats (parametrize where it's cheap and
// real). The dequant test body is one shared skeleton —
...
// Q4_0 dp4a-GEMV vs the fp64 reference. The oracle pins the nibble layout:
// standard ggml Q4_0 is SPLIT (element e = qs[e] low, e+16 = qs[e] high), NOT
// interleaved (2k = qs[k] low, 2k+1 = qs[k] high) — an interleaved read
// mispairs weights with the natural-order Q8_1 activations (~6x off).
// Q4_0_Traits::dp4a_block must use the same split extraction as the Q4_K path.
