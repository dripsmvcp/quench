// E2E proof for [runtime] deterministic — issue item 3.
//
...
// drift had a separate root cause — quench_perplexity nulled active_request
// before quench_context_reset, leaking the KV sequence + SSM/GDN slot per call
// (fixed in quench_api.cpp alongside this test).
//
// Gated on QUENCH_TEST_MOE_MODEL (point it at an MoE/hybrid model dir or .gguf;
// dense models pass trivially since they skip the routed-expert kernels).
// The deterministic flag reaches the engine via the legacy QUENCH_DETERMINISTIC
...
const char* model_path() { return std::getenv(quench_test::kEnvMoeModel); }
...
protected:
    void SetUp() override {
        ...
        if (!path)
            GTEST_SKIP() << "Set QUENCH_TEST_MOE_MODEL to run deterministic-mode E2E";
        // Must be set BEFORE quench_context_create: engine init pulls the
        // env-seeded RuntimeConfig via take_pending_runtime_config().
        setenv("QUENCH_DETERMINISTIC", "1", 1);
        path_ = path;
...
TEST_F(DetEvalE2ETest, PerplexityBitIdenticalSameContext) {
    ...
    MakeContext();
    double ppl1 = 0.0;
    ASSERT_EQ(quench_perplexity(ctx_, tokens.data(), static_cast<int>(tokens.size()), &ppl1), QUENCH_SUCCESS);
    double ppl2 = 0.0;
    ASSERT_EQ(quench_perplexity(ctx_, tokens.data(), static_cast<int>(tokens.size()), &ppl2), QUENCH_SUCCESS);
    FreeContext();
