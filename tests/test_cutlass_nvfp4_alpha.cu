...
// encoder it depends on. These guard against a subtle clamp bug:
//
//   `float_to_fp8_e4m3` clamping to (e=14, m=7) → bits 0x77, decode
//   240, instead of the correct E4M3-fn max 448 = (e=15, m=6) → 0x7E.
//   Any input value > 240 that falls into the e=15 exponent slot gets
...
// guard the CUTLASS prefill path against that precision cliff.
...
__global__ void encode_fp8_e4m3_kernel(const float* in, uint8_t* out, int n) {
    ...
    if (i < n)
        out[i] = quench::float_to_fp8_e4m3(in[i]);
}
...
// prequant NVFP4. The two distinguishing properties vs quench's auto-quant
...
//
// quench's auto-quant uses tensor_scale = max(|W|)/6 (large, ~0.73) and
...
TEST_F(CutlassNvfp4AlphaTest, MistralL0PrequantByteLayout) {
    ...
    auto quantize_fp4 = [](float v) -> uint8_t {
        // quench's E2M1 LUT thresholds: midpoints between {0, .5, 1, 1.5, 2, 3, 4, 6}
        float a = std::fabs(v);
    ...
    auto float_to_fp8_e4m3 = [](float v) -> uint8_t {
        // Match quench's float_to_fp8_e4m3 (clamped, RNE-rounded). Use stdlib bit ops.
        if (v <= 0.0f)
            return 0u;
        if (v >= 448.0f)
    ...
    for (int n = 0; n < N; ++n) {
        for (int b = 0; b < n_blocks_k; ++b) {
            ...
            // (decode FP8 via standard formula, matching quench)
            float reconstructed;
...
// ---------------------------------------------------------------------------
// The contract that lets the GEMM live without lazy workspace growth.
//
// A GEMM that cudaFree+cudaMallocs a file-scope workspace at GEMM time
// whenever the caller's buffer is too small does so on a path reachable under
// CUDA-graph capture, where cudaMalloc is illegal. It refuses instead, and
