...
//
// Ensures the planner doesn't propose NVFP4 for sub-5-bit-source weights
// (where NVFP4 is a representation change at similar bit-width, no
// compression win).
// ---------------------------------------------------------------------------
...
TEST(StoragePlanner, Q4KSourceDoesNotPickNVFP4UnderPreferHint) {
    ...
    // floor to FP16). Guards against a Gemma-3-12B Q4_K_M failure mode
    // where the runtime ends up with zero cache coverage.
    Model m;
    m.config_.n_layers = 1;
        ...
        if (e.kind == TensorKind::W_GATE || e.kind == TensorKind::W_UP ||
            ...
            EXPECT_EQ(e.tier, StorageTier::FP16)
                << "Q4_K-source FFN weights must land on FP16, NOT NVFP4 — "
                   ...
                   "(guards the Q4_K coverage-gap failure mode)";
            gate_count++;
        }
    }
