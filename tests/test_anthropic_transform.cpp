...
// cache_control mapping and cache usage accounting (issue item 1).
//
...
// prompt-KV pinning. Since the LAST marked system/message block also
...
using quench_server::anthropic::anthropic_to_openai_body;
using quench_server::anthropic::openai_to_anthropic_response;
...
// ({type:"enabled",budget_tokens:N} | {type:"disabled"}); quench's orchestrator
// reads `enable_thinking` (bool) + `think_budget` on the OpenAI body. Note quench's
...
// ---- tool_choice conversion + parallel_tool_calls (issue) -------------
...
//: images inside tool_result blocks (screenshot-returning tools) must
...
// context", so quench namespaces its own rather than guessing at their schema.
...
TEST(AnthropicResponse, EvictedTokensSurfacedUnderImpNamespace) {
    ...
    oai["usage"] = usage;
    json u = openai_to_anthropic_response(oai, "m")["usage"];
    EXPECT_EQ(u["quench_evicted_tokens"], 512);
...
TEST(AnthropicResponse, NoEvictionMeansNoImpKey) {
    ...
    json oai = oai_response(json{{"content", "x"}});
    oai["usage"] = usage;
    EXPECT_FALSE(openai_to_anthropic_response(oai, "m")["usage"].contains("quench_evicted_tokens"));
}
...
// ---------------------------------------------------------------------------
// The Messages API has no "system" role — sending one is an error there. quench
// accepts it, and must not render it as a USER turn: rendered that way the
// text still reaches the model (no data loss)
...
// ---------------------------------------------------------------------------
// Constrained-decoding extensions have no Anthropic equivalent, and a shim
// that drops them makes the same server honour guided_regex on
// /v1/chat/completions while ignoring it on /v1/messages — constrained output
// vs free-form prose. The silence would be total: a malformed pattern is not
// rejected either, because the admission check never sees the field.
// ---------------------------------------------------------------------------
