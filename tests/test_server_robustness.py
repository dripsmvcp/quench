...
Needs a running quench-server on :8080 (default model Qwen3-8B-NVFP4-cortecs).

Robustness battery for the HTTP surface (DEBUG-500-on-bad-input.md). quench must
...
HOST = os.environ.get("QUENCH_HOST", "localhost")
PORT = int(os.environ.get("QUENCH_PORT", "8080"))
M = os.environ.get("QUENCH_MODEL", "Qwen3-8B-NVFP4-cortecs")
...
def check_no5xx(label, path, body_bytes):
    """A lenient case: quench may accept it (2xx) or reject it (4xx+envelope), but it
...
def main():
    print(f"quench HTTP robustness battery — base={BASE} model={M}\n")
