...
// cached-block eviction, pool integrity). What it does NOT cover
// is the part that makes the feature *correct*:
//
...
// The heart of the suite. Reuse is only correct if the reused physical block
...
// hash-collision / stale-block guard — the determinism
// blocker for enabling the feature.
...
TEST(PrefixPersistTest, QuantizedKvRestoresItsScales) {
    ...
    const std::string path = "/tmp/quench_prefix_scale_roundtrip.bin";
    ::remove(path.c_str());
