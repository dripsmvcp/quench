...
// perplexity over the probe prompt (quench_perplexity applies the LM head to
...
// file it was calibrated on — re-downloaded quants of the SAME model
...
//
// Uses quench_generate (the high-level API) which uses the well-tested
// tokenize→prefill→decode path via quench_generate_streaming internally.
// This avoids having to manage the low-level quench_prefill_with_params
// + quench_decode_step loop manually in tests.
static std::string generate_greedy(const char* model_path, const std::string& prompt,
    ...
    if (quench_model_load(model_path, QUENCH_FORMAT_GGUF, &model) != QUENCH_SUCCESS || !model)
        return {};
    ...
    if (use_fp8_kv)
        cfg.kv_cache_dtype = QUENCH_DTYPE_FP8_E4M3;

    QuenchContext ctx = nullptr;
    if (quench_context_create(model, &cfg, &ctx) != QUENCH_SUCCESS || !ctx) {
...
static double ppl_for_chunk(const char* model_path, const std::string& prompt,
    ...
    int n_tokens = 0;
    if (quench_tokenize(model, prompt.c_str(), tokens.data(), &n_tokens,
                     static_cast<int>(tokens.size())) != QUENCH_SUCCESS ||
        n_tokens < 2) {
        quench_model_free(model);
        return -1.0;
    }
    ...
    double ppl = -1.0;
    QuenchError err = quench_perplexity(ctx, tokens.data(), n_tokens, &ppl);

    quench_context_free(ctx);
    quench_model_free(model);

    return (err == QUENCH_SUCCESS) ? ppl : -1.0;
}
...
// Measured cross-kernel-path noise (Qwen3-8B + Llama-3.2-3B,
...
protected:
    ...
    // QUENCH_TEST_MODEL_QWEN4B=path overrides the Qwen3-4B path. Deliberately NOT
    // the generic QUENCH_TEST_MODEL: suite runs set that to whatever model is
    ...
    // QUENCH_TEST_MODEL_LLAMA=path overrides Llama-3B path.
    static const char* qwen3_4b_path() {
        const char* p = std::getenv(quench_test::kEnvModelQwen4b);
        if (p) return p;
        return "/models/Qwen3-4B-Instruct-2507-Q8_0.gguf";
    }
    static const char* llama_3b_path() {
        const char* p = std::getenv(quench_test::kEnvModelLlama);
        if (p) return p;
...
// ---------------------------------------------------------------------------
// Test 3b (gate — ENABLED): LONG-context chunk invariance.
//
// Two failure modes this gate watches. Chunked continuations routed
// through the fp8/e4m3 FMHA family drift teacher-forced NLL up to ~25%.
// And a host rewrite of the pinned prefill staging
// (h_pf_token_ids_/h_pf_positions_) while earlier chunks' H2D copies are
// still queued produces CATASTROPHIC NLL on the Llama family
// (0.29 -> 7.13) — the race pf_staging_evt_ exists to prevent; the kernel
// itself is not at fault there. With the staging serialized, chunked
// continuations route through the FP16-QK FA2 kernel at every ctx_len
...
// replay path: the captured chunk forward bakes ctx_len/q_offset as
...
TEST_F(ChunkedPrefillTest, LongContext_Chunk_Invariance_GraphsOn) {
    ...
    for (int chunk : {512, 1024}) {
        ...
        ASSERT_GT(ppl, 0.0) << "chunk=" << chunk << " perplexity failed";
        EXPECT_NEAR(ppl, base, base * kFp16RelTol)
            << "chunk=" << chunk << " graphs-on long-context PPL diverges ( class)";
    }
}
...
TEST_F(ChunkedPrefillTest, Qwen3_4B_GenerationCoherent) {
    ...
    // with a particular Qwen3-4B-Instruct-2507 Q8_0 download). Only
