...
// quench-server): a tool request suppresses thinking, the chat template renders a
...
using quench::server::StreamReasoningSplitter;
using quench::server::ThinkPhase;
...
// Mirrors quench::server::kAgentScanLimit in the stream driver.
...
// a way to give it up — without that, preventing the leak above would break
