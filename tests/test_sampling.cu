...
QUENCH_TEST_ENGINE_ARENA(64ull << 20);  // T2 arena for the migrated scratches (A7 step 8)
namespace {
...
// Issue: the sampler drew the SAME quantile on every token.
//
    ...
    auto draw = [&](int top_k) {
        ...
        for (int tok : {1000, 2000, 3000})
            if (auto it = hist.find(tok); it != hist.end())
                top3 += it->second;
        fprintf(stderr, "[] k=%d: %zu distinct tokens, top-3 share %.2f\n", top_k, hist.size(),
                top3 / 200.0);
        return hist;
    };
...
// Issue: the CUB path (top_k > SAMPLE_MAX_TOP_K) served STALE candidates.
//
