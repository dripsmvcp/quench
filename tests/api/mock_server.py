"""
Mock quench server for GPU-free CI testing.

Implements the OpenAI-compatible API surface of quench-server without any model
...
class MockHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            body = (
                f"# HELP quench_uptime_seconds Server uptime\n"
                f"# TYPE quench_uptime_seconds gauge\n"
                f"quench_uptime_seconds {uptime:.1f}\n"
                f"# HELP quench_requests_total Total requests\n"
                f"# TYPE quench_requests_total counter\n"
                f"quench_requests_total {metrics.requests_total}\n"
                f"# HELP quench_requests_failed_total Failed requests\n"
                f"# TYPE quench_requests_failed_total counter\n"
                f"quench_requests_failed_total {metrics.requests_failed}\n"
                f"# HELP quench_tokens_prompt_total Total prompt tokens\n"
                f"# TYPE quench_tokens_prompt_total counter\n"
                f"quench_tokens_prompt_total {metrics.tokens_prompt_total}\n"
                f"# HELP quench_tokens_completion_total Total completion tokens\n"
                f"# TYPE quench_tokens_completion_total counter\n"
                f"quench_tokens_completion_total {metrics.tokens_completion_total}\n"
                f"# HELP quench_model_loaded Model loaded\n"
                f"# TYPE quench_model_loaded gauge\n"
                f"quench_model_loaded 1\n"
                f"# HELP quench_queue_depth Queue depth\n"
                f"# TYPE quench_queue_depth gauge\n"
                f"quench_queue_depth 0\n"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Mock quench server for testing")
    ...
    print(f"Mock quench server listening on http://127.0.0.1:{args.port}", flush=True)
