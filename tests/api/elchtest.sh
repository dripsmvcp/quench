...
# Needs a running quench-server on :8080 with the model below loaded.
...
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  ELCHTEST (moose test) — quench-server stability & correctness    ║"
...
r=$(health "/metrics")
t "GET /metrics → Prometheus format" "endpoint" "$(echo "$r" | grep -q 'quench_requests_total' && echo 1 || echo 0)"
