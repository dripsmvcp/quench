...
BASE_URL = os.environ.get("QUENCH_TEST_URL", "http://localhost:8080")
MODEL = os.environ.get("QUENCH_TEST_MODEL", "")

# Auto-start mock server when no real server URL or model is configured
USE_MOCK = os.environ.get("QUENCH_USE_MOCK", "0") == "1" or (not MODEL and BASE_URL == "http://localhost:8080")
MOCK_PORT = int(os.environ.get("QUENCH_MOCK_PORT", "9099"))
...
def model():
    if not MODEL:
        pytest.skip("QUENCH_TEST_MODEL not set and mock mode disabled")
    return MODEL
