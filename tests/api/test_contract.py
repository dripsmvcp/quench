...
External state:  Running quench-server or mock server.
"""
...
class TestMetricsEndpoint:
    ...
    def test_metrics_has_required_counters(self, client):
        r = client.get("/metrics")
        text = r.text
        assert "quench_requests_total" in text
        assert "quench_requests_failed_total" in text

    def test_metrics_has_model_loaded(self, client):
        r = client.get("/metrics")
        assert "quench_model_loaded" in r.text
