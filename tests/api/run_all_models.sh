...
for MODEL in "${MODELS[@]}"; do
  ...
  # Start server
  QUENCH_MODEL="/models/$MODEL" docker compose up quench-server -d 2>/dev/null
  ...
  for i in $(seq 1 60); do
    ...
    if ! docker compose ps quench-server 2>/dev/null | grep -q "Up"; then
      LOGS=$(docker compose logs quench-server --tail 3 2>/dev/null)
  ...
  OUTPUT=$(docker run --rm --network host \
    ...
    "pip install -q httpx pytest 2>/dev/null && QUENCH_TEST_MODEL=$MODEL python -m pytest test_errors.py test_chat.py test_streaming.py -v --tb=line 2>&1" \
  )
