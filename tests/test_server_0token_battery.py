#!/usr/bin/env python3
"""MANUAL tool — not wired into ctest/CI/verify.sh.
Needs a running quench-server on :8080 with an instruction-following model that
also serves /v1/embeddings (default Qwen3-8B-NVFP4-cortecs).

Regression battery for the quench "0 completion tokens" bug (DEBUG-0tokens.md,
fixed in PR / v0.11.1). Under interleaved embeddings + chat the embeddings
...
Env:
    QUENCH_BASE         (default http://localhost:8080)
    QUENCH_MODEL        (default: auto-detected from /v1/models)
...
BASE = os.environ.get("QUENCH_BASE", "http://localhost:8080").rstrip("/")
...
def model_id():
    if os.environ.get("QUENCH_MODEL"):
        return os.environ["QUENCH_MODEL"]
...
def main():
    print(f"quench 0-token battery — model={M} base={BASE} N={N} LOAD={LOAD} thr={FAIL_THRESHOLD:.0%}\n")
