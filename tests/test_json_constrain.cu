...
// ===========================================================================
// FSM grammar tests — a container close must restore the continuation
...
// Issue: a constrainer that cannot mask must never decode SILENTLY
// unconstrained.
//
// Were the device allow list allocated lazily inside apply_mask() — on
// the serving path, mid-decode — then on a model that loads with no free VRAM
// left that allocation fails and apply_mask returns without masking and
// without logging, so the first constrained request per process comes back as
// prose where JSON was promised: deterministic, byte-identical across restarts,
// and invisible in `quench_constrained_eager_fallback_total` because no fallback
// is taken. The three sibling constrainers (regex, grammar, schema)
// allocate this at init and fail the load loudly.
//
...
TEST(JsonConstrainTest, InitReservesTheAllowListSoApplyMaskNeverAllocates) {
    ...
    EXPECT_TRUE(jc.has_device_allow_list())
        << "init() succeeded without reserving the allow list — apply_mask would allocate on the "
           "serving path, and a failure there masks nothing at all (issue)";
