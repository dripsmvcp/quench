...
TEST(TokenizerGemma4Test, TruncatedUTF8AtEndDoesNotCrash) {
    // nfc_decode_utf8 must not advance pos past end-of-string
    // on a truncated multi-byte sequence (that yields a partial codepoint).
...
// \p{M} in the letter run — identical on ASCII). Letting the routing fall
// through to the gpt2 per-char-punct fallback over-splits symbol runs
// (+13% tokens measured on a 35B corpus) and makes canonical merges
// unreachable.
...
// ---- o200k pre-tokenizer (gpt-oss / GPT-4o,) ----
//
...
TEST(O200kPreTokenizeTest, NonAsciiSymbolsAreSymbolsNotLetters) {
    ...
    // from canonical (the 4 residual corpus diffs in).
...
// ---- cl100k pre-tokenizer (GPT-4/tiktoken lineage, Phi-4;) ----
//
...
static std::string write_temp_tokenizer_json(const std::string& body) {
    std::string path = std::string("/tmp/quench_tok_test_") + std::to_string(::getpid()) +
