...
static std::string write_temp_gguf(const std::vector<uint8_t>& data) {
    char path[] = "/tmp/quench_test_XXXXXX.gguf";
...
// ---------------------------------------------------------------------------
//: encoder-only architectures must be rejected at load, not fall through
...
TEST(GgufLoaderTest, EncoderOnlyArchDetection) {
    ...
    // to miss them, letting NomicBertModel fall through to the decoder.
...
TEST(GgufLoaderTest, EncoderArchGgufRejectedAtLoad) {
    ...
    // of mapping to the generic decoder. nomic-bert is admitted
