// GGUF loader fault-injection tests.
//
// Charter: "header fault injection → clean error". The contract
...
// the real loader, not a mock.
//
// Several of these tests guard specific weakness classes (length/count
// overflow, unknown-array-type spin, unbounded reserve, unchecked tensor
// offsets).
...
std::string write_temp(const std::vector<uint8_t>& data) {
    char path[] = "/tmp/quench_fault_XXXXXX.gguf";
...
bool load_is_clean(const std::vector<uint8_t>& data) {
    ...
    try {
        ...
        // QuenchError at the API boundary). Catching here proves we unwound
        // rather than faulted.
        clean = true;
    }
    unlink(path.c_str());
