// E2E greedy locks — frozen full-stack token sequences.
// Consumed by tests/test_e2e_greedy_lock.cpp. See that file's header for the
// lock lifecycle (generate with QUENCH_LOCK_PRINT=1 → verify EXTERNALLY →
// commit). Every entry documents its verification in the comment above it:
//   "verified: llama.cpp <image>" — same raw prompt, greedy, the
...
//   "verified: internal" — no external engine loads this format
...
struct GreedyLock {
    const char* model_basename;  // matched against basename(QUENCH_TEST_MODEL)
...
inline constexpr GreedyLock kGreedyLocks[] = {
    // verified: llama.cpp ghcr.io/ggml-org/llama.cpp:server-cuda.
    // Cross-engine note: llama.cpp greedy continues " Paris. The capital of
    // Italy is Rome..."; quench continues ". What is the capital of Germany?
    ...
    // trajectories). Locked sequence is quench's own — the lock is a
    ...
    // verified: llama.cpp server-cuda — both engines answer " 42"
    // (quench: "42\n\nOkay, let me try to figure out what 17 plus 25 is...";
    // llama.cpp: " 42\n\nQ: What is 17 + 25?\nA: 42..."). Arithmetic
    // grounding agrees; this is the prompt-blindness detector (prompt-blind
    // bugs scramble exactly this prompt shape).
    {"Qwen3-8B-Q8_0.gguf",
     "Q: What is 17 + 25?\nA:",
     31,
    ...
    // verified: internal — no external engine loads NVFP4
    // SafeTensors. Coherent capital-Q&A with correct facts (Berlin/Rome/
    // Madrid/Lisbon); degen_suite 27/0 on the same model+build.
    // This entry locks the SafeTensors loader + RoPE path (RoPE-NeoX-
    // class bugs produce prompt-blind output exactly here).
    ...
    // verified: internal — "17+25=42": arithmetically correct,
    ...
    // primary colors are red, blue, and" — quench's greedy output is
    ...
    // likely an exact logit tie broken by a non-deterministic
