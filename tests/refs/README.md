...
1. **No magic constants.** Every golden value in a test traces to a committed
   generator in this directory. The generator header states tool versions and
   seed; regenerating must reproduce the committed file bit-exactly
...
2. **Tolerance policy** (state source + tolerance + why, per test):
   ...
   - fp8-class attention (fp8 FMHA, FA2 e4m3 QK): a 5e-2 single-op
     bound is **provably not meetable on short rows** (measured 0.58-0.71 rel
     on mild data at 2-24 keys: e4m3 score noise with no
     averaging). These paths are CHARACTERIZED (envelope assert +
   ...
   - WMMA attention (fmha_sm120, flash_attention_blackwell): **≤ 1e-2 rel**
     ...
     f32 softmax + f16 P). Both paths measure ~4e-4 on all configs and are
     held strict.
...
3. **Realistic data, bit-identical across languages.** Kernel fixtures are
   ...
   where e4m3 loses mantissa bits and where benign constant fills would be
   vacuous.
...
| `gen_reference.py` | — (none committed) | **dormant** — HF layer-by-layer dump infra for future use; no test consumes it |

gpt-oss YaRN goldens: the generator reimplements YaRN-scaled RoPE
...
golden (**1e-9 rel**), then judges quench's `rope_forward` kernel against the
...
and asserts SENSITIVITY to a rope_freq_scale inversion (kernel must NOT
match the 1024×-wrong ref).

Harmony goldens: render strings from the REAL HF reference
...
quench's own jinja engine and compares EXACTLY (the one strftime_now `Current
...
The vision encoder golden is the one exception to rule 1's "independent
...
the test itself in dump mode (`QUENCH_VISION_GOLDEN_DUMP=1`), which prints the
golden arrays measured on the current build; the header records the producing
setup (GPU/CUDA + model file sizes) so a drift is attributable. Tolerance is the **f16
...
embedding. Regenerate via `make test-vision
QUENCH_VISION_GOLDEN_DUMP=1` and paste the emitted blocks; needs the RTX 5090 +
the gemma-3/gemma4v mmproj GGUFs (encoded standalone, no LM).

NVFP4 outlier goldens: the generator reimplements NVFP4's two-level
dequant (E2M1 + UE4M3 micro-scale + f32 tensor-scale, incl. the 1/512 floor)
in fp64 from the format definition — never from quench. Adversarial weight
distributions (Gaussian, 64× outliers, a single 512× outlier, all-tiny) probe
the per-tensor-scale floor that can collapse Gemma mode-2. The test
