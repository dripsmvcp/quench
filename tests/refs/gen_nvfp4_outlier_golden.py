...
Why this exists: round-trip-via-own-code tests on benign Gaussian data cannot
catch scale-collapse bugs in the NVFP4 quantize pipeline (the product's core
decode path). The Gemma mode-2 collapse — a single outlier weight inflates the
per-tensor scale until normal micro-blocks underflow the 1/512 FP8-E4M3 floor
and dequant to zero, ultimately NaN logits at decode step 2 — is invisible to
such tests because nothing feeds an adversarial (outlier / all-tiny) weight
...
two-level dequant in fp64 FROM THE FORMAT DEFINITION (not by calling quench):
...
adjacent representable values (the same midpoint thresholds quench's
float_abs_to_fp4_e2m1 uses). FP8 E4M3: 4-exp/3-man, bias 7, min subnormal
2^-9 = 1/512, max normal 448 — the quench/NVIDIA fp8_e4m3 definition.

The C++ test generates IDENTICAL inputs with the same integer-LCG + f32
multiply-only recipe (bit-exact across numpy/C++, no libm), runs quench's real
...
# RNE midpoints between adjacent magnitudes (quench float_abs_to_fp4_e2m1 thresholds).
...
def fp8_e4m3_round(x: float) -> float:
    ...
    Mirrors quench float_to_fp8_e4m3 / fp8_e4m3_to_float: bias 7, 3 mantissa
    ...
    if x >= FP8_MAX:
        return FP8_MAX
    if x < FP8_MIN_SUB:
        # quench returns 0 for |val| < 1/512 (sign-only byte).
        return 0.0
...
def e2m1_rne(mag: float) -> float:
    ...
    Uses the same midpoint thresholds as quench's branchless code. Saturates to 6.
    """
...
def dequant_ref(w_f16: np.ndarray) -> np.ndarray:
    ...
    if tensor_scale == 0.0:
        tensor_scale = 1.0  # quench: all-zero tensor -> scale 1
    out = np.zeros_like(w)
    nmb = K // MICRO
    for r in range(N):
        for g in range(nmb):
            ...
            if ms < FP8_MIN_SUB:
                ms = FP8_MIN_SUB          # quench floor clamp
            if ms > FP8_MAX:
                ms = FP8_MAX
            ms_actual = fp8_e4m3_round(ms)
            if ms_actual == 0.0:
                ms_actual = FP8_MIN_SUB   # quench guards 0 -> 1/512
...
def main():
    lines = [
        ...
        "// tensorscale) from the format definition — NOT produced by quench code.",
        "// Regenerating must reproduce this file (see tests/refs/README.md).",
        "#pragma once",
        "",
        "namespace quench_refs {",
        "",
    ]
    for idx, (name, N, K, amp, om, period, tiny) in enumerate(CONFIGS):
        ...
        lines.append("")
    lines.append("}  // namespace quench_refs")
    lines.append("")
