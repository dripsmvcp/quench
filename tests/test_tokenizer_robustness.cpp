// Tokenizer robustness / round-trip tests.
//
// The class (NUL string terminators leaking into SSE deltas) lived in the
...
TEST(TokenizerRobustness, RoundtripEmbeddedNUL) {
    // THE class: a NUL byte in the middle of the text. std::string holds
...
//
// These pin the ACTUAL behavior (characterize, don't wish). The
...
TEST(TokenizerRobustness, LiteralMarkerWithControlIsPreSplit) {
    ...
    // And decoding that control token renders the literal text back (the
