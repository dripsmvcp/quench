...
//
// The sibling tests for the NVFP4 quantizer (the product's core decode path)
// are round-trip-via-own-code on benign Gaussian data (test_nvfp4_quant.cu
// asserts "no crash"; test_nvfp4_quant_ref.cu round-trips through its own
// reference kernel). Nothing else feeds an ADVERSARIAL weight distribution
// through quantize -> dequant -> GEMV against an INDEPENDENT oracle. That
// blind spot is exactly where a Gemma-class outlier collapse hides: a single
...
// tensor-scale, incl. the 1/512 floor) — NOT by calling quench. The committed
...
// Quantize a distribution's weight tensor with quench's REAL production
