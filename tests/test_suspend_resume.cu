...
// QUENCH_TEST_MODEL or the default /models/Qwen3-8B-Q8_0.gguf.
//
// Deliberately does NOT exercise quench_gpu_release(device_reset=1): a
...
static std::map<std::string, uint64_t> checksum_records(QuenchModel model, size_t count) {
    std::map<std::string, uint64_t> sums;
    const quench::WeightUploadLog* log = model->model->upload_log();
    if (!log)
        return sums;
...
struct Cycle {
    ...
    bool up(const char* path) {
        if (quench_model_load(path, QUENCH_FORMAT_GGUF, &model) != QUENCH_SUCCESS)
            return false;
        QuenchConfig config = quench_config_default();
        config.max_seq_len = 1024;
        config.max_batch_size = 1;
        if (quench_context_create(model, &config, &ctx) != QUENCH_SUCCESS)
            return false;
        return true;
    }
    ...
    bool generate_greedy() {
        ...
        char buf[2048] = {};
        size_t n = 0;
        if (quench_generate(ctx, "Count from 1 to 5.", &params, buf, sizeof(buf), &n) != QUENCH_SUCCESS)
            return false;
        output.assign(buf, n);
        return n > 0;
    }
...
static void run_roundtrip(bool drop_one_key) {
    ...
    QuenchWeightSnapshot snap = nullptr;
    ASSERT_EQ(quench_weights_snapshot_capture(first.model, /*headroom_mb=*/256, &snap), QUENCH_SUCCESS);
    ASSERT_NE(snap, nullptr);
    EXPECT_GT(quench_weights_snapshot_bytes(snap), size_t{1} << 20);
    ...
    ASSERT_EQ(quench_gpu_release(/*device_reset=*/0), QUENCH_SUCCESS);
    ...
    // EngineRelaunchTest: cudaMemGetInfo under-reports on WSL2/WDDM).
    {
        ...
        if (free_now_mib + 2048 < free_before_mib + (quench_weights_snapshot_bytes(snap) >> 20)) {
            // Weights-sized memory should be free again; probe a big chunk.
            size_t probe_mib = quench_weights_snapshot_bytes(snap) >> 20;
            if (probe_mib > 1024) {
                void* probe = nullptr;
    ...
    ASSERT_EQ(quench_weights_snapshot_arm(snap), QUENCH_SUCCESS);
    Cycle second;
    ASSERT_TRUE(second.up(get_model_path()));
    EXPECT_GT(quench_weights_snapshot_hits(snap), 0) << "no warm hits — snapshot was ignored";
    ...
    second.down();
    quench_weights_snapshot_free(snap);
}
...
TEST(SuspendResumeTest, ArmedMismatchIsIgnored) {
    ...
    QuenchWeightSnapshot snap = nullptr;
    ASSERT_EQ(quench_weights_snapshot_capture(c.model, 256, &snap), QUENCH_SUCCESS);
    ASSERT_EQ(quench_weights_snapshot_arm(snap), QUENCH_SUCCESS);
    quench_weights_snapshot_free(snap);  // must disarm
    EXPECT_EQ(quench::weight_snapshot_take_armed(), nullptr);
    c.down();
}
