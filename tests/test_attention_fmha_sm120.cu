...
protected:
    ...
    void run_test(int B, int Sq, int Skv, int NH, int NKV, int HD, bool causal, int sliding_window = 0,
        ...
        // NOTE: the int cast before the -6 is load-bearing (/). With
...
// --- residue: WMMA fallback at hd=256, production-like sizes ---
...
TEST_F(FmhaSm120Test, DispatchSelectsSm120FMHA) {
    ...
    // in the same test binary context, without the subtle stream/memory
    // ordering pitfalls of a hand-rolled launch.
    run_test(1, 64, 64, 4, 4, 128, true, 0, 0.0f);
}

TEST_F(FmhaSm120Test, DispatchManual) {
    // The %13 fills below cast to int BEFORE subtracting 6. With size_t
    // arithmetic the subtraction underflows — ~46% of inputs become ±inf in
    // half, so NaN output is then CORRECT — while a comparator built on
    // std::max silently drops NaNs and looks green. The int cast keeps this
    // manual path and run_test() in agreement.
