...
struct TempFile {
    ...
    explicit TempFile(const std::string& body) {
        path = "/tmp/quench_test_config_" + std::to_string(::getpid()) + ".conf";
...
// (bool literals stay accepted so quench.conf files using them keep parsing).
...
TEST(RuntimeConfigTest, KvFp8HintDefaultSafeAllowlist) {
    ...
    // LLAMA: verified via Phi-4-reasoning-plus-NVFP4 (+0.25% PPL).
    EXPECT_TRUE(kv_fp8_hint_default_safe(ModelArch::LLAMA));
    // NEMOTRON_H_MOE: verified via Nemotron-3-Nano-30B — FP8 vs FP16 mean PPL
    // ~0.00% over a 26.5k-token corpus, 5 runs each (a single A/B is
    // run-to-run noise on this MoE+Mamba2 hybrid).
...
// evidence and the measured exclusions (QWEN36_MOE: +1.47% real on
...
TEST(RuntimeConfigTest, MissingFileFallsBackToDefaults) {
    RuntimeConfig cfg;
    EXPECT_FALSE(cfg.load_from_file("/nonexistent/path/quench.conf"));
