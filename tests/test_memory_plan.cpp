...
PlanInput dense_input() {
    ...
    in.library = LibraryReserve{3900 * kMiB, "measured"};
...
TEST(ShadowPlan, ReportSaysWhatItDoesNotModel) {
    ...
    // The plan is applied — the KV block count the report prints is the one
    // the pool uses — so the wording must not describe it as "computed, NOT
    // applied".
    ...
    // The note says the shadow plan does not model the tower — not that its
    // size is unknowable (the arena reserves it from a pre-load probe).
    // Match on the durable wording.
