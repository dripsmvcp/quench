...
// test_nvfp4_gemv_kpar_loop.cu) all roundtrip quench's own quantizer through
// quench's own dequantizer. They cannot detect a paired sign-flip / nibble-order
...
//
// and asserts that quench's gemv_nvfp4_kpar produces the same Y = W·X as a pure-host
...
// the sequential reference and quench's parallel-warp accumulator dominates;
...
TEST_F(NvFP4CompressedTensorsRef, BaselineUnityScales) {
    ...
    // ---- quench path via gemv_nvfp4_kpar ----
    ...
    int worst_idx = -1;
    for (int n = 0; n < N; ++n) {
        float quench_v = __half2float(h_y[n]);
        float diff = std::fabs(quench_v - y_ref_f32[n]);
    ...
    EXPECT_LT(max_abs_diff, 1e-2f) << "Compressed-tensors NVFP4 GEMV diverges from spec reference. "
                                   ...
                                   << " quench=" << __half2float(h_y[worst_idx])
...
// Test 3: Zero tensor_scale → output must be exactly zero (defensive
// zeroing). Validates that promoted_scale=0 actually produces
...
// Test 5: Modelopt path defensively zeros NaN/+Inf weight_scale_2.
...
TEST(NvFP4PromoteWeightScale2, LlmCompressorZeroNoInf) {
    // The bug PR first guarded: 1/0 = +Inf without the defensive zeroing.
    bool zeroed = false;
...
TEST(NvFP4ValidatePackedScaleShapes, AcceptsTinyTestShape) {
    // [16, 64] packed inner with [16, 8] scale inner — the baseline GEMV test shape.
    std::string err;
