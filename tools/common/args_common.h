...
// The CLI flags quench-cli and quench-server both accept, in one place.
//
...
// `max_tokens` — 256 in quench-cli, 8192 in quench-server. That gap turned out to be
...
// Flags shared by quench-cli and quench-server. Parsed by parse_common_flag().
struct CommonArgs {
    // quench.conf integration. --config overrides the search-path default; --set is
    ...
    // 8192 for both tools. quench-cli sat at 256 until — a default from
