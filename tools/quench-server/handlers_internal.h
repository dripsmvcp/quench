...
// Internal (server-private) shared declarations for the quench-server HTTP
...
struct ChatRequestParams {
    ...
    // cache_control breakpoint boundary: number of leading chat
    ...
    // 0 = force off, 1 = force on. From the quench extension body field
    ...
    // Enforced tool calling: filled for tool_choice=required / forced
    ...
    // Messages + image
    std::vector<quench::ChatMessage> chat_msgs;
...
struct ChatStateSnapshot {
    ...
    std::shared_ptr<quench::ImageData> vision_image;
    ...
    std::vector<std::shared_ptr<quench::QwenPatches>> qwen_patches;
    ...
    quench::ChatTemplateFamily tpl_family = quench::ChatTemplateFamily::CHATML;
    std::vector<quench::ToolFunction> tool_defs;
    ...
    // cache_control breakpoint boundary in TOKENS: -1 = pin the whole
...
inline int cache_creation_tokens_(const std::shared_ptr<quench::Request>& req, int n_prompt_tokens) {
    if (!req || !req->pin_kv_prefix)
        return 0;
    int rounded = (n_prompt_tokens / quench::kKVBlockSize) * quench::kKVBlockSize;
...
inline nlohmann::json prompt_tokens_details_(const std::shared_ptr<quench::Request>& req, int n_prompt_tokens) {
    if (!req)
...
// Rejects a constraint quench cannot compile with 400 instead of answering
// unconstrained. Called from validate_sampling_params.
...
// Build an quench::Request from a parsed+snapshotted chat request context — the
...
// streaming); this was hand-copied per site and drifted.
// completion_idx offsets the seed for n>1 choice generation; stream keeps the
// request on per-step decode for real per-token SSE.
