...
namespace quench_server::responses {
...
json responses_to_openai_body(const json& rsp) {
    // Statelessness guards: quench keeps no response store, so continuation via
    ...
    if (rsp.contains("previous_response_id") && !rsp["previous_response_id"].is_null())
        throw std::invalid_argument(
            "previous_response_id is not supported (quench-server is stateless — send the "
            "full transcript in `input`, e.g. store=false clients like Codex do)");
    if (rsp.value("store", false))
        throw std::invalid_argument(
            "store=true is not supported (quench-server keeps no response store); use "
...
json openai_to_responses_response(const json& oai, const std::string& req_model,
    ...
    if (oai.contains("usage")) {
        ...
        // quench extension, forwarded rather than dropped: context this request
