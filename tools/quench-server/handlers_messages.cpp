...
bool run_anthropic_stream_(httplib::DataSink& sink, ChatRequestContext& ctx, ServerState& state,
    ...
    // ---- message_start ----------------------------------------------------
    {
        // Cache accounting: harnesses read cache_read/creation from
        ...
        if (active_req) {
            for (int i = 0; i < 50 && active_req->status == quench::RequestStatus::PENDING; i++)
...
void handle_messages(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    // fail to parse. res is untouched by the shim (it writes a separate
...
static void handle_messages_impl(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    if (want_stream) {
        ...
        // SSE is real per-token rather than one burst at generation end.
        auto quench_req = build_imp_request_(ctx, ctx.snap.tokens, /*completion_idx=*/0,
                                          /*stream=*/true);
