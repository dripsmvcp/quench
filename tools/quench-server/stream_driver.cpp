...
bool run_stream_loop_(httplib::DataSink& sink, ChatRequestContext& ctx, ServerState& state,
    ...
    quench::server::StreamToolCallFilter tool_filter(tpl_family);
    ...
    quench::server::ThinkPhase think_start_phase;
    if (enable_thinking)
        think_start_phase = quench::server::ThinkPhase::REASONING;  // <think> in prefill
    else if (use_reasoning && think_budget > 0.0f)
        think_start_phase = quench::server::ThinkPhase::SCAN;  // model decides
    else
        think_start_phase = quench::server::ThinkPhase::CONTENT;  // no extraction
    ...
    // pointing Claude Code at quench-server: it printed raw reasoning as the reply.)
    ...
    quench::server::StreamReasoningSplitter think_split(think_start_phase, ctx.snap.think_start_id,
    ...
    // emitted mid-codepoint.
    const bool harmony = (tpl_family == quench::ChatTemplateFamily::HARMONY);
    ...
    auto flush_buffered_content = [&]() -> bool {
        if (stop_sequences.empty()) {
            ...
            auto hd = quench::stream::holdback_decision(pending_text, max_stop_len, stop_sequences);
    ...
    for (;;) {
        ...
        // the final token is swallowed by one of those paths (/).
        if (finish)
            break;
        ...
        if (think_active) {
            ...
            if (rs.content.empty()) {
                ...
                if (has_tools && think_split.phase() == quench::server::ThinkPhase::SCAN &&
        ...
        if (has_tools) {
            ...
            piece.clear();
            using SegKind = quench::server::StreamToolCallFilter::Segment::Kind;
    ...
    out.reasoning_truncated = think_active && think_split.phase() == quench::server::ThinkPhase::REASONING &&
...
void finish_stream_accounting_(ServerState& state, ChatRequestContext& ctx,
                               const std::shared_ptr<quench::Request>& active_req, const StreamLoopResult& out,
