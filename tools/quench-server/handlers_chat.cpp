...
void handle_chat_completions(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    auto quench_req = build_imp_request_(ctx, saved_tokens, /*completion_idx=*/0, ctx.params.stream);
    ...
    if (ctx.params.stream) {
        ...
        nonstream_chat_response_(res, state, ctx, quench_req, server_req, saved_tokens, comp_id, created);
    }
}
...
void handle_completions(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    // Create an quench::Request and submit to batching engine (/v1/completions is
    // text-only — no vision).
    auto quench_req = std::make_shared<quench::Request>();
    quench_req->input_tokens = std::move(tokens);
    quench_req->max_tokens = max_tokens;
    quench_req->temperature = temperature;
    quench_req->top_p = top_p;
    quench_req->top_k = top_k;
    quench_req->seed = seed;
    quench_req->min_p = min_p;
    quench_req->typical_p = typical_p;
    quench_req->repetition_penalty = repetition_penalty;
    quench_req->frequency_penalty = frequency_penalty;
    quench_req->presence_penalty = presence_penalty;
    quench_req->repeat_last_n = repeat_last_n;
    quench_req->dry_multiplier = dry_multiplier;
    quench_req->dry_base = dry_base;
    quench_req->dry_allowed_length = dry_allowed_length;
    quench_req->dry_penalty_last_n = dry_penalty_last_n;
    quench_req->mirostat = mirostat;
    quench_req->mirostat_tau = mirostat_tau;
    quench_req->mirostat_eta = mirostat_eta;
    quench_req->logprobs = req_logprobs;
    quench_req->top_logprobs = top_logprobs;
    quench_req->logit_bias = std::move(logit_bias);
    quench_req->think_budget = body.value("think_budget", state.default_think_budget);
    quench_req->pin_kv_prefix = body.value("cache_prompt", false);
    if (body.contains("speculative") && body["speculative"].is_boolean())
        quench_req->spec_ngram_override = body["speculative"].get<bool>() ? 1 : 0;
    ...
    if (body.contains("prediction") && body["prediction"].is_object()) {
        ...
        if (pred.value("type", "content") == "content" && pred.contains("content") &&
            ...
            quench_req->prediction_tokens = snap_tok->encode(pred["content"].get<std::string>());
            if (snap_max_seq_len > 0 &&
                quench_req->prediction_tokens.size() > static_cast<size_t>(snap_max_seq_len))
                quench_req->prediction_tokens.resize(snap_max_seq_len);
        }
    }
    // Stream requests stay on per-step decode for real per-token SSE.
    quench_req->stream = stream;
    quench_req->status = quench::RequestStatus::PENDING;
        ...
        res.set_chunked_content_provider(
            ...
            [&state, server_req, comp_id, created, n_prompt_tokens, t_start, stop_sequences,
             ...
             snap_is_think_model](size_t /*offset*/, httplib::DataSink& sink) -> bool {
                ...
                // </think> that never comes (: completions stream arrived as
                ...
                for (;;) {
                    //: the is_last token sets `finish` then falls through
                    ...
                    if (stop_sequences.empty()) {
                        ...
                        pending_text += piece;
                        auto d = quench::stream::holdback_decision(pending_text, max_stop_len,
                                                                stop_sequences);
...
void handle_count_tokens(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    namespace anth = quench_server::anthropic;
