...
///. Body parsing for the shared chat-completion machinery:
...
bool parse_chat_request_params(const httplib::Request& req, httplib::Response& res, ServerState& state,
    ...
    // Per-request speculative-decode override (quench extension). Absent → leave
    ...
    bool tool_xml_dialect = false;
    {
        ...
        ctx.snap.tpl_family = state.have_template ? state.chat_tpl.family() : quench::ChatTemplateFamily::CHATML;
    ...
    // Surface it: one WARN per request + a /metrics counter.
    ...
    // Enforced tool calling is collected POST-snapshot in
    ...
    for (const auto& msg : messages) {
        ...
        if (role == "tool") {
            ...
            if (ctx.snap.tpl_family == quench::ChatTemplateFamily::GEMMA && !ctx.params.chat_msgs.empty() &&
...
#else
                            ctx.params.image_error = "https image_url needs an quench built with OpenSSL";
#endif
                        } else {
