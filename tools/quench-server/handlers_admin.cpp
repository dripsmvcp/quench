...
// (quench_weights_snapshot_capture), tear down engine + model (all VRAM freed),
// then quench_gpu_release() — with [suspend] device_reset (default) that includes
...
void handle_suspend(const httplib::Request& /*req*/, httplib::Response& res, ServerState& state) {
    ...
    QuenchError err = quench_weights_snapshot_capture(state.model, headroom_mb, &snap);
    if (err != QUENCH_SUCCESS) {
        if (state.batching)
            state.batching->resume();
        res.status = (err == QUENCH_ERROR_OUT_OF_MEMORY) ? 507
                     : (err == QUENCH_ERROR_UNSUPPORTED) ? 501
                                                      : 500;
        json jerr = {{"error",
                      {{"message", std::string("suspend failed (nothing torn down): ") +
                                       quench_error_string(err) + " — see server log for details"},
                       {"type", "server_error"}}}};
    ...
    // Measure the reclaim BEFORE quench_gpu_release when resetting: any CUDA call
    ...
    quench_gpu_release(device_reset ? 1 : 0);
    if (!device_reset) {
        size_t f = 0;
    ...
    printf("[suspend] %s suspended to host RAM (%.2f GiB snapshot)%s\n", model_name.c_str(),
           quench_weights_snapshot_bytes(snap) / (1024.0 * 1024.0 * 1024.0),
           device_reset ? ", CUDA context reset" : "");
    fflush(stdout);

    json body = {{"suspended", true},
                 {"model", model_name},
                 {"snapshot_bytes", quench_weights_snapshot_bytes(snap)},
...
void handle_resume(const httplib::Request& /*req*/, httplib::Response& res, ServerState& state) {
    ...
    if (state.weight_snapshot)
        quench_weights_snapshot_arm(state.weight_snapshot);
    ...
    const int warm_hits = quench_weights_snapshot_hits(state.weight_snapshot);
    quench_weights_snapshot_free(state.weight_snapshot);
