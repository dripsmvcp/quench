...
std::string build_tool_prompt(quench::ChatTemplateFamily family, const json& tools, const json& tool_choice) {
    if (tools.empty())
        return "";
    ...
    if (family == quench::ChatTemplateFamily::LLAMA3) {
...
std::vector<std::pair<std::string, std::string>> collect_tool_constraint(quench::ChatTemplateFamily family,
                                                                         const json& tools,
                                                                         const json& tool_choice) {
    std::vector<std::pair<std::string, std::string>> out;
    if (tools.empty())
        return out;
    // Only the ChatML `<tool_call>` JSON envelope — the
    ...
    if (family != quench::ChatTemplateFamily::CHATML)
        return out;
...
std::pair<std::string, std::string> collect_llama3_forced_tool(quench::ChatTemplateFamily family,
    ...
    if (family != quench::ChatTemplateFamily::LLAMA3 || tools.empty())
        return {};
    if (!tool_choice.is_object() || !tool_choice.contains("function"))
        return {};
...
std::vector<std::pair<std::string, std::string>> collect_strict_tool_constraint(
    quench::ChatTemplateFamily family, const json& tools, const json& tool_choice) {
    std::vector<std::pair<std::string, std::string>> out;
    if (tools.empty())
        return out;
...
std::pair<std::string, std::vector<ParsedToolCall>> parse_tool_calls(
    quench::ChatTemplateFamily family, const std::string& text, std::atomic<int>& next_tool_call_id,
    const std::vector<std::string>& known_tool_names) {
    if (family == quench::ChatTemplateFamily::LLAMA3)
        return parse_tool_calls_llama3(text, next_tool_call_id, known_tool_names);
    if (family == quench::ChatTemplateFamily::GEMMA)
...
ToolTagScan scan_tool_tag(const std::string& buf, quench::ChatTemplateFamily family) {
    ToolTagScan r;
    ...
    const bool gemma = (family == quench::ChatTemplateFamily::GEMMA);
...
std::string reconstruct_tool_call_output(quench::ChatTemplateFamily family, const json& tool_calls,
    ...
    for (const auto& tc : tool_calls) {
        ...
        if (family == quench::ChatTemplateFamily::LLAMA3) {
            ...
            result += args;
            result += "</function>";
        } else if (family == quench::ChatTemplateFamily::GEMMA) {
...
std::string format_tool_response(quench::ChatTemplateFamily family, const json& msg) {
