...
// Enforced tool calling: derive the FSM constraint from the
...
static void collect_tool_enforcement_(ChatRequestContext& ctx) {
    ...
    if (!ctx.params.tool_constraint_tools.empty()) {
        ...
        // the body FSM engages only if the model opens a tool call.
    ...
    if (!ctx.params.tool_constraint_tools.empty() &&
        ctx.snap.tpl_family == quench::ChatTemplateFamily::CHATML && ctx.snap.have_template &&
    ...
    // bare parameter schema with a per-tool envelope. Only when the
...
bool snapshot_state_and_tokenize_(httplib::Response& res, ServerState& state, ChatRequestContext& ctx) {
    ...
    // This protects against concurrent model load/unload invalidating pointers.
    {
        ...
        ctx.snap.tpl_family = ctx.snap.have_template ? ctx.snap.chat_tpl.family()
                                                     : quench::ChatTemplateFamily::CHATML;
    ...
    // Enforced tool calling: tool_choice=required / forced function /
    ...
    if (ctx.snap.has_vision_request) {
        ...
        for (const auto& bytes : ctx.params.images)
            ctx.snap.vision_content_hash = quench::combine_image_hash(ctx.snap.vision_content_hash,
                                                                   quench::image_content_hash(bytes.data(),
                                                                                           bytes.size()));
        if (state.ctx->engine->has_qwen_vision()) {
            ...
            for (const auto& bytes : ctx.params.images) {
                auto patches = std::make_shared<quench::QwenPatches>();
            ...
            auto img = std::make_shared<quench::ImageData>();
    ...
    // content. The Anthropic "disabled" path already zeroes the budget.
    ...
    if (ctx.snap.have_template && !ctx.snap.qwen_patches.empty()) {
        ...
        std::string exp_err;
        if (pad_id < 0 ||
            !quench::expand_image_placeholders(ctx.snap.tokens, pad_id, ctx.snap.qwen_image_tokens, exp_err)) {
            res.status = 400;
            json error = {{"error",
        ...
        if (ctx.snap.tokens.empty()) {
            QUENCH_LOG_INFO("Jinja2 tools path failed, falling back to text-based tool prompt");
    ...
    // cache_control per-breakpoint boundary: re-render the leading
    ...
    if (ctx.params.cache_prompt && ctx.params.cache_prefix_messages > 0 &&
        ...
        std::vector<quench::ChatMessage> prefix_msgs(
    ...
    // by promoting them at AutoTokenizer load; quench's tokenizer doesn't, so
    ...
    // if the heuristic defaulted it ON (: Qwen3.5-4B mentions enable_thinking
    ...
    // fall in the same tail (a lone in-window "<think>" would else read as open).
    {
        ...
        ctx.snap.enable_thinking = quench::server::reconcile_thinking_with_prompt_tail(
    ...
    // Per-request LoRA selection: swap the engine-global adapter
    ...
    // scope (quench is batch=1-first by mission).
    {
        int32_t want = 0;
        ...
        quench_lora_set(state.ctx, want);
    }
...
// handlers_internal.h).
std::shared_ptr<quench::Request> build_imp_request_(const ChatRequestContext& ctx,
    ...
    // Stream requests stay on per-step decode for real per-token SSE.
    req->stream = stream;
...
void nonstream_chat_response_(httplib::Response& res, ServerState& state, ChatRequestContext& ctx,
                              std::shared_ptr<quench::Request>& quench_req,
    ...
    for (int ci = 0; ci < ctx.params.n_completions; ci++) {
        ...
        if (ci > 0) {
            quench_req = build_imp_request_(ctx, saved_tokens, ci, /*stream=*/false);
            server_req = std::make_shared<ServerRequest>();
            server_req->request = quench_req;
            {
                std::lock_guard<std::timed_mutex> lock(state.mtx);
        ...
        if (ctx.snap.tpl_family == quench::ChatTemplateFamily::HARMONY) {
            ...
            // markup leaks verbatim into content.
    ...
    if (json details = prompt_tokens_details_(quench_req, ctx.snap.n_prompt_tokens); !details.is_null())
        usage["prompt_tokens_details"] = std::move(details);
    if (quench_req && (quench_req->cached_tokens > 0 || quench_req->pin_kv_prefix))
        state.metrics.tokens_cached_total += quench_req->cached_tokens;
    ...
    if (quench_req && !quench_req->prediction_tokens.empty()) {
        usage["completion_tokens_details"] = {{"accepted_prediction_tokens", quench_req->pred_accepted},
                                              {"rejected_prediction_tokens", quench_req->pred_rejected}};
    }
