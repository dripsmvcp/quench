...
namespace quench_server::anthropic {
...
void push_user_turn(json& out, const json& anth_msg) {
    ...
    for (const auto& block : content) {
        ...
        if (type == "tool_result") {
            ...
            // marker in the tool body tying them back to this result.
            std::string body;
            int n_images = 0;
...
// Any cache_control marker anywhere in the request? quench maps markers to
...
json anthropic_to_openai_body(const json& anth) {
    ...
    // quench extension: per-request speculative-decode override, passed through
    ...
    // Constrained decoding. These are quench/vLLM/llama.cpp extensions with no
    ...
    // quench's orchestrator (handlers.cpp) reads `enable_thinking` (bool) and
    ...
    // one is an error there. quench accepted it and, falling through to the
    ...
    if (anth.contains("tool_choice")) {
        ...
        // suppression actually reaches the /v1/messages path.
    ...
    // granularity: the last marked system/message block bounds the
...
std::string tool_call_id_to_anthropic(const std::string& openai_id) {
    // OpenAI-style id from quench: "call_imp_<counter>". Rewrite to Anthropic's
...
json openai_to_anthropic_response(const json& oai, const std::string& anth_model) {
    ...
    if (oai.contains("usage") && oai["usage"].is_object()) {
        ...
        // excluded from input_tokens. quench surfaces the prefix-cache hit count
        ...
        // is an quench-namespaced extension rather than a guess at their schema.
        // Only present when eviction actually fired.
        if (evicted > 0)
            usage_out["quench_evicted_tokens"] = evicted;
    }
