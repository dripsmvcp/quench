...
void handle_tokenize(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    int n_tokens = 0;
    QuenchError err = quench_tokenize(snap_model, content.c_str(), tokens.data(), &n_tokens, tok_cap);
    if (err != QUENCH_SUCCESS) {
        res.status = 500;
        json error = {{"error",
                       {{"message", std::string("Tokenize failed: ") + quench_error_string(err)},
                        {"type", "server_error"}}}};
...
void handle_detokenize(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    QuenchError err = quench_detokenize(snap_model, tokens.data(), static_cast<int>(tokens.size()), buf.data(),
                                  buf.size());
    if (err != QUENCH_SUCCESS) {
        res.status = 500;
        json error = {{"error",
                       {{"message", std::string("Detokenize failed: ") + quench_error_string(err)},
                        {"type", "server_error"}}}};
        res.set_content(dump_safe(error), "application/json");
        return;
    }
...
void handle_metrics(const httplib::Request& /*req*/, httplib::Response& res, ServerState& state) {
    ...
    std::string out;
    out.reserve(1024);
    out += "# HELP quench_uptime_seconds Server uptime in seconds\n";
    out += "# TYPE quench_uptime_seconds gauge\n";
    out += "quench_uptime_seconds " + std::to_string(uptime) + "\n";
    out += "# HELP quench_requests_total Total inference requests\n";
    out += "# TYPE quench_requests_total counter\n";
    out += "quench_requests_total " + std::to_string(m.requests_total.load()) + "\n";
    out += "# HELP quench_requests_failed_total Failed inference requests\n";
    out += "# TYPE quench_requests_failed_total counter\n";
    out += "quench_requests_failed_total " + std::to_string(m.requests_failed.load()) + "\n";

    out += "# HELP quench_constrained_eager_fallback_total Constrained requests that requested "
           "logprobs and fell back from the ConstrainedPipeline to eager decode\n";
    out += "# TYPE quench_constrained_eager_fallback_total counter\n";
    out += "quench_constrained_eager_fallback_total " +
           std::to_string(m.constrained_eager_fallback.load()) + "\n";
    out += "# HELP quench_tokens_prompt_total Total prompt tokens processed\n";
    out += "# TYPE quench_tokens_prompt_total counter\n";
    out += "quench_tokens_prompt_total " + std::to_string(m.tokens_prompt_total.load()) + "\n";
    out += "# HELP quench_tokens_completion_total Total completion tokens generated\n";
    out += "# TYPE quench_tokens_completion_total counter\n";
    out += "quench_tokens_completion_total " + std::to_string(m.tokens_completion_total.load()) + "\n";
    out += "# HELP quench_last_request_duration_ms Duration of last request in milliseconds\n";
    out += "# TYPE quench_last_request_duration_ms gauge\n";
    out += "quench_last_request_duration_ms " + std::to_string(m.last_request_duration_ms.load()) + "\n";
    out += "# HELP quench_model_loads_total Total model loads\n";
    out += "# TYPE quench_model_loads_total counter\n";
    out += "quench_model_loads_total " + std::to_string(m.model_loads_total.load()) + "\n";
    out += "# HELP quench_tokens_cached_total Total prompt tokens served from prefix cache\n";
    out += "# TYPE quench_tokens_cached_total counter\n";
    out += "quench_tokens_cached_total " + std::to_string(m.tokens_cached_total.load()) + "\n";
    out += "# HELP quench_requests_cancelled_total Requests cancelled by client disconnect\n";
    out += "# TYPE quench_requests_cancelled_total counter\n";
    out += "quench_requests_cancelled_total " + std::to_string(m.requests_cancelled.load()) + "\n";
    out += "# HELP quench_last_ttft_ms Time to first token of last request in milliseconds\n";
    out += "# TYPE quench_last_ttft_ms gauge\n";
    out += "quench_last_ttft_ms " + std::to_string(m.last_ttft_ms.load()) + "\n";
    ...
    emit_histogram("quench_request_duration_seconds", "Request end-to-end latency in seconds",
                   m.request_duration);
    emit_histogram("quench_ttft_seconds", "Time to first token in seconds", m.ttft);
    emit_histogram("quench_inter_token_seconds", "Mean inter-token latency (ITL) per request in seconds",
                   m.inter_token);
    out += "# HELP quench_model_loaded Whether a model is currently loaded\n";
    out += "# TYPE quench_model_loaded gauge\n";
    bool loaded = false;
    int queue = -1;
    {
        // Bounded lock so a scrape can't hang behind a long /v1/embeddings
        // holder; fall back to the lock-free status snapshot.
    ...
    out += "quench_model_loaded " + std::string(loaded ? "1" : "0") + "\n";
    out += "# HELP quench_queue_depth Current number of active and pending requests\n";
    out += "# TYPE quench_queue_depth gauge\n";
    out += "quench_queue_depth " + std::to_string(queue) + "\n";
...
void handle_embeddings(const httplib::Request& req, httplib::Response& res, ServerState& state) {
    ...
    //: embeddings ride the normal request queue as prefill-only
    ...
    if (!is_encoder && state.batching && state.batching->is_running()) {
        ...
        const int d_model = quench_model_d_model(state.model);
        ...
        for (const auto& text : inputs) {
            std::vector<int32_t> tokens(tok_cap);
            int n_tokens = 0;
            QuenchError terr = quench_tokenize(state.model, text.c_str(), tokens.data(), &n_tokens, tok_cap);
            if (terr != QUENCH_SUCCESS) {
                send_json_error(res, 500, "server_error",
                                std::string("Tokenize failed: ") + quench_error_string(terr));
                return;
            }
            if (n_tokens == 0) {
            ...
            // Chunked prefill pools per chunk, so the only hard bound
        ...
        for (size_t i = 0; i < submitted.size(); i++) {
            ...
            if (ereq->status != quench::RequestStatus::FINISHED || ereq->embedding_out.empty()) {
                send_json_error(res, 500, "server_error",
    ...
    // Pause the batching engine for exclusive C-API access (quench_prefill drives
    ...
    int d_model = quench_model_d_model(state.model);
    int total_prompt_tokens = 0;
    ...
    for (size_t input_idx = 0; input_idx < inputs.size(); ++input_idx) {
        ...
        std::vector<int32_t> tokens(tok_cap);
        int n_tokens = 0;
        QuenchError err = quench_tokenize(state.model, text.c_str(), tokens.data(), &n_tokens, tok_cap);
        if (err != QUENCH_SUCCESS) {
            res.status = 500;
            json error = {{"error",
                           {{"message", std::string("Tokenize failed: ") + quench_error_string(err)},
        ...
        // Encoder-only embedder (e.g. nomic-bert): the dedicated bidirectional
        ...
        if (state.ctx && state.ctx->engine && state.ctx->engine->is_encoder_model()) {
            ...
            const int32_t cls = quench_model_bos_token(state.model);
        ...
        //      and abort the whole server (Tensor::slice QUENCH_CHECK). max_tokens
        ...
        err = quench_prefill(state.ctx, tokens.data(), n_tokens);
        if (err != QUENCH_SUCCESS) {
            quench_context_reset(state.ctx);
            res.status = 500;
            json error = {{"error",
                           {{"message", std::string("Prefill failed: ") + quench_error_string(err)},
                            {"type", "server_error"}}}};
            res.set_content(dump_safe(error), "application/json");
            return;
        }
        ...
        quench::Tensor hidden_view = executor->view_hidden(n_tokens);
