...
// A constraint quench cannot compile used to be dropped and the request answered
// anyway: HTTP 200, free-form text, nothing in the reply to distinguish it from
// a satisfied constraint. Constrained decoding is a guarantee, so a
...
bool validate_constraints(const json& body, httplib::Response& res) {
    ...
    if (!grammar.empty()) {
        std::vector<quench::GbnfRule> rules;
        int32_t root = -1;
        std::string err;
        if (!quench::parse_gbnf(grammar, rules, root, &err))
    ...
    if (!schema.empty() && !quench::parse_json_schema(schema))
...
// in silence: `video_url` (quench has no video path at all) produced a 200
