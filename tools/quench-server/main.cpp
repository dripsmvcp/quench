...
#include "handlers.h"
#include "utils.h"
#include "webui_asset.h"  // generated: QUENCH_WEBUI_HTML
...
static bool is_inference_endpoint(const std::string& path) {
    return path == "/v1/chat/completions" || path == "/v1/completions" || path == "/v1/responses" ||
           path == "/v1/messages" || path == "/v1/embeddings";
}
...
int main(int argc, char** argv) {
    ...
    // Engine::init to pick up. The server may load
    ...
    state.runtime_config = quench::RuntimeConfig::load(args.config_path, args.config_overrides, &rejected_overrides);
    ...
    quench::process_diag_install(state.runtime_config);
    quench::set_pending_runtime_config(state.runtime_config);

    QuenchModelFormat resolved_format = QUENCH_FORMAT_GGUF;
    std::string resolved_model = quench::resolve_model_auto(args.model_path, resolved_format, args.revision);
    ...
    if (resolved_model != args.model_path) {
        printf("Resolved model: %s -> %s (%s)\n", args.model_path.c_str(), resolved_model.c_str(),
               resolved_format == QUENCH_FORMAT_SAFETENSORS ? "SafeTensors" : "GGUF");
    }
    ...
    // model load. Routes are registered once the model is ready;
    ...
    for (const auto& [name, path] : args.loras) {
        int32_t id = 0;
        if (quench_lora_load(state.ctx, path.c_str(), &id) != QUENCH_SUCCESS) {
    ...
    svr.set_pre_routing_handler([&state](const httplib::Request& req, httplib::Response& res) {
        ...
        // --metrics-require-auth folds it back under the api_key check.
    ...
    svr.Get("/", [](const httplib::Request&, httplib::Response& res) {
        res.set_content(QUENCH_WEBUI_HTML, "text/html; charset=utf-8");
    });
    ...
    // read quench's context length without a hard-coded table.
    ...
    quench_weights_snapshot_free(state.weight_snapshot);  // non-null only when suspended
    return 0;
}
