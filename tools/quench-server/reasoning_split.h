...
#include "stream_pipeline.h"  // quench::stream::utf8_complete_len

namespace quench::server {
...
// The closed-block downgrade is: templates such as Qwen3.5-4B default
...
inline bool reconcile_thinking_with_prompt_tail(bool current, bool explicit_set,
    ...
    if (tail_has_think && tail_has_close && !explicit_set)
        return false;  // pre-closed block: model answers directly
...
public:
    ...
    Result feed(std::string piece, int token) {
        ...
        for (;;) {
            ...
            size_t utf8_tail = cbuf_.size() - quench::stream::utf8_complete_len(cbuf_);
            if (utf8_tail > hold)
                hold = utf8_tail;
            if (cbuf_.size() > hold) {
...
private:
    ...
    static void emit_with_overlap(std::string& buf, std::string& out) {
        size_t complete = quench::stream::utf8_complete_len(buf);
