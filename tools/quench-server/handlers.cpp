...
std::string make_completion_id(ServerState& state) {
    return "quench-" + std::to_string(state.next_id.fetch_add(1));
}
...
void handle_health(const httplib::Request& /*req*/, httplib::Response& res, ServerState& state) {
    ...
    // liveness probe. Grab it with a short timeout and fall back to the
    ...
    if (faulted)
        res.status = 503;  // let orchestrators restart a wedged server
...
std::vector<std::pair<std::string, std::string>> scan_model_files(const std::string& dir) {
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(dir, ec)) {
        ...
        if (entry.is_directory()) {
            ...
            if (quench::is_safetensors_dir(dpath)) {
                std::error_code ec2;
                auto real = std::filesystem::canonical(path, ec2);
                if (ec2)
                    continue;
...
void handle_models(const httplib::Request& /*req*/, httplib::Response& res, ServerState& state) {
    ...
    // Snapshot state fields under a bounded lock — do not block
...
// prompt). We expose both so a TGI-shaped auto-detect path works against quench.
...
std::string find_model_path(const ServerState& state, const std::string& name) {
    ...
    if (name.find('/') != std::string::npos) {
        QuenchModelFormat fmt;
        std::string resolved = quench::resolve_model_auto(name, fmt);
        if (!resolved.empty())
            return resolved;
    }
...
// Build QuenchConfig from default args + optional JSON overrides.
// Engine auto-detects max_seq_len, max_batch_size, KV dtype, FP8 prefill, NVFP4 decode.
QuenchConfig build_config(const ServerArgs& args, const quench::RuntimeConfig& runtime_cfg,
                       const std::string& model_path, const json& overrides, QuenchModel model = nullptr) {
    (void)model_path;
    (void)model;
    ...
    // flag > [runtime] max_batch_size from quench.conf > 0 (engine auto-sizes from
    // the model's weight footprint; a >20 GiB MoE auto-picks 1). The quench.conf
    ...
    // quench.conf (the engine bridges the quench.conf key itself when this is 0).
    ...
    // Prefix caching: [server] prefix_cache, default ON since the/
...
std::string load_model_into_state(ServerState& state, const std::string& path, const json& config_overrides) {
    ...
    if (state.ctx) {
        quench_context_free(state.ctx);
        state.ctx = nullptr;
    }
    if (state.model) {
        quench_model_free(state.model);
        state.model = nullptr;
    }
    state.tok = nullptr;
    ...
    // once the context is up and the name is set (observability snapshot).
    state.publish_model_status(false, "");

    // Auto-detect format from path
    QuenchModelFormat format = quench::is_safetensors_dir(path) ? QUENCH_FORMAT_SAFETENSORS : QUENCH_FORMAT_GGUF;
    ...
    QuenchError err = quench_model_load_ex(path.c_str(), format, /*load_mtp_head=*/mtp_k > 0 ? 1 : 0,
                                     &state.model);
    if (err != QUENCH_SUCCESS) {
        std::string msg = std::string("Failed to load model: ") + quench_error_string(err);
        state.model = nullptr;
        return msg;
    }
    ...
    quench::set_pending_runtime_config(state.runtime_config);
    QuenchConfig config = build_config(state.default_args, state.runtime_config, path, config_overrides,
                                    state.model);
    err = quench_context_create(state.model, &config, &state.ctx);
    if (err != QUENCH_SUCCESS) {
        std::string msg = std::string("Failed to create context: ") + quench_error_string(err);
        quench_model_free(state.model);
        state.model = nullptr;
        return msg;
    }
    ...
    if (mtp_k > 0) {
        QuenchError mtp_err = quench_enable_mtp_spec_decode(state.ctx, mtp_k);
        if (mtp_err != QUENCH_SUCCESS)
            fprintf(stderr, "Warning: speculative.mtp_k=%d requested but MTP enable failed (%s); "
                    ...
                    mtp_k, quench_error_string(mtp_err));
    }
    ...
    // unaddressable over the HTTP API.
    ...
    const quench::ChatTemplate& engine_tpl = state.ctx->engine->chat_template();
    ...
    if (chat_tpl_name == "none") {
        ...
        auto family = quench::ChatTemplate::parse_family(chat_tpl_name);
    ...
    state.max_seq_len = quench_context_max_seq_len(state.ctx);
    if (state.max_seq_len <= 0)
        state.max_seq_len = quench_model_max_seq_len(state.model);
    if (state.max_seq_len <= 0)
    ...
    if (state.have_template) {
        printf("Chat template: %s\n", quench::chat_template_family_name(state.chat_tpl.family()));
    } else {
