...
void BatchingEngine::start(QuenchContext ctx) {
    if (running_.load()) {
        stop();
    }
    ctx_ = ctx;
...
void BatchingEngine::worker_loop() {
    quench::Engine* engine = ctx_->engine.get();
    quench::KVCacheManager* kv_mgr = engine->kv_manager();
    ...
    while (!stop_requested_.load(std::memory_order_relaxed)) {
            ...
            if (!pause_requested_.load(std::memory_order_relaxed)) {
                ...
                while (!pending_queue_.empty()) {
                    ...
                    if (sr->request->image && !engine->encode_image_for(*sr->request)) {
                        sr->request->status = quench::RequestStatus::CANCELLED;
        ...
        for (auto& sr : active_requests_) {
            if (sr->is_cancelled() && sr->request->status != quench::RequestStatus::FINISHED &&
                sr->request->status != quench::RequestStatus::CANCELLED) {
        ...
        try {
            ...
            QUENCH_LOG_ERROR("BatchingEngine: engine->step() threw %s — cancelling %zu active request(s)",
                          e.what(), active_requests_.size());
            for (auto& sr : active_requests_) {
                if (sr->request->status != quench::RequestStatus::FINISHED &&
                    sr->request->status != quench::RequestStatus::CANCELLED) {
                    sr->request->status = quench::RequestStatus::CANCELLED;
                    kv_mgr->free_sequence(sr->request->id);
                    engine->reset_ssm_state(sr->request->id);
            ...
            // the next forward() and the server returns garbage on every request.
            {
                ...
                if (cuda_error_is_unrecoverable(sync_err) || cuda_error_is_unrecoverable(sticky)) {
                    QUENCH_LOG_ERROR("BatchingEngine: CUDA context poisoned (%s / %s) — stopping the "
        ...
        quench::Tokenizer* tok = engine->model()->tokenizer();
        ...
        while (it != active_requests_.end()) {
            ...
            bool is_done = (req->status == quench::RequestStatus::FINISHED ||
                            req->status == quench::RequestStatus::CANCELLED);
            ...
            for (size_t i = sr->notified_count; i < current_count; i++) {
                ...
                if (is_last_token) {
                    ...
                    if (req->status == quench::RequestStatus::CANCELLED) {
                        ...
                        reason = req->cancel_reason == quench::CancelReason::KvCapacity ? "capacity"
                                                                                     : "cancelled";
                    } else {
                        if (token == tok->eos_id()) {
                            reason = "stop";
            ...
            if (is_done) {
                ...
                if (!had_new_tokens) {
                    const char* reason =
                        (req->status != quench::RequestStatus::CANCELLED) ? "length"
                        : (req->cancel_reason == quench::CancelReason::KvCapacity) ? "capacity"
                                                                                : "cancelled";
