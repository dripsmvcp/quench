...
int main(int argc, char** argv) {
    ...
    // Load quench.conf (if present) + apply --set overrides, then stash for
    // Engine::init to pick up.
    std::vector<std::string> rejected_overrides;
    quench::RuntimeConfig runtime_cfg =
        quench::RuntimeConfig::load(args.config_path, args.config_overrides, &rejected_overrides);
    if (!rejected_overrides.empty()) {
        ...
        fprintf(stderr, "See quench.conf.example for the key names.\n");
        return 1;
    }
    ...
    if (args.bench) {
        ...
        // Graph-captured verify changes verify-step timing (and pads
    ...
    quench::process_diag_install(runtime_cfg);
    quench::set_pending_runtime_config(runtime_cfg);
    ...
    printf("quench Inference Engine %s\n", quench_version());
    ...
    QuenchModelFormat format = QUENCH_FORMAT_GGUF;
    ...
    // Check if path is a directory with SafeTensors files
    {
        ...
        if (stat(args.model_path.c_str(), &st) == 0 && S_ISDIR(st.st_mode)) {
            ...
            if (stat(index.c_str(), &idx_st) == 0 || stat(single.c_str(), &idx_st) == 0) {
                format = QUENCH_FORMAT_SAFETENSORS;
            }
        }
    }

    if (format == QUENCH_FORMAT_GGUF) {
        resolved_model = quench::resolve_model_gguf(args.model_path, args.revision);
    ...
    printf("Loading model: %s (%s)\n", resolved_model.c_str(),
           format == QUENCH_FORMAT_SAFETENSORS ? "SafeTensors" : "GGUF");
    ...
    QuenchError err = quench_model_load_ex(resolved_model.c_str(), format,
                                     /*load_mtp_head=*/args.mtp_spec_decode_k > 0, &model);
    if (err != QUENCH_SUCCESS) {
        fprintf(stderr, "Error loading model: %s\n", quench_error_string(err));
        return 1;
    }
    ...
    quench::SamplingDefaults sampling = quench::get_sampling_defaults(model->model->config().arch);
    ...
    if (args.kv_fp8)
        config.kv_cache_dtype = QUENCH_DTYPE_FP8_E4M3;
    if (args.kv_int8)
        config.kv_cache_dtype = QUENCH_DTYPE_INT8;
    if (args.kv_int4)
        config.kv_cache_dtype = QUENCH_DTYPE_INT4;
    if (args.kv_nvfp4)
        config.kv_cache_dtype = QUENCH_DTYPE_NVFP4;
    if (args.kv_mxfp4)
        config.kv_cache_dtype = QUENCH_DTYPE_MXFP4_KV;
    if (args.ssm_fp16)
        config.ssm_state_dtype = QUENCH_DTYPE_FP16;
    ...
    if (!args.perplexity_file.empty()) {
        ...
        while ((r = std::fread(buf, 1, sizeof(buf), f)) > 0)
            text.append(buf, r);
        std::fclose(f);
        int vocab_size = quench_model_vocab_size(model);
        (void)vocab_size;
        int max_tok = static_cast<int>(text.size()) + 16;
        ppl_tokens.resize(max_tok);
        int n_tok = 0;
        QuenchError te = quench_tokenize(model, text.c_str(), ppl_tokens.data(), &n_tok, max_tok);
        if (te != QUENCH_SUCCESS || n_tok < 2) {
            fprintf(stderr, "Error tokenizing perplexity corpus (%s, n=%d)\n", quench_error_string(te), n_tok);
            quench_model_free(model);
            return 1;
        }
        ...
        int32_t bos = quench_model_bos_token(model);
        ...
        // per line on stderr) — cross-engine tokenizer forensics diffs
        ...
        // became chunk-aware: per-chunk NLL accumulation makes the
        ...
        // family beyond the S-matrix cap — the WMMA hd=256 remnant).
    ...
    err = quench_context_create(model, &config, &ctx);
    if (err == QUENCH_SUCCESS && args.mtp_spec_decode_k > 0) {
        QuenchError mtp_err = quench_enable_mtp_spec_decode(ctx, args.mtp_spec_decode_k);
        if (mtp_err != QUENCH_SUCCESS) {
            fprintf(stderr, "Warning: --mtp-spec-decode %d failed (%s); continuing without spec-decode\n",
                    args.mtp_spec_decode_k, quench_error_string(mtp_err));
        }
    }
    if (err != QUENCH_SUCCESS) {
    ...
    if (!args.perplexity_file.empty()) {
        double ppl = -1.0;
        QuenchError pe = quench_perplexity(ctx, ppl_tokens.data(), static_cast<int>(ppl_tokens.size()), &ppl);
        if (pe != QUENCH_SUCCESS) {
            fprintf(stderr, "perplexity failed: %s\n", quench_error_string(pe));
        ...
        if (!args.calibrate_out.empty()) {
            QuenchError ce = quench_calibration_write(ctx, args.calibrate_out.c_str());
            if (ce != QUENCH_SUCCESS) {
                fprintf(stderr, "calibration write failed: %s\n", quench_error_string(ce));
                quench_context_free(ctx);
                quench_model_free(model);
                return 1;
            }
    ...
    if (args.bench) {
        ...
        QuenchGenerateParams bench_params = quench_generate_params_default();
        bench_params.temperature = 0.0f;
        bench_params.ignore_eos = 1;  // Don't stop on EOS during benchmark
        // +1 because quench_prefill already produces the first output token;
        ...
        quench_prefill_with_params(ctx, tokens.data(), args.bench_pp, &bench_params);
        for (int s = 0; s < tg_tokens; s++) {
            int32_t tok = 0;
            quench_decode_step(ctx, &bench_params, &tok);
        }
        ...
        for (int rep = 0; rep < args.bench_reps; rep++) {
            ...
            err = quench_prefill_with_params(ctx, tokens.data(), args.bench_pp, &bench_params);
            auto t1 = std::chrono::high_resolution_clock::now();
            if (err != QUENCH_SUCCESS) {
                fprintf(stderr, "Prefill error on rep %d: %s\n", rep, quench_error_string(err));
                break;
            }
        ...
        for (int rep = 0; rep < args.bench_reps; rep++) {
            ...
            for (int s = 0; s < tg_tokens; s++) {
                int32_t tok = 0;
                err = quench_decode_step(ctx, &bench_params, &tok);
                if (err != QUENCH_SUCCESS)
                    break;
            }
            auto t1 = std::chrono::high_resolution_clock::now();
        ...
        quench::Tokenizer* tok = model->model->tokenizer();
        const quench::ChatTemplate& engine_tpl = ctx->engine->chat_template();

        // Resolve effective chat template: CLI override or engine-detected
        quench::ChatTemplate chat_tpl;
        ...
        if (args.chat_template == "none") {
            ...
            auto family = quench::ChatTemplate::parse_family(args.chat_template);
            if (family != quench::ChatTemplateFamily::RAW) {
        ...
        if (have_template) {
            printf("Chat template: %s\n", quench::chat_template_family_name(chat_tpl.family()));
        } else {
        ...
        std::vector<quench::ChatMessage> history;
        char line[4096];
        ...
        while (true) {
            ...
            if (input.rfind("/image ", 0) == 0) {
                ...
                const bool first = quench_pending_image_tokens(ctx) == 0;
                err = first ? quench_set_image(ctx, img_path.c_str()) : quench_add_image(ctx, img_path.c_str());
                if (err != QUENCH_SUCCESS) {
            ...
            if (have_template) {
                ...
                const int pending_img_tokens = quench_pending_image_tokens(ctx);
                ...
                err = quench_prefill_with_params(ctx, tokens.data(), static_cast<int>(tokens.size()), &params);
                if (err != QUENCH_SUCCESS) {
                ...
                char output[8192];
                size_t output_len = 0;
                err = quench_generate(ctx, input.c_str(), &params, output, sizeof(output), &output_len);
                if (err != QUENCH_SUCCESS) {
        ...
        if (args.prompt.empty()) {
            ...
            for (size_t i = 0; i < args.image_paths.size(); ++i) {
                ...
                err = (i == 0) ? quench_set_image(ctx, p) : quench_add_image(ctx, p);
                if (err != QUENCH_SUCCESS) {
                    fprintf(stderr, "Error loading image '%s': %s\n", p, quench_error_string(err));
                    quench_context_free(ctx);
                    quench_model_free(model);
                    return 1;
                }
            ...
            if (have_template && pending_img_tokens > 0) {
                ...
                for (size_t i = 0; i < counts.size(); ++i)
                    blocks += "<|vision_start|><|image_pad|><|vision_end|>";
                std::vector<quench::ChatMessage> msgs = {{"user", blocks + args.prompt}};
                tokens = chat_tpl.apply(*tok, msgs);
                const int32_t pad_id = tok->find_token("<|image_pad|>");
                std::string exp_err;
                if (pad_id < 0 || !quench::expand_image_placeholders(tokens, pad_id, counts, exp_err)) {
                ...
                std::vector<quench::ChatMessage> msgs = {{"user", args.prompt}};
            ...
            err = quench_prefill_with_params(ctx, tokens.data(), n_prompt_tokens, &params);
            ...
            for (int step = 0; step < params.max_tokens; step++) {
                ...
                // finishes the request the next quench_decode_step returns
                // QUENCH_ERROR_INTERNAL and we exit above. Bailing here on the
            ...
            // MTP telemetry: report draft accuracy if measured.
            if (quench::Engine* engine = ctx->engine.get(); engine && engine->mtp_spec_decode_enabled()) {
                ...
                // Tree-ceiling table: per-depth top-w hit rate.
                ...
                constexpr int W = quench::Engine::kMtpMeasureW;
                if (!cw.empty()) {
