...
void print_usage(const char* prog) {
    ...
    // and corrupted the output ("+4 pct" was the workaround in PR).
    ...
    fputs(
            ...
            "  --config <path>       quench.conf path (default: ./quench.conf, ~/.config/quench/quench.conf)\n"
            "  --set <sec.key=val>   Override one quench.conf key (repeatable; an\n"
            ...
            "  --decode-nvfp4-only   Force mode 2 (replacement: NVFP4-only, no duplicate FP8\n"
            "                        cache). Saves VRAM and gives faster prefill at -3 % decode\n"
            "                        — use for long-prompt workloads where prefill dominates\n"
            "                        wallclock.\n"
...
CliArgs parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // Shared quench-cli/quench-server flags first: a tool must not be able
