...
struct CliArgs : CommonArgs {
    // Shared flags live in CommonArgs. Inherited, not composed, so
    // every existing `args.<field>` use site is unchanged.

    // quench.conf integration. --config overrides the search-path default;
    // --set is a repeatable key=value (e.g. --set kv_cache.dtype=fp8) that
    // applies on top of the loaded config. See quench.conf.example for keys.
    ...
    // --perplexity pass and write them for quench-quantize --calib.
