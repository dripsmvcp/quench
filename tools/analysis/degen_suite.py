#!/usr/bin/env python3
"""Extended degeneration suite for quench-server — on-demand, server-level.

Probes a RUNNING quench-server through /v1/chat/completions (stream and
...
# Gemma channels/turns, Llama instruct, generic pads. (PR regression
...
class Suite:
    ...
    def cat_repetition(self):
        ...
        # NO coverage here, which is why issue shipped: DeviceTopK served
    ...
    # -- anthropic-thinking ---------------------------------------------------
    # /v1/messages DEFAULT path: thinking blocks must appear WITHOUT
    # --reasoning-format too, not only with it.
    ...
    # Agentic surface). quench's schema FSM is sampler-independent by construction,
    ...
    def cat_constrained(self):
        ...
        if calls:
            ...
            # Required-key presence is the target (prompt-hint-only today,
        ...
        # 6. Strict OPTIONAL tool calling: strict:true + tool_choice auto.
