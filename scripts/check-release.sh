...
# A "see docs/<name>.md" in a comment is a promise the file is there.
# Section 1 covers markdown links; this covers code and
...
# The version lives in two places that have to agree, and nothing pinned
# them: CMakeLists.txt is the source of truth and CHANGELOG.md must carry a
# released section for it. Bumping one and forgetting the other is the classic
# release slip — this makes it a failure.
section "version consistency"
CM_VER=$(sed -nE 's/^project\(quench .*VERSION ([0-9]+\.[0-9]+\.[0-9]+)\).*/\1/p' CMakeLists.txt | head -1)
CL_VER=$(grep -oE '^## \[[0-9]+\.[0-9]+\.[0-9]+\]' CHANGELOG.md | head -1 | tr -d '#[] ')
if [ -z "$CM_VER" ]; then
    fail "no project(quench ... VERSION X.Y.Z) in CMakeLists.txt"
elif [ "$CM_VER" = "$CL_VER" ]; then
    pass "CMakeLists and CHANGELOG both say $CM_VER"
else
    fail "version drift — CMakeLists '$CM_VER', CHANGELOG '${CL_VER:-none}'"
fi
...
if [ "${SKIP_VERIFY:-0}" = "1" ]; then
    ...
    if make verify-fast >/tmp/quench_check_release_verify.log 2>&1; then
        pass "make verify-fast"
    else
        echo "  log: /tmp/quench_check_release_verify.log"
        tail -30 /tmp/quench_check_release_verify.log | sed 's/^/  /'
