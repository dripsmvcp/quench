...
- **Same-session A/B.** Your merged tree and `main` are both built from source on a rented RTX 5090 and benched in one box session (`quench-cli --bench`, median of alternated reps). The verdict gates on decode throughput (`tg`) only — prefill varies up to 2.6× with cuBLAS algorithm selection — with a ±1.5% noise bar. Verdict ladder, strongest first: suite failure → `eval:reject`; any gated regression beyond the bar → `eval:reject` (a regression outranks any improvement); a taint → `eval:tainted`; gain beyond the bar → `eval:pass`; otherwise `eval:noise`.
- **Your PR does not grade itself.** `bench/` and `tests/` are overlaid from the base commit before your tree reaches the runner: your engine code is measured by main's bench and gated by main's tests against main's oracles. A PR that *modifies* pinned harness files (or the eval scripts, or workflows) still gets measured, but its best verdict is `eval:tainted` — that diff needs human review. The same cap applies to the supply-chain paths `cmake/`, `Dockerfile`, `docker-entrypoint.sh` and `docker-compose.yml`: they decide what the build pulls in and how the container starts, and nothing in the A/B measures them. Files *added* under pinned paths don't taint, but they also don't run during the eval; they join the gate once merged. Because the suite is compiled by your tree's CMakeLists, the pipeline also asserts your build's `ctest -N` list is a superset of main's.
...
- **The verdict is enforced, and it decides the merge.** It lands as the `quench/eval` commit status on your head sha (required by branch protection), an `eval:*` label, a PR comment with the measured table, and the git note. A new push resets the status and re-queues evaluation. `eval:pass` and `eval:noise` arm GitHub's squash auto-merge, so a measured, non-regressing PR merges without anyone clicking — once every required check is green. `eval:tainted`, `eval:reject` and `eval:error` hold the PR for a human. The bot itself calls no merge API; it only writes the label that `.github/workflows/auto-merge.yml` keys on.

- **Some paths always need a human, measured or not.** `.github/CODEOWNERS` names them: the eval bot and its policy, the pinned harness (`bench/`, `tests/`), CI and the merge workflow, and the supply-chain paths (`cmake/`, `Dockerfile`, `docker-entrypoint.sh`, `docker-compose.yml`). A PR touching any of them needs a code-owner approval before it can merge, on top of the checks. This is deliberately belt-and-braces with the taint policy above, which only applies if an eval actually ran — a PR touching only `cmake/` is not a runtime path, so it is never evaluated and the taint cap never gets a say. The bot can also be down. CODEOWNERS is enforced by GitHub either way, and the two lists are kept in sync by a test.
