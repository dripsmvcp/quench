...
public:
    bool alloc(size_t bytes, HostPinnedKind kind, void** out_host, void** out_device) override {
        ...
        if (herr != cudaSuccess) {
            QUENCH_LOG_WARN("pinned host alloc failed (%zu B, %s): %s", bytes,
        ...
        if (kind == HostPinnedKind::Mapped) {
            ...
            if (derr != cudaSuccess) {
                QUENCH_LOG_WARN("pinned host device pointer failed: %s", cudaGetErrorString(derr));
                cudaFreeHost(hp);
                return false;
            }
            if (out_device)
    ...
    void free(void* host) override {
        if (host)
            QUENCH_CUDA_CHECK_LOG(cudaFreeHost(host));
    }
};
...
public:
    bool register_read_only(void* ptr, size_t bytes) override {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_WARN("cudaHostRegister failed (%.2f MiB): %s — H2D will be slower",
