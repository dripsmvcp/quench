...
// Default cache directory: $XDG_CACHE_HOME/quench/warm, else $HOME/.cache/quench/warm,
// else /tmp/quench-warm-cache. Deliberately NOT next to the model: model mounts
...
static std::string default_warm_cache_dir() {
    if (const char* xdg = getenv("XDG_CACHE_HOME"); xdg && *xdg)
        return std::string(xdg) + "/quench/warm";
    if (const char* home = getenv("HOME"); home && *home)
        return std::string(home) + "/.cache/quench/warm";
    return "/tmp/quench-warm-cache";
}
...
WeightCacheFingerprint weight_cache_fingerprint(const std::string& model_path) {
    ...
    if (fs::is_directory(model_path, ec)) {
        const std::string cache_name = ".quench_warm_cache";
...
bool weight_cache_write(const Model& model, const std::string& cache_path) {
    ...
    if (recs.empty()) {
        QUENCH_LOG_INFO("Warm cache: nothing to persist — every upload is raw-from-source");
        return false;
    }
    ...
    if (!f) {
        ...
        QUENCH_LOG_INFO("Warm cache: %s not writable — skipping (set [warm_cache] dir to enable)",
                     tmp_path.c_str());
        return false;
    }
    ...
    if (sync != cudaSuccess) {
        QUENCH_LOG_WARN("Warm cache: device sync failed (%s) — skipping write", cudaGetErrorString(sync));
        return false;
    }
    ...
    for (const WeightUploadRecord* rec : recs) {
        ...
        for (const auto& a : rec->allocs) {
            ...
            if (cudaMemcpy(bounce.data(), a.ptr, a.bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
                QUENCH_LOG_WARN("Warm cache: D2H failed for %s — aborting write", rec->key.c_str());
                f.close();
                std::error_code ec;
    ...
    f.flush();
    if (!f) {
        QUENCH_LOG_WARN("Warm cache: write to %s failed (disk full?) — skipping", tmp_path.c_str());
        f.close();
        std::error_code ec;
        fs::remove(tmp_path, ec);
        return false;
    ...
    if (ec) {
        QUENCH_LOG_WARN("Warm cache: rename to %s failed: %s", cache_path.c_str(), ec.message().c_str());
        fs::remove(tmp_path, ec);
        return false;
    }
    QUENCH_LOG_INFO("Warm cache: persisted %zu transformed uploads (%.2f GiB) to %s", recs.size(),
...
std::unique_ptr<WeightSnapshot> weight_cache_load(const std::string& cache_path, const Model& model) {
    ...
    if (!take(&hdr, sizeof(hdr)) || memcmp(hdr.magic, kMagic, sizeof(kMagic)) != 0 ||
        ...
        QUENCH_LOG_INFO("Warm cache: %s has an incompatible format — ignoring (cold load)",
                     cache_path.c_str());
        return nullptr;
    }
    if (hdr.arch_id != static_cast<int32_t>(model.config().arch) || hdr.n_layers != model.n_layers()) {
        QUENCH_LOG_INFO("Warm cache: %s does not match this model (arch/layers) — ignoring", cache_path.c_str());
        return nullptr;
    }
    ...
    if (now.total_bytes != hdr.fp_total_bytes || now.mtime_max != hdr.fp_mtime_max) {
        QUENCH_LOG_INFO("Warm cache: %s is stale (model file changed) — ignoring (cold load)",
                     cache_path.c_str());
        return nullptr;
    }
    ...
    for (uint32_t i = 0; i < hdr.record_count; ++i) {
        ...
        if (!take(&rh, sizeof(rh)) || rh.key_len == 0 || rh.key_len > 256 || rh.n_allocs == 0 ||
            rh.n_allocs > 16) {
            QUENCH_LOG_WARN("Warm cache: %s truncated/corrupt at record %u — ignoring (cold load)",
                         cache_path.c_str(), i);
            return nullptr;
        }
        ...
        if (!take(rec.key.data(), rh.key_len) || !take(&rec.tensor, sizeof(Tensor))) {
            QUENCH_LOG_WARN("Warm cache: %s truncated in record %u — ignoring", cache_path.c_str(), i);
            return nullptr;
        }
        ...
        for (uint32_t a = 0; a < rh.n_allocs; ++a) {
            ...
            if (!take(&bytes, sizeof(bytes)) || bytes == 0 ||
                ...
                QUENCH_LOG_WARN("Warm cache: %s corrupt alloc size at record %u — ignoring",
                             cache_path.c_str(), i);
                return nullptr;
            }
    ...
    QUENCH_LOG_INFO("Warm cache: mapped %zu transformed uploads (%.2f GiB) from %s", snap->record_count(),
