...
void MemAccount::note(const char* pool, std::ptrdiff_t delta_bytes) {
    ...
    // unattributed_bytes() mean "what quench cannot account for"; gated, it is
    ...
    // from an empty ledger collapses Qwen3.6-35B-A3B-NVFP4's KV pool from
    // 4096 tokens to
    // 512, and a run that happens to have --mem-report on masks the failure.
    //
...
void MemAccount::sampler_start(int interval_us) {
    ...
    QUENCH_LOG_INFO("MemAccount: peak sampler started (interval=%d us)", sampler_interval_us_);
}
...
#pragma GCC diagnostic pop
    ...
    // (e.g. the ms_ref cudaFree's) sits here as reserved and still counts
    ...
    if (!pools_.empty()) {
        ...
        // structurally cannot see — not quench allocations at all in two of the
...
void trim_device_mempool() {
    ...
    if (cudaGetDevice(&dev) == cudaSuccess &&
        ...
        QUENCH_LOG_INFO("mempool trim: reserved %.0f->%.0f MiB used %.0f->%.0f MiB (rc=%s)",
