...
MemError GraphSlotPool::open(Backend& backend, HostPinnedAllocator& host, const GraphSlotCaps& caps,
    ...
    QUENCH_LOG_INFO("graph slot pool: %d slots (%.1f KiB device + %.1f KiB pinned host), caps: "
...
void GraphSlotPool::close() {
    ...
    if (leaked > 0) {
        ...
        QUENCH_LOG_ERROR("graph slot pool: closing with %d slot(s) still leased", leaked);
    }
    if (declines_exhausted_ || declines_too_small_) {
        QUENCH_LOG_INFO("graph slot pool: %llu declines (%llu exhausted, %llu too small)",
...
void graph_slot_pool_open_for(Backend& backend, int max_seq_len) {
    ...
    if (e != MemError::Ok)
        QUENCH_LOG_WARN("graph slot pool: open failed (%s) — per-burst allocation stays",
                     mem_error_name(e));
}
