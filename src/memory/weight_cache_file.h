...
// into a dedicated cache directory (~/.cache/quench/warm by default). Raw-from-source records are deliberately NOT stored: a
...
// ($XDG_CACHE_HOME/quench/warm, else ~/.cache/quench/warm, else
// /tmp/quench-warm-cache). The directory is created best-effort.
