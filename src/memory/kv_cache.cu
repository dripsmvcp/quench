...
KVCache::KVCache(int n_layers, int n_kv_heads, int head_dim, QType dtype, int max_blocks, int block_size,
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemset(pool_, 0, total));
    ...
    if (needs_scales) {
        ...
        // Always 2x: K scales region + V scales region
        ...
        if (!scale_pool_) {
            if (alloc_)
                ...
                QUENCH_CUDA_CHECK_LOG(cudaFree(pool_));
            pool_ = nullptr;
            char msg[256];
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemset(scale_pool_, 0, scale_total));
    }
...
KVCache::KVCache(int n_layers, const std::vector<int>& n_kv_heads_per_layer,
    ...
    if (dtype == QType::NVFP4 || dtype == QType::MXFP4_KV) {
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemset(scale_pool_, 0, sc_total));
    ...
    if (swa_max_blocks_ > 0) {
        ...
        for (char f : layer_is_swa_)
            if (f)
                n_swa_layers++;
        QUENCH_LOG_INFO("KVCache (per-layer): SWA group %d blocks × %d windowed layers "
                     "(global group %d blocks × %d layers)",
                     swa_max_blocks_, n_swa_layers, max_blocks_, n_layers_ - n_swa_layers);
    }

    QUENCH_LOG_INFO("KVCache (per-layer): %zu layers, pool %.2f MiB, max nkv=%d, max hd=%d", (size_t)n_layers_,
...
KVCache::~KVCache() {
    ...
    // away with the last int-based caller (A7 step 3).
    blocks_.abandon();
    swa_blocks_.abandon();
    if (d_copy_meta_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_copy_meta_));
    ...
    if (scale_pool_) {
        if (alloc_)
            ...
            QUENCH_CUDA_CHECK_LOG(cudaFree(scale_pool_));
        scale_pool_ = nullptr;
    }
    if (pool_) {
        if (alloc_)
...
int KVCache::allocate_block() {
    ...
    if (!ref) {
        if (graph_diag::enabled()) {
            QUENCH_LOG_ERROR("[graph_diag:kv_alloc] OOM (phase=%s, 0 free blocks left)",
    ...
    if (graph_diag::enabled()) {
        ...
        if (p == graph_diag::Phase::REPLAY) {
            ...
            QUENCH_LOG_INFO("[graph_diag:kv_alloc] allocate_block id=%d free_left=%d phase=%s", block_id,
...
void* KVCache::k_ptr(int layer, int block_id) {
    ...
    if (layer < 0 || layer >= n_layers_ || block_id < 0 || block_id >= max_blocks_) {
        QUENCH_LOG_ERROR("KV cache k_ptr bounds violation: layer=%d/%d, block=%d/%d", layer, n_layers_, block_id,
                      max_blocks_);
    }
#endif
...
#ifdef QUENCH_DEBUG
    if (layer < 0 || layer >= n_layers_ || block_id < 0 || block_id >= max_blocks_) {
        QUENCH_LOG_ERROR("KV cache v_ptr bounds violation: layer=%d/%d, block=%d/%d", layer, n_layers_, block_id,
                      max_blocks_);
    }
#endif
...
#ifdef QUENCH_DEBUG
    if (layer < 0 || layer >= n_layers_ || block_id < 0 || block_id >= max_blocks_) {
        QUENCH_LOG_ERROR("KV cache k_scale_ptr bounds violation: layer=%d/%d, block=%d/%d", layer, n_layers_,
...
#ifdef QUENCH_DEBUG
    if (layer < 0 || layer >= n_layers_ || block_id < 0 || block_id >= max_blocks_) {
        QUENCH_LOG_ERROR("KV cache v_scale_ptr bounds violation: layer=%d/%d, block=%d/%d", layer, n_layers_,
                      block_id, max_blocks_);
    }
#endif
    if (!layer_scale_block_bytes_.empty()) {
...
void KVCache::copy_blocks_device(const int* srcs, const int* dsts, int n_pairs,
    ...
    if (!d_copy_meta_) {
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemcpy(d_copy_meta_, meta.data(), meta.size() * sizeof(size_t),
