...
bool KVCacheManager::enable_residual_buffer(int max_seqs, int residual_n, VRAMAllocator* alloc) {
    if (residual_pool_) {
        QUENCH_LOG_WARN("KVCacheManager: residual buffer already enabled, ignoring re-enable");
        return false;
    }
    ...
    if (residual_n_kv_heads_ <= 0 || residual_head_dim_ <= 0) {
        QUENCH_LOG_WARN("KVCacheManager: residual buffer needs uniform per-layer head_dim — disabled (multi-head_dim model)");
        return false;
    }
    ...
    if (!pool) {
        QUENCH_LOG_ERROR("KVCacheManager: residual buffer allocation failed (%.2f MiB)",
    ...
    if (cudaMalloc(&d_residual_widx_, state_bytes) != cudaSuccess ||
        ...
        QUENCH_LOG_ERROR("KVCacheManager: residual state buffer alloc failed (%zu bytes)", state_bytes);
    ...
    QUENCH_LOG_INFO("KVCacheManager: residual buffer enabled — max_seqs=%d, residual_n=%d, %.2f MiB",
...
bool KVCacheManager::allocate_blocks(int seq_id, int num_blocks) {
    ...
    for (int i = 0; i < num_blocks; ++i) {
        BlockRef ref = allocate_block_ref_with_eviction();
        if (!ref) {
            // Rollback: dropping the refs added in *this* call frees them.
...
int KVCacheManager::append_block(int seq_id) {
    ...
    const int block_id = ref.id();
    it->second.push(std::move(ref));
    return block_id;
}
    ...
    for (size_t i = 0; i < seq.size(); ++i) {
        ...
        if (pinned_blocks_.find(block_id) != pinned_blocks_.end()) {
            ...
            if (cached_blocks_map_.find(block_id) == cached_blocks_map_.end()) {
                // The cache does not own this block yet: MOVE this sequence's
                // reference into it. Same refcount as before, now held by an
                // owner rather than retained by an omitted free.
                cached_blocks_lru_.push_back(block_id);
                cached_blocks_map_.emplace(
                    block_id, CachedEntry{std::prev(cached_blocks_lru_.end()), seq.take(i)});
                // NOT incrementing reclaimable_cached_count_ — pinned blocks excluded
            }
            // Otherwise the cache already owns it and this sequence's
            // reference simply drops with the table below.
            continue;
        }
...
int KVCacheManager::evict_lru() {
    ...
    // ones), and quench has no recompute-on-resume path, so freeing a live
...
int KVCacheManager::allocate_blocks_with_prefix(int seq_id, std::span<const int32_t> tokens,
    ...
    for (int b = 0; b < total_blocks; ++b) {
        ...
        if (prefix_caching_enabled_ && is_full_block) {
            ...
            if (hit != block_hash_to_id_.end() && cache_->ref_count(hit->second) == 0 &&
                ...
                QUENCH_LOG_WARN("prefix cache: stale hash entry for free block %d — dropping", hit->second);
            ...
            BlockRef plain = allocate_block_ref_with_eviction();
            if (!plain) {
                rollback_partial_allocation(seq_id, blocks, hashes, original_size);
                return -1;
            }

            blocks.push(std::move(plain));
            if (is_full_block) {
    ...
    if (reused_blocks > 0) {
        QUENCH_LOG_DEBUG("PrefixCache: seq %d reused %d/%d blocks (%d tokens skippable)", seq_id, reused_blocks,
...
void KVCacheManager::register_block_hashes(int seq_id, std::span<const int32_t> tokens, size_t content_salt) {
    ...
    for (int b = 0; b < total_blocks; ++b) {
        ...
        int block_id = blocks.id_at(static_cast<size_t>(b));
...
BlockRef KVCacheManager::allocate_block_ref_with_eviction() {
    ...
    return BlockRef();  // Caller should try evict_lru() if needed.
}
...
void KVCacheManager::pin_prefix(int seq_id, int num_blocks) {
    ...
    for (int i = 0; i < to_pin; ++i) {
        int bid = blocks.id_at(static_cast<size_t>(i));
        if (bid < 0)
    ...
    QUENCH_LOG_DEBUG("PinPrefix: seq %d pinned %d blocks (%zu unique pinned total)", seq_id, n_owned,
                  pin_refcount_.size());
}
...
void KVCacheManager::unpin_prefix(int seq_id) {
    ...
    QUENCH_LOG_DEBUG("UnpinPrefix: seq %d unpinned (%zu unique pinned remain)", seq_id,
                  pin_refcount_.size());
}
...
void KVCacheManager::enable_swa_sizing(int window_tokens, int slack_tokens) {
    if (!cache_->swa_enabled() || window_tokens <= 0) {
        QUENCH_LOG_WARN("KVCacheManager: enable_swa_sizing ignored (cache swa group %s, window=%d)",
    ...
    QUENCH_LOG_INFO("KVCacheManager: SWA sizing enabled (window=%d, slack=%d, group=%d blocks)",
...
bool KVCacheManager::enable_swa_snapshots() {
    ...
    if (cudaMallocHost(reinterpret_cast<void**>(&h_swa_descs_),
        ...
        cudaMalloc(reinterpret_cast<void**>(&d_swa_descs_),
                   static_cast<size_t>(cap) * sizeof(KVCache::CopyDesc)) != cudaSuccess) {
        QUENCH_LOG_WARN("KVCacheManager: SWA snapshot desc alloc failed — snapshots disabled");
        if (h_swa_descs_) {
            cudaFreeHost(h_swa_descs_);
            h_swa_descs_ = nullptr;
        }
    ...
    swa_desc_cap_ = cap;
    QUENCH_LOG_INFO("KVCacheManager: SWA snapshots enabled (%zu KiB/snapshot, %d layers x %d blocks)",
...
bool KVCacheManager::swa_snapshot_copy_(int seq_id, int upto_tokens, void* slab, bool to_slab,
    ...
    // one extra boundary block (the floor/ceil lesson) and zero-fill
    ...
    if (to_slab) {
        for (int b = first; b < needed_start; ++b) {
            if (swa[b] < 0) {
                QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(slab, 0, swa_snap_bytes_, stream));
                zero_filled = true;
                break;
            }
        }
    ...
    if (n == 0 || n > swa_desc_cap_)
        return false;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_swa_descs_, h_swa_descs_,
        ...
        if (id < 0) {
            ...
            for (int b2 = first; b2 < end_b; ++b2)
                if (std::find(fresh.begin(), fresh.end(), swa[b2]) != fresh.end())
                    swa[b2] = -1;
            QUENCH_LOG_WARN("KVCacheManager: SWA snapshot restore failed (group exhausted) — "
...
int KVCacheManager::evict_middle_blocks(int seq_id, int n_sink_tokens, int n_window_tokens) {
    ...
    // Retain ONE extra boundary block beyond the ceil-aligned window:
    ...
    for (int i = sink_end_block; i < window_start_block; ++i) {
        ...
        // Dropping the reference frees the block; the SLOT stays, as a hole,
        // so the table length (and the kernels' positional alignment) is
        // unchanged.
        blocks.make_hole(static_cast<size_t>(i));
        ++freed;
    }
...
void KVCacheManager::rollback(int seq_id, int new_seq_len) {
    ...
    while (static_cast<int>(blocks.size()) > blocks_needed) {
        ...
        drop_stale_hash_if_last(blocks.back_id());
        blocks.pop_back();  // dropping the reference is the free
    }
...
int KVCacheManager::save_prefix_cache(const std::string& path, uint64_t model_fingerprint,
    ...
    if (!prefix_caching_enabled_ || cached_blocks_lru_.empty()) {
        QUENCH_LOG_INFO("Prefix cache: nothing to save (0 cached blocks)");
        return 0;
    }
    ...
    if (!f) {
        if (fd >= 0)
            ::close(fd);
        QUENCH_LOG_ERROR("Failed to open %s for writing", path.c_str());
        return -1;
    }
    ...
    if (fwrite(&hdr, sizeof(hdr), 1, f) != 1) {
        QUENCH_LOG_ERROR("Failed to write prefix cache header to %s", path.c_str());
        fclose(f);
        return -1;
    }
    ...
    if (entries.empty()) {
        fclose(f);
        QUENCH_LOG_INFO("Prefix cache: nothing to save (0 valid cached blocks)");
        return 0;
    }
    ...
    for (size_t bi = 0; bi < entries.size(); ++bi) {
        ...
        for (int l = 0; l < nl; l++) {
            ...
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed for block %d layer %d K: %s", block_id, l,
            ...
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed for block %d layer %d V: %s", block_id, l,
                              cudaGetErrorString(err));
                block_ok[bi] = false;
                break;
            }
            ...
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed for block %d layer %d scales: %s", block_id, l,
                              cudaGetErrorString(err));
                block_ok[bi] = false;
                break;
            }
    ...
    for (size_t bi = 0; bi < entries.size(); ++bi) {
        ...
        if (fwrite(&hash, sizeof(hash), 1, f) != 1) {
            QUENCH_LOG_ERROR("Failed to write block hash to %s", path.c_str());
            fclose(f);
            return -1;
        }
        if (fwrite(host_buf.data() + bi * per_block_total, per_block_total, 1, f) != 1) {
            QUENCH_LOG_ERROR("Failed to write block data to %s", path.c_str());
            fclose(f);
            return -1;
        }
        saved++;
    ...
    fclose(f);
    QUENCH_LOG_INFO("Prefix cache: saved %d blocks (%.1f MiB) to %s", saved,
...
int KVCacheManager::load_prefix_cache(const std::string& path, uint64_t model_fingerprint,
                                      cudaStream_t stream) {
    if (!prefix_caching_enabled_) {
        QUENCH_LOG_WARN("Prefix cache: loading disabled (prefix caching not enabled)");
        return -1;
    }

    FILE* f = fopen(path.c_str(), "rb");
    if (!f) {
        QUENCH_LOG_INFO("Prefix cache: no cache file at %s", path.c_str());
        return 0;
    }

    PrefixCacheHeader hdr{};
    if (fread(&hdr, sizeof(hdr), 1, f) != 1 || hdr.magic != kPrefixCacheMagic) {
        QUENCH_LOG_WARN("Prefix cache: invalid header in %s", path.c_str());
        fclose(f);
        return -1;
    }

    if (hdr.version != kPrefixCacheVersion) {
        QUENCH_LOG_WARN("Prefix cache: version mismatch (%u vs %u)", hdr.version, kPrefixCacheVersion);
        fclose(f);
        return -1;
    }
    ...
    if (hdr.model_fingerprint != model_fingerprint) {
        QUENCH_LOG_WARN("Prefix cache: model fingerprint mismatch (cache=0x%016llx, current=0x%016llx) "
    ...
    if (hdr.scale_block_bytes != sbb) {
        QUENCH_LOG_WARN("Prefix cache: scale geometry mismatch (cache=%llu, current=%zu) — discarding",
    ...
    struct LoadEntry {
        ...
        bool copy_ok;
        BlockRef ref;  // owns the block until it is handed to the cached LRU
    };
    ...
    for (int i = 0; i < n_blocks; i++) {
        ...
        BlockRef block_ref = cache_->acquire_block_ref();
        const int block_id = block_ref.id();
        if (!block_ref) {
            QUENCH_LOG_INFO("Prefix cache: pool exhausted after %d blocks", loaded);
            break;
        }
        ...
        load_entries.push_back({hash, block_id, buf_offset, true, std::move(block_ref)});
        loaded++;
    }
    ...
    for (auto& entry : load_entries) {
        ...
        for (int l = 0; l < nl; l++) {
            cudaError_t err = cudaMemcpyAsync(cache_->k_ptr(l, entry.block_id),
                                              ...
                                              cudaMemcpyHostToDevice, stream);
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed loading block %d layer %d K: %s", entry.block_id, l,
                              cudaGetErrorString(err));
                entry.copy_ok = false;
                break;
            }
            ...
            err = cudaMemcpyAsync(cache_->v_ptr(l, entry.block_id),
                                  ...
                                  stream);
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed loading block %d layer %d V: %s", entry.block_id, l,
                              cudaGetErrorString(err));
                entry.copy_ok = false;
                break;
            }
            ...
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("cudaMemcpyAsync failed loading block %d layer %d scales: %s", entry.block_id,
    ...
    QUENCH_LOG_INFO("Prefix cache: restored %d blocks (%.1f MiB) from %s%s", loaded,
