...
std::string library_reserve_cache_default_path() {
    if (const char* xdg = std::getenv("XDG_CACHE_HOME"); xdg && *xdg)
        return std::string(xdg) + "/quench/library_reserve";
    if (const char* home = std::getenv("HOME"); home && *home)
        return std::string(home) + "/.cache/quench/library_reserve";
    return {};
}
...
bool library_reserve_cache_store(const std::string& path, const LibraryReserveKey& key,
    ...
    const std::string tmp = path + ".tmp";
    {
        ...
        if (!out)
            return false;
        out << "# quench library-reserve cache — what the first forward pass actually\n"
               "# claimed, per (model, quant path, CUDA runtime). Safe to delete;\n"
               "# quench re-measures and rewrites it. See docs/MEMORY_ARCHITECTURE.md A1.5.\n";
