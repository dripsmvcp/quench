...
// own budgets from live free VRAM again. The guarantee against that is a
// planned floor, deliberately NOT a physical balloon allocation whose only
// job would be to hide
// bytes from the planner — but the live re-derivation is still here.
//
...
// The fixed charge that is not quench's memory and cannot be planned away.
// Measured: ~3.9 GiB claimed by CUDA/cuBLAS/CUTLASS on the first forward
...
// Measured on the target (RTX 5090 / sm_120a, CUDA 13.3, WSL2 WDDM
...
// controls. Refuted as explanations: quench's own lazy module loading
// (CUDA_MODULE_LOADING=EAGER moves 124 MiB), and the default cudaMallocAsync
// pool (reserved/used unchanged across the request).
//
// Re-measure after a driver or CUDA bump; quench.conf `vram.library_reserve_mb`
...
struct PlanInput {
    ...
    // WSL2/WDDM box). Gone before quench allocates anything.
...
struct MemoryPlan {
    ...
    // fits and leaves the rest as native blocks), so committing it whole would
    // make the plan reject configurations that serve fine.
