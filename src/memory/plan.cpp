...
PlanResult plan_memory(const PlanInput& in) {
    ...
    // itself drop under pressure is not a commitment.
    ...
    if (p.kv.bytes > 0) {
        // FP8 KV halves the pool; the accuracy trade is per-family (see the
        // kv_fp8 allowlists in src/model/model.cpp), so it is offered, not applied.
