...
void RecurrentSnapshotStore::init(size_t entry_bytes, size_t budget_bytes) {
    ...
    if (want <= 0) {
        QUENCH_LOG_INFO("RecurrentSnapshotStore: disabled (budget %.0f MiB < one %.1f MiB slot)",
    ...
    if (capacity_ == 0)
        pool_.reset();
    QUENCH_LOG_INFO("RecurrentSnapshotStore: %d/%d slots x %.1f MiB pre-allocated (budget %.0f MiB)",
