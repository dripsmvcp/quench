...
//
// Multiple quench-server processes sharing one GPU need each process to size
...
// memory, and NOT charged against the budget — the budget covers what quench
...
// The pct parameter is a budget-planner knob (quench.conf vram.reserve_floor_pct,
...
// was that mismatch between Engine::init and the VRAM budget (1118 MiB
// on a 32 GiB card); was the same mismatch in the NVFP4 MoE cache's
...
//
// `Floored` has had its own message: nothing was left to size
...
// `pending_reserve_bytes` is memory that is still FREE at sizing time but
// already spoken for by a claim that lands after it — the library reserve,
// which cuBLAS/CUTLASS take on the first forward pass. The measured residual
// contains those bytes, so a clamp that ignores them sizes the pool into the
// claim: on Mistral-Nemo-12B Q8_0 / 32 GiB the clamp kept 1605 MiB of
// headroom against a 2297 MiB pending claim, the first forward ran the card
// to 2 MiB free, and warmup graph capture OOM'd the CUDA context. The
// parameter is not defaulted on purpose: whether a pending claim exists is a
// question every caller must answer, not one it can silently skip.
inline KvResidualSizing kv_blocks_from_residual(size_t free_bytes, size_t headroom_bytes,
    ...
    if (per_block_bytes == 0 || planned_blocks <= 0)
        return out;
    const size_t reserved = headroom_bytes + pending_reserve_bytes;
    const size_t room = (free_bytes > reserved) ? (free_bytes - reserved) : 0;
