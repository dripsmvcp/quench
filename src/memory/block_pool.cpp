...
void BlockPool::close() {
    ...
    if (outstanding != 0) {
        QUENCH_LOG_ERROR("BlockPool::close with %zu blocks still referenced — a block outlived its "
...
void BlockPool::dec_ref_impl_(int id, bool strict) {
    ...
    if (rc <= 0) {
        ...
        if (!strict)
            return;
        QUENCH_LOG_ERROR("BlockPool: dec_ref on block %d with refcount %d (double release)", id, rc);
#ifndef NDEBUG
        std::abort();
#endif
        return;
