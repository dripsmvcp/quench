...
public:
    ...
    // WARNING: every lru_order_ entry is a LIVE sequence and quench has no
    ...
    //
    // This class provides the allocation + accessor surface only; the
    // decode kernel and the write path build on top of it.
    //
...
private:
    ...
    // Without it, free_sequence() would have to deliberately
    // skip the free so the block survives at refcount 1 — liveness by
    // omission. Erasing an entry drops the reference, which is what
