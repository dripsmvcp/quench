...
void vram_budget_install(size_t budget_mb) {
    ...
    if (!have_info) {
        QUENCH_LOG_WARN("vram_budget: cudaMemGetInfo failed — budget disabled");
        g_budget_bytes.store(0, std::memory_order_relaxed);
        return;
    }
    if (budget > total_b) {
        QUENCH_LOG_WARN("vram_budget: %zu MiB exceeds device total %zu MiB — clamping", budget_mb,
                     total_b >> 20);
        budget = total_b;
    }
    g_free_at_install.store(free_b, std::memory_order_relaxed);
    g_budget_bytes.store(budget, std::memory_order_relaxed);
    QUENCH_LOG_INFO("VRAM budget: %.0f MiB (device free at install: %.0f MiB) — sizing sees a "
                 "virtual %.0f MiB GPU",
...
bool vram_budget_mem_get_info(size_t* free_bytes, size_t* total_bytes) {
    ...
    // which the peak forms. Once serving starts quench allocates nothing (I2), so
