...
bool VRAMAllocator::init(float headroom_fraction) {
    ...
    if (!vram_budget_mem_get_info(&free_mem, &total_)) {
        QUENCH_LOG_ERROR("VRAMAllocator: cudaMemGetInfo failed");
        return false;
    }
    ...
    QUENCH_LOG_INFO("VRAMAllocator: total=%.0f MiB, headroom=%.0f MiB (%.0f%%)", total_ / (1024.0 * 1024.0),
...
void* VRAMAllocator::allocate(size_t bytes, const char* tag, bool bypass_headroom) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("VRAMAllocator: cudaMalloc failed for %s (%.2f MiB): %s", tag,
...
void VRAMAllocator::report() const {
    ...
    for (const auto& [tag, info] : by_tag) {
        QUENCH_LOG_INFO("  %-24s %8.1f MiB  (%d allocs)", tag.c_str(), info.first / (1024.0 * 1024.0),
                     info.second);
    }
}
