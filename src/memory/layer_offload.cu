...
LayerOffloadManager::~LayerOffloadManager() {
    for (auto& slot : slots_) {
        if (slot.gpu_buf) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(slot.gpu_buf));
            slot.gpu_buf = nullptr;
        }
        if (slot.ready_event) {
            QUENCH_CUDA_CHECK_LOG(cudaEventDestroy(slot.ready_event));
            slot.ready_event = nullptr;
        }
        if (slot.transfer_stream) {
            QUENCH_CUDA_CHECK_LOG(cudaStreamDestroy(slot.transfer_stream));
...
bool LayerOffloadManager::init(Model* model, int gpu_layers) {
    ...
    if (n_offloaded == 0 || max_layer_bytes == 0) {
        enabled_ = false;
        QUENCH_LOG_INFO("Layer offloading: nothing to offload");
        return true;
    }
    ...
    for (int s = 0; s < 2; s++) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to allocate offload slot %d (%zu bytes): %s", s, max_layer_bytes,
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to create offload event: %s", cudaGetErrorString(err));
            return false;
        }

        err = cudaStreamCreateWithFlags(&slots_[s].transfer_stream, cudaStreamNonBlocking);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to create offload stream: %s", cudaGetErrorString(err));
            return false;
        }
    }
...
void LayerOffloadManager::upload_layer_to_slot(int layer, int slot_idx) {
    ...
    for (const auto& e : entries) {
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyWithAttributesAsync(dst_base + e.offset_in_slot, e.host_ptr,
                                                         e.raw_bytes, &attrs, slot.transfer_stream));
    }

    // Record event so compute stream can wait on it
    QUENCH_CUDA_CHECK_LOG(cudaEventRecord(slot.ready_event, slot.transfer_stream));
...
void LayerOffloadManager::ensure_layer(int layer, cudaStream_t compute_stream) {
    ...
    for (int s = 0; s < 2; s++) {
        if (slots_[s].loaded_layer == layer) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaStreamWaitEvent(compute_stream, slots_[s].ready_event, 0));
    ...
    // Wait for transfer
    QUENCH_CUDA_CHECK_LOG(cudaStreamWaitEvent(compute_stream, slots_[target_slot].ready_event, 0));
