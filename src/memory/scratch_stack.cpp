...
void ScratchStack::close() {
    ...
    if (!region_.valid())
        return;
    if (depth_ != 0) {
        QUENCH_LOG_ERROR("ScratchStack::close with %u live marks — a forward pass did not unwind",
                      depth_);
#ifndef NDEBUG
        std::abort();
#endif
...
void ScratchStack::rewind_(size_t offset, uint32_t depth) {
    ...
    if (depth != depth_) {
        ...
        QUENCH_LOG_ERROR("ScratchStack: mark released out of order (depth %u, expected %u) — LIFO "
                      "discipline violated",
                      depth, depth_);
#ifndef NDEBUG
        std::abort();
