...
// forward pass. `kMeasuredLibraryReserveBytes` is a single constant, and it is
...
// Default cache location. `$XDG_CACHE_HOME/quench/library_reserve` when set,
// otherwise `$HOME/.cache/quench/library_reserve`. Empty when neither is set —
...
// record 0, and a `> 0` test on the return value would silently throw their
// measurement away and charge the 3900 MiB constant instead.
