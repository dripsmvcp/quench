...
// (docs/MEMORY_ARCHITECTURE.md A6).
//
...
// redirects quench's references to __wrap_*, we record and forward to __real_*.
//
...
//
// Built only when QUENCH_ALLOC_INTERPOSE=ON, because --wrap has to be on the
...
void print_sites() {
    ...
    QUENCH_LOG_DEBUG("    by call site (addr2line -e <binary> <offset>):");
    for (const auto& s : g_sites) {
        ...
        QUENCH_LOG_DEBUG("      %8llu calls  %9.3f MiB   %s +0x%llx", (unsigned long long)s.calls,
                      s.bytes / (1024.0 * 1024.0), obj, off);
    }
    if (g_sites_overflow_calls)
        QUENCH_LOG_DEBUG("      %8llu calls  (further sites, table full)",
                      (unsigned long long)g_sites_overflow_calls);
}

void record(Counter& c, size_t bytes, quench::RegionTag tag, const void* site, bool device) {
    if (quench::alloc_phase() != quench::AllocPhase::Serving)
        return;
    c.calls.fetch_add(1, std::memory_order_relaxed);
    c.bytes.fetch_add(bytes, std::memory_order_relaxed);
    tally_site(site, bytes);
    if (device)
        quench::note_serving_allocation(tag, bytes, site);
}
...
cudaError_t __wrap_cudaMalloc(void** devPtr, size_t size) {
    record(g_dev_sync, size, quench::RegionTag::Other, __builtin_return_address(0), true);
...
cudaError_t __wrap_cudaMallocAsync(void** devPtr, size_t size, cudaStream_t stream) {
    record(g_dev_async, size, quench::RegionTag::Other, __builtin_return_address(0), true);
...
cudaError_t __wrap_cudaMallocHost(void** ptr, size_t size) {
    record(g_host_pinned, size, quench::RegionTag::HostStaging, __builtin_return_address(0), false);
