...
bool SSMState::init(int n_ssm_layers, int max_sequences, int conv_channels, int conv_kernel, int n_heads,
    ...
    if (alloc_) {
        ...
        if (!pool_) {
            QUENCH_LOG_WARN("SSM state: allocator rejected, trying raw cudaMalloc (%zu bytes)", total_bytes_);
    ...
    if (!pool_) {
        QUENCH_LOG_ERROR("Failed to allocate SSM state pool (%zu bytes)", total_bytes_);
        return false;
    }

    // Zero-initialize all state
    QUENCH_CUDA_CHECK_LOG(cudaMemset(pool_, 0, total_bytes_));
...
void SSMState::reset_sequence(int seq_id, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(base, 0, per_seq_bytes_, stream));
}
