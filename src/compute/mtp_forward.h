...
//
// Phase status:
//   - 2.1 (shipped PR): reduced forward (emb → pre_fc_norm → fc →
//          final_norm → lm_head → argmax). Skips the transformer block.
//          Acceptance rate will be far below trained-MTP optimum.
//   - 2.2.MoE (this file): MoE block plumbed via existing quench::gemm /
...
struct MtpDraftWorkspace {
    ...
    // degrading acceptance with position (issue). rope_freq_scale is the
...
// identically on rope-scaled models (issue): inv_scaling = 1/rope_freq_scale,
