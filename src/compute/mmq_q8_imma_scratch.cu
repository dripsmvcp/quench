...
bool imma_ensure_act(int M, int K, bool capturing) {
    ...
    if (g_imma_act.xs8) {
        ...
        // arena — cannot happen.
        QUENCH_LOG_WARN("mmq_q8_imma: activation scratch regrew to M=%d K=%d (%.1f MiB) past the "
    ...
    if (xs8.empty() || xscale.empty() || xrowsum.empty()) {
        QUENCH_LOG_WARN("mmq_q8_imma: activation scratch for M=%d K=%d unavailable from the T2 arena "
...
bool imma_ensure_splitk(size_t floats, bool capturing) {
    ...
    if (g_imma_splitk.buf) {
        QUENCH_LOG_WARN("mmq_q8_imma: split-K scratch regrew to %.1f MiB — the N*used bound in "
...
void imma_quantize_act(const __half* x, int M, int K, cudaStream_t stream) {
    ...
    // REUSED across layers with the same pointer — a (ptr, M, K) memo would
    // serve layer-1 activations to every later layer (PPL 31.6 → 441k). The
    // kernel costs ~7 µs; quantize unconditionally.
...
void mmq_q8_imma_preallocate(int rows, int k) {
    ...
    if (!imma_ensure_act(rows, k, /*capturing=*/false)) {
        QUENCH_LOG_WARN("mmq_q8_imma: could not preallocate the %dx%d activation scratch — the IMMA "
    ...
    QUENCH_LOG_DEBUG("mmq_q8_imma: preallocated %dx%d activation scratch (%.1f MiB)", rows, k,
