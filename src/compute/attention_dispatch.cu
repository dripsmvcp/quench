...
int get_device_sm_version() {
    ...
    QUENCH_LOG_INFO("Device SM version: %d.%d (sm_%d)", major, minor, cached_sm_version);
    return cached_sm_version;
}
...
void attention_prefill_dispatch(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor& O, float scale,
    ...
    // Learned attention sinks (gpt-oss/): only the FP16 WMMA FMHA
    ...
    // silently wrong output (the pre- executor WARN case).
    if (has_sinks) {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "declined head_dim=%d (or fmha_sm120=never) — no sink-capable kernel",
    ...
    // ( SageAttention3-recipe spike) are read from process_diag inside
    // the launcher.
    ...
    // via [attention] fmha_fa2, env QUENCH_FMHA_FA2. head_dim=128. QK^T runs in f16
    ...
    // scores compound per layer into garbage on real activations.
    if (rcfg.attention.fmha_fa2 == "on") {
        ...
        if ((sup.fa2_accepts = fmha_sm120_fa2_prefill(Q, K, V, O, scale, causal, sliding_window, softcap,
            ...
            QUENCH_LOG_DEBUG("FMHA dispatch: using FA2 register-resident kernel (hd=%d)",
                          static_cast<int>(Q.shape[3]));
            return;
        }
    ...
    // score error that compounds across layers: teacher-forced PPL
    ...
    if (rcfg.attention.fp8_fmha == "on") {
        ...
        if (sup.fp8_accepts) {
            ...
            QUENCH_LOG_DEBUG("FMHA dispatch: using FP8 sm120 kernel (hd=%d)", static_cast<int>(Q.shape[3]));
            return;
        }
    }
    ...
    // produced garbage logits (teacher-forced PPL ~1e10,). Reaching this
    ...
    snprintf(msg, sizeof(msg),
             ...
             "(check attention.fmha_sm120/fmha_fa2 config)",
             static_cast<int>(Q.shape[3]));
    throw std::runtime_error(msg);
}
