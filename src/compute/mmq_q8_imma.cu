...
// notes (SMEM-staged scales, 128x128x64 tiles, +16-B row pad killing 28.3M
// bank conflicts, one syncthreads pair per K-step; 3-stage cp.async
// REGRESSED -21% — do not re-add).
//
...
// K-loop latency (the spec-decode verify bottleneck, issue). Each split
...
// TUNING LADDER (Qwen3-8B Q8_0 pp512, baseline 12 131 tok/s) —
...
//   NT=64 tile (acc 32):       9 685 (-20%) — bigger tiles win, consistent
//                              with the +108% that tile growth bought.
//                              1 CTA/SM stands.
//   ldmatrix.x4 A+B fetch:    11 619 (-4%)  — smem fetch is NOT the binding
//                              constraint after the +16-B row pad.
// Remaining structural ideas (BK=128 sync-halving w/ dynamic smem, warp
// specialization) are larger rewrites. Spend elsewhere first.
bool gemm_common(const void* w_blocks, int qkind /*0=q8 1=q4k 2=q6k*/, const __half* x_f16,
    ...
    // bottleneck (issue). Split the K-steps across gridDim.z into fp32
...
bool mmq_imma_moe_gemm(const void* w_blocks, int qkind, const __half* x_f16, __half* out_f16,
    ...
    if (ok && !logged) {
        logged = true;
        QUENCH_LOG_INFO("MoE IMMA prefill ACTIVE (%s, ne=%d N=%d K=%d max_rows=%d)",
                     qkind == 1 ? "Q4_K"
