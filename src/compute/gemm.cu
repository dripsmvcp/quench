...
#define CUBLASLT_CHECK(call)                                                    \
    do {                                                                        \
        ...
        if (_st != CUBLAS_STATUS_SUCCESS) {                                     \
            QUENCH_LOG_ERROR("quench::gemm: %s failed (status %d)", #call, (int)_st); \
        }                                                                       \
    } while (0)

namespace quench {

// graph-captured verify: see gemm.h. Single engine thread writes it;
...
static cublasHandle_t get_cublas_handle() {
    if (!s_cublas_handle) {
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            QUENCH_LOG_ERROR("quench::gemm: cublasCreate failed (status %d)", (int)st);
            abort();
        }
...
static cublasLtHandle_t get_cublaslt_handle() {
    if (!s_cublaslt_handle) {
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            QUENCH_LOG_ERROR("quench::gemm: cublasLtCreate failed (status %d)", (int)st);
            abort();
        }
    }
...
static cudaDataType_t dtype_to_cuda(QType dt) {
    switch (dt) {
        ...
        case QType::INT32:
            return CUDA_R_32I;
        default:
            QUENCH_LOG_ERROR("quench::gemm: unsupported dtype %d", std::to_underlying(dt));
...
//     spread, not guessed — a 3 % margin sits below the noise and leaves the
//     picks unstable.
...
static int gemm_algo_log_enabled() { return quench::process_diag_log_gemm_algo() ? 1 : 0; }
...
static void benchmark_and_select_algo(cublasLtHandle_t lt, GemmCacheEntry& entry, const void* A_data,
    ...
    if (gemm_algo_log_enabled() && M > 0) {
        QUENCH_LOG_DEBUG("[gemm-algo] shape M=%d N=%d K=%d  candidates=%d", M, N, K, nresults);
        for (int i = 0; i < nresults; i++) {
            ...
            QUENCH_LOG_DEBUG("[gemm-algo]   cand[%d]: numImplFlags=0x%x tile=%d ws=%zu", i, algo_id, tile_id,
    ...
    const bool s_deterministic_gemm = quench::process_diag_deterministic_gemm();
    if (s_deterministic_gemm || nresults == 1) {
        ...
        // model-wide on models FA2 doesn't serve — pre- that included all
        ...
        entry.has_algo = true;
        if (gemm_algo_log_enabled() && M > 0) {
            QUENCH_LOG_DEBUG("[gemm-algo]   PICKED cand[%d] (deterministic, warmup-validated)", pick);
        }
        return;
    }
    ...
    if (gemm_algo_log_enabled() && M > 0) {
        for (int i = 0; i < nresults; i++)
            QUENCH_LOG_DEBUG("[gemm-algo]   cost[%d] per_ms=%.5f reps=%d%s", i, cand_ms[i], reps[i],
                          algo_ok[i] ? "" : " REJECTED");
    }
    ...
    if (gemm_algo_log_enabled() && M > 0) {
        ...
        QUENCH_LOG_DEBUG("[gemm-algo]   PICKED cand[%d] tile=%d  best_ms=%.3f", best_idx, picked_tile, best_ms);
    }
}
...
namespace {
QUENCH_REGISTER_CUDA_STATIC_RESET(gemm_reset_static_cuda_state);
}  // namespace
...
static void gemm_cublaslt_generic(const Tensor& A, const Tensor& B, Tensor& C, float alpha, float beta,
    ...
    // the graph-captured verify chunk warms every shape eagerly before
    ...
    if (A.qtype != C.qtype && A.qtype == QType::F16 && B.qtype == QType::F16 && C.qtype == QType::F32) {
        ...
        cublasStatus_t st = cublasGemmEx(fb_handle, CUBLAS_OP_T, CUBLAS_OP_N, (int)N, (int)M, (int)K, &alpha,
                                         ...
                                         CUBLAS_GEMM_DEFAULT);
        if (st != CUBLAS_STATUS_SUCCESS) {
            QUENCH_LOG_WARN("gemm: cublasGemmEx FP16→FP32 failed status=%d M=%ld K=%ld N=%ld", (int)st, (long)M,
                         (long)K, (long)N);
        }
        return;
    }
    ...
    if (compute_type == CUBLAS_COMPUTE_32I) {
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            ...
            if (fb_st != CUBLAS_STATUS_SUCCESS) {
                ...
                // the throw is translated to QuenchError at the API boundary.
                char msg[192];
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            ...
            if (st != CUBLAS_STATUS_SUCCESS) {
                ...
                if (fb_st != CUBLAS_STATUS_SUCCESS) {
                    ...
                    // garbage; continuing corrupts the forward pass silently
                    // (repeated-token gibberish + downstream IMA). Fail loudly
                    // instead — the throw is translated to QuenchError at the API
...
void gemm_cublaslt(const Tensor& A, const Tensor& B, Tensor& C, float alpha, float beta, const float* aScale,
    ...
    // This is safe because quench enforces single-stream GEMM execution — all GEMM
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            ...
            if (st != CUBLAS_STATUS_SUCCESS) {
                ...
                if (d_a16 && d_b16) {
                    ...
                    QUENCH_LOG_ERROR("gemm_cublaslt: FP8 dequant fallback alloc failed M=%ld K=%ld N=%ld",
