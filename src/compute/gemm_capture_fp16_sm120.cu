...
bool gemm_capture_fp16_sm120(const void* A, const void* B, void* D, int M, int N, int K, float alpha,
    ...
    // status 14 and abort the whole decode graph (, GDN N=32 projections).
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("gemm_capture_fp16_sm120: launch failed M=%d N=%d K=%d BM=%d smem=%zu: %s", M, N, K,
