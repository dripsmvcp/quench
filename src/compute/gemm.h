...
// Graph-captured verify: allow cuBLASLt calls to record into an active
...
//
// LAYOUT: qs first + alignas(16) + pad to a 48-B stride. The ggml-style
// 36-B layout puts qs at offset 4 — never 16-B aligned — which chains every
// dp4a GEMV activation read to 8× LDG.32 per block (the measured structural
// ceiling of the kernel family: 31-42% DRAM BW). With qs at offset 0 and
// 16-B block alignment the existing 32-B memcpy loads compile to 2×
// LDG.128. Costs 12 pad bytes per block in tiny scratch buffers. mmvq keeps
// its own ggml_block_q8_1 (the imported ggml vec_dot kernels expect the
// ggml layout).
...
// Batched-activation variant for the spec-verify LM head ( lever 2):
