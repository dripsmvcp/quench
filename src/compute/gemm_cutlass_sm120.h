...
// Convert quench NvFP4QuantResult to CUTLASS block-scaled format.
...
//
// Today the upstream `moe_gather` still runs, so this saves only the
// gathered_base HBM read which is an L2 hit on a 96 MB-L2 RTX 5090 — net
...
// future lazy-gather addition in the legacy fallback path. The kernel +
// dispatch wiring already exist so that fix is one branch + one
