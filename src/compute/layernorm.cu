...
// --------------------------------------------------------------------------
// Warp-per-row FP16 RMSNorm — the batch-prefill variant.
//
...
void rmsnorm(const Tensor& x, const Tensor& weight, Tensor& out, float eps, cudaStream_t stream,
    ...
    switch (x.qtype) {
        case QType::F32:
            ...
            break;
        case QType::F16:
            // Batch prefill: warp-per-row — no barriers/smem; the
...
// --------------------------------------------------------------------------
// True LayerNorm with bias + optional residual (, encoder post-LN).
