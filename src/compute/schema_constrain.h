...
struct SchemaFrame {
    ...
    // degenerating model could not be forced to close the number ( —
...
public:
    ...
    // the `unit` lane — the grammar bugs this surface has shipped (,,
    //) all escaped CI, which has no GPU runner. Not for engine use:
    ...
    // Jump-ahead: appends to `out` the characters every schema-legal
    ...
    // Tool-call envelope: when set, generation is framed by forced
    ...
    // Strict OPTIONAL tool call (, OpenAI `strict: true` with a model-chosen
    ...
    // parallel_tool_calls (, strict optional mode only): when true, the gate
...
private:
    ...
    // Strict optional tool call: the preamble gate has just seen the
