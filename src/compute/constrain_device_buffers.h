...
// inside apply_mask() until, where a failure mid-decode returned without
...
//
// Allocation happens at init and fails loudly, which is the invariant
...
public:
    ...
    [[nodiscard]] bool alloc_token_allow(const char* owner, int vocab_size) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("%s: failed to allocate the %d-token allow list: %s", owner, vocab_size,
    ...
    [[nodiscard]] bool alloc_categories(const char* owner, const uint16_t* host, int vocab_size) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("%s: failed to allocate device categories: %s", owner, cudaGetErrorString(err));
            d_token_categories_ = nullptr;
            return false;
        }
        err = cudaMemcpy(d_token_categories_, host, bytes, cudaMemcpyHostToDevice);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("%s: failed to copy categories to device: %s", owner, cudaGetErrorString(err));
            return false;
        }
        return true;
    }
    ...
    [[nodiscard]] bool alloc_allowed_mask(const char* owner) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("%s: failed to allocate mask buffer: %s", owner, cudaGetErrorString(err));
    ...
    void free() {
        if (d_token_categories_) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_token_categories_));
            d_token_categories_ = nullptr;
        }
        if (d_token_allow_) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_token_allow_));
            d_token_allow_ = nullptr;
        }
        if (d_allowed_mask_) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_allowed_mask_));
            d_allowed_mask_ = nullptr;
        }
    }
