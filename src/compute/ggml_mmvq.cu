...
static __device__ __forceinline__ int ggml_dp4a(const int a, const int b, int c) { return __dp4a(a, b, c); }
...
template <int vdr>
static __device__ __forceinline__ float vec_dot_q5_1_q8_1_impl(const int* vl, const int* vh, const int* u,
...
static __device__ __forceinline__ float vec_dot_q5_1_q8_1(const void* __restrict__ vbq,
...
#pragma unroll
    ...
    // Construct half2 from quench's separate d/m and d/s fields
...
// File-local template tag for dispatch (distinct from quench::QType).
...
void ggml_mmvq_q4k(const void* W, const half* x, half* y, int M, int N, int K, void* scratch,
...
void ggml_mmvq_q5_1(const void* W, const half* x, half* y, int M, int N, int K, void* scratch,
                    size_t scratch_size, cudaStream_t stream) {
    const int num_q8_blocks = (K / QK8_1);
    const int total_q8_blocks = M * num_q8_blocks;
    size_t need = (size_t)total_q8_blocks * sizeof(ggml_block_q8_1);
...
// No Q5_1 mmvq variant here — Q5_1 GEMV dispatch goes through
// `gemv_q5_1_q8_1` (dp4a path) in gemm.cu.

void ggml_mmvq_q5k(const void* W, const half* x, half* y, int M, int N, int K, void* scratch,
                   size_t scratch_size, cudaStream_t stream) {
    const int num_q8_blocks = (K / QK8_1);
    const int total_q8_blocks = M * num_q8_blocks;
    size_t need = (size_t)total_q8_blocks * sizeof(ggml_block_q8_1);
...
void ggml_mmvq_q8_0(const void* W, const half* x, half* y, int M, int N, int K, void* scratch,
                    size_t scratch_size, cudaStream_t stream) {
    const int num_q8_blocks = (K / QK8_1);
    const int total_q8_blocks = M * num_q8_blocks;
    size_t need = (size_t)total_q8_blocks * sizeof(ggml_block_q8_1);
