...
void gemm_dispatch(cublasLtHandle_t, const WeightHandle& w, const Tensor& x, Tensor& y, float alpha,
    ...
    switch (w.primary_tier) {
        ...
        case StorageTier::NVFP4: {
            ...
            const bool force_dequant = quench::process_diag_nvfp4_force_dequant();
        ...
        case StorageTier::CUTLASS_NVFP4: {
            ...
            if (M == 1) {
                ...
                QUENCH_LOG_ERROR("gemm_dispatch CUTLASS_NVFP4 M=1: no valid fallback available");
                return;
            }
        ...
        case StorageTier::MXFP4: {
            ...
            if (M == 1) {
                ...
                if (mw.linear_scales != nullptr) {
                    ...
                    QUENCH_LOG_ERROR("gemm_dispatch MXFP4 M=1: linear_scales is null, cannot GEMV");
                }
                return;
            }
        ...
        case StorageTier::Undefined:
            QUENCH_LOG_FATAL("gemm_dispatch: handle in invalid tier %d", std::to_underlying(w.primary_tier));
            return;
    }
}
...
void gemv_dispatch(const WeightHandle& w, const Tensor& x, Tensor& y, cudaStream_t stream) {
    switch (w.primary_tier) {
        ...
        case StorageTier::MXFP4: {
            ...
            if (mw.linear_scales == nullptr) {
                QUENCH_LOG_ERROR("gemv_dispatch MXFP4: linear_scales is null");
                return;
            }
        ...
        default:
            QUENCH_LOG_FATAL("gemv_dispatch: handle in invalid tier %d", std::to_underlying(w.primary_tier));
            return;
    }
}
...
void gemm_grouped_dispatch(cublasLtHandle_t /*lt*/, std::span<const WeightHandle* const> experts,
    ...
    for (int e = 1; e < ne; ++e) {
        if (experts[e]->primary_tier != tier) {
            QUENCH_LOG_FATAL("gemm_grouped_dispatch: mixed tiers in expert set (%d vs %d)",
    ...
    switch (tier) {
        ...
        default:
            QUENCH_LOG_FATAL("gemm_grouped_dispatch: undefined tier %d", std::to_underlying(tier));
            return;
    }
}
