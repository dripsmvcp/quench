...
//   - Stripped the `permute` path (quench's attention doesn't need it at the
//     quant step — permutation happens inside the FMHA tile loop).
//   - Adapted to quench's coding style, logging, and namespace.
...
bool nvfp4_quant_hw_fp16(const half* d_input, uint8_t* d_nvfp4, uint8_t* d_sf, int batch_size, int n_heads,
    ...
    if (head_dim != 64 && head_dim != 128) {
        QUENCH_LOG_ERROR("nvfp4_quant_hw: head_dim %d unsupported (only 64 / 128)", head_dim);
        return false;
    }
    if (n_tokens <= 0 || batch_size <= 0 || n_heads <= 0) {
        QUENCH_LOG_ERROR("nvfp4_quant_hw: invalid dims batch=%d heads=%d tokens=%d", batch_size, n_heads,
                      n_tokens);
        return false;
    }
