...
// measured 2.1x on 512x1024x5120 (27.0us -> 12.8us) in a standalone
// config sweep on Qwen3-14B shapes. It LOSES ~25% on large-N
...
__device__ __forceinline__ void quantize_micro_block_nvfp4_from_vals(const float vals[kSFVecSize],
    ...
    // division by zero — matches the >=2^-9 clamp used elsewhere in quench.
...
__global__ void quantize_fp16_nvfp4_cutlass_moe_kernel(
    ...
    // skipped — that skip is gated on a future lazy-gather addition in the
    // legacy MoE fallback path.
...
void convert_nvfp4_to_cutlass(const NvFP4QuantResult& src, CutlassNvFP4Weight& dst, cudaStream_t stream) {
    QUENCH_CHECK(src.packed_data != nullptr, "convert_nvfp4_to_cutlass: src.packed_data is null");
    int64_t N = src.N;
    int64_t K = src.K;
    ...
    void* d_sf = nullptr;
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_sf, sf_bytes));
    QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(d_sf, 0, sf_bytes, stream));  // zero-init for padding
    ...
    QUENCH_LOG_DEBUG("convert_nvfp4_to_cutlass: N=%lld K=%lld sf=%.2f MiB (data borrowed)", (long long)N,
...
void convert_nvfp4_to_cutlass_borrowed(const NvFP4QuantResult& src, CutlassNvFP4Weight& dst, void* sf_dst,
                                       cudaStream_t stream) {
    QUENCH_CHECK(src.packed_data != nullptr, "convert_nvfp4_to_cutlass_borrowed: src.packed_data is null");
    QUENCH_CHECK(sf_dst != nullptr, "convert_nvfp4_to_cutlass_borrowed: sf_dst is null");
    int64_t N = src.N;
    int64_t K = src.K;
...
void free_cutlass_nvfp4_weight(CutlassNvFP4Weight& w) {
    ...
    if (w.scale_factors && !w.sf_borrowed) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(w.scale_factors));
    }
...
void convert_nvfp4_moe_scales_to_sfatom(const void* src_native_ms, void* dst_sfatom_sf, int ne, int N,
    ...
    QUENCH_CHECK(K % kSFVecSize == 0, "convert_nvfp4_moe_scales_to_sfatom: K=%d must be multiple of %d",
              K, kSFVecSize);
    int K_groups = K / kSFVecSize;
    int n_k_tiles = (K + kAtomKElems - 1) / kAtomKElems;
...
void quantize_fp16_to_nvfp4_cutlass(const void* src_fp16, void* dst_data, void* dst_sf, int M, int K,
                                    cudaStream_t stream) {
    QUENCH_CHECK(K % kSFVecSize == 0, "quantize_fp16_to_nvfp4_cutlass: K=%d must be multiple of %d",
              K, kSFVecSize);
...
void quantize_fp16_to_nvfp4_cutlass_moe(const void* src_fp16, void* dst_packed, uint8_t* const* d_sfa_bases,
                                        ...
                                        cudaStream_t stream) {
    QUENCH_CHECK(K % kSFVecSize == 0, "quantize_fp16_to_nvfp4_cutlass_moe: K=%d must be multiple of %d",
              K, kSFVecSize);
    if (expanded == 0)
        return;
...
static bool gemm_nvfp4_cutlass_sm120_impl(const void* a_data, const void* a_sf, const CutlassNvFP4Weight& b,
    ...
    // cuTensorMapEncodeTiled return 719 (LAUNCH_FAILED) instead of the real code.
    {
        ...
        if (prior != cudaSuccess) {
            QUENCH_LOG_ERROR("CUTLASS sm120: prior CUDA error before GEMM: %s", cudaGetErrorString(prior));
            return false;
        }
    }
    ...
    if (st != cutlass::Status::kSuccess) {
        QUENCH_LOG_WARN("CUTLASS sm120 NVFP4 GEMM: can_implement failed (%d) for M=%d N=%d K=%d", (int)st, M, N,
                     K);
        return false;
    }
    ...
    if (st != cutlass::Status::kSuccess) {
        QUENCH_LOG_ERROR("CUTLASS sm120 NVFP4 GEMM: initialize failed (%d) M=%d N=%d K=%d", (int)st, M, N, K);
        return false;
    }

    st = gemm.run(stream);
    if (st != cutlass::Status::kSuccess) {
        QUENCH_LOG_ERROR("CUTLASS sm120 NVFP4 GEMM: run failed (%d)", (int)st);
        return false;
    }
