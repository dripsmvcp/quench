        ...
        while (!eof()) {
            ...
            if (key == "type") {
                ...
                if (name.empty() || name.find('/') != std::string::npos) {
                    ...
                    QUENCH_LOG_WARN("JSON schema: unsupported $ref '%s'", ref.c_str());
                    ref_error_ = true;
                } else {
...
static void compile_patterns(SchemaNode* node) {
    ...
    if (!node->pattern.empty() && !node->pattern_nfa) {
        ...
        if (nfa->compile(node->pattern)) {
            ...
            QUENCH_LOG_WARN("JSON schema: unsupported regex pattern '%s' — pattern not enforced",
                         node->pattern.c_str());
        }
    }
...
static bool validate_refs(const SchemaNode* root, const SchemaNode* node) {
    ...
    if (node->type == SchemaType::REF && resolve_schema_ref(root, node) == nullptr) {
        QUENCH_LOG_WARN("JSON schema: unresolvable $ref '%s'", node->ref_name.c_str());
        return false;
    }
    for (auto& [name, prop] : node->properties)
...
std::unique_ptr<SchemaNode> parse_json_schema(const std::string& json) {
    ...
    if (parser.ref_error()) {
        QUENCH_LOG_ERROR("Failed to parse JSON schema: unsupported $ref form");
        return nullptr;
    }
    ...
    if (!validate_refs(root.get(), root.get())) {
        QUENCH_LOG_ERROR("Failed to parse JSON schema: unresolvable $ref");
        return nullptr;
    }
...
// mapped name. Used to namespace a tool's hoisted $defs ( stage 2).
...
static bool collect_tool_defs(const std::vector<std::pair<std::string, std::string>>& tools,
    ...
    for (auto& [name, params_json] : tools) {
        ...
        // Hoist any per-tool $defs into the TOOL_CALL root ( stage 2).
