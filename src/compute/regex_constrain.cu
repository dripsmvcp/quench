...
bool RegexConstrainer::init_pattern_only(const std::string& pattern) {
    ...
    if (const char* bad = unsupported_construct(effective)) {
        QUENCH_LOG_WARN("RegexConstrainer: pattern '%s' uses %s — not enforcing it", pattern.c_str(), bad);
        return false;
    }
    if (!nfa_.compile(effective)) {
        QUENCH_LOG_WARN("RegexConstrainer: unsupported or malformed pattern '%s'", pattern.c_str());
        return false;
    }
    ...
    if (!language_non_empty()) {
        QUENCH_LOG_WARN("RegexConstrainer: pattern '%s' matches nothing — not enforcing it", pattern.c_str());
        return false;
    }
    initialized_ = true;
    return true;
...
bool RegexConstrainer::init(const std::string& pattern, Tokenizer* tokenizer, int vocab_size) {
    ...
    QUENCH_LOG_INFO("RegexConstrainer: pattern '%s' active over %d tokens", pattern.c_str(), vocab_size_);
    return true;
}
...
const std::vector<uint8_t>& RegexConstrainer::allow_for_current_state(int vocab_size) {
    ...
    if (n_allowed == 0) {
        for (int32_t eid : eos_ids_)
            if (eid >= 0 && eid < vocab_size)
                allow[static_cast<size_t>(eid)] = 1;
        QUENCH_LOG_WARN("RegexConstrainer: no token continues pattern '%s' — allowing EOS", pattern_.c_str());
    }
...
void RegexConstrainer::apply_mask(float* d_logits, int vocab_size, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(dev_.token_allow(), allow.data(), static_cast<size_t>(vocab_size),
