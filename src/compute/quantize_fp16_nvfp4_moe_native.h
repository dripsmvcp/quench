...
// pattern in executor_forward_moe.cu. Eliminating both syncs is a
// graph-capture prerequisite for MoE prefill.
//
...
// index), preserving the host-loop semantics that the device-built ptr
// arrays downstream mirror. Single-block 256-thread launch — safe inside
...
// (a further graph-capture prerequisite). Output is exclusive prefix
...
// that H2D from the dispatch path.
//
