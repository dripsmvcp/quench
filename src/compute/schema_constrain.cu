...
// of a reasoning model whose think block was budget-force-closed (:
// `["tech","tech","tech",...` until max_tokens). Same anti-runaway class as
// the number-digit cap. INT_MAX = unbounded.
...
bool SchemaConstrainer::init(const Tokenizer& tok, std::unique_ptr<SchemaNode> schema) {
    ...
    QUENCH_LOG_INFO("SchemaConstrainer: initialized with %d tokens, schema type=%d", vocab_size_,
...
void SchemaConstrainer::reset() {
    ...
    if (strict_optional_envelope_) {
        ...
        // value; when the value pops it flips to ENVELOPE_CLOSE.
        SchemaFrame env;
...
// argument schema is bound before any argument content is generated.
...
uint16_t SchemaConstrainer::compute_category_mask() const {
    ...
    switch (f.phase) {
        ...
        case SchemaPhase::NUMBER_VALUE: {
            ...
            // and leave the JSON unterminated. f.string_len counts digits;
...
void SchemaConstrainer::compute_token_allow_mask(uint16_t cat_mask) {
    ...
    // just as much to the JSON free-string shortcut a few lines up.
...
void SchemaConstrainer::apply_mask(float* d_logits, int vocab_size, cudaStream_t stream) {
    ...
    if (stack_.empty()) {
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(dev_.token_allow(), token_allow_.data(), vocab_size_ * sizeof(uint8_t),
        ...
        if (n_allowed == 0) {
            ...
            for (int32_t e : eos_tokens_)
                if (e >= 0 && e < vocab_size_)
                    token_allow_[e] = 1;
            cat_mask = 0xFFFF;
            QUENCH_LOG_WARN("SchemaConstrainer: no token satisfies the schema in phase %d — allowing EOS",
                         std::to_underlying(top().phase));
        }
    }

    QUENCH_LOG_DEBUG("SchemaConstrainer::apply_mask phase=%d cat_mask=0x%04x need_allow=%d stack=%zu",
...
void SchemaConstrainer::update(int32_t token) {
    ...
    // parallel_tool_calls: a strict tool-call body just drained the
    ...
    if (!stack_.empty()) {
        QUENCH_LOG_DEBUG("SchemaConstrainer::update token=%d [%s] phase %d->%d stack=%zu", token, text.c_str(),
                      std::to_underlying(before), std::to_underlying(top().phase), stack_.size());
    }
}

// ---------------------------------------------------------------------------
// Jump-ahead: forced-continuation text probe
// ---------------------------------------------------------------------------
...
bool SchemaConstrainer::sim_advance(std::vector<SchemaFrame>& stk, char c) const {
    ...
    switch (f.phase) {
        case SchemaPhase::VALUE_START: {
            ...
            switch (f.node->type) {
                ...
                case SchemaType::ANY_OF:
                    ...
                    return true;
                case SchemaType::TOOL_CALL:
                    // Tool-call body — an object with ordered keys.
                    if (c == '{') {
                        if (stk.size() >= kMaxSchemaStackDepth)
                            return false;
                        f.phase = SchemaPhase::OBJECT_OPEN;
        ...
        case SchemaPhase::OBJECT_KEY: {
            ...
            // No escapes in keys: accepting '\' and dropping it let
        ...
        case SchemaPhase::OBJECT_AFTER_KEY: {
            ...
            if (c == ':') {
                ...
                // the tool chosen by the completed "name" enum.
        ...
        case SchemaPhase::ENUM_VALUE: {
            if (c == '"') {
                ...
                // TOOL_CALL name binding: the completed "name" enum
