...
// given config + head_dim) is covered by a cheap CPU unit test — the
...
// model does —. Renaming a tier breaks both sides together.
...
// has_sinks mirrors the pre-gate: learned sinks (gpt-oss) route straight
...
inline AttnPrefillPath select_attn_prefill_path(const DispatchPolicy& rcfg, const AttnKernelSupport& sup,
    ...
    // 0. Learned sinks: FP16 WMMA FMHA or nothing.
    if (has_sinks) {
    ...
    //   : teacher-forced PPL gemma-3-12b 16.6→549, Qwen3-8B 40.5→4506
