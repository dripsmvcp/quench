...
// and `{"a":1,}` passed the mask and the reply did not parse.
...
public:
    ...
    // the document always ends well-formed instead of being truncated.
    ...
    // force-close narrowing lives in this mask, not in the grammar
    ...
    // initialize() and never false again — the point of issue, where it
...
private:
    ...
    // max_tokens and returned truncated, unparseable JSON.
    ...
    void enter_number_(char c);  // seed the RFC 8259 number sub-state
};
