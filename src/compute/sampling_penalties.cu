...
void apply_dry_penalty(float* d_logits, int vocab_size, const int32_t* host_token_ids, int n_tokens,
    ...
    if (needed > s_dry_buf_cap) {
        ...
        QUENCH_LOG_WARN("apply_dry_penalty: %zu tokens exceeds the %zu-slot capacity — applying the "
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(s_dry_tokens_buf, h_tokens.data(), n * sizeof(int32_t),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(s_dry_values_buf, h_values.data(), n * sizeof(float),
                                       cudaMemcpyHostToDevice, stream));
...
__global__ void mirostat_v2_sample_kernel(const float* __restrict__ logits, int vocab_size, float mu,
    ...
    if (tid == 0) {
        ...
        unsigned int rng = sampler_seed_scramble(seed);  // issue
...
int32_t sample_mirostat_v2(const Tensor& logits, float temperature, float tau, float eta, float* mu,
    ...
    if (cudaMalloc(&d_result, 2 * sizeof(int32_t)) != cudaSuccess) {
        QUENCH_LOG_ERROR("sample_mirostat_v2: cudaMalloc failed");
        return 0;
    }
...
void sampling_preallocate_dry(int max_seq_len, cudaStream_t /*stream*/) {
    ...
    if (tokens.empty() || values.empty()) {
        QUENCH_LOG_WARN("sampling_preallocate_dry: %zu slots unavailable from the T2 arena — the DRY "
    ...
    s_dry_buf_cap = cap;
    QUENCH_LOG_DEBUG("sampling_preallocate_dry: pre-allocated %zu DRY penalty slots", cap);
}
