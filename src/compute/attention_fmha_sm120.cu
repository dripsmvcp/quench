...
__global__ void __launch_bounds__(SM120_BLOCK_THREADS, 2) fmha_sm120_kernel(
    ...
    // is fundamental (Bq=16 -> O(n) KV re-reads, no TMEM on sm_120 to widen
    // Bq). The WMMA loops are parametric in Bkv.
    ...
    if (tid < Bq) {
        if (sinks != nullptr) {
            // gpt-oss learned sink (/): a virtual extra logit column
...
#pragma unroll
            ...
            // scores, worst at short seq_q; issue). Stage this thread's
...
bool fmha_sm120_prefill(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor& O, float scale,
    ...
    int Bq;
    {
        ...
        if (smem_128 <= occ2_cap) {
            ...
            QUENCH_LOG_DEBUG("FMHA sm120: no Bq fits smem (hd=%d, smem_16=%zu, max=%d)", head_dim, smem_16,
                          max_smem);
            return false;
        }
    }
    ...
    if (smem > (size_t)max_smem) {
        QUENCH_LOG_DEBUG("FMHA sm120: smem %zu > device max %d, skipping", smem, max_smem);
        return false;
    }
...
#define LAUNCH_FMHA_SM120(BQ, HD)                                                                        \
    do {                                                                                                 \
        ...
        if (attr_err != cudaSuccess) {                                                                   \
            QUENCH_LOG_WARN("FMHA sm120: cudaFuncSetAttribute failed for Bq=%d HD=%d smem=%zu: %s", BQ, HD, \
...
// Scaled variant ( fp8-QK campaign): multiply by 1/s before the convert so
// the operand uses the full e4m3 dynamic range. The raw (unscaled) conversion
// is the quality cliff — ~10% relative score error on real activations.
...
#pragma unroll
            ...
            // (SP_half row r aliases the bytes of float row r/2 — issue,
...
// replaces 4-6 scalar LDS per MMA operand fetch (without it this kernel is
// LSU-instruction-bound: ncu measured 3.4 shared-LDS per tensor op;
...
// Bank-conflict padding (ncu: unpadded, this FA2 kernel is mio_throttle-bound —
...
// context — e4m3 score noise compounds across layers at short seq,/).
...
// the occupancy "smem surgery" lever for the grid-underfill band.
// TWOSLOT ( second cut): keeps the FULL Bkv=64 tile at the same ~35 KB by
...
// ( follow-up to the QK f16acc). The online-softmax O rows are convex
...
// FP8SCALED: amax-scaled e4m3 conversion for the fp8-QK path. Q and K
...
__global__ void fmha_sm120_fa2_kernel(const half* __restrict__ Q, const half* __restrict__ K,
    ...
    // Graph-captured verify: the real KV length lives on device and the
...
#pragma unroll
            for (int n = 0; n < N_S; n += 2) {
                ...
                if constexpr (F16ACC) {
                    // f16-accumulate QK^T (, opt-in attention.fa2_f16acc).
                    // GeForce sm_120 runs f16-src/f32-acc HMMA at 1/4 rate (253
                    // of 838 TFLOPS,); f16-acc lifts the score MMA to the
...
// Registered as a pre-cudaDeviceReset hook; see core/cuda_static_reset.h.
namespace {
QUENCH_REGISTER_CUDA_STATIC_RESET(fmha_sm120_reset_static_cuda_state);
}  // namespace
...
bool fmha_sm120_fa2_prefill(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor& O, float scale,
    ...
    // stage-1 port behind attention.fa2_hd256 (default ON: fp16-qk only,
    ...
    if (head_dim != 128 && !(head_dim == 256 && fp16_qk && quench::process_diag_fa2_hd256()))
        return false;
    ...
    // together fill the SMs. Three bands (fp16qk path; occupancy surgery):
    ...
    // f16-acc QK^T only applies to the fp16_qk path (the fp8 path keeps
    // its f32 accumulate). Opt-in: +3-4% pp2048/pp4096 NVFP4 for +0.37% PPL.
    const bool f16acc = fp16_qk && quench::process_diag_fa2_f16acc();
    ...
    const bool pv_f16 = f16acc && quench::process_diag_fa2_pv_f16acc();
    int Bq, Bkv;
    bool twoslot = false;
    ...
    if (head_dim == 256) {
        ...
        if (blocks_128 >= (long)sm_count) {
            ...
            // Underfill band: Bq=64 doubles the grid and TWOSLOT halves
        ...
        Bq = use_bq64 ? 64 : 128;
        Bkv = 64;
        // fp8-QK campaign: amax-scaled e4m3 conversion (the raw variant
        // is the quality cliff and stays the default for A/B).
        if (quench::process_diag_fp8_qk_scaled()) {
            fp8_scaled = true;
