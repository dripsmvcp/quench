...
__global__ void mtp_mrope_kernel(
    ...
    // rotates identically to the verifier (issue). inv_scaling = 1/freq_scale;
...
// step and the rope-parity unit test (issue).
...
bool mtp_draft_step(int prev_token_id, const void* d_h_prev,
                    ...
                    int32_t* d_out_token) {
    if (!mtp.loaded) {
        QUENCH_LOG_ERROR("mtp_draft_step: MTP head not loaded");
        return false;
    }
    ...
    if (!ws.d_emb_norm || !ws.d_h_norm || !ws.d_fc_in || !ws.d_fc_out ||
        ...
        QUENCH_LOG_ERROR("mtp_draft_step: workspace not allocated");
        return false;
    }
    if (main_tok_emb.data == nullptr || main_lm_head.data == nullptr) {
        QUENCH_LOG_ERROR("mtp_draft_step: main embedding or lm_head not on GPU");
        return false;
    }
    if (d_prev_token == nullptr && (prev_token_id < 0 || prev_token_id >= vocab_size)) {
        QUENCH_LOG_ERROR("mtp_draft_step: token_id %d out of range [0,%d)",
    ...
    // of input. quench::embedding_lookup handles the qtype dispatch.
    if (d_prev_token != nullptr) {
        ...
        quench::embedding_lookup(main_tok_emb, d_prev_token, /*n_tokens=*/1, out_view,
        ...
        if (cudaMalloc(&d_tok, sizeof(int32_t)) != cudaSuccess) {
            QUENCH_LOG_ERROR("mtp_draft_step: token-id scratch alloc failed");
            return false;
        }
        cudaMemcpyAsync(d_tok, &h_tok, sizeof(int32_t), cudaMemcpyHostToDevice, stream);
        int64_t out_shape[2] = {1, hidden_dim};
        Tensor  out_view(ws.d_fc_in, QType::F16, 2, out_shape, /*on_device=*/true);
        quench::embedding_lookup(main_tok_emb, d_tok, /*n_tokens=*/1, out_view, main_tok_emb.qtype,
                              stream);
        cudaFreeAsync(d_tok, stream);
    }

    // Step 2: emb_norm = RMSNorm(emb, pre_fc_norm_embedding)
    // quench::rmsnorm dispatcher reads x.shape[0]=rows + x.shape[1]=d_model and
    ...
    quench::rmsnorm(emb_view, mtp.pre_fc_norm_embedding, emb_n, 1e-6f, stream);
    quench::rmsnorm(h_view,   mtp.pre_fc_norm_hidden,    h_n,   1e-6f, stream);
    ...
    // Step 4: fc_out = fc @ fc_in  ([hidden_dim, 2*hidden_dim] x [2*hidden_dim] = [hidden_dim])
    {
        ...
        quench::gemm(fc_in_view, mtp.fc, fc_out_view, 1.0f, 0.0f, stream);
    }
    ...
    if (ws.num_heads > 0 && ws.head_dim > 0 &&
        ...
        // 5.A.1 — input_layernorm(fc_out) → d_input_norm
        {
            ...
            quench::rmsnorm(fc_out_view, mtp.input_layernorm, in_view, 1e-6f, stream);
        }
        // 5.A.2 — Q (full, including gate): q_proj @ d_input_norm → [2 * nh * hdh]
        {
            ...
            quench::gemm(in_view, mtp.q_proj, out_view, 1.0f, 0.0f, stream);
        }
        ...
        if (ws.d_k_proj && nkv > 0) {
            ...
            quench::gemm(in_view, mtp.k_proj, out_view, 1.0f, 0.0f, stream);
        }
        if (ws.d_v_proj && nkv > 0) {
            ...
            quench::gemm(in_view, mtp.v_proj, out_view, 1.0f, 0.0f, stream);
        }
        ...
        if (use_kv_scan) {
            ...
            if (mtp.q_norm.data) {
                ...
                quench::rmsnorm(q_view, mtp.q_norm, q_view, ws.rms_norm_eps, stream,
                             ws.arch_norm_offset);
            }
            if (mtp.k_norm.data) {
                int64_t k_shape[2] = {nkv, hdh};
                Tensor k_view(ws.d_k_proj, QType::F16, 2, k_shape, /*on_device=*/true);
                quench::rmsnorm(k_view, mtp.k_norm, k_view, ws.rms_norm_eps, stream,
                             ws.arch_norm_offset);
            }
            ...
            if (ws.rope_dim > 0 && ws.mrope_sec0 + ws.mrope_sec1 + ws.mrope_sec2 == ws.rope_dim / 2) {
                // RoPE-scaling params mirrored from the main forward (issue):
        ...
        // 5.A.5 — o_proj @ d_attn_out → d_attn_residual
        {
            ...
            quench::gemm(in_view, mtp.o_proj, out_view, 1.0f, 0.0f, stream);
        }
    ...
    //     sigmoid gating (quench::moe_gate_topk_fused / swiglu /
    ...
    if (mtp_has_moe || mtp_has_dense_mlp) {
        ...
        // 5.B.1 — post_attention_layernorm(fc_out) → d_post_norm
        {
            ...
            quench::rmsnorm(fc_out_view, mtp.post_attention_layernorm, pn_view, 1e-6f, stream);
        }
        ...
        quench::moe_gate_topk_fused(mtp.router.data, ws.d_post_norm, ne, hd, top_k,
        ...
        for (int k = 0; k < top_k; ++k) {
            ...
            if (e_idx < 0 || e_idx >= ne) {
                QUENCH_LOG_WARN("mtp MoE: invalid expert index %d (top_k=%d)", e_idx, k);
                continue;
            }
            ...
            // gate_up = gu_view @ post_norm
            {
                ...
                quench::gemm(in_view, gu_view, out_view, 1.0f, 0.0f, stream);
            }
            // swiglu: gate = first half, up = second half → act = silu(gate)*up
            {
                ...
                quench::swiglu(gate_view, up_view, act_view, stream);
            }
            // down = dn_view @ act → write directly into d_expert_outputs[k * hd]
            {
                ...
                quench::gemm(in_view, dn_view, out_view, 1.0f, 0.0f, stream);
            }
        }

        // 5.B.5 — Weighted sum + residual: moe_out = fc_out + Σ_k w[k]*expert_outputs[k]
        quench::moe_weighted_sum_residual(
        ...
        if (d_ff_s > 0 && mtp.shared_expert_gate_proj.data && mtp.shared_expert_up_proj.data &&
            ...
            // shared_gate = shared_expert_gate_proj @ post_norm  → [d_ff_s]
            {
                ...
                quench::gemm(in_view, mtp.shared_expert_gate_proj, out_view, 1.0f, 0.0f, stream);
            }
            // shared_up = shared_expert_up_proj @ post_norm  → [d_ff_s]
            {
                ...
                quench::gemm(in_view, mtp.shared_expert_up_proj, out_view, 1.0f, 0.0f, stream);
            }
            ...
            // shared_out = shared_expert_down_proj @ shared_act  → [hd]
            {
                ...
                quench::gemm(in_view, mtp.shared_expert_down_proj, out_view, 1.0f, 0.0f, stream);
            }
            ...
            if (mtp.shared_expert_gate.data != nullptr) {
                quench::shared_expert_gate_scale(
                    /*x=*/ ws.d_post_norm,
    ...
    // Step 6: h_final = RMSNorm(fc_out, final_norm)
    {
        ...
        quench::rmsnorm(fc_out_view, mtp.final_norm, h_final_view, 1e-6f, stream);
    }
    ...
    if (nvfp4_lm) {
        ...
        quench::gemm(h_final_view, main_lm_head, logits_view, 1.0f, 0.0f, stream);
    }
    ...
    if (want_topk) {
        ...
        if (ws.d_topk == nullptr) {
            QUENCH_LOG_ERROR("mtp_draft_step: top-W requested but ws.d_topk not allocated");
            return false;
        }
        if (nvfp4_lm) {
    ...
    if (d_idx == nullptr) {
        if (cudaMallocAsync(&d_idx, sizeof(int), stream) != cudaSuccess) {
            QUENCH_LOG_ERROR("mtp_draft_step: argmax scratch alloc failed");
            return false;
        }
        owned_idx = true;
    }
