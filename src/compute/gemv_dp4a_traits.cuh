...
// File-local tag enum for template dispatch (distinct from quench::QType).
...
//: the 12 scale bytes arrive as three words (s0 = sc[0..3],
...
__device__ __forceinline__ float q4k_dp4a_sub(const uint8_t* __restrict__ qs,  // Q4_K qs base (128 bytes)
    ...
    // Two LDG.128 instead of eight LDG.32: this kernel class is
...
__device__ __forceinline__ float q5k_dp4a_sub(
    ...
    // Vectorized loads (, same rationale as q4k_dp4a_sub): qs sits at
...
struct DequantTraits<DPQTag::Q4_0> {
    ...
    static __device__ __forceinline__ float dp4a_block(const uint8_t* bp, int /*sub*/, const int* xi,
        ...
        // (same convention as quench's dequant_q4_0_kernel). The Q8_1 activations
...
struct DequantTraits<DPQTag::Q4_K> {
    ...
    static __device__ __forceinline__ float dp4a_block(const uint8_t* bp, int sub, const int* xi, float dq,
        ...
        // bytes) instead of 2x LD.16 + per-byte scale loads. The
...
struct DequantTraits<DPQTag::Q5_K> {
    ...
    static __device__ __forceinline__ float dp4a_block(const uint8_t* bp, int sub, const int* xi, float dq,
                                                       float /*q8_sum*/) {
        // Header via one LDG.128; 176-byte superblock is 16B-aligned.
        const uint4 hdr = *reinterpret_cast<const uint4*>(bp);
        const half2 dd = *reinterpret_cast<const half2*>(&hdr.x);
        float d_super = __half2float(__low2half(dd));
        float dmin_super = __half2float(__high2half(dd));
...
// lever 2). MR activation rows share ONE pass over W — the per-row loop
// re-read the full vocab x d_model LM head once per chunk row (0.44 ms/row on
// Qwen3-8B Q8_0, the dominant term of the GGUF verify cycle.
