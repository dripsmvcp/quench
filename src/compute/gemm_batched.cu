...
void gemm_kv_batched(const Tensor& input, const Tensor& weight_kv, Tensor& k_out, Tensor& v_out,
    ...
    // the V batch landed inside the K buffer and v_out stayed stale (:
    ...
    if (st != CUBLAS_STATUS_SUCCESS) {
        QUENCH_LOG_ERROR("quench::gemm_kv_batched: cublasGemmStridedBatchedEx failed (status %d)", (int)st);
    }
}
...
void gemm_pair_batched(const Tensor& input, const Tensor& weight_fused, Tensor& out1, Tensor& out2,
    ...
    if (st != CUBLAS_STATUS_SUCCESS) {
        QUENCH_LOG_ERROR("quench::gemm_pair_batched: cublasGemmStridedBatchedEx failed (status %d)", (int)st);
    }
}
