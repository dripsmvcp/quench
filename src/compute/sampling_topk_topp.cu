...
//
// A single-block <<<1, BLOCK_SIZE>>> kernel uses 1 of 170 SMs and scans the
// full vocab (~150k) three times — ~737 us/call, the #1 GPU consumer in
// batched server decode. This splits the full-vocab work across
...
//   an LCG. Distribution-identical to a single-block formulation (not
...
struct CubSortScratch {
    ...
    bool ensure(int vocab_size, cudaStream_t stream) {
        ...
        *this = CubSortScratch{};
        ...
        capacity = vocab_size;
        gen = g;
        QUENCH_LOG_DEBUG("CUB sort scratch: %.2f MiB from the T2 arena for vocab=%d (CUB temp %.2f MiB)",
...
static int32_t sample_topk_topp_cub(const float* d_logits, int vocab_size, int top_k, float top_p,
    ...
    if (!s_cub_scratch.ensure(vocab_size, stream)) {
        QUENCH_LOG_ERROR("CUB sort scratch allocation failed for vocab_size=%d", vocab_size);
        return 0;
    }
    ...
    // It is gone because it does not work here (issue): instrumented on
    ...
    int32_t h_result = 0;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(&h_result, d_result, sizeof(int32_t), cudaMemcpyDeviceToHost, stream));
...
static int32_t sample_topk_topp_impl(const float* d_logits, int vocab_size, int top_k, float top_p,
    ...
    if (top_k > MAX_TOP_K) {
        int32_t result = sample_topk_topp_cub(d_logits, vocab_size, top_k, top_p, inv_temperature, seed,
                                              d_result, stream);
        if (owns_result)
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_result));
        return result;
    }
...
int32_t sample_topk_topp(const Tensor& logits, int top_k, float top_p, float temperature, unsigned int seed,
    ...
    if (cudaMalloc(&d_result, SAMPLE_SCRATCH_BYTES) != cudaSuccess) {
        QUENCH_LOG_ERROR("sample_topk_topp: cudaMalloc failed");
        return 0;
    }
...
void sample_topk_topp_device(const Tensor& logits, int top_k, float top_p, float temperature,
    ...
    if (top_k <= 0 || top_k > vocab_size)
        top_k = vocab_size;
    if (top_k > MAX_TOP_K) {
        QUENCH_LOG_WARN("top_k=%d exceeds MAX_TOP_K=%d, clamping", top_k, MAX_TOP_K);
        top_k = MAX_TOP_K;
    }
    if (temperature <= 0.0f)
        temperature = 1.0f;
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(h_mapped, d_result, sizeof(int32_t), cudaMemcpyDeviceToHost, stream));
}
