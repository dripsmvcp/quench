...
static cublasHandle_t get_attn_cublas_handle() {
    if (!s_attn_cublas_handle) {
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            QUENCH_LOG_ERROR("quench::attention_cublas: cublasCreate failed (status %d)", (int)st);
            abort();
        }
...
__global__ void causal_softmax_fp32_to_fp16_kernel(const float* __restrict__ S_in,
    ...
    // gpt-oss sink: virtual extra column — joins max + denominator,
...
static void ensure_attn_ptr_arrays(int n_heads) {
    ...
    if (needed <= s_attn_d_ptrs_capacity)
        return;
    if (s_attn_d_ptrs)
        QUENCH_CUDA_CHECK_LOG(cudaFree(s_attn_d_ptrs));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&s_attn_d_ptrs, needed * sizeof(void*)));
...
namespace {
QUENCH_REGISTER_CUDA_STATIC_RESET(attention_cublas_reset_static_cuda_state);
}  // namespace
...
void attention_cublas_prefill(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor& O, Tensor& S,
    ...
    // FP32-S needs 3× the score element count, not 2×: the FP32 QK^T
    ...
    if (gqa_ratio == 1) {
        ...
        // S_prob (non-overlapping FP16 region on the FP32-S path,).
