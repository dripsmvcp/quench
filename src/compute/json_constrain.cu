...
bool JsonConstrainer::init(const Tokenizer& tok) {
    ...
    // Per-token allow list. Allocated HERE, not on first use (issue).
    ...
    // the allocation failed and apply_mask returned WITHOUT applying
    ...
    // process, invisible in `quench_constrained_eager_fallback_total` because no
    ...
    reset();
    initialized_ = true;
    QUENCH_LOG_INFO("JsonConstrainer initialized (%d tokens classified)", vocab_size_);
    return true;
}
...
void JsonConstrainer::reset() {
    ...
    ws_run_ = 0;
    enter_number_('0');  // clear the number sub-state
...
uint16_t JsonConstrainer::compute_allowed_mask() const {
    ...
    // Force-close near the budget. A constrainer can forbid illegal
    ...
    switch (current_state_) {
        ...
        case JsonState::ARRAY_NEED_VALUE:
            // After , in array: a value is mandatory — no closer.
            mask |= CAT_VALUE_START;
            break;

        case JsonState::OBJECT_NEED_KEY:
            // After , in object: a key is mandatory — no closer.
            mask |= CAT_QUOTE;
            break;
...
bool JsonConstrainer::advance_char(char c) {
    ...
    // state. update() ignores the result (tolerant); the
    ...
    // let the emitted text interleave digits with spaces. Close the
    ...
    switch (current_state_) {
        ...
        case JsonState::AFTER_VALUE:
            if (c == ',') {
                ...
                // accept `}` and turn `{"a":1,}` into legal output.
        ...
        case JsonState::IN_NUMBER: {
            ...
            // legal and left a degenerating model with no forced way out.
            bool ok = false;
            if (c >= '0' && c <= '9') {
                ok = true;
                num_need_digit_ = false;
...
bool JsonConstrainer::sim_token_valid(const std::string& text) {
    ...
    // num_seen_frac_/num_need_digit_ mutated on the real state.
...
// inside a string and the reply did not parse.
...
void JsonConstrainer::apply_mask(float* d_logits, int vocab_size, cudaStream_t stream) {
    ...
    // (host SIGBUS, killed quench-server on the first json_mode request).
    ...
    if (n_allowed == 0) {
        ...
        QUENCH_LOG_WARN("JsonConstrainer: no token satisfies the grammar in state %d — allowing EOS",
