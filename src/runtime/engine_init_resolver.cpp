...
//
// Split out of engine.cpp to keep init orchestration in one place.
// Methods remain Engine::* with declarations in engine.h.
...
// QUENCH_DEBUG_RAW meta-flag: forces the engine into a "naked" FP16 forward pass
...
void Engine::init_apply_debug_raw_overrides_() {
    ...
    setenv("QUENCH_NO_CUDA_GRAPH", "1", 0);
    // No warmup (warmup can leak state into first request)
    setenv("QUENCH_NO_WARMUP", "1", 0);
    // Deterministic cuBLAS (bit-exact across runs, no algo jitter)
    setenv("QUENCH_DETERMINISTIC_GEMM", "1", 0);
    setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", 0);
    // MoE: no expert LRU cache (state-carrying)
    setenv("QUENCH_NO_EXPERT_CACHE", "1", 0);
    // GDN: use reference unfused scan (no register-state reordering)
    setenv("QUENCH_GDN_REF", "1", 0);
    // NOTE: intentionally NOT forcing QUENCH_FORCE_CUBLAS_DECODE / QUENCH_NO_FMHA_SM120 /
    // QUENCH_NO_MMVQ — those trigger incompatible kernel paths that produce IMAs on
    // some combinations. The RAW flag is about disabling *caches and approximations*,
    // not about swapping kernel variants.
}

// Runtime RoPE-scaling override (quench.conf [rope]): inject YaRN/linear
...
bool apply_rope_override(ModelConfig& mcfg, const RuntimeConfig::Rope& rope) {
    ...
    if (rope.scaling != "yarn" && rope.scaling != "linear") {
        QUENCH_LOG_WARN("[rope] scaling=\"%s\" unknown (yarn|linear) — override ignored",
                     rope.scaling.c_str());
        return false;
    }
    if (rope.factor <= 1.0f) {
        QUENCH_LOG_WARN("[rope] factor=%.3f must be > 1.0 — override ignored", rope.factor);
        return false;
    }
    ...
    // MLA (YaRN mscale is entangled with the softmax scale ratio,),
    // and NoPE (no rotary embedding to scale).
    if (!mcfg.rope_short_factor.empty() || !mcfg.rope_long_factor.empty()) {
        QUENCH_LOG_ERROR("[rope] model uses per-dimension RoPE tables (LongRoPE/llama3) — "
                      "override refused");
        return false;
    }
    if (mcfg.is_mla()) {
        QUENCH_LOG_ERROR("[rope] MLA models entangle YaRN mscale with the softmax scale — "
                      "override refused");
        return false;
    }
    if (mcfg.rope_attn_disabled) {
        QUENCH_LOG_ERROR("[rope] model uses NoPE attention (no rotary embedding) — override refused");
        return false;
    }
    ...
    int orig_ctx = rope.orig_ctx > 0 ? rope.orig_ctx
                   : mcfg.rope_n_ctx_orig > 0 ? mcfg.rope_n_ctx_orig
                                              : mcfg.max_seq_len;
    if (orig_ctx <= 0) {
        QUENCH_LOG_WARN("[rope] cannot resolve original context (orig_ctx=0, model ctx=%d) — "
                     "override ignored",
                     mcfg.max_seq_len);
        return false;
    }
    ...
    if (mcfg.rope_freq_scale != 1.0f || mcfg.yarn_ext_factor > 0.0f) {
        QUENCH_LOG_WARN("[rope] model already declares scaling (freq_scale=%.3f, yarn_ext=%.2f) — "
    ...
    if (mcfg.sliding_window_pattern > 0 || !mcfg.swa_layers.empty()) {
        QUENCH_LOG_INFO("[rope] note: sliding-window layers keep freq_scale=1.0 by design — the "
                     "override affects global-attention layers only");
    }
    QUENCH_LOG_INFO("[rope] override applied: %s factor=%.2f orig_ctx=%d → model ctx %d "
...
// are opt-in. Rationale is inline below.
void Engine::init_resolve_kv_dtype_policy_() {
    ...
    if (config_.kv_cache_dtype == QType::F16) {
        ...
        if (kv_str == "fp8") {
            ...
            config_.kv_cache_dtype = QType::FP8_E4M3;
            QUENCH_LOG_INFO("KV cache dtype: FP8_E4M3 (auto — honoring model author's "
            ...
            config_.kv_cache_dtype = QType::FP8_E4M3;
            QUENCH_LOG_INFO("KV cache dtype: FP8_E4M3 (auto — %s measured PPL-neutral for "
    ...
    if (fp8_auto_legacy && config_.kv_cache_dtype == QType::F16 && !debug_raw_ && !force_kv_fp16) {
        config_.kv_cache_dtype = QType::FP8_E4M3;
        QUENCH_LOG_INFO("KV cache dtype: kv_cache.fp8_auto_legacy → FP8_E4M3 (legacy auto-upgrade)");
    } else if (config_.kv_cache_dtype == QType::F16) {
        QUENCH_LOG_INFO("KV cache dtype: FP16 (default — pass --kv-fp8 for FP8 E4M3 memory savings)");
    } else if (config_.kv_cache_dtype == QType::NVFP4) {
        QUENCH_LOG_INFO("KV cache dtype: NVFP4 (FP4 E2M1 + UE4M3 per-16-elem scales, ~3.6× compression)");
        if (config_.use_fp8_prefill) {
            QUENCH_LOG_INFO("NVFP4 KV: disabling FP8 prefill cache (avoid stacked low-precision drift)");
            config_.use_fp8_prefill = 0;
        }
    } else if (config_.kv_cache_dtype == QType::MXFP4_KV) {
        QUENCH_LOG_INFO("KV cache dtype: MXFP4_KV (FP4 E2M1 + UE8M0 per-16-elem scales, ~3.6× compression)");
        if (config_.use_fp8_prefill) {
            QUENCH_LOG_INFO("MXFP4_KV: disabling FP8 prefill cache (avoid stacked low-precision drift)");
            config_.use_fp8_prefill = 0;
        }
    }

    // Scope: the deterministic forcing below is
    // a workaround for NaNs in the cuBLAS-attention + FP8
    // round-trip. When the
    ...
    // hd=256 rides the FA2 port (attention.fa2_hd256 is
    ...
    if (config_.kv_cache_dtype == QType::FP8_E4M3 && fa2_serves_attention &&
        ...
        QUENCH_LOG_INFO("FP8 KV cache: FA2 serves all attention (hd=128/256) — skipping the legacy "
    ...
    if (config_.kv_cache_dtype == QType::FP8_E4M3 && !fa2_serves_attention &&
        ...
        !runtime_config_.runtime.deterministic_gemm) {
        // Mutate the per-Engine RuntimeConfig in place. Free-
        // function readers (gemm.cu's algo-selection skip-benchmark branch)
        // read from the process_diag cache; update it here too so the
    ...
    if (config_.max_batch_size <= 0) {
        ...
        QUENCH_LOG_INFO("max_batch_size: auto → %d (approx_weights=%.1f GB, post-load headroom=%.1f GB, "
                     "tier-floor=%d, VRAM-aware)",
                     config_.max_batch_size, approx_weight_bytes / (1024.0 * 1024.0 * 1024.0),
                     headroom / (1024.0 * 1024.0 * 1024.0), tier);
    } else {
        QUENCH_LOG_INFO("max_batch_size: %d (configured)", config_.max_batch_size);
    }
}
...
void Engine::init_resolve_ssm_dtype_() {
    ...
    if (config_.ssm_state_dtype == QType::F32 && mcfg.ssm_state_size > 0 && !has_gdn_for_dtype) {
        ...
        QUENCH_LOG_INFO("SSM state dtype: auto → FP16 (hybrid SSM model, state_size=%d)", mcfg.ssm_state_size);
    }
}
...
void Engine::init_resolve_fp8_prefill_() {
    ...
    if (is_nvfp4_native && !config_.use_fp8_prefill) {
        QUENCH_LOG_INFO("FP8 prefill: disabled for native NVFP4 (CUTLASS NVFP4 GEMM used instead)");
        ...
        if (sm >= 120 && runtime_config_.attention.fp8_prefill != "always") {
            ...
            QUENCH_LOG_INFO("FP8 prefill: auto → enabled");
        }
    } else if (no_fp8_prefill) {
        QUENCH_LOG_INFO("FP8 prefill: disabled (attention.fp8_prefill=never)");
    }
}
...
void Engine::init_resolve_quant_flags_() {
    ...
    // FP32-accumulate FP16 tensor-core rate; 16F
    // accumulate restores full rate (+24.9% q8 pp512,
    ...
    if (runtime_config_.gemm.cublas_fp16_acc == "auto") {
        ...
        QUENCH_LOG_INFO("cuBLAS FP16-accumulate prefill: auto → %s (arch=%s)", deny ? "OFF" : "ON",
                     model_arch_name(mcfg.arch));
    }
    ...
    if (config_.use_nvfp4_decode < 0) {
        ...
        if (nvfp4_beneficial_qtype && !is_moe && !is_gdn && !sub8bit_qtype) {
            ...
            // Q6_K @ ctx=2048 (151 vs 145.6 tok/s). Decode is ranked #1 for
            // the north-star, so dense Q*_K defaults to mode 1.
            config_.use_nvfp4_decode = 1;
            QUENCH_LOG_INFO("NVFP4 decode: auto → mode 1 (dense Q*_K — decode-first)");
            ...
            QUENCH_LOG_INFO("NVFP4 decode: auto → mode 2 (sub-8-bit Q*_K + decode_all)");
        } else if (is_gdn) {
            ...
            config_.use_nvfp4_decode = 2;
            QUENCH_LOG_INFO("NVFP4 decode: auto → mode 2 (%s)", why);
        }
    }
    ...
    // handles these models by dequanting Q4_K→FP16 on each forward.
    if (config_.use_fp8_prefill) {
        ...
        if (sub_8bit) {
            config_.use_fp8_prefill = 0;
            QUENCH_LOG_INFO("FP8 prefill cache: auto-disabled (sub-8-bit weights → dequant fallback)");
        }
    }
    ...
    if (config_.dual_path_quant) {
        if (config_.use_nvfp4_decode <= 0) {
            QUENCH_LOG_WARN("Dual-path quant requires NVFP4 decode — enabling mode 2 (NVFP4 only)");
            config_.use_nvfp4_decode = 2;
        }
        if (!config_.use_fp8_prefill) {
            QUENCH_LOG_INFO("Dual-path quant: auto-enabling FP8 prefill for attention weight quality");
            config_.use_fp8_prefill = true;
        }
    }
    ...
    // and then never used (deciding later, e.g. inside
    // init_kv_cache, wastes ~3 MiB). Dual-path quant keeps the FP8 path
    // for FFN even on GDN — only attention drops to FP16.
    if (config_.use_fp8_prefill && !config_.dual_path_quant && model_->profile().is_gdn) {
        QUENCH_LOG_INFO("GDN model: disabling FP8 prefill (recurrent state needs FP16 precision)");
        config_.use_fp8_prefill = 0;
    }
    // DENSE Gemma (3 and 4) from GGUF: mode-2 NVFP4 conversion is validated
    // here — gemma-3-12b mode 2 is
    // degen-suite clean CLI+server, gemma-4-31B mode 2 answers coherently
    // (55 tok/s vs 21.7 at mode 1) with multi-turn green. So
    ...
    if (model_->profile().is_gemma4) {
        ...
        // FP8 prefill stays enabled on Gemma-4. Measured on Q4_K_M vs FP16:
        ...
        if (config_.use_nvfp4_decode) {
            ...
            // since each entry carries its own (N, K) shape. Verified
            // on Q4_K_M + UD-Q4_K_M: tg256 184 → 204 tok/s (+11%), pp512
            // 1795 → 2347 tok/s (+30%) vs no cache. Coherent on chat prompts;
            // the known Q4_K_M code-gen drift (see roadmap) is orthogonal.
            QUENCH_LOG_INFO("Gemma 4: NVFP4 decode cache enabled (use_nvfp4_decode=%d, prequant=%d)",
                         config_.use_nvfp4_decode,
                         (int)model_->config().is_nvfp4_prequant);
        }
        if (config_.dual_path_quant) {
            QUENCH_LOG_INFO("Gemma 4: disabling dual_path_quant");
            config_.dual_path_quant = false;
        }
        // FP8 KV is safe to opt into on Gemma-4. The hazard it must avoid:
        // FP8 KV calibration reading garbage beyond the per-layer
        // live K/V region — Gemma-4 has dual head_dim (256 SWA / 512 global)
        // and the workspace is allocated for max_head_dim, leaving a
        // tail-region of uninitialized memory on SWA layers.
        // src/exec/executor_kv_write.cu narrows the calibration view to
        // `nkv * hd` per layer, which closes that path.
        ...
        if (!runtime_config_.runtime.deterministic_gemm) {
            // Mutate the per-Engine RuntimeConfig in place. Also
        ...
        if (!getenv("CUBLAS_WORKSPACE_CONFIG")) {
            ...
            QUENCH_LOG_INFO("Gemma 4: setting CUBLAS_WORKSPACE_CONFIG=:4096:8 for deterministic grouped GEMM");
        }
        ...
        if (!model_->config().overrides.gemma4.force_mmvq) {
            ...
            QUENCH_LOG_INFO("Gemma 4: enabling MMVQ for all weight GEMMs (numerical parity with llama.cpp)");
        }
    }
}
...
// override to exceed the auto cap (documented in quench.conf.example); the manual
...
void Engine::init_compute_max_seq_len_() {
    ...
    if (int v = runtime_config_.runtime.max_seq_len; v > 0) {
        ...
        QUENCH_LOG_INFO("max_seq_len: runtime.max_seq_len=%d", v);
    }
    ...
    if (config_.max_seq_len <= 0) {
        ...
        if (swa_mode == SwaSizingMode::On ||
            ...
            if (swa_layers > 0 && swa_layers < kv_layer_count) {
                QUENCH_LOG_INFO("max_seq_len auto: SWA sizing — %d/%d windowed layers excluded "
        ...
        // default. Cap lifted 16K → 64K → 128K (: coding-agent
