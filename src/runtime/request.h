...
struct Request {
    ...
    // Pin boundary in tokens: -1 = whole prompt; >=0 = pin only the
    ...
    // surfaces tokens per step while the burst is in flight (a blocking
    // per-burst sync would deliver tokens only in burst-sized
    // groups, which would force streaming onto per-step decode).
    bool stream = false;
    ...
    // Embedding request: prefill-only. Hidden states are mean-pooled
