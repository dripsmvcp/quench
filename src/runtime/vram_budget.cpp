...
VRAMBudget compute_vram_budget(const Model& model, const EngineConfig& config, int n_kv_layers, int head_dim,
    ...
    // Budget-planner knobs (quench.conf [vram]): clamp once here so direct
    ...
    // costing ~7x decode on gpt-oss-20b at server defaults.
    ...
    // pass does not charge this"). This closes that divergence.
    {
        ...
        if (lib_reserve > budget.reserve_bytes) {
            QUENCH_LOG_INFO("VRAM budget: reserve floored at the library charge %.0f -> %.0f MiB",
    ...
    // projections — see pre_dequant_phase3_cutlass.cu), so decode REQUIRES
    // the FP16
    ...
    // never sees the fallback, so without charging it here the KV clamp eats
    // the headroom (server default max_seq_len fills VRAM): the fallback
    // then fails to allocate and decode runs against no valid weights →
    // uniform logits → token-0 ("!") garbage. Charge it as overhead
    // so the KV pool sizes down to leave room, mirroring the SWA/SSM
    // footprints above. Same failure class as the NVFP4 and Q4_K
    // weight-cache reserves.
    ...
    if (mcfg.ssm_inner_size > 0) {
        ...
        if (mxfp4_fp16_fallback_bytes > 0)
            QUENCH_LOG_INFO("VRAM budget: MXFP4-GDN FP16 decode fallback reserve %.1f MiB (mandatory — "
    ...
    if (swa_live_tokens > 0 && n_swa_layers > 0 && n_swa_layers <= n_kv_layers) {
        ...
        QUENCH_LOG_INFO("VRAM budget: SWA group %d blocks × %d layers = %.1f MiB "
    ...
    // which the GGUF branch below folds into the weight-cache reserve.
    ...
    // separately in QuantPipeline::build (executor_pre_dequant.cu).
    {
        ...
        if (mcfg.is_nvfp4_prequant) {
            ...
            // The floors are unconditional. An unbacked floor could in theory
            // over-commit the SF slab on a
            // tight card, but the caches are built BEFORE
            // the KV pool, i.e. at the moment VRAM is least contended.
            // What the floor is actually for:
            // cudaMemGetInfo under-reports free while async frees are still
            ...
            if (native_need > cutlass_sf_estimate) {
                QUENCH_LOG_INFO("VRAM budget: native-NVFP4 weight-cache reserve %.1f MiB "
            ...
            // KV pool nobody asked for ( follow-up). Same failure class as
    ...
    // One pool per advertised sequence set, not two. A `* 2`
    // here is not a sizing decision — it makes the live pass
    // diverge from plan_memory(): measured on
    // Qwen3-8B-Q8_0 at two different (seq, batch) combinations, a doubled pool
    // comes out 4096 blocks against the plan's 2048, while mode-2 models —
    // which have no doubling — agree with the plan.
    //
    // Measured: the doubling buys no prefix-cache
    // reuse — 12656 cached tokens and the same warm latency with half the pool —
    // and it binds rather than being VRAM-clamped, holding ~2.3 GiB for
    ...
    // max_batch_size that product blows past real headroom on a
    ...
    if (per_block_total > 0) {
        ...
        if (budget.kv_max_blocks > max_fit_blocks) {
            QUENCH_LOG_INFO("VRAM budget: KV clamped %d → %d blocks to fit post-weight VRAM "
    ...
    if (!user_requested_min) {
        ...
        // Cover the full advertised context when that is CHEAP (
    ...
    if (budget.kv_max_blocks < min_kv_blocks) {
        QUENCH_LOG_INFO("VRAM budget: raising KV from %d to %d blocks (min_kv_tokens=%d)", budget.kv_max_blocks,
