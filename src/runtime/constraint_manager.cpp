...
void ConstraintManager::prepare(bool json_mode, const std::string& json_schema, Tokenizer* tokenizer,
    ...
    //     deciding to call get_weather. A 64-token slack (sized for
    //     "Sure! "-style preambles) expires mid-sentence, and the schema
    //     mask then forces a contentless `{"answer":""}` INSTEAD of the
    //     tool call the model was about to make. 512 covers real
    //     deliberation while still bounding a model that never produces
    //     structure.
    //   - no tools: 0 — even an 8-token "markdown fence" slack lets the model
    //     open ```json fences that then leak into content around otherwise
    ...
    if (has_tools) {
        ...
        if (dialect.empty()) {
            ...
            QUENCH_LOG_INFO("ConstraintManager: no tool-tag dialect for family %d, dropping schema/json_mode",
    ...
    if (use_schema) {
        ...
        if (probe_res && probe_res->type == SchemaType::OBJECT && probe_res->properties.empty() &&
            ...
            QUENCH_LOG_INFO("ConstraintManager: free-form object schema → any-JSON constrainer");
            use_schema = false;
            json_mode = true;
        }
    }
        ...
        if (schema_constrainer_ && schema_constrainer_->is_initialized() &&
            ...
            if (schema) {
                ...
                if (tokenizer && schema_constrainer_->init(*tokenizer, std::move(schema))) {
                    ...
                    QUENCH_LOG_ERROR("Failed to initialize schema constrainer");
                    schema_constrainer_.reset();
                    cached_schema_string_.clear();
                }
            } else {
                QUENCH_LOG_ERROR("Failed to parse JSON schema");
            }
        }
        return;
    }
    ...
    if (json_mode) {
        if (!json_constrainer_) {
            ...
            if (!tokenizer || !json_constrainer_->init(*tokenizer)) {
                QUENCH_LOG_ERROR("Failed to initialize JSON constrainer");
...
bool ConstraintManager::prepare_tool_call(const std::vector<std::pair<std::string, std::string>>& tools,
    ...
    if (bare_args) {
        if (tools.size() != 1) {
            QUENCH_LOG_INFO("ConstraintManager: bare-args tool call needs exactly one tool");
            return false;
        }
        ...
        if (!res || res->type != SchemaType::OBJECT || res->properties.empty()) {
            QUENCH_LOG_INFO("ConstraintManager: bare-args parameter schema not enforceable");
            return false;
        }
    ...
    if (!schema) {
        QUENCH_LOG_INFO("ConstraintManager: tool schemas not enforceable — keeping prompt-hint tool choice");
        return false;
    }
    ...
    // Optional calls keep the tool-deliberation slack: the model spends
    ...
    if (!(schema_constrainer_ && schema_constrainer_->is_initialized() && key == cached_schema_string_)) {
        schema_constrainer_ = std::make_unique<SchemaConstrainer>();
        if (!tokenizer || !schema_constrainer_->init(*tokenizer, std::move(schema))) {
            QUENCH_LOG_ERROR("Failed to initialize tool-call schema constrainer");
            schema_constrainer_.reset();
            cached_schema_string_.clear();
            return false;
        }
    ...
    schema_constrainer_->reset();
    active_schema_ = true;
    QUENCH_LOG_INFO("ConstraintManager: enforcing %s tool call (%zu tool(s), %s body, envelope %zu+%zu chars)",
...
bool ConstraintManager::prepare_regex(const std::string& pattern, Tokenizer* tokenizer,
    ...
    if (!regex_constrainer_->init(pattern, tokenizer, tokenizer->vocab_size())) {
        QUENCH_LOG_WARN("ConstraintManager: regex '%s' rejected — not enforcing it", pattern.c_str());
        return false;
    }
    const int32_t think_close = detect_think_close(tokenizer);
    if (think_close >= 0)
    ...
    active_regex_ = true;
    QUENCH_LOG_INFO("ConstraintManager: constraining output to regex '%s'", pattern.c_str());
    return true;
}
...
bool ConstraintManager::prepare_grammar(const std::string& gbnf, Tokenizer* tokenizer, bool thinking_open) {
    ...
    if (!reuse) {
        ...
        if (!grammar_constrainer_->init(gbnf, tokenizer, tokenizer->vocab_size())) {
            QUENCH_LOG_WARN("ConstraintManager: GBNF grammar rejected (%s) — not enforcing it",
    ...
    QUENCH_LOG_INFO("ConstraintManager: constraining output to a GBNF grammar (%zu bytes)", gbnf.size());
    return true;
}
