...
//
// Why "relaxed" is the default: CUTLASS 3.x grouped GEMM has been observed
// to silently HANG under cudaStreamCaptureModeGlobal during NVFP4 MoE
// prefill capture. Relaxed drops the cross-thread synchronization
// constraint that the
// CUTLASS collective scheduler is suspected to deadlock on, and is a strict
// superset of capturable behaviors — the decode fast path works under
// either mode, so nothing is lost by defaulting to "relaxed".
cudaStreamCaptureMode get_capture_mode() {
    static cudaStreamCaptureMode cached = []() {
        ...
        if (mode == "global") {
            ...
            if (prefill_graph) {
                QUENCH_LOG_WARN(
                    ...
                    "runtime.prefill_graph=true is known to hang CUTLASS "
                    "grouped GEMM during NVFP4 MoE prefill capture. "
                    "Recommended: keep graph_capture_mode=\"relaxed\" (current "
                    "default) when prefill_graph is enabled.");
            }
            QUENCH_LOG_INFO("CudaGraphCapture: using cudaStreamCaptureModeGlobal");
            return cudaStreamCaptureModeGlobal;
        }
        if (mode == "thread_local") {
            QUENCH_LOG_INFO("CudaGraphCapture: using cudaStreamCaptureModeThreadLocal (runtime.graph_capture_mode=thread_local)");
        ...
        QUENCH_LOG_INFO("CudaGraphCapture: using cudaStreamCaptureModeRelaxed (default for prefill-safe capture)");
...
int apply_pdl_edges(cudaGraph_t graph) {
    ...
    for (size_t i = 0; i < num_edges; i++) {
        ...
        // pass (which would log every request as
    ...
    if (skipped_non_pdl > 0)
        QUENCH_LOG_DEBUG("apply_pdl_edges: skipped %d edges (source kernel not PDL-enabled)", skipped_non_pdl);
...
bool CudaGraphCapture::begin_capture(cudaStream_t stream) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("CudaGraphCapture: begin_capture failed: %s", cudaGetErrorString(err));
        return false;
    }
...
bool CudaGraphCapture::end_capture() {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("CudaGraphCapture: end_capture failed: %s", cudaGetErrorString(err));
    ...
    if (pdl::is_available()) {
        ...
        if (converted > 0)
            QUENCH_LOG_DEBUG("CudaGraphCapture: %d edges converted to PDL", converted);
    }
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("CudaGraphCapture: instantiate failed: %s", cudaGetErrorString(err));
        graph_.reset();
        capture_stream_ = nullptr;
        return false;
    }
...
bool CudaGraphCapture::replay(cudaStream_t stream) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("CudaGraphCapture: replay failed: %s", cudaGetErrorString(err));
        return false;
    }
...
bool CudaGraphCapture::end_capture_and_update() {
    ...
    // Log node composition (gated by diagnostics.graph_diag in quench.conf). Symmetric
    ...
    if (graph_exec_) {
        ...
        // this fallback is expected and handled, but an uncleared stale error
        // lingers until engine teardown's leak net clears it (WARN noise).
...
void abort_stream_capture(cudaStream_t stream) {
    ...
    if (g)
        cudaGraphDestroy(g);
    (void)cudaGetLastError();
    QUENCH_LOG_WARN("abort_stream_capture: closed a stray open capture (stream %p)", (void*)stream);
}
...
bool CudaGraphRunner::execute(cudaStream_t stream) {
    if (!decode_fn_) {
        QUENCH_LOG_ERROR("CudaGraphRunner: no decode function set");
        return false;
    }
    ...
    if (!graph_.is_captured()) {
        ...
        QUENCH_LOG_INFO("CudaGraphRunner: capturing CUDA graph (step %d)", step_count_);
        ...
        try {
            ...
            //: the forward threw while the stream was capturing (e.g. the
...
bool CudaGraphRunner::replay_only(cudaStream_t stream) {
    if (!graph_.is_captured())
        return false;
    if (!graph_.replay(stream)) {
        QUENCH_LOG_ERROR("CudaGraphRunner: pipelined replay failed — invalidating graph.");
        graph_.reset();
        step_count_ = 0;
        return false;
    }
...
// Host sets to 1 via cudaMemcpyToSymbol when QUENCH_GRAPH_DIAG is enabled.
...
__global__ void post_decode_step_kernel(
    ...
    if (track_think && d_content_after_think && d_think_exit_step && *d_think_exit_step >= 0 &&
        ...
        // and must not release the grace (post- 0-content fix).
...
bool CudaGraphConditionalRunner::setup(GraphExecutor* executor, const InferenceState& state_template,
    ...
    // Propagate QUENCH_GRAPH_DIAG to device-side tracing in post_decode_step_kernel.
    ...
    // Without the slot pool, every buffer below is a cudaMalloc/cudaFree pair
    // per burst — the whole of the engine's steady-state allocation
    // traffic. The slot is held for exactly
    ...
    if (!config_.stop_ids.empty()) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: stop_ids upload failed: %s", cudaGetErrorString(err));
            goto fail;
        }
    }
    ...
    if (config_.think_end_id >= 0) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: think_count init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_in_think_, &init_think, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: in_think init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_think_exit_step_, &neg_one, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: think_exit_step init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_content_after_think_, &zero, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: content_after_think init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
    }
    ...
    // ---- Allocate penalty ring buffer (prefix history + generated tokens) ----
    {
        ...
        if (has_penalties) {
            ...
            err = cudaMemcpyAsync(d_penalty_count_, &penalty_prefix_len_, sizeof(int), cudaMemcpyHostToDevice,
                                  stream);
            if (err != cudaSuccess) {
                QUENCH_LOG_ERROR("ConditionalRunner: penalty_count init failed: %s", cudaGetErrorString(err));
                goto fail;
            }
        }
    }
    ...
    // ---- Initialize device state ----
    {
        ...
        // the context window covering it ( — the former +1 here shifted
        ...
        *h_burst_done_ = 0;
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: first_token upload failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_position_, &init_pos, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: position init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_context_len_, &init_ctx, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: context_len init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_step_counter_, &init_step, sizeof(int), cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: step_counter init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_step_limit_, &config_.step_limit, sizeof(int),
                              cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: step_limit init failed: %s", cudaGetErrorString(err));
            goto fail;
        }
        err = cudaMemcpyAsync(d_think_limit_, &config_.think_budget_limit, sizeof(int),
                              cudaMemcpyHostToDevice, stream);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: think_limit init failed: %s",
                          cudaGetErrorString(err));
            goto fail;
        }
    }
    ...
    // We override positions, context_lens, token_ids with our device ptrs.
    {
        ...
        // WHILE body, but the dispatch detects stream
        ...
        if (pdl::is_available()) {
            ...
            if (converted > 0)
                QUENCH_LOG_INFO("ConditionalRunner: %d body graph edges converted to PDL", converted);
        }
        ...
        QUENCH_LOG_INFO("ConditionalRunner: graph built (max_steps=%d)", config_.max_steps);
    }
...
bool CudaGraphConditionalRunner::launch(cudaStream_t stream) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("ConditionalRunner: launch failed: %s", cudaGetErrorString(err));
        return false;
    }
...
bool CudaGraphConditionalRunner::rearm(int32_t first_token, int position, int context_len,
    ...
    auto up = [&](void* dst, const void* src, size_t n, const char* what) {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("ConditionalRunner: rearm %s upload failed: %s", what,
                          cudaGetErrorString(err));
            return false;
        }
        return true;
...
std::vector<int32_t> CudaGraphConditionalRunner::wait_and_get_tokens(cudaStream_t stream) {
    ...
    if (sync_err != cudaSuccess) {
        QUENCH_LOG_ERROR("AsyncGraphLoop: stream sync failed (%s) — discarding burst",
...
bool CudaGraphConditionalRunner::try_finish_burst(cudaStream_t stream) {
    ...
    launched_ = false;
    if (sync_err != cudaSuccess) {
        QUENCH_LOG_ERROR("AsyncGraphLoop: burst failed (%s) — no further tokens from this burst",
                      cudaGetErrorString(sync_err));
    }
    return true;
}
...
void CudaGraphConditionalRunner::finish_burst_blocking(cudaStream_t stream) {
    ...
    launched_ = false;
    if (sync_err != cudaSuccess) {
        QUENCH_LOG_ERROR("AsyncGraphLoop: blocking burst finish failed (%s)",
                      cudaGetErrorString(sync_err));
    }
}
...
void CudaGraphConditionalRunner::cleanup() {
    ...
    if (d_token_id_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_token_id_));
        d_token_id_ = nullptr;
    }
    if (d_position_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_position_));
        d_position_ = nullptr;
    }
    if (d_step_limit_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_step_limit_));
        d_step_limit_ = nullptr;
    }
    if (d_think_limit_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_think_limit_));
        d_think_limit_ = nullptr;
    }
    if (d_context_len_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_context_len_));
        d_context_len_ = nullptr;
    }
    if (d_step_counter_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_step_counter_));
        d_step_counter_ = nullptr;
    }
    if (d_stop_ids_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_stop_ids_));
        d_stop_ids_ = nullptr;
    }
    if (d_think_count_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_think_count_));
        d_think_count_ = nullptr;
    }
    if (d_in_think_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_in_think_));
        d_in_think_ = nullptr;
    }
    if (d_think_exit_step_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_think_exit_step_));
        d_think_exit_step_ = nullptr;
    }
    if (d_content_after_think_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_content_after_think_));
        d_content_after_think_ = nullptr;
    }
    if (d_penalty_ring_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_penalty_ring_));
        d_penalty_ring_ = nullptr;
    }
    if (d_penalty_count_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_penalty_count_));
        d_penalty_count_ = nullptr;
    }
    penalty_prefix_len_ = 0;
