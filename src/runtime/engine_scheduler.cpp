...
#include "compute/dispatch_record.h"  // resolved-path summary
...
//
// put log_resolved_dispatch_once_() before the final `return` of step().
...
void Engine::demote_graphs_(GraphDemotionReason reason) {
    ...
    QUENCH_LOG_INFO("CUDA graphs disabled: %s%s", graph_demotion_reason_name(reason),
                 graph_demotion_is_mid_run(reason) ? " (mid-run, one-way)" : "");
}

// One-shot summary of the kernels this model ACTUALLY resolved to.
//
...
void Engine::log_resolved_dispatch_once_() {
    ...
    QUENCH_LOG_INFO("Resolved dispatch: attn_prefill=%s attn_decode=%s moe_prefill=%s graphs=%s", prefill,
...
int Engine::step_async_graph_resume() {
    if (async_graph_runner_.is_setup() && async_graph_req_) {
        ...
        // Incremental harvest: poll the mapped ring buffer instead of
        // blocking until the whole burst retires, so tokens reach the caller
        // (server SSE / quench_decode_step) as the device loop produces them.
        ...
        // quench_decode_step zero-token retry bound (8) never trips; the device
        ...
        if (async_pending_cursor_ >= static_cast<int>(async_pending_tokens_.size()) &&
            ...
            while (async_graph_runner_.poll_new_tokens(async_pending_tokens_) == 0) {
                ...
                if (std::chrono::steady_clock::now() > deadline) {
                    QUENCH_LOG_ERROR("AsyncGraphLoop: no token for 30 s and no done flag — "
        ...
        QUENCH_LOG_DEBUG("AsyncGraphLoop: graph tokens exhausted, continuing with step decode");
    }
...
bool Engine::supports_chunked_prefill_() const {
    ...
    // head_dim 256/512) is supported via:
    //   - the cuBLAS softmax sliding_window param,
...
int Engine::resolve_prefill_chunk_size_() const {
    ...
    if (explicit_val < 0) {
        // Default 2048. Chunked prefill re-reads
...
// (quench_perplexity). See engine.h for the contract.
// =====================================================================
...
bool Engine::begin_perplexity_capture(const int32_t* tokens, int n) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy(ppl_capture_.d_tokens, tokens,
                                  static_cast<size_t>(n) * sizeof(int32_t), cudaMemcpyHostToDevice));
    QUENCH_CUDA_CHECK_LOG(cudaMemset(ppl_capture_.d_nll, 0, static_cast<size_t>(n) * sizeof(double)));
...
bool Engine::end_perplexity_capture(double* out_ppl) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaDeviceSynchronize());
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy(h_nll_pos.data(), ppl_capture_.d_nll,
    ...
    if (ppl_capture_.d_match) {
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemcpy(h_match.data(), ppl_capture_.d_match,
    ...
    // QUENCH_PPL_DUMP=1: sparse per-position NLL (first 16, every 16th, tail).
    // QUENCH_PPL_DUMP=full: every position — needed for cross-mode forensics;
    ...
    if (!runtime_config_.diagnostics.ppl_dump.empty()) {
        ...
        QUENCH_LOG_DEBUG("[PPL-DUMP] per-pos nll:%s", nll_line.c_str());
    }
    double h_nll = 0.0;
    for (int i = first; i <= last; ++i)
        h_nll += h_nll_pos[i];
    ...
    QUENCH_LOG_INFO("perplexity_nll: n=%d first=%d last=%d counted=%d mean_nll=%.4f  PPL=%.4f", n,
                 first, last, counted, h_nll / counted, ppl);
    if (match_sum >= 0)
        QUENCH_LOG_INFO("greedy top1 match: %ld/%d (%.2f%%)", match_sum, counted,
...
bool Engine::prefill_allocate_kv_blocks_(std::shared_ptr<Request>& req, int kv_bs,
    ...
    // bias the pooled vector. Same class as the ppl_capture guard.
    ...
    if (kv_manager_->prefix_caching_enabled() && existing == 0 && offset == 0 && !ppl_capture_.active &&
        ...
        if (prefix_reused > 0) {
            ...
            if (skip_tokens > offset) {
                QUENCH_LOG_INFO("PrefixCache: seq %d skipping %d/%d prefill tokens (%d blocks reused)",
...
bool Engine::prefill_upload_metadata_(std::shared_ptr<Request>& req,
    ...
    auto check = [&req](cudaError_t err, const char* op) {
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Engine::step prefill %s failed: %s", op, cudaGetErrorString(err));
            req->status = RequestStatus::CANCELLED;
        }
    ...
    if (prefill_pool_ && chunk_len <= config_.max_seq_len) {
        ...
        if (!check(cudaMallocAsync(&d_token_ids, chunk_len * sizeof(int32_t), pf_stream),
            ...
            if (d_token_ids)
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_token_ids, pf_stream));
            if (d_positions)
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_positions, pf_stream));
            if (d_block_tables)
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables, pf_stream));
            if (d_block_tables_swa)
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables_swa, pf_stream));
            if (d_context_lens)
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_context_lens, pf_stream));
            kv_manager_->free_sequence(req->id);
            return false;
        }
    }
    ...
    // (: catastrophic chunked-prefill NLL, timing/arch-dependent).
    ...
    if (pf_staging_evt_ && (h_pf_token_ids_.as<int32_t>() || h_pf_positions_.as<int>()) && chunk_len <= config_.max_seq_len)
        QUENCH_CUDA_CHECK_LOG(cudaEventSynchronize(pf_staging_evt_));
    ...
    if (pf_staging_evt_ && (h_pf_token_ids_.as<int32_t>() || h_pf_positions_.as<int>()) && chunk_len <= config_.max_seq_len)
        QUENCH_CUDA_CHECK_LOG(cudaEventRecord(pf_staging_evt_, pf_stream));
...
void Engine::step_prefill_one(std::shared_ptr<Request>& req, int effective_chunk, cudaStream_t pf_stream) {
    ...
    // usable here (quench_prefill seeds a 4096 placeholder; decode_step drives the
    ...
    // stay large and only late chunks shrink (clamping EVERY chunk to the
    // final-chunk worst case cap²/total_input would mean e.g. 32-token
    ...
    if (swa_sizing_active_) {
        ...
        if (req->swa_restore && offset > 0 && offset == req->cached_tokens) {
            ...
            if (!ok) {
                QUENCH_LOG_WARN("SwaSnapshot: restore failed for req %d at %d tokens — cancelling",
                             req->id, offset);
                kv_manager_->free_sequence(req->id);
                req->status = RequestStatus::CANCELLED;
                return;
            }
            QUENCH_LOG_INFO("SwaSnapshot: restored %d-token window for req %d (enqueue %.2f ms)",
                         offset, req->id,
    ...
    if (offset == 0) {
        ...
        // The legacy global-image path (quench_set_image + the mmproj pipeline)
    ...
    // this runs exactly once regardless of how the prompt is chunked.
    {
        if (!req->qwen_patches.empty() && !encode_qwen_image_for_(*req, pf_stream)) {
            QUENCH_LOG_ERROR("Qwen3-VL: image encode failed — cancelling request %llu",
    ...
    if (req->vision_emb && offset == 0) {
        ...
        // Global path: the C-API (quench_set_image) / quench-cli set ONE image on the
        // engine for the next generation; its request carries no per-request
        // embeddings, so bind the global ones. (The server binds embeddings
        // per request, but quench_prefill_with_params builds a
        // bare request, so without this the CLI's image would be silently
        // ignored.)
    ...
    if (!is_last_chunk) {
        ...
        // aborted capture costs a wasted forward per chunk. Run eager.
        ...
        // truncated long-context prefill (: teacher-forced PPL
        ...
        // Teacher-forced NLL for this chunk's positions (quench_perplexity).
        ...
        // Embedding pooling for this chunk — hidden_ still holds it.
        ...
        QUENCH_LOG_DEBUG("Chunked prefill: req %d chunk [%d, %d) of %d", req->id, offset, offset + chunk_len,
                      total_input);
        ...
        // Embedding request: last chunk — forward, pool, finish.
        ...
        if (use_event_sync) {
            ...
            if (state.banned_tokens != nullptr && state.n_banned_tokens > 0) {
                ...
                for (int bi = 0; bi < state.n_banned_tokens; bi++) {
                    ...
                    if (tid >= 0 && tid < vocab) {
                        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(lp + tid, &neg_inf, sizeof(float),
                                                           cudaMemcpyHostToDevice, pf_stream));
                    }
                }
            }
        ...
        if (req->logprobs && prefill_logits_out.data != nullptr) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(executor_->h_logits_pinned(), d_logits,
                                               vocab_size * sizeof(float), cudaMemcpyDeviceToHost,
                                               pf_stream));
            QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(pf_stream));
        ...
        // Teacher-forced NLL for the LAST chunk's positions (quench_perplexity).
        ...
        QUENCH_LOG_DEBUG("Prefill -> token %d (ctx=%d): id=%d [%s]", (int)req->output_tokens.size(),
...
void Engine::step_decode(cudaStream_t dec_stream) {
    ...
    // stage 1: at batch > 1 (dense, non-recurrent), ONE request per
    ...
    for (auto& req : decode_batch) {
        ...
        if (blocks_needed > blocks_have) {
            ...
            if (new_block < 0) {
                ...
                // entry is LIVE and quench has no recompute path, so evicting one
                ...
                // Log loudly: a silent cancel surfaces as a
                // bare "internal error" at the API and is very hard to
                // attribute (e.g. Gemma-4-12B at ctx 16384 on a
                // 1024-block FP16-KV pool).
...
void Engine::decode_build_inference_state_(GPUBatch& gpu_batch,
    ...
    // Deliberately not one flag per constrainer (json / schema / regex / …):
    // every consumer only ever asks for
    // their disjunction, and a constrainer missing from one of several
    // per-kind flags could never
    // reach the constrained pipeline and would silently stay on the eager path.
    ...
    // constraints stay enforced at batch>1 (otherwise they would be silently
    // dropped whenever a constrained request shares a decode batch).
...
void Engine::step_decode_forward(std::vector<std::shared_ptr<Request>>& valid_decode,
    ...
    auto sample_per_request = [&](const Tensor& logits) -> std::vector<int32_t> {
        ...
        // gathers all tokens with ONE pinned D2H + ONE stream sync. A
        // per-row synchronous readback would block the engine thread
        // ~850 us per sequence per step (pageable 4-byte D2H + sync each) —
        // 29% GPU idle at n=16 sustained serving (per nsys). Rows
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (req_needs_pen && !req->output_tokens.empty() && d_penalty_tokens_) {
                ...
                if (rn <= d_penalty_tokens_capacity_) {
                    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_penalty_tokens_, req->output_tokens.data(),
    ...
    if (config_.use_cuda_graphs && !profiling && gpu_batch.n_sequences > 0 && graph_idx < kMaxGraphPoolSize &&
        ...
        //: the decode-attention LAUNCH topology derives from
    ...
    if (residual_meta_d_buf_ != nullptr) {
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(residual_meta_d_buf_, dec_stream));
    ...
    if (mtp_spec_decode_enabled() && model_ && model_->mtp_.has_value() &&
        ...
        if (mtp_pending_prediction_ >= 0) {
            ...
            if (s_pattern_log) {
                ...
                QUENCH_LOG_INFO("MTP-PAT %s pred=%d '%s' actual=%d '%s'",
                             match ? "+" : "-",
        ...
        // Verify-consumer sync gate: the chain feed below appends a
        ...
        auto* ws_gate = static_cast<quench::MtpDraftWorkspace*>(mtp_ws_storage_);
        ...
        if (h_view.data != nullptr && mtp_synced) {
            ...
            if (s_pre_norm_h) {
                ...
                quench::rmsnorm(in_view, model_->output_norm(), out_view,
            ...
            // Verify-consumer bookkeeping: the kept pair covers
...
void Engine::step_decode_process_outputs(std::vector<std::shared_ptr<Request>>& valid_decode,
    ...
    if (needs_logprobs && decode_logits_out.data != nullptr) {
        ...
        for (int i = 0; i < static_cast<int>(valid_decode.size()); i++) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(h_base + static_cast<size_t>(slot) * vocab_size, d_logits,
                                               vocab_size * sizeof(float), cudaMemcpyDeviceToHost,
                                               dec_stream));
            slot++;
        }
        QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(dec_stream));
    ...
    for (int i = 0; i < static_cast<int>(valid_decode.size()); i++) {
        ...
        QUENCH_LOG_DEBUG("Decode step %d (ctx=%d, pos=%d): id=%d [%s]", (int)req->output_tokens.size(),
    ...
    if (decode_graph_pool_[0].graph_path_available() && valid_decode.size() == 1 && !offload_mgr_ &&
        ...
        if (async_compatible && !needs_eager_for_text_think && dreq->status == RequestStatus::DECODING &&
            ...
            // Streaming requests run the loop too:
            // step_async_graph_resume polls the mapped ring buffer and
            // surfaces tokens per-step while the burst is still running —
            // SSE stays real per-token (a blocking per-burst
            // sync would deliver tokens only in burst-sized groups, forcing
            // streaming onto per-step decode and forfeiting the +27-45% loop
            // win).
            //
            ...
            // limit; <=0 leaves the loop unbounded.
...
// (the ~15-20% step tail at n=16 sustained serving, per nsys).
//
...
bool Engine::pipeline_enqueue_next_(int parity, cudaStream_t stream) {
    ...
    if (!runner.is_ready())
        return false;
    // guard: the captured decode-attention launch topology covers the
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!executor_->sample_single_from_logits_async(seq_logits, per, i, stream)) {
            ...
            QUENCH_LOG_ERROR("decode-pipeline: unexpected sampler decline (row %d) — not chaining", i);
            return false;
        }
    }
...
bool Engine::pipeline_enter_(std::vector<std::shared_ptr<Request>>& rows, const GPUBatch& gpu_batch,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (needs_pen && count > 0) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(row_hist, req->output_tokens.data(),
...
void Engine::step_decode_pipeline_(cudaStream_t stream) {
    ...
    //: yield the chain periodically so the plain path's round-robin
