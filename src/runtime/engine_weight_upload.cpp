...
//
// Split out of engine.cpp. Method remains Engine::* with declaration in engine.h.
...
bool Engine::init_weights() {
    ...
    executor_->set_runtime_config(dispatch_policy_);
    {
        ...
        if (config_.dual_path_quant) {
            ...
            QUENCH_LOG_INFO("Dual-path quant: attention weights → FP8, FFN weights → NVFP4");
        }

        if (runtime_config_.calibration.enabled) {
            executor_->enable_calibration();
            QUENCH_LOG_INFO("Activation calibration ON — collecting per-channel activation magnitudes.");
        ...
        if (config_.streaming_kv_enabled) {
            ...
            if (config_.kv_cache_dtype != QType::F16) {
                ...
                if (n_sinks > 0 && win > 0) {
                    QUENCH_LOG_INFO("StreamingLLM smart KV cache enabled: %d sinks + %d-token window", n_sinks,
                                 win);
    ...
    // poison the stream (cross-model GDN→Gemma-4 garbage repro).
    {
        ...
        if (max_persist > 0) {
            ...
            QUENCH_LOG_INFO("L2 persisting cache: reserved %zu MB / %zu MB total", reserve >> 20,
                         max_persist >> 20);
        }
    }
    ...
    size_t expert_reserve = executor_->workspace_estimate();
    {
        ...
        // Packing- and scale-aware K+V per-layer block bytes: raw
        // dtype_size() returns 0 for NVFP4/MXFP4_KV,
        // which would zero the KV headroom out of the expert-offload decision.
        ...
        QUENCH_LOG_INFO("Expert upload reserve: %.2f MiB (workspace=%.2f, kv=%.2f, ssm+safety=rest)",
    ...
    QUENCH_LOG_INFO("GPU memory before weight upload: %zu MiB free / %zu MiB total",
    ...
    if (!upload_stream_raii.create(cudaStreamNonBlocking))
        QUENCH_LOG_WARN("Failed to create weight-upload stream; uploading on the main stream.");
    ...
    if (!model_->upload_weights_gpu(config_.compute_dtype, upload_stream ? upload_stream : stream_,
        ...
        QUENCH_LOG_ERROR("Weight upload failed. Try a smaller quantization.");
        return false;
    }
    ...
    if (upload_stream) {
        ...
        if (upload_done.record(upload_stream))
            QUENCH_CUDA_CHECK_LOG(cudaStreamWaitEvent(stream_, upload_done.get()));
    }

    size_t free_after = 0, total_after = 0;
    cudaMemGetInfo(&free_after, &total_after);
    QUENCH_LOG_INFO("GPU memory after weight upload: %zu MiB free / %zu MiB total (weights ~%zu MiB)",
    ...
    if (mcfg.n_experts > 0 && !model_->profile().is_gpt_oss) {
        ...
        if (experts_on_host_ && config_.use_cuda_graphs) {
            // The opt-in `moe.allow_graphs_under_offload` flag skips
            ...
            // architectural caveat; the durable fix is dispatch kernels
            // that read the device mirror at runtime so the captured graph
            // adapts correctly.
            if (runtime_config_.moe.allow_graphs_under_offload) {
                ...
                QUENCH_LOG_INFO(
                    "  Tip: if model+KV fits in VRAM, set QUENCH_EXPERT_OVERHEAD_PCT=10 "
    ...
    // The mandatory native-NVFP4 decode caches are deliberately NOT protected
    // by a physical balloon (a cudaMalloc taken right after the upload purely so
    // every later live-free reader sees fewer bytes, released again before the
    // cache build). The cache build runs BEFORE the KV pool, and a balloon
    // measured across five configs binds on
    // exactly one, where it costs 5x KV capacity (16 vs 87 blocks) to buy coverage
    // whose absence changes neither throughput nor correctness. The guarantee
    // comes from the plan instead: vram_budget floors phase 3 at the
    // measured demand (mandatory_sf_bytes / mandatory_moe_bytes), which is what
    // invariant I4 asks for — capacity decided up front, not hidden from a live query.
    ...
    if (config_.gpu_layers >= 0) {
        ...
        if (!offload_mgr_->init(model_.get(), config_.gpu_layers)) {
            QUENCH_LOG_WARN("Layer offloading init failed, continuing without it");
            offload_mgr_.reset();
        }
    }
