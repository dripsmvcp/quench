// =============================================================================
// engine_spec_capture.cpp — graph-captured verify chunk
// =============================================================================
//
// The eager verify forward pays ~1800 kernel launches per cycle in host
// launch pacing (~8 ms on Qwen3-Coder-30B after the LM-head batching).
...
// code path (cuBLASLt/CUTLASS algo warmup — census PR showed only the
...
// mid-serving claim into a planned one, which is the difference that was
...
void Engine::prewarm_spec_scratch_() {
    ...
    if (!ensure_spec_buffers_(chunk_cap, table_cap)) {
        QUENCH_LOG_WARN("[spec] scratch prewarm failed (chunk_cap=%d table_cap=%d) — "
                     "buffers will be taken on first use instead",
                     chunk_cap, table_cap);
        return;
    }
    executor_->prewarm_verify_scratch();
    QUENCH_LOG_INFO("[spec] scratch prewarmed: chunk_cap=%d table_cap=%d ctx_cap=%d", chunk_cap,
...
int Engine::spec_capture_bucket_(int chunk_len) const {
    ...
    // 3/5 buckets: the decode-attention verify route derives its
    ...
    // 4 = the token-recycling depth-3 chunk: exactly one batched-GEMV
...
bool Engine::spec_capture_ready_(int ctx_padded) {
    ...
    if (spec_capture_ctx_cap_ < 0) {  // resolve once per engine
        ...
        QUENCH_LOG_INFO("[spec-capture] %s (ctx_cap=%d)",
...
bool Engine::spec_captured_forward_(InferenceState& state, Tensor& logits_out,
    ...
    if (ws_gen != spec_capture_ws_gen_) {
        if (!spec_graphs_.empty()) {
            QUENCH_LOG_INFO("[spec-capture] workspace reallocated — dropping %zu cached graphs",
                         spec_graphs_.size());
            free_spec_graphs_();
        }
    ...
    if (slot.exec) {
        ...
        if (err == cudaSuccess)
            return true;
        QUENCH_LOG_WARN("[spec-capture] graph launch failed (%s) — dropping graph cache",
    ...
    auto doom_check = [this](const char* why) {
        if (++spec_capture_failures_ >= 2) {
            ...
            QUENCH_LOG_WARN("[spec-capture] disabled after repeated failures (%s)", why);
        }
    };

    cudaError_t err = cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal);
    if (err != cudaSuccess) {
        QUENCH_LOG_WARN("[spec-capture] begin capture failed: %s", cudaGetErrorString(err));
        cudaGetLastError();
    ...
    if (forward_threw || err != cudaSuccess || !graph) {
        QUENCH_LOG_WARN("[spec-capture] capture failed: %s%s%s",
    ...
    graph.reset();
    if (err != cudaSuccess) {
        QUENCH_LOG_WARN("[spec-capture] instantiate failed: %s", cudaGetErrorString(err));
        cudaGetLastError();
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_WARN("[spec-capture] first launch failed: %s", cudaGetErrorString(err));
        cudaGetLastError();
    ...
    QUENCH_LOG_INFO("[spec-capture] verify chunk graph cached (n_tokens=%d, ctx_tier=%d, rec_slot=%d)",
                 state.n_tokens, state.ctx_capacity, rec_slot);
    return true;
}

// =============================================================================
// graph-captured-verify feasibility probe (diagnostics.spec_capture_probe)
// =============================================================================
...
void Engine::spec_capture_probe_forward_(InferenceState& state, Tensor& logits_out,
    ...
    cudaError_t err = cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal);
    if (err != cudaSuccess) {
        QUENCH_LOG_WARN("[spec-capture-probe] begin capture failed: %s", cudaGetErrorString(err));
        cudaGetLastError();
        executor_->forward_logits(state, logits_out, stream);
        return;
    }
    ...
    if (!forward_threw && err == cudaSuccess && graph) {
        ...
        if (err == cudaSuccess) {
            ...
            if (err == cudaSuccess) {
                ...
                QUENCH_LOG_WARN("[spec-capture-probe] graph launch failed: %s",
                             cudaGetErrorString(err));
            }
        } else {
            QUENCH_LOG_WARN("[spec-capture-probe] instantiate failed: %s", cudaGetErrorString(err));
        }
    } else {
        QUENCH_LOG_WARN("[spec-capture-probe] capture failed: %s%s%s",
                     err != cudaSuccess ? cudaGetErrorString(err) : "(forward threw)",
                     forward_threw ? " forward exception: " : "", forward_threw ? what : "");
    }
    cudaGetLastError();
    QUENCH_LOG_INFO("[spec-capture-probe] chunk n_tokens=%d %s (launched %ld/%ld)", state.n_tokens,
