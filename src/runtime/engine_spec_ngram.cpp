...
bool Engine::ensure_spec_buffers_(int chunk_cap, int max_blocks) {
    if (spec_chunk_cap_ >= chunk_cap && spec_block_table_cap_ >= max_blocks)
        return true;
    free_spec_buffers_();
    // consolidated staging: tokens/positions/row-ctx-lens (chunk_cap
    ...
    bool ok = cudaMalloc(&d_spec_stage_, stage_ints * sizeof(int32_t)) == cudaSuccess &&
              ...
              // decode-attention verify route staging (see engine.h).
    ...
    if (!ok) {
        QUENCH_LOG_WARN("spec-ngram: buffer allocation failed — speculation disabled this step");
        free_spec_buffers_();
        return false;
    }
...
void Engine::log_spec_stats_() const {
    if (spec_stats_.verify_steps + spec_stats_.miss_steps == 0)
        return;
    QUENCH_LOG_INFO("[spec-ngram] verify_steps=%lld miss_steps=%lld drafted=%lld accepted=%lld "
...
bool Engine::ensure_spec_state_scratch_() {
    ...
    if (spec_state_scratch_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(spec_state_scratch_));
        spec_state_scratch_ = nullptr;
        spec_state_scratch_bytes_ = 0;
    }
    if (cudaMalloc(&spec_state_scratch_, bytes) != cudaSuccess) {
        QUENCH_LOG_WARN("spec-hybrid: state scratch alloc failed (%zu bytes) — "
...
void Engine::free_spec_buffers_() {
    ...
    if (d_spec_stage_) QUENCH_CUDA_CHECK_LOG(cudaFree(d_spec_stage_));
    if (d_spec_block_table_) QUENCH_CUDA_CHECK_LOG(cudaFree(d_spec_block_table_));
    if (d_spec_block_table_swa_) QUENCH_CUDA_CHECK_LOG(cudaFree(d_spec_block_table_swa_));
    if (d_spec_row_block_tables_) QUENCH_CUDA_CHECK_LOG(cudaFree(d_spec_row_block_tables_));
    if (d_spec_argmax_) QUENCH_CUDA_CHECK_LOG(cudaFree(d_spec_argmax_));
...
bool Engine::spec_burst_launch_ok_(const Request& req) const {
    // QUENCH_SPEC_TRACE: dump every term — the launch decision decides WHICH
    ...
    if (runtime_config_.diagnostics.spec_trace) {
        QUENCH_LOG_INFO("[burst-ok?] req=%d out=%zu pool_avail=%d offload=%d graphs=%d setup=%d parked=%d "
...
bool Engine::spec_ngram_gates_ok_(const Request& req, bool ignore_think) const {
    ...
    if (req.logprobs || req.json_mode || !req.json_schema.empty() ||
        ...
        return false;  // constrained decode: verify replicates no FSM masks
    ...
    // Long-context economics on the DENSE path: the captured chunk
    ...
    // the live context. Measured on Qwen3-8B Q8_0: a verify step
    ...
    // on draft-rich code-edit (Qwen3-Coder-30B-FP4) with a -3-7%
...
bool Engine::step_spec_verify_(std::shared_ptr<Request>& req, cudaStream_t stream, int min_draft) {
    ...
    // stage 2 — depth-aware long-context gate: a verify step costs
    ...
    // (78-94% depth-1 accept on Qwen3.6). Subject to the economics
    ...
    // where the suffix/n-gram sources measure 0 drafts.
    ...
    // (1.55-1.9 emitted/verify vs ~1.9x step cost). Falls back
    ...
    // batch-RR economics: at batch > 1 the WHOLE batch waits for the
    ...
    if (draft.empty() && !mc_on) {
        ...
        if (++req->spec_consecutive_misses >= scfg.give_up_after && scfg.give_up_after > 0 &&
            ...
            QUENCH_LOG_INFO("spec-ngram: req %d gave up after %d consecutive draft misses — "
    ...
    // Graph-captured verify: pad the chunk up to its bucket length so
    ...
    //: dense verify chunks route their attention through the batched-
    ...
    //: [tokens | positions | row_ctx_lens | ctx_len, past_len,
    ...
    auto check = [&](cudaError_t err, const char* op) {
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("spec-ngram: %s failed: %s", op, cudaGetErrorString(err));
            return false;
        }
        return true;
    };
    ...
    // decode-attention route: present the chunk rows as n same-KV
    ...
    // FP16 KV gather (557 vs ~44 us/layer at 16k, per nsys).
    ...
    if (penalties) {
        ...
        if (pstate.penalty_tokens == nullptr) {
            ...
            if (hybrid)
                QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(
    ...
    // One D2H covers [argmax | topm] (contiguous block,); without the
    ...
    if (!argmax_ok) {
        ...
        if (hybrid)
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(ssm_state_->seq_base(rec_slot), spec_state_scratch_,
    ...
    if (hybrid && matched < K && req->status != RequestStatus::FINISHED) {
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_spec_context_len_, &replay_ctx, sizeof(int),
                                           cudaMemcpyHostToDevice, stream));
    ...
    if (draft_from_mtp) {
        ...
        // the verify-chunk cost, both of which have moved.
    ...
    if (!req->spec_ngram_given_up && scfg.give_up_after > 0 &&
        ...
        QUENCH_LOG_INFO("spec-ngram: req %d gave up (%s: verifies=%d accepted=%lld/%lld misses=%d) — "
                     "re-enabling async graph loop",
...
// graph-captured verify live in engine_spec_capture.cpp.
