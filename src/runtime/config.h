...
// quench.conf — central runtime configuration.
//
// Replaces ~50 ad-hoc QUENCH_*-prefixed environment variables that were scattered
...
//   3. ./quench.conf                   working-dir relative
//   4. ~/.config/quench/quench.conf       user config directory
...
struct RuntimeConfig {
    struct Runtime {
        ...
        // Legacy env: QUENCH_DETERMINISTIC=1.
        ...
        // quench-server processes can share one card without overcommitting it.
        ...
        // CLI flag --vram-budget / C-API QuenchConfig.vram_budget_mb override.
        ...
        // capture. "relaxed" is the default because
        // `cudaStreamCaptureModeRelaxed` is a strict superset of
        // capturable behaviors: the decode fast path
        // works under either mode, while the prefill path
        // gets a real chance of capturing without hanging.
        //
        // Set graph_capture_mode = "global" via quench.conf to opt into
        // the strict mode (any decode regression should also be
        ...
        // ON, backed by an A/B sweep across
        // Gemma-4-26B-NVFP4 / Qwen3.6-35B-NVFP4 / Qwen3-Coder-30B-FP4
        // (3 trials × 4 capture modes each): no hang on any
        // (model, capture_mode) combination with
        // `graph_capture_mode = "relaxed"` as the default. Decode tg
        ...
        // `--set runtime.prefill_graph=false` or quench.conf if a model
        // regresses.
        ...
        // and speculation paths are unaffected. <=0 leaves the loop
        // unbounded. IGNORED when `deterministic` is set: the unbounded loop is
        ...
        // instead of idling it (the ~15-20% step tail at n=16).
    ...
    struct Vram {
        ...
        // batch (1..16) and context (1024..4096) — it is not a workspace quench
        ...
        // constant. Empty = the default cache location;
    ...
    // (Gemma-4 knobs live in ModelConfig::Overrides::Gemma4 —
    ...
    struct Server {
        ...
        // quench.conf.example) and what delivers the advertised warm-prompt TTFT
        // win + cache_read_input_tokens reporting (: shipping OFF meant the
        // prebuilt image never cached unless an quench.conf opted in). Library /
        ...
        // requesting call (the warm weight cache,, makes repeats cheap);
        ...
        // by design) and accounted in the expert-offload reserve. quench-cli
    ...
    struct WarmCache {
        ...
        // ("<file>.impwcache" / "<dir>/.quench_warm_cache"). Point this at a
    ...
    struct Suspend {
        ...
        // reset would orphan (quench itself re-arms everything at the next init).
    ...
    // model, no MTP head. Greedy-only: the verify step replays the
    ...
    // burst_rearm + miss_burst keep draft-miss fragmentation ~free, with a
    // draft-poor floor of only -0.2%/-0.9% on
    // dense Q8/NVFP4 — hence default-ON. spec_ngram_gates_ok_
    ...
    struct Constrained {
        // Jump-ahead over schema-forced spans: when the schema FSM
        ...
        // -3-5% on Qwen3-8B (Q8 + NVFP4) — the model picks
    ...
    // write them for quench-quantize's AWQ scale search. Not an inference feature:
    ...
    struct Calibration {
        bool enabled = false;
        // Where quench_calibration_write() puts the file. Empty means the caller
        // supplies the path.
        std::string out_path;
    } calibration;
    ...
    // that stops. (An unknown key in quench.conf stays a warning: a config file
...
//
// The C API constructs Engine inside src/api/quench_api.cpp. Tool mains
// (quench-cli, quench-server) load a RuntimeConfig from quench.conf + CLI
// overrides at startup and need to hand that to Engine::init without
// passing it through the ABI-stable QuenchConfig C struct.
//
// Workflow: tool main calls set_pending_runtime_config(loaded_cfg) once,
// then later quench_context_create() pulls it via take_pending_runtime_config()
// and passes to Engine::init. The lifetime is bounded to a single
// Engine construction; there is deliberately no process-wide singleton
// and no per-call accessor.
