...
// net for exception paths that unwound past an active capture.
...
public:
    ...
    // Close an in-flight capture without keeping its graph. Called
...
public:
    ...
    struct Config {
        ...
        // after </think> must not release the grace (post- 0-content fix).
...
private:
    ...
    // copy. Deliberately NOT aliased with h_step_counter_: aliasing is
    // harmless when tokens are only harvested after a full-burst sync, but a
    // polling host could read
