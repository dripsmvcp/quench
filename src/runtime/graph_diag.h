...
namespace quench::graph_diag {

enum class Phase { NORMAL, CAPTURE, REPLAY };

inline thread_local Phase g_phase = Phase::NORMAL;

inline bool enabled() { return quench::process_diag_graph_diag(); }

inline const char* dump_path() { return quench::process_diag_graph_dump_dir(); }
...
// Post-launch sync + error check. Only runs when QUENCH_GRAPH_DIAG is set.
...
inline void check_post_launch(cudaStream_t stream, const char* label) {
    ...
    if (launch_err != cudaSuccess) {
        QUENCH_LOG_ERROR("[graph_diag:%s] cudaGetLastError after launch: %s", label,
                      cudaGetErrorString(launch_err));
    }
    cudaError_t sync = cudaStreamSynchronize(stream);
    if (sync != cudaSuccess) {
        QUENCH_LOG_ERROR("[graph_diag:%s] cudaStreamSynchronize after launch: %s", label,
                      cudaGetErrorString(sync));
    }
}

// Dump verbose DOT of a captured graph if QUENCH_GRAPH_DUMP is set.
...
inline void dump_graph(cudaGraph_t g, const char* label) {
    ...
    cudaError_t err = cudaGraphDebugDotPrint(g, path.c_str(), cudaGraphDebugDotFlagsVerbose);
    (void)cudaGetLastError();  // clear sticky state unconditionally
    if (err != cudaSuccess) {
        static bool warned = false;
        if (!warned) {
            QUENCH_LOG_WARN(
                "[graph_diag:%s] cudaGraphDebugDotPrint(%s) failed: %s "
                "(known WSL2 limitation; kernel-node summary is the fallback)",
                label, path.c_str(), cudaGetErrorString(err));
            warned = true;
        }
    } else {
        QUENCH_LOG_INFO("[graph_diag:%s] wrote %s", label, path.c_str());
    }
}
...
inline void log_kernel_nodes(cudaGraph_t g, const char* label) {
    ...
    if (cudaGraphGetNodes(g, nullptr, &num_nodes) != cudaSuccess || num_nodes == 0) {
        QUENCH_LOG_INFO("[graph_diag:%s] graph has 0 nodes", label);
        return;
    }
    std::vector<cudaGraphNode_t> nodes(num_nodes);
    if (cudaGraphGetNodes(g, nodes.data(), &num_nodes) != cudaSuccess)
        return;

    int n_kernel = 0, n_memcpy = 0, n_memset = 0, n_host = 0, n_alloc = 0, n_free = 0, n_cond = 0,
        n_child = 0, n_other = 0;
    ...
    QUENCH_LOG_INFO(
        "[graph_diag:%s] %zu nodes (kernel=%d memcpy=%d memset=%d "
        "host=%d alloc=%d free=%d cond=%d child=%d other=%d)",
        label, num_nodes, n_kernel, n_memcpy, n_memset, n_host, n_alloc, n_free, n_cond, n_child, n_other);
    ...
    for (size_t i = 0; i < num_nodes; ++i) {
        ...
        if (verbose) {
            QUENCH_LOG_INFO(
                "[graph_diag:%s]   kernel[%d] func=%p grid=(%u,%u,%u) "
                "block=(%u,%u,%u) smem=%u",
                label, idx, kp.func, kp.gridDim.x, kp.gridDim.y, kp.gridDim.z, kp.blockDim.x, kp.blockDim.y,
                kp.blockDim.z, kp.sharedMemBytes);
        } else {
            QUENCH_LOG_DEBUG("[graph_diag:%s]   kernel[%d] func=%p grid=(%u,%u,%u)", label, idx, kp.func,
