// =============================================================================
// engine_spec_mtp.cpp — MTP head as a draft source for the verify loop
// =============================================================================
//
...
// the prefill feed here aligns with it (the pre- prefill fed the
...
void Engine::mtp_unbind_(const char* why) {
    if (mtp_bound_req_ >= 0 && !mtp_stale_logged_) {
        QUENCH_LOG_INFO("mtp-spec: drafting off for req %d (%s)", mtp_bound_req_, why);
...
bool Engine::mtp_feed_pairs_(const int32_t* tokens, const void* d_hidden_rows, int n_pairs,
    ...
    const bool device_chain = chain_after && ws->n_experts == 0 &&
                              ws->d_chain_tokens != nullptr && K <= quench::kMtpMaxChainK;
    ...
    if (device_chain) {
        ...
        int32_t h_chain[quench::kMtpMaxChainK];
...
bool Engine::enable_mtp_spec_decode(int k) {
    if (k <= 0) {
        QUENCH_LOG_ERROR("enable_mtp_spec_decode: k must be > 0 (got %d)", k);
        return false;
    }
    if (!model_) {
        QUENCH_LOG_ERROR("enable_mtp_spec_decode: no model loaded");
        return false;
    }
    if (!model_->mtp_.has_value() || !model_->mtp_->loaded) {
        QUENCH_LOG_ERROR("enable_mtp_spec_decode: model has no MTP head loaded");
        return false;
    }
    if (mtp_ws_storage_ != nullptr) {
        QUENCH_LOG_WARN("enable_mtp_spec_decode: already enabled, k=%d -> %d", mtp_spec_k_, k);
        mtp_spec_k_ = k;
        return true;
    }
    const int hidden_dim = model_->config_.d_model;
    ...
    // (~16 MiB each for K and V at Qwen3.6 dims).
    constexpr int kMtpKvCap = 16384;
    int mtp_kv_max = std::min(model_->config_.max_seq_len, kMtpKvCap);
    if (mtp_kv_max <= 0)
        mtp_kv_max = kMtpKvCap;

    auto* ws = new quench::MtpDraftWorkspace();
    if (!quench::mtp_workspace_allocate(*ws, hidden_dim, vocab_size, n_experts, top_k, expert_d_ff, shared_d_ff,
                                     mtp_num_heads, mtp_num_kv_heads, mtp_head_dim, mtp_kv_max)) {
        delete ws;
        QUENCH_LOG_ERROR("enable_mtp_spec_decode: workspace alloc failed");
        return false;
    }
    // Configure RoPE for the MTP attention.
    ...
    // same way as the verifier at extended positions (issue). Without
    ...
    if (model_->config_.yarn_ext_factor > 0.0f) {
        ...
        quench::rope_yarn_corr_dims(n_dims, n_ctx_orig, model_->config_.rope_theta,
                                 model_->config_.yarn_beta_fast, model_->config_.yarn_beta_slow, corr);
        ws->yarn_corr_dim_0 = corr[0];
        ws->yarn_corr_dim_1 = corr[1];
        QUENCH_LOG_INFO("MTP YaRN: ext=%.2f attn=%.3f freq_scale=%.4f corr_dims=[%.1f, %.1f]",
...
bool Engine::mtp_draft_one(int prev_token_id, const void* d_h_prev, int hidden_dim, int vocab_size,
                           ...
                           int32_t* d_out_token) {
    if (mtp_ws_storage_ == nullptr) {
        QUENCH_LOG_ERROR("mtp_draft_one: spec-decode not enabled");
        return false;
    }
    if (!model_ || !model_->mtp_.has_value() || !model_->mtp_->loaded) {
        QUENCH_LOG_ERROR("mtp_draft_one: MTP head not loaded");
        return false;
    }
    auto* ws = static_cast<quench::MtpDraftWorkspace*>(mtp_ws_storage_);
    // Chain lm_head via the NVFP4 decode cache when available: the full-vocab
    // FP16 GEMV is the dominant per-draft cost ( lever 3). Draft-only
    ...
    quench::NvFP4QuantResult lm_nvfp4;
    const quench::NvFP4QuantResult* lm_nvfp4_p = nullptr;
    if (runtime_config_.speculative.mtp_nvfp4_head && executor_ && executor_->lm_head_nvfp4_view(lm_nvfp4)) {
        ...
        if (!logged) {
            logged = true;
            QUENCH_LOG_INFO("MTP chain lm_head: NVFP4 decode-cache view engaged (N=%lld K=%lld)",
                         static_cast<long long>(lm_nvfp4.N), static_cast<long long>(lm_nvfp4.K));
        }
    }
    return quench::mtp_draft_step(prev_token_id, d_h_prev, *model_->mtp_, model_->tok_emb_, model_->out_proj_,
