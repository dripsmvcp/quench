...
//
// Split out of engine.cpp as its own per-subsystem translation unit.
...
bool Engine::should_stop(Request& req, int32_t token) const {
    ...
    if (req.think_exit_idx >= 0 && is_stop_token(token)) {
        ...
        // 0-content completion (the post- regression).
...
void Engine::fill_sampling_params(Request& req, InferenceState& state) const {
    ...
    // document closed before max_tokens truncates it mid-structure.
...
void Engine::upload_penalties(const Request& req, InferenceState& state, cudaStream_t stream) {
    ...
    if (n > d_penalty_tokens_capacity_) {
        ...
        if (!d_penalty_tokens_) {
            QUENCH_LOG_ERROR("VRAMAllocator failed for penalty tokens (%zu)", d_penalty_tokens_capacity_);
            d_penalty_tokens_capacity_ = 0;
            return;
        }
    }
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_penalty_tokens_, req.output_tokens.data(), n * sizeof(int32_t),
...
int Engine::acquire_recurrent_slot_(int req_id) {
    ...
    if (!free_recurrent_slots_.empty()) {
        ...
        slot = req_id % cap;
        QUENCH_LOG_WARN("recurrent slot pool exhausted (cap=%d) — falling back to id%%cap for req %d", cap,
                     req_id);
    }
    ...
    if (ssm_state_) {
        ...
        if (reset) {
            ...
            if (req.recurrent_restore && req.recurrent_restore->data &&
                ...
                QUENCH_LOG_DEBUG("RecurrentSnapshot: restored %d-token state for req %d (slot %d)",
...
void Engine::maybe_save_recurrent_snapshot_(const Request& req, int snap_end, cudaStream_t stream) {
    ...
    if (recurrent_snapshots_->save(key, snap_end, ssm_state_->seq_base(it->second), stream)) {
        ...
        QUENCH_LOG_DEBUG("RecurrentSnapshot: saved %d-token state for req %d (%d/%d slots)", snap_end,
...
void Engine::maybe_save_swa_snapshot_span_(int seq_id, std::span<const int32_t> tokens,
    ...
    if (swa_snapshots_->save(key, snap_end, swa_snap_slab_, stream)) {
        ...
        // Sync policy (measured: a prefill-boundary sync waits ~50 ms on
        // in-flight prefill compute and would be the entire warm-TTFT
        ...
        QUENCH_LOG_INFO("SwaSnapshot: saved %d-token window for seq %d (%d/%d slots, "
