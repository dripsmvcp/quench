...
bool Engine::init_kv_cache() {
    ...
    QUENCH_LOG_INFO("KV cache layers: %d attention out of %d total", n_kv_layers, mcfg.n_layers);
    ...
    if (config_.kv_block_size <= 0) {
        ...
        QUENCH_LOG_INFO("KV block size: auto → %d (n_kv_heads=%d)", config_.kv_block_size, mcfg.n_kv_heads);
    }
    ...
    if (swa_mode != SwaSizingMode::Off) {
        ...
        if (off_reason) {
            QUENCH_LOG_INFO("kv_cache.swa_sizing=%s ignored: %s",
            ...
            if (config_.use_prefix_caching && runtime_config_.kv_cache.swa_snapshot_mb <= 0) {
                ...
                QUENCH_LOG_INFO("kv_cache.swa_sizing=on: prefix caching disabled (freed window "
    ...
    // VRAM budget. Nothing is held back from effective_free_vram()
    // (there is no balloon): the prequant reserve charges KV the full
    // measured cache demand and phase 3 is floored at it.
    // Charge what the first forward actually claimed LAST time, if we
    // know. The plan needs this number before the forward that
    ...
    if (config_.library_reserve_mb < 0 && runtime_config_.vram.library_reserve_cache != "off") {
        ...
        if (remembered_found) {
            // Test `remembered_found`, NOT `remembered > 0`: the latter
            // discards a recorded
            // ZERO and charges the 3900 MiB constant instead — on exactly the
            // models whose first forward claims nothing.
            config_.library_reserve_mb = static_cast<int>(remembered >> 20);
            QUENCH_LOG_INFO("library reserve: %d MiB from the measurement cache (%s) — the default "
                         "constant is %zu MiB",
            ...
            QUENCH_LOG_INFO("library reserve: no measurement for this model in %s — planning with the "
    ...
    auto vram_budget = compute_vram_budget(*model_, config_, n_kv_layers, head_dim,
                                           ...
                                           &native_cache_demand());
    // One number, two readers: the shadow-plan probe charges it, and the
    // measured-residual clamp further down keeps it out of the KV pool. It is
    // config_.library_reserve_mb, NOT the runtime-config field — the loader
    // above writes the REMEMBERED measurement into the former, and the plan
    // must read the same figure the live pass uses.
    const size_t library_reserve_bytes =
        config_.library_reserve_mb < 0 ? kMeasuredLibraryReserveBytes
                                       : static_cast<size_t>(config_.library_reserve_mb) << 20;
    // A7 step 2 — APPLIED. The KV block count comes from plan_memory(), not
    // from the live-free-derived pass. What makes that safe: no balloon hides
    // bytes from the live read, the live pass applies no doubling the plan
    // does not model,
    // and V8 asserts the live pass never exceeds the plan. The
    ...
    int max_blocks = 0;
    {
        ...
        probe.library_reserve_bytes = library_reserve_bytes;
        ...
        QUENCH_LOG_INFO("%s", shadow_plan_report(probe, plan, vram_budget.kv_max_blocks).c_str());
        ...
        if (config_.kv_cache_max_blocks > 0) {
            ...
            if (max_blocks != vram_budget.kv_max_blocks) {
                ...
                // because a silent divergence is how passes drift apart.
                QUENCH_LOG_INFO("KV blocks: plan %d (live pass would have said %d)", max_blocks,
            ...
            QUENCH_LOG_WARN("KV blocks: the plan rejects this configuration — falling back to the "
    ...
    // KV pool — the elastic tier — takes what is actually left.
    //
    ...
    // GDN detection
    {
        if (model_->profile().is_gdn) {
            if (config_.use_cuda_graphs) {
                QUENCH_LOG_INFO("GDN model: CUDA graphs enabled (recurrent state in-place)");
            } else {
                QUENCH_LOG_INFO(
    ...
    // it cannot overcommit; and it keeps free what everything allocated after
    // this point needs to succeed — the allocator's headroom AND the library
    // reserve, which is still unclaimed at this point (cuBLAS/CUTLASS take it
    // on the first forward). The measured free still contains those bytes, so
    // without the explicit charge the clamp sizes the pool into the claim.
    ...
    if (per_block_total_bytes > 0) {
        ...
        const auto sizing = kv_blocks_from_residual(free_now, headroom, library_reserve_bytes,
                                                    per_block_total_bytes, max_blocks, 16);
        if (sizing.clamped) {
            QUENCH_LOG_INFO("KV cache: %d -> %d blocks from the measured post-cache residual "
                         "(%.0f MiB free, %.0f MiB allocator headroom + %.0f MiB pending "
                         "library reserve kept)",
                         max_blocks, sizing.blocks, free_now / (1024.0 * 1024.0),
                         headroom / (1024.0 * 1024.0), library_reserve_bytes / (1024.0 * 1024.0));
        ...
        // default path learns about it from cancelled generations.
        if (sizing.floored) {
            QUENCH_LOG_WARN(
                "KV cache: only %.0f MiB was left after the weight caches, and %.0f MiB of it "
                "is spoken for (%.0f MiB allocator headroom + %.0f MiB library reserve claimed "
                "on the first forward) — nothing remained to size the pool from, so "
                ...
                (headroom + library_reserve_bytes) / (1024.0 * 1024.0),
                headroom / (1024.0 * 1024.0), library_reserve_bytes / (1024.0 * 1024.0),
                sizing.blocks, static_cast<double>(sizing.blocks) * kv_bs, max_blocks_planned,
        ...
        // design, so it has to at least say so.
        //
    ...
    // makes the whole path graph-capture-safe — graphs stay enabled.
    {
        ...
        if (residual_n > 0 && config_.kv_cache_dtype == QType::NVFP4) {
            ...
            QUENCH_LOG_INFO("kv_cache.bitdecoding_residual_tokens=%d ignored (only active with kv_cache_dtype=NVFP4)",
                         residual_n);
        }
    }
    ...
    if (config_.use_prefix_caching) {
        ...
        QUENCH_LOG_INFO("Prefix caching enabled (pin budget %d blocks)", pin_budget);
        ...
        if (mcfg.ssm_inner_size == 0 && !config_.prefix_cache_path.empty()) {
            ...
            if (restored > 0)
                QUENCH_LOG_INFO("Restored %d prefix cache blocks from %s", restored,
    ...
    if (mcfg.ssm_inner_size > 0) {
        ...
        if (n_ssm > 0) {
            ...
            if (!ssm_state_->init(n_ssm, config_.max_batch_size, conv_ch, mcfg.ssm_conv_kernel, n_heads, hd,
                ...
                QUENCH_LOG_WARN("Failed to init SSM state, continuing without it");
                ssm_state_.reset();
            }
        }
    ...
    if (swa_sizing_active_ && kv_manager_->prefix_caching_enabled()) {
        ...
        if (budget_mb > 0 && kv_manager_->enable_swa_snapshots()) {
            ...
            if (cudaMalloc(&swa_snap_slab_, slab_bytes) == cudaSuccess) {
                ...
                if (swa_snapshots_->enabled()) {
                    ...
                    QUENCH_LOG_INFO("SWA snapshots: %d MiB budget, %zu KiB/snapshot, capacity %d",
        ...
        if (!swa_snapshots_) {
            ...
            if (budget_mb > 0 && static_cast<size_t>(budget_mb) < need_mb) {
                QUENCH_LOG_WARN("Prefix caching DISABLED: kv_cache.swa_snapshot_mb=%d is below one "
                ...
                QUENCH_LOG_INFO("Prefix caching disabled under SWA sizing (snapshot store off — "
    ...
    if (mcfg.is_nvfp4_prequant && mcfg.n_experts > 0) {
        ...
        if (moe_layers > 0 && covered == moe_layers) {
            QUENCH_LOG_INFO("NVFP4 decode caches: FULL (%d/%d MoE layers) — decode graph "
                         "capture eligible",
                         covered, moe_layers);
    ...
    if (config_.use_fp8_prefill)
        QUENCH_LOG_INFO("Weight cache: FP8 E4M3 (2x prefill throughput on sm_120)");
                                ...
                                /*with_swa_tables=*/swa_sizing_active_);
    {
        ...
        if (!d_penalty_tokens_) {
            QUENCH_LOG_WARN("Failed to pre-allocate penalty token buffer");
    ...
    // Pre-allocate prefill metadata pool (avoids per-request cudaMallocAsync)
    {
        ...
        if (prefill_pool_) {
            ...
            QUENCH_LOG_WARN("Failed to pre-allocate prefill pool, will use per-request malloc");
        }
    ...
    // Report memory
    {
        ...
        if (cudaMemGetInfo(&free_mem, &total_mem) == cudaSuccess)
            QUENCH_LOG_INFO("GPU memory: %.0f MiB used / %.0f MiB total (%.0f MiB free)",
