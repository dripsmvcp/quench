...
namespace quench::engine_internal {
...
inline void free_prefill_buffers(int32_t* d_token_ids, int* d_positions, int* d_block_tables, int* d_context_lens,
                                 cudaStream_t stream) {
    QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_token_ids, stream));
    QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_positions, stream));
    QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables, stream));
    QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_context_lens, stream));
}
...
// The library reserve the PLAN charges: quench.conf's vram.library_reserve_mb, or
