...
struct EngineConfig {
    ...
    // Budget-planner tuning (quench.conf [vram], see RuntimeConfig::Vram).
...
// Apply the quench.conf [rope] runtime override to a loaded ModelConfig (see
...
public:
    ...
    // Chunked-prefill-aware teacher-forced perplexity (quench_perplexity).
    ...
    // Scaffolding: API in place; auto-invoke from the decode path is
    // future work.
    ...
    // One draft step (mtp_draft_one below). Public for smoke testing;
    // production callers should not invoke it directly.
    // Encoder embedder: pooled + L2-normalized embedding for `tokens`.
...
private:
    ...
    // Engine::init(). Tool mains (quench-cli, quench-server) stash the loaded
    ...
    // several init phases need it (auto
    ...
    //
    // Pool size 64 (was 32 — P5 §2.2 M4): the quench-server's continuous-batching
    ...
    // one (: IMA when a request with a long prompt follows short ones).
    ...
    struct BatchedDecodePipeline {
        ...
        //: chained steps since the last spec-yield probe — the chain
    ...
    // Gated by runtime.prefill_graph.
    ...
    // greedy sample. NOT the executor's same-named member.
    ...
    struct ConstrainedPipeline {
        ...
        // Jump-ahead: forced-span speculative precompute. fdraft is
    ...
    // One-shot guard for the resolved-dispatch summary.
    ...
    // The banned-token list is built once at warmup and never mutated, so
    // per-launch device copies in each of the three graph paths would be
    // pure per-burst allocation
    // traffic. One engine-owned copy, uploaded
    ...
    // json_schema or enforced tool call —). No-op when none apply or
    // the request already holds one.
    void ensure_constraints_(const std::shared_ptr<Request>& req);
    // Embedding requests: pool the chunk that hidden_ currently holds
    ...
    // tokens/positions (: catastrophic chunked-prefill NLL on Llama).
    cudaEvent_t pf_staging_evt_ = nullptr;

    // ── Penalty token buffer ─────────────────────────────────────────
    //: round-robin cursor for batched spec verify — id of the request
    ...
    // Embedding pooling scratch: [d_model] fp32 chunk partial sums,
    ...
    void* encoder_ws_storage_ = nullptr;  // type-erased EncoderWorkspace*
    ...
    // ── MTP verify consumer — implemented in engine_spec_mtp.cpp ──
    ...
    // min_draft > 0 ( batch-RR): soft-decline (return false, no miss
    ...
    // capturability census for the verify chunk forward (see
    ...
    void spec_capture_probe_forward_(InferenceState& state, Tensor& logits_out,
                                     cudaStream_t stream);
    // ── Graph-captured verify chunk, engine_spec_capture.cpp ──
    ...
    // Compare the first forward's actual claim against what the plan
    // charged. Diagnostic; logs the number to pin.
    ...
    // all, and a `> 0` guard would silently fall back to the assumed 3900 MiB
    // there, reporting a charge that was never taken (residual −3486 MiB, 149 %).
    size_t measured_library_reserve_ = SIZE_MAX;
    // Identity + location for persisting the measurement. Filled in
    ...
    // (There is deliberately no verify-in-loop recycle variant: it measured a
    // loss on every prompt class tried. The eager
    // token-recycling drafter above is the form that pays.)
    // Device/pinned staging for the verify chunk (lazy-init, K+1 capacity).
    // Tokens/positions/row-ctx-lens + the 3 length scalars live in ONE
    ...
    // decode-attention verify route (dense, eager): per-row context lens
    ...
    // seen both a prefill and a decode.
    ...
    void decode_build_inference_state_(GPUBatch& gpu_batch,
                                       std::vector<std::shared_ptr<quench::Request>>& valid_decode,
    ...
    // graphs-ON decode path leaves through the first of them ( placed the
    ...
    // Jump-ahead: probe the schema FSM for a forced continuation and
