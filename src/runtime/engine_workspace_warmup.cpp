...
// becomes ready.
//
// Split out of engine.cpp as its own translation unit.
...
bool Engine::init_features() {
    ...
    if (config_.use_green_contexts) {
        if (!green_ctx_.init(0, config_.green_ctx_prefill_ratio)) {
            QUENCH_LOG_WARN("Green context init failed — falling back to regular streams");
        ...
        if (green_ctx_.is_available() && resolve_prefill_chunk_size_() > 0)
            if (executor_->allocate_decode_workspace(stream_, config_.max_batch_size))
                QUENCH_LOG_INFO("Concurrent prefill/decode overlap enabled");
    }
    ...
    if (Tokenizer* tok = model_->tokenizer()) {
        ...
        if (family == ChatTemplateFamily::RAW) {
            ...
            if (family != ChatTemplateFamily::RAW)
                QUENCH_LOG_INFO("No chat template in metadata, using %s default for %s",
        ...
        if (family != ChatTemplateFamily::RAW && !chat_template_.init(family, *tok, tok->chat_template_str()))
            // Was silently discarded until marked init() [[nodiscard]]. A
        ...
        if (ptok) {
            ...
            if (accept) {
                ...
                if (he >= 0) {
                    ...
                    QUENCH_LOG_INFO("Harmony reasoning: <|end|>=%d, force opener=[ %s]", he, seq.c_str());
                }
            }
            ...
            if (think_end_id_ >= 0) {
                ...
                if (d_token_is_whitespace_) {
                    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_token_is_whitespace_,
                                                       token_is_whitespace_.data(),
                                                       vocab * sizeof(uint8_t),
                                                       cudaMemcpyHostToDevice, stream_));
                    QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(stream_));
                }
            }
        }
    }
    ...
    if (model_->vision_tower) {
        ...
        if (!qwen_vision_.init(*model_->vision_tower, budget)) {
            QUENCH_LOG_WARN("Qwen3-VL vision tower failed to initialise — continuing text-only");
        } else if (model_->tokenizer()) {
            qwen_image_pad_id_ = model_->tokenizer()->find_token("<|image_pad|>");
            if (qwen_image_pad_id_ < 0)
                QUENCH_LOG_WARN("Qwen3-VL: no <|image_pad|> in the tokenizer — images unavailable");
        }
    }
...
void Engine::build_banned_token_list() {
    ...
    // ban list. Useful for bisecting long-form repetition
    // (e.g. Mistral-Small-3.2-NVFP4: ban list vs weight quality).
    if (runtime_config_.generation.no_ban) {
        banned_token_ids_.clear();
        QUENCH_LOG_WARN("generation.no_ban=true: skipping banned-token list (debug)");
        return;
    }
    banned_token_ids_.clear();
    ...
    if (tok) {
        // Harmony (gpt-oss,): the model is TRAINED to emit its own
    ...
    if (!banned_token_ids_.empty()) {
        QUENCH_LOG_INFO("Banned %zu special tokens from generation", banned_token_ids_.size());
        if (tok) {
            ...
            QUENCH_LOG_INFO("  banned: %s", bl.c_str());
        }
    }
}
...
// (kMeasuredLibraryReserveBytes) with an quench.conf override. Measurement shows
...
// context (M5), so one line in quench.conf fixes that deployment for good.
...
static size_t report_library_reserve(size_t free_before, int library_reserve_mb) {
    ...
    if (diff <= tol) {
        QUENCH_LOG_INFO("library reserve: charged %.0f MiB, first forward claimed %.0f MiB (within "
                     "tolerance)",
                     chg_mib, meas_mib);
        return measured;
    }
    QUENCH_LOG_WARN("library reserve MISMATCH: the plan charged %.0f MiB, the first forward actually "
                 "claimed %.0f MiB (%+.0f MiB). %s Pin it for this model with "
                 "'vram.library_reserve_mb = %.0f' in quench.conf — it is stable per model and quant "
                 "path, and invariant to batch and context.",
...
void Engine::warmup() {
    ...
    if (has_mxfp4_weights) {
        QUENCH_LOG_INFO("Warmup skipped (MXFP4 model)");
        return;
    }
    ...
    // backtick/markdown degeneration. QUENCH_NO_WARMUP=1 was the manual
    // mitigation; make it automatic for the arch.
    if (model_->profile().is_gemma4) {
        QUENCH_LOG_INFO("Warmup skipped (Gemma-4 algo-jitter protection)");
        return;
    }
    ...
    // claim, the graph captures, or quench's own lazy workspaces. These three cost
    // one cudaMemGetInfo each, at init only.
    ...
    // runs CUTLASS two phases earlier and measures ~0. What the
    // device holds that quench's own allocations cannot account for IS the rest —
    // the allocator and the async mempool are both fully named,
    // and the invariance A1.5 defines the charge by confirms it.
    //
    // This reads the per-pool ledger, which is why note() must not be gated on
    // --mem-report: with an empty ledger the residual is the whole device, and
    // charging from it collapses a 35B's KV pool 4096 -> 512 tokens.
    //
    ...
    // instead of the constant. A write failure is a warning, never a
    // load failure — refusing to serve a model over a cache file would be absurd.
    //
    // NEVER record from an errored init. The claim is opportunistic: a first
    // forward that hit OOM claims whatever was left, not what the libraries
    // need, so the measurement under pressure is "all remaining VRAM". Persist
    // that and the next start reserves more, frees more for the claim to
    // swallow, and measures higher still — observed ratcheting 2297 -> 7348
    // -> 9647 MiB across three starts on a 32 GiB card until the KV pool had
    // collapsed. Zero errors is the only state the number is worth keeping.
    if (measured_library_reserve_ != SIZE_MAX && !library_reserve_cache_path_.empty() &&
        log_error_count() > 0) {
        QUENCH_LOG_WARN(
            "library reserve: NOT recording the %.0f MiB measurement — %llu error(s) were "
            "logged during init, so it reflects a degraded first forward, not the libraries' "
            "real demand. The next start plans with the previous value or the constant.",
            measured_library_reserve_ / (1024.0 * 1024.0),
            static_cast<unsigned long long>(log_error_count()));
    } else if (measured_library_reserve_ != SIZE_MAX && !library_reserve_cache_path_.empty()) {
        if (library_reserve_cache_store(library_reserve_cache_path_, library_reserve_key_,
            ...
            // OUTLIVES this process. The supported way to run quench is a
            ...
            QUENCH_LOG_INFO("library reserve: recorded %.0f MiB in %s — the next start on this model "
            ...
            QUENCH_LOG_WARN("library reserve: could not write %s — the next start will charge the "
                         "constant again",
                         library_reserve_cache_path_.c_str());
        }
    }
    ...
    cudaDeviceSynchronize();
    {
        ...
        if (e != cudaSuccess)
            QUENCH_LOG_ERROR("warmup CUDA error: %s", cudaGetErrorString(e));
    }
    ...
    QUENCH_LOG_INFO("Warmup complete");
}
