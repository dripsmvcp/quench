...
bool GreenContextManager::init(int device, float prefill_sm_ratio) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("GreenContextManager: failed to set device %d: %s", device, cudaGetErrorString(err));
        return false;
    }
    ...
    if (err != cudaSuccess || total_sms_ <= 0) {
        QUENCH_LOG_ERROR("GreenContextManager: failed to query SM count");
        return false;
    }
    ...
    // --- Green Contexts (Runtime API): true SM partitioning ---
    {
        ...
        err = cudaExecutionCtxStreamCreate(&prefill_stream_, prefill_green_ctx_, cudaStreamNonBlocking,
                                           low_prio);
        if (err != cudaSuccess) {
            QUENCH_LOG_WARN("GreenContextManager: failed to create prefill stream (%s)",
                         cudaGetErrorString(err));
            goto cleanup_green;
        }
        ...
        err = cudaExecutionCtxStreamCreate(&decode_stream_, decode_green_ctx_, cudaStreamNonBlocking,
                                           high_prio);
        if (err != cudaSuccess) {
            QUENCH_LOG_WARN("GreenContextManager: failed to create decode stream (%s)", cudaGetErrorString(err));
...
fallback:
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("GreenContextManager: failed to create prefill stream");
        return false;
    }
    ...
    if (err != cudaSuccess) {
        cudaStreamDestroy(prefill_stream_);
        prefill_stream_ = nullptr;
        QUENCH_LOG_ERROR("GreenContextManager: failed to create decode stream");
        return false;
    }
