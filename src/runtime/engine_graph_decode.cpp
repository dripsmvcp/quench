...
const int32_t* Engine::banned_tokens_device_(cudaStream_t stream) {
    ...
    if (cudaMalloc(&p, bytes) != cudaSuccess) {
        QUENCH_LOG_WARN("banned tokens: device upload failed (%zu B) — masking disabled", bytes);
        return nullptr;
    }
    d_banned_tokens_ = static_cast<int32_t*>(p);
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_banned_tokens_, banned_token_ids_.data(), bytes,
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(stream));  // once, at first use
    return d_banned_tokens_;
}
...
std::vector<int32_t> Engine::try_graph_loop_decode(std::shared_ptr<Request> req, int32_t first_token,
    ...
    if (cudaMallocAsync(&d_block_tables, max_blocks_per_seq * sizeof(int), stream) != cudaSuccess)
        return {};
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_block_tables, full_bt.data(), max_blocks_per_seq * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    ...
    if (swa_sizing_active_) {
        ...
        if (static_cast<int>(swa_bt.size()) == max_blocks_per_seq &&
            cudaMallocAsync(&d_block_tables_swa, max_blocks_per_seq * sizeof(int), stream) ==
                cudaSuccess) {
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_block_tables_swa, swa_bt.data(),
                                               max_blocks_per_seq * sizeof(int),
                                               cudaMemcpyHostToDevice, stream));
        } else {
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables, stream));
            return {};
        }
    }
    ...
    if (!runner.setup(executor_.get(), state_template, first_token, gcfg, stream)) {
        if (d_block_tables_swa)
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables_swa, stream));
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables, stream));
        return {};
    }
    ...
    if (d_block_tables_swa)
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables_swa, stream));
    QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_block_tables, stream));
    QUENCH_LOG_INFO("ConditionalGraph: generated %zu tokens in graph loop", tokens.size());
    runner.cleanup();
    return tokens;
}
...
bool Engine::try_launch_async_graph_loop(std::shared_ptr<Request> req, int32_t first_token,
    ...
    // postscript: the "rearm emits a wrong token" artifact was the
    ...
    if (runtime_config_.speculative.burst_rearm && async_graph_runner_.is_setup() &&
        ...
        if (async_parked_req_id_ == req->id && async_d_block_tables_ != nullptr && swa_ok &&
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(async_d_block_tables_, bt.data(),
                                               bt.size() * sizeof(int), cudaMemcpyHostToDevice,
                                               stream));
            if (swa_sizing_active_)
                QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(async_d_block_tables_swa_, swa_bt.data(),
            ...
            if (runtime_config_.diagnostics.spec_trace)
                QUENCH_LOG_INFO("[burst-launch] REARM seed=%d pos=%d ctx=%d limit=%d", (int)first_token,
            ...
            if (async_graph_runner_.rearm(first_token, /*position=*/ctx - 1, /*context_len=*/ctx,
                ...
                QUENCH_LOG_DEBUG("AsyncGraphLoop: rearmed for %d-step burst (ctx=%d)", step_limit,
                              ctx);
                return true;
            }
        }
    ...
    if (cudaMalloc(&d_bt, max_blocks_per_seq * sizeof(int)) != cudaSuccess)
        return false;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_bt, full_bt.data(), max_blocks_per_seq * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    ...
    if (swa_sizing_active_) {
        ...
        if (static_cast<int>(swa_bt.size()) != max_blocks_per_seq ||
            ...
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_bt));
            return false;
        }
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_bt_swa, swa_bt.data(), max_blocks_per_seq * sizeof(int),
                                           cudaMemcpyHostToDevice, stream));
    }
    ...
    if (runtime_config_.diagnostics.spec_trace)
        QUENCH_LOG_INFO("[burst-launch] FRESH seed=%d ctx=%d limit=%d remaining=%d", (int)first_token,
    ...
    if (!async_graph_runner_.setup(executor_.get(), state_template, first_token, gcfg, stream)) {
        if (d_bt_swa)
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_bt_swa));
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_bt));
        return false;
    }
    ...
    async_parked_req_id_ = -1;
    QUENCH_LOG_DEBUG("AsyncGraphLoop: launched with %d banned tokens", state_template.n_d_banned_tokens);
    async_pending_tokens_.clear();
    async_pending_cursor_ = 0;
    QUENCH_LOG_INFO("AsyncGraphLoop: launched for %d remaining tokens", remaining);
    return true;
}
...
bool Engine::try_launch_constrained_pipeline(std::shared_ptr<Request> req, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_bt, full_bt.data(), max_blocks_per_seq * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    int32_t first_token = req->output_tokens.back();
    int ctx = req->context_len();
    int pos = ctx - 1;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_token, &first_token, sizeof(int32_t), cudaMemcpyHostToDevice,
                                       stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_pos, &pos, sizeof(int), cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_ctx, &ctx, sizeof(int), cudaMemcpyHostToDevice, stream));

    if (!banned_token_ids_.empty()) {
        if (cudaMalloc(&p.d_banned, banned_token_ids_.size() * sizeof(int32_t)) == cudaSuccess) {
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_banned, banned_token_ids_.data(),
    ...
    // jump-ahead chunks share THIS workspace with the captured
    ...
    st.grammar_constrainer = req->constraints ? req->constraints->grammar_constrainer() : nullptr;
    ...
    p.req = req;
    p.active = true;
    QUENCH_LOG_INFO("ConstrainedPipeline: launched for %d budgeted tokens (%s)", budget,
...
int Engine::step_constrained_pipeline() {
    ...
    // Jump-ahead span consumption: while fnext > 0, the tick's
    ...
    // Remaining output allowance for the force-close mask — the
    ...
    QUENCH_CUDA_CHECK_LOG(cudaEventRecord(p.ev, stream));
    ...
    QUENCH_CUDA_CHECK_LOG(cudaEventSynchronize(p.ev));
    ...
    if (consuming) {
        ...
        if (runtime_config_.diagnostics.jump_trace) {
            ...
            QUENCH_LOG_INFO("[jump] exit at %d/%d: sampled %d [%s] draft %d [%s]", p.fnext, p.frows,
        ...
        const int ctx = pos + 1;
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_token, &token, sizeof(int32_t),
                                           cudaMemcpyHostToDevice, stream));
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_pos, &pos, sizeof(int), cudaMemcpyHostToDevice,
                                           stream));
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(p.d_ctx, &ctx, sizeof(int), cudaMemcpyHostToDevice,
                                           stream));
        p.runner.execute(stream);
        p.forward_in_flight = true;
        p.fdraft.clear();
    ...
    // Jump-ahead, two-step: a pending draft is confirmed token by
    ...
    if (!p.fpending.empty()) {
        ...
        if (runtime_config_.diagnostics.jump_trace) {
            Tokenizer* tk = model_->tokenizer();
            QUENCH_LOG_INFO("[jump] pending dropped at %d: sampled %d [%s] draft %d [%s]",
...
// Jump-ahead probe: derive the forced continuation TEXT from the
...
void Engine::constrained_jump_probe_(std::shared_ptr<Request>& req) {
    ...
    p.fpend_cursor = 0;
    if (runtime_config_.diagnostics.jump_trace)
        QUENCH_LOG_INFO("[jump] pended %d tokens for forced span \"%s\"",
                     static_cast<int>(p.fpending.size()), text.c_str());
}

// Jump-ahead commit: the pipeline just confirmed the draft's first
...
void Engine::constrained_jump_commit_(std::shared_ptr<Request>& req, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_spec_tokens_, draft.data(), K * sizeof(int32_t),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_spec_positions_, h_positions.data(), K * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_spec_context_len_, &chunk_ctx, sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    ...
    if (runtime_config_.diagnostics.jump_trace) {
        ...
        QUENCH_LOG_INFO("[jump] committed %d-token chunk after free first-token match (ids: %s)", K,
                     ids.c_str());
    }
    QUENCH_LOG_DEBUG("ConstrainedPipeline: jump-ahead committed a %d-token draft chunk", K);
}

void Engine::teardown_constrained_pipeline(bool synchronize) {
    auto& p = cpipe_;
    if (p.jumps > 0) {
        QUENCH_LOG_INFO("ConstrainedPipeline: jump-ahead saved %lld forwards across %d drafted "
                     "spans (%d tokens produced)",
                     p.jumped_tokens, p.jumps, p.produced);
    }
    if (synchronize)
        QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(decode_stream()));
    p.runner.invalidate();
    if (p.d_bt) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_bt)); p.d_bt = nullptr; }
    if (p.d_token) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_token)); p.d_token = nullptr; }
    if (p.d_pos) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_pos)); p.d_pos = nullptr; }
    if (p.d_ctx) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_ctx)); p.d_ctx = nullptr; }
    if (p.d_banned) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_banned)); p.d_banned = nullptr; }
    p.h_token.reset();
    if (p.ev) { QUENCH_CUDA_CHECK_LOG(cudaEventDestroy(p.ev)); p.ev = nullptr; }
    if (p.d_frows) { QUENCH_CUDA_CHECK_LOG(cudaFree(p.d_frows)); p.d_frows = nullptr; }
    p.req = nullptr;
    p.active = false;
