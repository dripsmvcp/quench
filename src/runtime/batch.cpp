...
void GPUBatch::upload(const Batch& batch, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_token_ids, total_tokens * sizeof(int32_t)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_positions, total_tokens * sizeof(int)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_context_lens, n_sequences * sizeof(int)));

    if (n_sequences > 1) {
        QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_seq_offsets, (n_sequences + 1) * sizeof(int)));
    }
    ...
    // Async copy
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_token_ids, batch.token_ids.data(), total_tokens * sizeof(int32_t),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_positions, batch.positions.data(), total_tokens * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_context_lens, batch.context_lens.data(), n_sequences * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));

    if (d_seq_offsets && !batch.seq_offsets.empty()) {
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_seq_offsets, batch.seq_offsets.data(),
                                           (n_sequences + 1) * sizeof(int), cudaMemcpyHostToDevice, stream));
    }

    if (d_block_tables && !batch.block_tables.empty()) {
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_block_tables, batch.block_tables.data(),
...
void GPUBatch::free() {
    if (d_token_ids) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_token_ids));
        d_token_ids = nullptr;
    }
    if (d_positions) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_positions));
        d_positions = nullptr;
    }
    if (d_seq_offsets) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_seq_offsets));
        d_seq_offsets = nullptr;
    }
    if (d_block_tables) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_block_tables));
        d_block_tables = nullptr;
    }
    if (d_context_lens) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_context_lens));
...
void GPUBatchPool::allocate(int max_batch_size, int max_blocks_per_seq, bool with_swa_tables) {
    ...
    if (slab.empty()) {
        QUENCH_LOG_ERROR("decode batch pool: engine arena exhausted for %zu bytes — the arena was "
...
GPUBatch GPUBatchPool::upload_into_pool(const Batch& batch, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_token_ids_, batch.token_ids.data(),
                                       batch.total_tokens * sizeof(int32_t), cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_positions_, batch.positions.data(), batch.total_tokens * sizeof(int),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_context_lens_, batch.context_lens.data(),
                                       batch.n_sequences * sizeof(int), cudaMemcpyHostToDevice, stream));

    if (batch.n_sequences > 1 && !batch.seq_offsets.empty()) {
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_seq_offsets_, batch.seq_offsets.data(),
    ...
    if (batch.max_blocks_per_seq > 0 && !batch.block_tables.empty()) {
        ...
        // NOT sufficient — see last_upload_block_tables_ in batch.h.
        if (batch.n_sequences == 1 && batch.block_tables == last_upload_block_tables_) {
            // Skip — block_table content is identical to last upload
        } else {
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_block_tables_, batch.block_tables.data(),
    ...
    if (gpu.d_block_tables_swa && batch.max_blocks_per_seq > 0) {
        if (batch.n_sequences == 1 && batch.block_tables_swa == last_upload_block_tables_swa_) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_block_tables_swa_, batch.block_tables_swa.data(),
                                               static_cast<unsigned long>(batch.n_sequences) *
                                                   batch.max_blocks_per_seq * sizeof(int),
                                               cudaMemcpyHostToDevice, stream));
            if (batch.n_sequences == 1)
