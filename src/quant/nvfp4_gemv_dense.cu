...
void gemv_nvfp4_kpar_batched_fp32(const NvFP4QuantResult& A, const half* x, float* y, int N_out, int K,
    ...
    while (done < n_act) {
        ...
        if (rem >= 4) {
            ...
            // Bucket-3 verify lm_head: one sweep instead of <2>+<1>.
...
// GEMMs: one block per weight row n, the row decoded once and reused
// across the MR activation rows of this launch. kAcc adds into the existing
// output (cuBLAS beta=1 semantics) for the o/down residual-add GEMMs.
...
// Batched-M FP16 GEMM for spec-verify chunks. MR cap of 4 mirrors the
...
void gemm_nvfp4_batched(const NvFP4QuantResult& A, const half* x, half* y, int N_out, int K,
    ...
    while (done < n_act) {
        ...
        if (rem >= 4) {
            ...
            // otherwise pays TWO weight sweeps (<2> + <1>) —.
