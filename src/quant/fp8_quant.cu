...
//
// Phase 2A of the quench quantization pipeline.  Builds on the unscaled FP8
...
// Reduction scratch for calibrate_fp8_scale. cudaMalloc-ing the
// per-block buffer and the result scalar on every call and freeing them
// again would show up as post-warmup device allocations under the --wrap
// interposer. The calls are one-shot per layer, not per request, so this
// is not a hot-path win; per-call allocation is an I2 violation that costs
// nothing to avoid.
//
...
std::pair<float*, float*> absmax_scratch(int grid) {
    const bool live = g_absmax_scratch != nullptr &&
                      (g_absmax_scratch_gen == kOwnedGen ||
                       g_absmax_scratch_gen == quench::engine_arena().generation());
    ...
    auto slab = quench::engine_arena().take_bytes(bytes);
    if (!slab.empty()) {
        ...
        if (g_absmax_scratch_owned && g_absmax_scratch)
            QUENCH_CUDA_CHECK_LOG(cudaFree(g_absmax_scratch));
        void* p = nullptr;
...
float calibrate_fp8_scale(const Tensor& input, cudaStream_t stream) {
    if (!input.on_device || input.data == nullptr) {
        QUENCH_LOG_ERROR("calibrate_fp8_scale: input must be a non-null device tensor");
        return 1.0f;
    }

    const int n = (int)input.numel();
    if (n <= 0) {
        QUENCH_LOG_WARN("calibrate_fp8_scale: empty tensor, returning scale=1.0");
        return 1.0f;
    }
    ...
    if (!d_block_maxes) {
        QUENCH_LOG_ERROR("calibrate_fp8_scale: reduction scratch unavailable");
        return 1.0f;
    }
    ...
    float absmax = 0.0f;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(&absmax, d_result, sizeof(float), cudaMemcpyDeviceToHost, stream));
    ...
    if (absmax == 0.0f) {
        QUENCH_LOG_WARN("calibrate_fp8_scale: all-zero tensor, returning scale=1.0");
        return 1.0f;
    }

    float scale = absmax / kFP8E4M3Max;
    QUENCH_LOG_DEBUG("calibrate_fp8_scale: absmax=%.6f  scale=%.6f", absmax, scale);
    return scale;
}
...
void calibrate_and_quantize_fp8_async(const void* input_fp16, void* output_fp8, int64_t n_elements,
    ...
    if (n_elements > static_cast<int64_t>(INT_MAX)) {
        QUENCH_LOG_ERROR("calibrate_and_quantize_fp8_async: n_elements=%lld exceeds int range — skipping",
...
void quantize_fp16_to_fp8_e4m3(const Tensor& input, Tensor& output, float* d_scale_out, cudaStream_t stream,
    ...
    if (!input.on_device || input.data == nullptr) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3: input must be a non-null device tensor");
        return;
    }
    if (!output.on_device || output.data == nullptr) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3: output must be a pre-allocated device tensor");
        return;
    }
    if (input.qtype != QType::F16) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3: input dtype must be FP16");
        return;
    }
    ...
    if (output.numel() != input.numel()) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3: output numel (%lld) != input numel (%lld)",
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_block_maxes, (size_t)grid * sizeof(float)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_scale_device, sizeof(float)));
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3 kernel launch failed: %s", cudaGetErrorString(err));
    }

    QUENCH_CUDA_CHECK_LOG(cudaFree(d_block_maxes));
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_scale_device));

    QUENCH_LOG_DEBUG("quantize_fp16_to_fp8_e4m3: n=%d  scale=%.6f", n, scale);
}
...
void quantize_fp16_to_fp8_e4m3_scaled(const void* input_fp16, void* output_fp8, int n_elements, float scale,
    ...
    if (input_fp16 == nullptr || output_fp8 == nullptr) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3_scaled: null pointer");
        return;
    }
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3_scaled launch failed: %s", cudaGetErrorString(err));
    }
}
...
void dequantize_fp8_e4m3_to_fp16(const void* input_fp8, void* output_fp16, int n_elements, float scale,
    ...
    if (input_fp8 == nullptr || output_fp16 == nullptr) {
        QUENCH_LOG_ERROR("dequantize_fp8_e4m3_to_fp16: null pointer");
        return;
    }
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("dequantize_fp8_e4m3_to_fp16 launch failed: %s", cudaGetErrorString(err));
    }
}
...
void calibrate_fp8_scales_per_expert(const void* input_fp16, int K, const int32_t* d_offsets, int n_experts,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("calibrate_fp8_scales_per_expert launch failed: %s", cudaGetErrorString(err));
    }
}
...
void quantize_fp16_to_fp8_e4m3_per_expert(const void* input_fp16, void* output_fp8, int K,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("quantize_fp16_to_fp8_e4m3_per_expert launch failed: %s", cudaGetErrorString(err));
    }
}
