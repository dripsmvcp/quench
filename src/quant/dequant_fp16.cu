...
__global__ void dequant_int4_fp16_kernel(const uint8_t* __restrict__ input, half* __restrict__ output,
                                         const half* __restrict__ scales, int n, int group_size) {
...
void dequant_int4_fp16(const void* input, void* output, const void* scales, int n, int group_size,
                       cudaStream_t stream) {
    if (n <= 0)
        return;
    ...
    dequant_int4_fp16_kernel<<<grid, kBlockSize, 0, stream>>>(static_cast<const uint8_t*>(input),
                                                              static_cast<half*>(output),
                                                              static_cast<const half*>(scales), n,
                                                              group_size);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("dequant_int4_fp16 launch failed: %s", cudaGetErrorString(err));
    }
}
