...
void quant_gemm_int4(const Tensor& A, const Tensor& B_quant, const Tensor& scales, Tensor& C,
                     cudaStream_t stream) {
    // --- Validate dimensions ------------------------------------------------
    QUENCH_CHECK(A.ndim == 2, "quant_gemm_int4: A must be 2D [M, K], got ndim=%d", A.ndim);
    QUENCH_CHECK(B_quant.ndim == 2, "quant_gemm_int4: B_quant must be 2D [N, K/2], got ndim=%d", B_quant.ndim);
    QUENCH_CHECK(scales.ndim == 2, "quant_gemm_int4: scales must be 2D [N, num_groups], got ndim=%d",
              scales.ndim);
    QUENCH_CHECK(C.ndim == 2, "quant_gemm_int4: C must be 2D [M, N], got ndim=%d", C.ndim);

    const int M = static_cast<int>(A.shape[0]);
    const int K = static_cast<int>(A.shape[1]);
    const int N = static_cast<int>(B_quant.shape[0]);

    QUENCH_CHECK(static_cast<int>(B_quant.shape[1]) == K / 2,
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("quant_gemm_int4 launch failed: %s", cudaGetErrorString(err));
    }
}
