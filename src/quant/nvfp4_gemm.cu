...
void gemv_nvfp4(const NvFP4QuantResult& A, const Tensor& x, Tensor& y, cudaStream_t stream) {
    QUENCH_CHECK(A.packed_data != nullptr, "gemv_nvfp4: A.packed_data is null");
    QUENCH_CHECK(x.on_device, "gemv_nvfp4: x must be on device");
    QUENCH_CHECK(y.on_device, "gemv_nvfp4: y must be on device");
    QUENCH_CHECK(x.qtype == QType::F16, "gemv_nvfp4: x must be FP16, got qtype=%d", std::to_underlying(x.qtype));
    QUENCH_CHECK(y.qtype == QType::F16, "gemv_nvfp4: y must be FP16, got qtype=%d", std::to_underlying(y.qtype));
...
namespace {
QUENCH_REGISTER_CUDA_STATIC_RESET(nvfp4_gemm_reset_static_cuda_state);
}  // namespace
...
static void* ensure_dequant_buffer(size_t needed, cudaStream_t stream) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("gemm_nvfp4: failed to allocate %zu bytes for dequant buffer: %s", needed,
...
void gemm_nvfp4(const NvFP4QuantResult& A, const Tensor& B, Tensor& C, cudaStream_t stream, float beta) {
    QUENCH_CHECK(A.packed_data != nullptr, "gemm_nvfp4: A.packed_data is null");
    QUENCH_CHECK(B.on_device, "gemm_nvfp4: B (input) must be on device");
    QUENCH_CHECK(C.on_device, "gemm_nvfp4: C (output) must be on device");
    QUENCH_CHECK(B.ndim == 2, "gemm_nvfp4: B must be 2D [M, K], got ndim=%d", B.ndim);
    QUENCH_CHECK(C.ndim == 2, "gemm_nvfp4: C must be 2D [M, N], got ndim=%d", C.ndim);

    const int64_t N = A.N;
    const int64_t K = A.K;
    const int64_t M = B.shape[0];

    QUENCH_CHECK(B.shape[1] == K, "gemm_nvfp4: B.shape[1]=%lld must equal weight K=%lld",
              static_cast<long long>(B.shape[1]), static_cast<long long>(K));
    QUENCH_CHECK(C.shape[0] == M && C.shape[1] == N,
    ...
    if (!dequant_buf) {
        ...
        // LACKS this GEMM — the census "hybrid crash" was this exact
