...
//
// Phase 4 of the quench quantization pipeline.  Implements NVIDIA's FP4 format
...
float calibrate_nvfp4_scales(const Tensor& input, cudaStream_t stream, float* d_reusable_max) {
    QUENCH_CHECK(input.on_device, "calibrate_nvfp4_scales: input must be on device");
    QUENCH_CHECK(input.qtype == QType::F16, "calibrate_nvfp4_scales: input must be FP16, got qtype=%d",
    ...
    if (!d_global_max) {
        QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_global_max, sizeof(float)));
        own_alloc = true;
    }
    QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(d_global_max, 0, sizeof(float), stream));
    ...
    if (own_alloc)
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_global_max));

    if (h_absmax == 0.0f) {
        QUENCH_LOG_WARN("calibrate_nvfp4_scales: tensor is all zeros, using scale 1.0");
        return 1.0f;
    }

    float tensor_scale = h_absmax / kFP4E2M1Max;
    QUENCH_LOG_DEBUG("calibrate_nvfp4_scales: absmax=%.6f, tensor_scale=%.6f", h_absmax, tensor_scale);
    return tensor_scale;
}

void quantize_fp16_to_nvfp4(const Tensor& input, NvFP4QuantResult& result, cudaStream_t stream) {
    QUENCH_CHECK(input.on_device, "quantize_fp16_to_nvfp4: input must be on device");
    QUENCH_CHECK(input.qtype == QType::F16, "quantize_fp16_to_nvfp4: input must be FP16, got qtype=%d",
              std::to_underlying(input.qtype));
    QUENCH_CHECK(input.ndim == 2, "quantize_fp16_to_nvfp4: input must be 2D [N, K], got ndim=%d", input.ndim);

    int64_t N = input.shape[0];
    int64_t K = input.shape[1];
    QUENCH_CHECK(K % kMicroBlockSize == 0, "quantize_fp16_to_nvfp4: K=%lld must be multiple of %d",
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_packed, packed_bytes));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_micro_scales, micro_scale_bytes));
...
void quantize_fp16_to_nvfp4_async(const Tensor& input, NvFP4QuantResult& result, float* d_absmax_buf,
    ...
    QUENCH_CHECK(input.on_device, "quantize_fp16_to_nvfp4_async: input must be on device");
    QUENCH_CHECK(input.qtype == QType::F16, "quantize_fp16_to_nvfp4_async: input must be FP16, got qtype=%d",
              std::to_underlying(input.qtype));
    QUENCH_CHECK(input.ndim == 2, "quantize_fp16_to_nvfp4_async: input must be 2D [N, K], got ndim=%d",
              input.ndim);

    int64_t N = input.shape[0];
    int64_t K = input.shape[1];
    QUENCH_CHECK(K % kMicroBlockSize == 0, "quantize_fp16_to_nvfp4_async: K=%lld must be multiple of %d",
              static_cast<long long>(K), kMicroBlockSize);

    // Step 1: absmax reduction (async, no host sync)
    QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(d_absmax_buf, 0, sizeof(float), stream));
    int64_t n_elements = input.numel();
...
void dequantize_nvfp4_to_fp16(const NvFP4QuantResult& quant, void* output_fp16, cudaStream_t stream) {
    QUENCH_CHECK(quant.packed_data != nullptr, "dequantize_nvfp4_to_fp16: packed_data is null");
    QUENCH_CHECK(quant.micro_scales != nullptr, "dequantize_nvfp4_to_fp16: micro_scales is null");
    QUENCH_CHECK(output_fp16 != nullptr, "dequantize_nvfp4_to_fp16: output buffer is null");
...
void dequantize_nvfp4_moe_to_fp16(const NvFP4MoEQuantResult& result, void* output_fp16, cudaStream_t stream) {
    QUENCH_CHECK(result.packed_data != nullptr, "dequantize_nvfp4_moe_to_fp16: packed_data is null");
    QUENCH_CHECK(result.micro_scales != nullptr, "dequantize_nvfp4_moe_to_fp16: micro_scales is null");
    QUENCH_CHECK(result.tensor_scales != nullptr, "dequantize_nvfp4_moe_to_fp16: tensor_scales is null");
    QUENCH_CHECK(output_fp16 != nullptr, "dequantize_nvfp4_moe_to_fp16: output is null");
...
void free_nvfp4_result(NvFP4QuantResult& result) {
    ...
    if (result.owned) {
        if (result.packed_data)
            QUENCH_CUDA_CHECK_LOG(cudaFree(result.packed_data));
        if (result.micro_scales)
            QUENCH_CUDA_CHECK_LOG(cudaFree(result.micro_scales));
    }
...
void quantize_packed_experts_to_nvfp4(const void* packed_ggml_data, QType qtype, int n_experts, int eff,
                                      ...
                                      cudaStream_t stream) {
    QUENCH_CHECK(packed_ggml_data != nullptr, "quantize_packed_experts_to_nvfp4: packed_ggml_data is null");
    QUENCH_CHECK(dequant_scratch != nullptr, "quantize_packed_experts_to_nvfp4: dequant_scratch is null");
    QUENCH_CHECK(K % kMicroBlockSize == 0, "quantize_packed_experts_to_nvfp4: K=%d must be multiple of %d",
              K, kMicroBlockSize);
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_packed, total_packed));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_micro_scales, total_ms));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_tensor_scales, n_experts * sizeof(float)));
...
void free_nvfp4_moe_result(NvFP4MoEQuantResult& result) {
    ...
    if (result.tensor_scales) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(result.tensor_scales));
        result.tensor_scales = nullptr;
    }
    result.n_experts = 0;
    result.N = 0;
