...
inline bool debug_forward_enabled() { return quench::process_diag_debug_forward(); }
...
// QUENCH_DUMP_HIDDEN env) filename tagging so
...
// QUENCH_DUMP_HIDDEN env shorthand). Resolution happens in process_diag_install().
inline const char* dump_hidden_dir() { return quench::process_diag_dump_hidden_dir(); }
...
inline void write_npy_fp32(const std::string& path, const float* data, int rows, int cols) {
    ...
    if (!f) {
        QUENCH_LOG_ERROR("[DUMP_NPY] open failed: %s", path.c_str());
        return;
    }
...
// Early-returns when the diagnostics.dump_hidden_dir config flag (was QUENCH_DUMP_HIDDEN env) is
...
inline void dump_tensor_npy(const char* tag, const Tensor& t, cudaStream_t stream, int layer, int step) {
    ...
    char fname[512];
    std::snprintf(fname, sizeof(fname), "%s/quench_step%02d_L%02d_%s.npy", dir, step, layer, tag);
    if (bad > 0) {
        QUENCH_LOG_WARN("[DUMP_NPY] %s: %zu/%zu values are not finite — this view likely covers "
...
// QUENCH_DEBUG_FORWARD env) is active.
inline void debug_tensor_stats(const char* name, const Tensor& t, cudaStream_t stream, int row = 0,
    ...
    if (t.qtype == QType::F16) {
        std::vector<half> tmp(n);
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(tmp.data(), static_cast<const half*>(t.data) + (int64_t)row * cols,
                                           n * sizeof(half), cudaMemcpyDeviceToHost, stream));
        cudaStreamSynchronize(stream);
        for (int i = 0; i < n; i++)
            host[i] = __half2float(tmp[i]);
    } else if (t.qtype == QType::F32) {
        QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(host.data(),
                                           static_cast<const float*>(t.data) + (int64_t)row * cols,
                                           n * sizeof(float), cudaMemcpyDeviceToHost, stream));
        cudaStreamSynchronize(stream);
    } else {
        QUENCH_LOG_ERROR("[DEBUG_FWD] %s: unsupported dtype %d", name, std::to_underlying(t.qtype));
        return;
    }
    ...
    QUENCH_LOG_DEBUG("[DEBUG_FWD] %-30s  min=%+.6e  max=%+.6e  mean=%+.6e  sum=%+.4f  L2=%.6e%s", name, vmin,
...
inline void debug_tensor_rows(const char* name, const Tensor& t, cudaStream_t stream) {
    ...
    QUENCH_LOG_DEBUG("[DEBUG_FWD_ROW] %s: shape=[%d,%d] stride0=%lld nrows_dump=%d", name, nrows, cols,
    ...
    for (int r = 0; r < max_rows; r++) {
        ...
        QUENCH_LOG_DEBUG("[DEBUG_FWD_ROW] %s[%d] L2=%.4f  %+.4f %+.4f %+.4f ...  %+.4f %+.4f %+.4f", name, r,
...
inline void debug_tensor_stats_all(const char* name, const Tensor& t, cudaStream_t stream) {
    ...
    if (t.qtype != QType::F16 && t.qtype != QType::F32) {
        QUENCH_LOG_ERROR("[DEBUG_FWD_ALL] %s: unsupported dtype %d", name, std::to_underlying(t.qtype));
        return;
    }
    std::vector<float> host(n);
    if (t.qtype == QType::F16) {
    ...
    QUENCH_LOG_DEBUG("[DEBUG_FWD_ALL] %-30s  rows=%d cols=%d  sum=%+.4f  L2=%.4f", name, nrows, cols, vsum,
                  std::sqrt(vss));
}
