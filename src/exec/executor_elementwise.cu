...
// (was QUENCH_GEMMA4_FP32_GEMM_OUT env) for attention.
...
// calling thread ~165 us per call; per-MoE-layer residual copies make that
// ~8 ms of blocked host time per decode step at n=16 (nsys). A
...
void device_copy_async(void* dst, const void* src, size_t bytes, cudaStream_t stream) {
    ...
    if (e != cudaSuccess)
        QUENCH_LOG_ERROR("device_copy_async fallback memcpy failed: %s", cudaGetErrorString(e));
}
