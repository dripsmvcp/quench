...
// context and local variables in scope (including the `after_attention` label).
        ...
        if (q_offset > 0) {
            ...
            // on: same fp16-qk kernel family, Bq=64/TWOSLOT instance.
            ...
            // cuBLAS (folded into the WMMA FMHA above the threshold,).
            ...
            // reads ms/verify 78 → 60, +31% e2e,). The tiled FMHA keeps
            ...
            // 2.8-4.6x slower than cuBLAS there. It stays on cuBLAS
            ...
            // Graph-captured verify replay mode: kernels must not bake
            ...
            // KV-append-quant (opt-in): serve the whole chunk straight
            ...
            //
            // REVERTED ( routed ALL hd=128 prefill into FA2 "regardless of
            ...
            // threshold is tracked separately (see issue).
            // (chunk_fa2_serves / prefer_fmha / smatrix_fits computed above,
            // with the S-matrix capacity guard.)

            // Chunked attention routing: prefer the FP16-QK FA2 kernel
            ...
            // FP16 WMMA kernel — the fp8-QK family is opt-in only (:
            ...
            if (cap_replay) {
                ...
                // hd=512 kernel). The hd=256 SWA layers took FA2
                ...
                // fallback at Skv 8k/16k (the FMHA's Bq=16
                ...
                // for 16-row cuBLAS slices). Learned sinks ride along.
                int64_t q4s[4] = {1, (int64_t)n, (int64_t)nh, (int64_t)hd};
        ...
        // refinement the chunked path uses. Only truly
        // heterogeneous shapes (gemma-4 dual head_dim) and learned sinks
        // (gpt-oss) require cuBLAS. Without this, hybrids would never take
        ...
        // servable and follow the same threshold policy as every
        ...
        // layers (5/6 of layers in the 5:1 pattern) take FA2 f16-QK
        // per-layer. The hd=512 global layers stay on the materialized
        // cuBLAS path, which is 2.8-4.6x FASTER than the SMEM-capped fused hd=512
        // kernel (Bq=16 forced by the 99 KB opt-in). The
        ...
        // FMHA only above the S-matrix threshold.
        ...
        // NOTE: sliding-window prefill takes the cuBLAS path too —
        // attention_cublas_prefill applies the sliding_window softmax mask.
        // The fp8-QK kernel is opt-in only:
        // raw e4m3 Q/K conversion compounds per-layer score error on
        // real activations — measured PPL 549 vs 16.6 when it served
        // gemma-3 chunks. The FP16 WMMA kernel that
        ...
        // prefill restart noise, +24% pp1024, +52% pp2048).
        // Try it BEFORE the S-matrix gate so the
        // fast path does not depend on the attn_scores buffer being allocated or
        // large enough for n: a too-small buffer would make s_matrix_fits false
        ...
        // learned sinks. Gemma-4's hd=256 SWA layers take FA2
        // per-layer; its hd=512 global layers stay on cuBLAS (faster than the fused
        ...
        if (attn_sinks == nullptr &&
            ...
            // the S-matrix fits (faster than the SMEM-capped fused hd=512
            // kernel). The hd=256 SWA layers took FA2 above.
            dispatch_record::set_attn_prefill_outer(AttnPrefillOuter::CUBLAS);
            ...
            // 3.4-3.9× faster than the whole-prompt FMHA hd=512
            // fallback. False only when even a 16-row slice
            // overflows; then the FMHA below still serves.
        } else {
            // FMHA fallback: tiled O(n) memory chain. Sink-capable
