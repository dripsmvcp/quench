...
// Embedding requests mean-pool every token's hidden state; this runs
...
inline void apply_constraint_mask(const quench::InferenceState& st, float* logits, int vocab,
                                  cudaStream_t stream) {
    if (st.grammar_constrainer)
        ...
        // Budget-aware close: hand the remaining output allowance to
...
int32_t GraphExecutor::forward(const InferenceState& state, cudaStream_t stream) {
    ...
    // Check for CUDA errors after the forward pass (debug only)
    {
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("CUDA error after forward: %s", cudaGetErrorString(err));
        }
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("CUDA last error: %s", cudaGetErrorString(err));
        }
    }
#endif
    ...
    if (state.banned_tokens != nullptr && state.n_banned_tokens > 0) {
        ...
        for (int bi = 0; bi < state.n_banned_tokens; bi++) {
            ...
            if (tid >= 0 && tid < vocab_size) {
                QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(logits_ptr + tid, &neg_inf, sizeof(float),
...
std::vector<int32_t> GraphExecutor::sample_from_logits(const Tensor& logits, const InferenceState& state,
    ...
    auto apply_pre_sample = [&](Tensor& seq_logits, const InferenceState& st) {
        ...
        if (st.n_logit_bias > 0 && st.logit_bias != nullptr) {
            for (int i = 0; i < st.n_logit_bias; i++) {
                ...
                if (tid >= 0 && tid < vocab) {
                    float logit;
                    QUENCH_CUDA_CHECK_LOG(cudaMemcpy(&logit, lp + tid, sizeof(float), cudaMemcpyDeviceToHost));
                    logit += bias;
                    QUENCH_CUDA_CHECK_LOG(
    ...
    if (state.is_prefill || n_seq <= 1) {
        ...
        // pinned D2H + ONE stream sync. A per-sequence readback
        // (pageable 4-byte D2H + cudaStreamSynchronize each) serializes the
        // batch against ~200 us host round-trips: at n=16 sustained load that
        // is ~3 ms host time per decode step = 29% GPU idle
        // (nsys). Kernels, parameter normalization, and per-sequence
        ...
        if (can_batch) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpy2DAsync(h_sample_pinned_.as<int32_t>(), sizeof(int32_t), d_sample_result_,
...
bool GraphExecutor::sample_single_from_logits_async(const Tensor& logits, const InferenceState& state,
    ...
    // D2H + one sync). The synchronous per-row readback costs ~850 us of
    // blocked host time per sequence per step at n=16 sustained serving
    // (pageable 4-byte D2H + stream sync each, nsys).
...
void GraphExecutor::flush_pending_topk_rows_(cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_base, h_base, sizeof(TopkRowArgs) * n_pending_topk_rows_,
                                       cudaMemcpyHostToDevice, stream));
...
const int32_t* GraphExecutor::collect_sampled_tokens(int n_slots, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy2DAsync(h_sample_pinned_.as<int32_t>() + base, sizeof(int32_t),
...
bool GraphExecutor::gather_sampled_tokens_async(int n_slots, cudaStream_t stream) {
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy2DAsync(h_sample_pinned_.as<int32_t>() + base, sizeof(int32_t),
                                         ...
                                         SAMPLE_SCRATCH_BYTES, sizeof(int32_t), n_slots,
                                         cudaMemcpyDeviceToHost, stream));
    QUENCH_CUDA_CHECK_LOG(cudaEventRecord(sample_gather_evt_[sample_parity_], stream));
    return true;
}
...
const int32_t* GraphExecutor::wait_gathered_tokens(int parity) {
    ...
    if (!h_sample_pinned_.as<int32_t>() || !sample_gather_evt_[parity])
        return nullptr;
    QUENCH_CUDA_CHECK_LOG(cudaEventSynchronize(sample_gather_evt_[parity]));
...
void GraphExecutor::forward_decode_async(const InferenceState& state, int32_t* d_token_id, int32_t* h_mapped,
                                         cudaStream_t stream) {
    if (!initialized_) {
        QUENCH_LOG_ERROR("GraphExecutor::forward_decode_async called before init()");
        return;
    }
