...
// planner's per-weight tier decision selects FP16. A centralised
// `effective_capabilities(kind, qtype)` check mirrors the StoragePlanner's
// policy so Phase 1 and Phase 3 agree on which weights belong where —
// without it, Q4_K weights (e.g. Gemma-3-12B) can end up with zero cache
// (Phase 1 early-exiting while Phase 3 only caches nvfp4_beneficial
// weights).
//
...
using quench::pre_dequant_internal::create_fused_weight_pair;
...
void QuantPipeline::pre_dequant_phase1_fp16_cache_(
    ...
    //     not FP16 (the× starvation guard, now structural).
    ...
    auto cache_weight = [&](const Tensor& w, QType qtype, TensorKind kind) {
        ...
        if (!fp16_buf) {
            budget_exhausted = true;
            QUENCH_LOG_WARN("FP16 cache: allocation failed after %d tensors (%.1f MiB)", cached_count,
    ...
    if (cached_count > 0) {
        ...
        QUENCH_LOG_INFO("FP16 weight cache: %d tensors, %.2f MiB (incl. %d fused KV, %d fused gate+up)",
