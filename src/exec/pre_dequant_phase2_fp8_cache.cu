    ...
    if (wcache_->use_fp8) {
        ...
        size_t fp16_all_bytes = 0;
        {
            ...
            if (fp16_all_count > 0) {
                ...
                QUENCH_LOG_INFO("NVFP4→FP16 weight cache: %d tensors, %.2f MiB "
        ...
        if (!fp8_entries.empty() && qscratch_->dequant) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_block_maxes, (size_t)max_grid * sizeof(float)));
            QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_absmax, sizeof(float)));
            QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_scales_all, fp8_entries.size() * sizeof(float)));
            ...
            if (!d_fp8_bulk) {
                ...
                QUENCH_LOG_WARN("FP8 weight cache bulk alloc failed (%.1f MiB): %s",
            ...
            if (actual_count > 0) {
                ...
                QUENCH_CUDA_CHECK_LOG(cudaMemcpy(h_scales.data(), d_scales_all, actual_count * sizeof(float),
            ...
            QUENCH_CUDA_CHECK_LOG(cudaFree(d_absmax));
        ...
        if (fp8_count > 0) {
            ...
            QUENCH_LOG_INFO("FP8 weight cache: %d tensors, %.2f MiB (%.2f MiB saved vs FP16)", fp8_count,
                         fp8_total / (1024.0 * 1024.0), (fp16_equivalent - fp8_total) / (1024.0 * 1024.0));
        } else {
            QUENCH_LOG_INFO("FP8 prefill: no weights cached (budget=0 or no eligible weights)");
        }
    }
...
// decode slice (34.6% on Qwen3.6-35B per nsys). NVFP4 on these
...
// in no other cache, so their handles would be Undefined-tier → decode would
// pay a full dequant→cuBLAS round-trip per token. UD quants keep exactly these
...
void QuantPipeline::pre_dequant_phase2b_fp8_ssm_sidecar_(const ModelConfig& cfg,
    ...
    // gemm.fp8_attn_proj: same decode-only per-row-scale sidecar for
    ...
    if (!d_bulk) {
        cudaError_t e = cudaGetLastError();
        QUENCH_LOG_WARN("fp8_ssm_proj: sidecar alloc failed (%.1f MiB): %s — decode keeps FP16",
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_row_scales, total_rows * sizeof(float)));
    if (!d_row_scales) {
        vram_free(vram_alloc_, d_bulk);
        QUENCH_LOG_WARN("fp8_ssm_proj: row-scale alloc failed — decode keeps FP16");
        return;
    }
    ...
    QUENCH_LOG_INFO("fp8 decode sidecar: %zu projections%s%s (%.1f MiB, %zu per-row scales; "
