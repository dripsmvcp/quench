...
using quench::pre_dequant_internal::deduct_budget;
using quench::pre_dequant_internal::for_each_dense_weight;
using quench::pre_dequant_internal::nvfp4_beneficial;
using quench::pre_dequant_internal::nvfp4_lm_head_enabled;
...
void QuantPipeline::nvfp4_decode_collect_candidates_(const ModelConfig& cfg,
    ...
    if (wcache_->dual_path_quant) {
        ...
        QUENCH_LOG_INFO("Dual-path quant: excluding %zu attention weights from NVFP4 cache",
    ...
    // 9B+ models. (GGUF hybrids are served by the
    // GGUF branch of gemm.fp8_ssm_proj instead — see the
    // dispatch-policy comment.) Native-NVFP4 SSM
    // weights are handled identically by the phase0b register gate.
    {
        ...
        if (n_ssm_excluded > 0)
            QUENCH_LOG_INFO("GDN/SSM: excluding %d recurrent projections from NVFP4 cache", n_ssm_excluded);
    }
    ...
    // Both opt-outs must be honored here too — an unconditional call would
    // silently void them on every quantized-source (GGUF) checkpoint.
    {
        ...
        // net rule for quantized heads — see nvfp4_lm_head_enabled().
        ...
        if (quantized_head) {
            if (head_on && gdn_head_ok)
                ...
                QUENCH_LOG_INFO("NVFP4 LM head: skipped (%s)",
                             !gdn_head_ok ? "nvfp4_lm_head_gdn=false, GDN/SSM hybrid"
                                          : "gemm.nvfp4_lm_head off/auto net rule");
        }
    }
...
void QuantPipeline::nvfp4_decode_cache_fp16_lm_head_(const ModelConfig& cfg, cudaStream_t stream) {
    // Native-precision head (checked below): auto → ON per the net rule.
    ...
    if (!runtime_config().gemm.nvfp4_lm_head_gdn) {
        for (int i = 0; i < cfg.n_layers; i++) {
            ...
            if (L.ssm_in.data || L.ssm_out.data || L.gdn_gate.data) {
                QUENCH_LOG_INFO("NVFP4 LM head: skipped (GDN/SSM-hybrid model)");
                return;
            }
        }
    }
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_absmax_buf, sizeof(float)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_tscale_buf, sizeof(float)));
    ...
    float h_tscale = 1.0f;
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy(&h_tscale, d_tscale_buf, sizeof(float), cudaMemcpyDeviceToHost));
    result.tensor_scale = h_tscale;
    result.N = rows;
    result.K = cols;
    wcache_->nvfp4[lm.data] = result;

    QUENCH_CUDA_CHECK_LOG(cudaFree(d_absmax_buf));
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_tscale_buf));

    const double nvfp4_mib =
        (static_cast<size_t>(rows) * cols / 2 + static_cast<size_t>(rows) * cols / 16) /
        (1024.0 * 1024.0);
    QUENCH_LOG_INFO("NVFP4 LM head: quantized FP16 [%d x %d] → NVFP4 (%.1f MiB), decode GEMV fast path",
                 rows, cols, nvfp4_mib);
}
...
void QuantPipeline::nvfp4_decode_cache_fp16_projections_(const ModelConfig& cfg,
    ...
    if (n_attn > 0)
        QUENCH_LOG_INFO("NVFP4 attn proj: quantized %d BF16 q/k/v/o weights -> NVFP4 (%.1f MiB), decode GEMV fast path",
                     n_attn, bytes_attn / (1024.0 * 1024.0));
    else
        QUENCH_LOG_INFO("NVFP4 attn proj: no eligible BF16 q/k/v/o (flag on but model has none)");
}
...
void QuantPipeline::pre_dequant_phase3_nvfp4_decode_(
    ...
    if (runtime_config().diagnostics.no_nvfp4_decode_cache) {
        QUENCH_LOG_INFO("NVFP4 decode cache DISABLED (diagnostics.no_nvfp4_decode_cache) — "
    ...
    for (auto& e : nvfp4_entries) {
        ...
        if (e.from_scratch) {
            ...
            if (it == wcache_->fp16.end()) {
                QUENCH_LOG_ERROR("NVFP4 cache: FP16 companion for %p [%dx%d] VANISHED between "
        ...
        if (tmp_buf)
            QUENCH_CUDA_CHECK_LOG(cudaFree(tmp_buf));
        ...
        if (!e.from_scratch) {
            ...
            if (it != wcache_->fp16.end()) {
                ...
                QUENCH_LOG_DEBUG("NVFP4 cache: freeing FP16 companion %p (%zu B) after quantize",
                              e.orig_ptr, freed);
...
void QuantPipeline::nvfp4_decode_quantize_mode1_(size_t& remaining_budget, cudaStream_t stream,
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_tscales_all, budgeted.size() * sizeof(float)));
    ...
    for (size_t i = 0; i < budgeted.size(); i++) {
        ...
        if (e.from_scratch) {
            ...
            auto it = wcache_->fp16.find(e.orig_ptr);
            if (it == wcache_->fp16.end()) {
                QUENCH_LOG_ERROR("NVFP4 cache (batch): FP16 companion for %p [%dx%d] VANISHED between "
    ...
    QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(stream));
    for (void* p : tmp_bufs)
        QUENCH_CUDA_CHECK_LOG(cudaFree(p));

    std::vector<float> h_tscales(budgeted.size());
    QUENCH_CUDA_CHECK_LOG(cudaMemcpy(h_tscales.data(), d_tscales_all, budgeted.size() * sizeof(float),
    ...
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_absmax_buf));
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_tscales_all));
    ...
    if (nvfp4_from_scratch > 0) {
        ...
        QUENCH_LOG_INFO("NVFP4 decode cache: %d tensors, %.2f MiB (mode: %s)", nvfp4_count,
...
void QuantPipeline::nvfp4_decode_second_pass_(const VRAMBudget& budget, cudaStream_t stream,
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_absmax_buf2, sizeof(float)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_tscale_buf2, sizeof(float)));
    ...
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_absmax_buf2));
    QUENCH_CUDA_CHECK_LOG(cudaFree(d_tscale_buf2));

    if (second_count > 0) {
        wcache_->nvfp4_bytes += second_bytes;
        QUENCH_LOG_INFO("NVFP4 second pass: %d additional tensors, %.2f MiB", second_count,
