...
bool take_scratch(size_t bytes) {
    ...
    if (err != cudaSuccess) {
        QUENCH_LOG_ERROR("mmvq scratch: arena short by %zu B AND cudaMalloc failed: %s", bytes,
                      cudaGetErrorString(err));
        return false;
    }
    QUENCH_LOG_WARN("mmvq scratch: engine arena could not supply %.2f MiB (remaining %.2f MiB) — "
...
void prewarm_mmvq_scratch(int max_tokens, int max_K) {
    ...
    QUENCH_LOG_INFO("MMVQ scratch pre-warmed: %.2f KiB (max_tokens=%d, max_K=%d)",
