...
public:
    ...
    // is included only when the allocator would actually build one —
    ...
    // Per-chunk NLL accumulation (chunked-prefill-aware quench_perplexity).
    ...
    // Embedding pooling: column-wise sum of hidden_[0..n_tokens) into
    ...
    // The constrained-pipeline jump-ahead consumes them across later
    ...
    // allocator skip and workspace_estimate() so the two cannot drift.
    ...
    // Graph-captured verify chunk. The chunked continuation forward is
    ...
    // LoRA runtime delta (issue): activation-path low-rank deltas, no
    ...
    // Per-Engine RuntimeConfig (there is no
    // RuntimeConfig::current() singleton). Engine wires this via
    // set_runtime_config() during init; the contract is "set before
...
private:
    ...
    bool cur_spec_verify_ = false; // set by forward_logits: spec-verify chunk
    ...
    // Persistent K/V scratch for the graph-captured verify chunk,
    ...
    //. Grow-only; reused across layers (stream-ordered use).
...
public:
    ...
    // the scheduler must not attempt prefill-graph capture at all.
...
private:
    ...
    // Mode flags mirrored from wcache_ for PlanHints (hints_ is the plan-driven source of truth).
    PlanHints hints_;
    ...
    // --- Per-Engine RuntimeConfig (non-owning) ---
