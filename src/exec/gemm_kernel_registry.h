...
//
// Replaces the 21-parameter `gemm_dispatch_impl` god-dispatcher
// with a single small args struct and a (Strategy
...
//
// Live dispatch tries the registry first and falls back to the legacy
// `gemm_dispatch_impl` for strategies that return NoMatch /
// PreconditionFail or are not registered.
...
struct GemmKernelArgs {
    ...
    // (fall back to a non-workspace path) or fail loud via QUENCH_CHECK.
    ...
    // Dual-cache CUTLASS MXFP4 hand-off. When the
    ...
    // dp4a / Q8_1 activation quantization scratch (GGUF dp4a tier).
    ...
    // Per-model gemma4.force_mmvq override. Used by the
    ...
    // Per-Engine RuntimeConfig::gemm flags,
    // threaded from GemmContext at dispatch time (instead of the kernels
    // reading a RuntimeConfig::current() singleton).
    bool no_mmvq = false;
...
public:
    ...
    // entry and logs QUENCH_LOG_WARN. At most one registration is expected
    // per strategy.
...
private:
    ...
    // Small-N linear scan — the production table holds a few dozen entries
    // at most; a flat array + linear scan beats a
    // hash map for this size and avoids the std::unordered_map alloc.
    struct Entry {
        GemmStrategy strategy;
        GemmKernelFn fn;
    };
    // Capacity: 8 tier entries + 11 GGUF
    // strategies (4 mmvq qtypes + 7 dp4a qtypes) = 19; sized 32 for
    // headroom (additional GGUF qtype/backend
