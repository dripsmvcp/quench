...
void QuantPipeline::pre_dequant_phase0_promote_nvfp4_sidecars_(
    ...
    auto promote = [&](const NvFP4PreQuantWeight& sc, Tensor& w, const char* key) {
        ...
        if (!w.on_device || !sc.weight_scale.on_device) {
            QUENCH_LOG_DEBUG("NVFP4 prequant: skipping %s (data_dev=%d, scale_dev=%d)", key, w.on_device,
        ...
        if (!nvfp4_validate_weight_scale_dtype(sc.weight_scale.qtype, &scale_dtype_err)) {
            n_wrong_scale_dtype++;
            QUENCH_LOG_WARN("NVFP4 prequant: %s — %s. Skipping promotion (weight stays in "
        ...
        if (w.ndim == 2 && sc.weight_scale.ndim == 2) {
            ...
            if (!is_fused_sub) {
                ...
                if (!nvfp4_validate_packed_scale_shapes(w.shape[0], w.shape[1],
                                                       ...
                                                       &shape_err)) {
                    n_wrong_scale_dtype++;
                    QUENCH_LOG_WARN("NVFP4 prequant: %s — %s. Skipping promotion.", key, shape_err.c_str());
                    return false;
                }
            }
        }
        ...
        if (was_zeroed) {
            if (cfg.is_llm_compressor_nvfp4) {
                if (h_scale == 0.0f) {
                    n_zero_scale++;
                    QUENCH_LOG_WARN("NVFP4 prequant: %s has weight_scale_2=0 — zeroing tensor_scale "
                                 "(would otherwise produce Inf via reciprocal flip)", key);
                } else {
                    n_nonfinite_after_flip++;
                    QUENCH_LOG_WARN("NVFP4 prequant: %s reciprocal flip produced non-finite scale "
                ...
                if (h_scale == 0.0f) {
                    ...
                    QUENCH_LOG_DEBUG("NVFP4 prequant: %s has weight_scale_2=0 (Modelopt null layer)", key);
                } else {
                    n_nonfinite_after_flip++;
                    QUENCH_LOG_WARN("NVFP4 prequant: %s has non-finite weight_scale_2=%.6g — zeroing "
    ...
    for (auto& [key, sc] : mut_model->nvfp4_scratch_) {
        ...
        if (!w) {
            QUENCH_LOG_WARN("NVFP4 prequant: unresolved scratch key '%s'", key.c_str());
            continue;
        }
    ...
    // fix the sub-projection scales to point at the correct row offsets.
    {
        ...
        if (n_qkv_split > 0 || n_gateup_split > 0)
            QUENCH_LOG_INFO("Fused projection scale split: %d QKV + %d gate_up layers",
    ...
    // native NVFP4 (registered in Phase 0b, gemv_nvfp4 — same as Nemotron-H).
    {
        ...
        for (int i = 0; i < cfg.n_layers; i++) {
            ...
            for (Tensor* w : {&L.gdn_alpha, &L.gdn_beta}) {
                ...
                if (cudaMallocAsync(&fp16buf, static_cast<size_t>(N) * K * 2, stream) != cudaSuccess) {
                    QUENCH_LOG_WARN("GDN NVFP4→FP16 dequant: alloc failed (layer %d) — leaving NVFP4", i);
                    continue;
                }
        ...
        if (n_gdn_dequant > 0)
            QUENCH_LOG_INFO("GDN NVFP4 alpha/beta: dequantized %d FP16_ONLY projections to FP16",
                         n_gdn_dequant);
    }
    ...
    if (prequant_count > 0) {
        QUENCH_LOG_INFO("NVFP4 pre-quantized: promoted %d weights to Tensor sidecars", prequant_count);
        if (n_zero_scale > 0 || n_nonfinite_after_flip > 0) {
            QUENCH_LOG_WARN("NVFP4 prequant: %d zero weight_scale_2 / %d non-finite weight_scale_2 "
        ...
        if (n_wrong_scale_dtype > 0) {
            QUENCH_LOG_WARN("NVFP4 prequant: %d weights had non-FP8_E4M3 weight_scale dtype "
...
void QuantPipeline::pre_dequant_phase0b_register_cutlass_nvfp4_(
    ...
    //
    // Note: CUTLASS versions before v4.5.0 produced numerical regressions
    // on borderline tokens and cross-capture non-determinism on the sm_120
    // NVFP4×NVFP4 path (both llm-compressor and Modelopt formats);
    // v4.5.0 fixed this, so no format-specific guard is needed and all
    // NVFP4-prequant models take the CUTLASS fast path uniformly
    // (verifiable via `scripts/validate_safetensors.py`).
    if (cutlass_sm120_nvfp4_available()) {
        ...
        auto register_prequant = [&](const Tensor& w) {
            ...
            //
            // Do NOT build the CUTLASS SfAtom buffer
            ...
            // On Qwen3-Coder-30B-A3B NVFP4 that orphan is ~1782 MiB. Phase 3b
        ...
        if (ct_count > 0) {
            ...
            QUENCH_LOG_INFO("NVFP4 prequant: registered %d weights in decode cache "
        ...
        }
        {
            ...
            if (n_ssm > 0)
                QUENCH_LOG_INFO("GDN/SSM (native NVFP4): %d recurrent projections IN NVFP4 decode cache "
