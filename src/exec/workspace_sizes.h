...
// whatever free VRAM it finds and, on gpt-oss-20b at server defaults, can
// drive the card to 0.0 MiB free and leave a 31.64 MiB workspace to be
// rejected. Getting the capacity right is therefore not tuning; it is the
// mechanism.
//
...
// makes them testable in the CPU lane — taking a Model directly would make
// them untestable there. The arena is sized from these
// numbers, so a wrong one is the failure class (under-reservation), which
...
struct ExecShape {
    ...
    // NOT the context length — conflating the two wrongly sizes the
    // sampling scratch at ~115 MiB when it is ~1 MiB.
    ...
    // charge ~5x what either route can actually reach (measured:
...
// is assigned and therefore fires only for SSM+MoE, never for pure
// GDN. Replicating the *intended* condition would size the arena at
