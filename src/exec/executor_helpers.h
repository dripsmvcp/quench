...
static inline Tensor make_workspace_tensor(char*& ptr, QType dtype, int64_t rows, int64_t cols,
                                           size_t aligned_sz) {
...
static inline void* vram_alloc(VRAMAllocator* alloc, size_t bytes, const char* tag) {
    if (bytes == 0)
        return nullptr;
    if (alloc)
        return alloc->allocate(bytes, tag);
    void* ptr = nullptr;
...
static inline void* vram_alloc_force(VRAMAllocator* alloc, size_t bytes, const char* tag) {
    if (bytes == 0)
        return nullptr;
    if (alloc)
        return alloc->allocate(bytes, tag, /*bypass_headroom=*/true);
    void* ptr = nullptr;
    if (cudaMalloc(&ptr, bytes) != cudaSuccess)
        return nullptr;
    return ptr;
...
static inline void vram_free(VRAMAllocator* alloc, void* ptr) {
    ...
    if (alloc)
        ...
        QUENCH_CUDA_CHECK_LOG(cudaFree(ptr));
}
...
static inline void set_l2_streaming(cudaStream_t stream, const void* ptr, size_t bytes) {
    ...
    if (s_max_bytes == 0) {
        int max_w = 0;
        if (cudaDeviceGetAttribute(&max_w, cudaDevAttrMaxAccessPolicyWindowSize, 0) == cudaSuccess &&
            max_w > 0) {
