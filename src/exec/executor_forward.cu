...
// ---------------------------------------------------------------------------
// Forward pass diagnostics (diagnostics.debug_forward config flag; was QUENCH_DEBUG_FORWARD env)
// ---------------------------------------------------------------------------
...
void debug_top_logits(const Tensor& logits, cudaStream_t stream, int topk = 10) {
    ...
    QUENCH_LOG_DEBUG("[DEBUG_FWD] logits row=%d/%d  min=%+.4f max=%+.4f L2=%.4f", row, nrows, mn, mx,
                  std::sqrt(ss));
    ...
    QUENCH_LOG_DEBUG("[DEBUG_FWD] Top-%d logits:", topk);
    for (int i = 0; i < std::min(topk, vocab); i++) {
        QUENCH_LOG_DEBUG("  [%2d] token_id=%6d  logit=%+.6f", i, scored[i].second, scored[i].first);
    }
}
...
void GraphExecutor::build_lm_head_cutlass_(cudaStream_t stream) {
    ...
    if (lm_head_cutlass_ready_)
        QUENCH_LOG_INFO("LM head: CUTLASS NVFP4 tensor-core GEMM enabled for batched decode "
...
void GraphExecutor::forward_logits(const InferenceState& state, Tensor& logits_out, cudaStream_t stream) {
    if (!initialized_) {
        QUENCH_LOG_ERROR("GraphExecutor::forward_logits called before init()");
        return;
    }
    ...
    if (n <= 0) {
        QUENCH_LOG_ERROR("n_tokens must be positive, got %d", n);
        return;
    }
    if (n > max_tokens_) {
    ...
    // Clear any stale CUDA error state before starting the forward pass.
    {
        ...
        if (e_ != cudaSuccess)
            QUENCH_LOG_WARN("Cleared stale error before forward: %s", cudaGetErrorString(e_));
    }

    // ---- Optional per-component profiling (diagnostics.profile config flag, was QUENCH_PROFILE env) ----
    // Profiling disables CUDA graph capture (they are incompatible).
    // Use the diagnostics.profile config flag (was QUENCH_PROFILE env) for diagnostic runs only.
    ...
    if (debug_forward_enabled()) {
        ...
        QUENCH_LOG_DEBUG("[DEBUG_FWD] [step=%d] input_tokens (%d):%s", decode_step, n, ids_line.c_str());
        // Dump positions
        ...
        QUENCH_LOG_DEBUG("[DEBUG_FWD] [step=%d] positions (%d):%s", decode_step, n, pos_line.c_str());
    }
    ...
    if (fp32_accum_buf_ && n == 1 && debug_forward_enabled()) {
        ...
        cudaMemcpyAsync(tmp, view_tokens(fp32_hidden_, n).data, 4 * sizeof(float), cudaMemcpyDeviceToHost,
                        stream);
        cudaStreamSynchronize(stream);
        QUENCH_LOG_DEBUG("[DEBUG_FWD] [step=%d] fp32_accum_init: [%.4f %.4f %.4f %.4f]", decode_step, tmp[0],
                      tmp[1], tmp[2], tmp[3]);
    }
    ...
    if (!runtime_config().diagnostics.dump_hidden_dir.empty()) {
        ...
        char fname[256];
        snprintf(fname, sizeof(fname), "/tmp/quench_embed_step%d.bin", decode_step);
        FILE* f = fopen(fname, "wb");
        if (f) {
            fwrite(h_buf.data(), sizeof(half), h_buf.size(), f);
            fclose(f);
        }
        QUENCH_LOG_DEBUG("[DUMP_BIN] Wrote %s (%zu halfs)", fname, h_buf.size());
    }
    ...
    for (int i = 0; i < max_layer; ++i) {
        ...
        dump_tensor_npy("B_post_attn", view_tokens(h, n), stream, i, decode_step);
        {
            ...
            snprintf(buf, sizeof(buf), "[step=%d] after_layer%02d_%s", decode_step, i,
                     layer_has_attention(i) ? "attn" : "none");
        ...
        }
        {
            char buf[64];
            snprintf(buf, sizeof(buf), "[step=%d] after_layer%02d_%s", decode_step, i,
                     layer_has_dense_ffn(i) ? "ffn" : "no_ffn");
            debug_tensor_stats_all(buf, view_tokens(h, n), stream);
            const bool dump_this_layer = debug_forward_enabled();
            if (dump_this_layer) {
                char rbuf[64];
            ...
            // and nothing else; the default is empty and costs a string check.
            {
                ...
                if (!dh.empty() && (dh == "all" || i == 0 || i == 5 || i == 15 || i == 29)) {
                    ...
                    char fname[512];
                    snprintf(fname, sizeof(fname), "%s/quench_L%02d_step%d_n%d.bin",
        ...
        // next layer reads it. Without it, the residual stream grows unbounded.
        {
            ...
            if (ly.layer_out_scale.data != nullptr && ly.layer_out_scale.on_device && h.qtype == QType::F16) {
                ...
                if (debug_forward_enabled()) {
                    ...
                    if (i == 0 || i == 29)
                        QUENCH_LOG_DEBUG("[DEBUG_FWD] L%d_out_scale = %.6f", i, sval);
                    ...
                    if (n == 1 && debug_forward_enabled()) {
                        ...
                        cudaStreamSynchronize(stream);
                        QUENCH_LOG_DEBUG("[DUMP] step=%d L%02d h=[%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f]",
    ...
    // is not visible via primary_tier alone — probe wcache_.nvfp4 as well.
    ...
    // L2 streaming hint for the LM head projection:
    ...
    if (state.is_prefill && !state.all_logits) {
        ...
        if (lm_tier == StorageTier::MXFP4 && lm_h->payload.mxfp4.linear_scales) {
            ...
            if (runtime_config().generation.lm_dequant_fp16) {
                ...
                QUENCH_CUDA_CHECK_LOG(cudaMallocAsync(&w_fp16_dev, fp16_bytes, stream));
                ...
                QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(w_fp16_dev, stream));
                QUENCH_LOG_DEBUG("[DEBUG_FWD] LM head via dequant->FP16->cuBLAS path");
            } else {
        ...
        if (n == 1 && lm_tier == StorageTier::MXFP4 && lm_h->payload.mxfp4.linear_scales) {
            ...
            // Batched-M NVFP4 GEMV LM head for batched decode. Looping
            // a single M=1 GEMV per sequence would re-read the whole ~389 MiB
            // LM-head matrix from HBM once per row (it does not fit in L2) — the
            // #2 decode GPU consumer at batch>1. gemv_nvfp4_kpar_batched_fp32
            // reads the weight ONCE and reuses it across all rows.
            NvFP4QuantResult nvfp4_lm_r;
            if (lm_nvfp4_secondary) {
                nvfp4_lm_r = lm_nvfp4_it->second;
            } else {
