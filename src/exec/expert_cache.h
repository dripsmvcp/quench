...
struct ExpertLRUCache {
    ...
    // Pre-computed host source pointers for capture-safe memcpy.
    ...
    // tensors don't move on host). TODO (graphs under host-offload): use
    // these as the fixed `src` argument of cudaGraphAddMemcpyNode so the
    // graph can replay without recomputing host pointers each iteration.
    std::vector<std::vector<const void*>> host_expert_addrs_;

    // Canonical packed_ptr per (layer, proj). The ExpertCacheKey
    ...
    // Per-(layer, proj) expert byte size. The prefetch path can't
    ...
    // Device-side lookup mirror. Sized [n_layers × 3 ×
    // n_experts] int32 cells; cell value = **layer-relative** slot_idx
    // (0..slots_per_layer-1) or -1 if not cached. Read pattern (the
    // graphs-under-offload follow-up will do this from inside dispatch kernels):
    ...
    // nullptr if the slot is not occupied. Used by tests and as the
    // future kernel-side resolve helper — the graphs-under-offload
    // follow-up will inline this logic into the dispatch kernels.
    ...
    // Debug helper: copy the device lookup mirror back to host and
    ...
    // (via QUENCH_LOG_FATAL) on mismatch. Tests can call it directly.
