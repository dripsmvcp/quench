...
bool Workspace::resize_workspace(int new_max_tokens, cudaStream_t stream) {
    ...
    // memory (: server wedged with an illegal memory access whenever a
    ...
    *max_tokens_ = saved_max;
    ...
    if (new_shared > shared_workspace_size_) {
        ...
        if (shared_workspace_) {
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(shared_workspace_, stream));
        }
        ...
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to resize shared workspace to %zu bytes: %s", new_shared,
...
bool Workspace::allocate_decode_workspace(cudaStream_t stream, int max_batch) {
    ...
    if (!decode_workspace_) {
        QUENCH_LOG_ERROR("Failed to allocate decode workspace");
        return false;
    }
    ...
    *max_tokens_ = saved;
    ...
    if (shared > 0) {
        ...
        if (!decode_shared_workspace_) {
            QUENCH_LOG_ERROR("Failed to allocate decode shared workspace");
    ...
    QUENCH_LOG_INFO("Decode overlap workspace: %.2f MiB for max_batch=%d (persistent=%.1f KB, shared=%.1f KB)",
