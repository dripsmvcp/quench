...
void GraphExecutor::allocate_auxiliary_buffers(bool skip_batch_dequant) {
    ...
    if (cfg.is_mla() && max_tokens_ > 0) {
        ...
        auto alloc = [&](void** p, size_t cols, const char* name) {
            ...
            if (slab.empty()) {
                QUENCH_LOG_ERROR("MLA scratch %s (%.1f MiB) unavailable from the T2 arena "
        ...
        QUENCH_LOG_INFO("MLA QKV scratch: kv_a+latent+k_rope+kv_b for max_tokens=%d (graph-safe)",
                     max_tokens_);
        ...
        if (runtime_config().attention.mla_absorb && mla_absorb_max_seq_ > 0) {
            ...
            if (!all_fp16) {
                QUENCH_LOG_WARN("attention.mla_absorb: kv_b_proj is not FP16 on all layers — "
                ...
                if (cache_slab.empty() || scores_slab.empty()) {
                    QUENCH_LOG_ERROR("attention.mla_absorb: latent cache (%.1f MiB) unavailable from "
                    ...
                    QUENCH_LOG_INFO("MLA absorbed latent cache: %.1f MiB (n_layers=%d, max_seq=%d, "
    ...
    // Qwen3-14B-NVFP4).
    {
        ...
        if (max_weight_elems > 0) {
            ...
            if (!qscratch_.dequant) {
                QUENCH_LOG_ERROR("Failed to allocate dequant scratch (%.1f MiB)",
                              qscratch_.dequant_size / (1024.0 * 1024.0));
                qscratch_.dequant_size = 0;
            } else {
                QUENCH_LOG_INFO("Dequant scratch buffer: %.2f MiB", qscratch_.dequant_size / (1024.0 * 1024.0));
            }
        }
    }
    ...
    // exact pre-parity layout.
    {
        ...
        if (slab.empty()) {
            QUENCH_LOG_ERROR("Failed to obtain the sampling result buffer from the T2 arena");
    ...
    if (h_sample_pinned_.empty() && d_sample_result_) {
        ...
        if (h_sample_pinned_.empty())
            QUENCH_LOG_WARN("pinned sample buffer unavailable — async sample D2H disabled");
    }
    ...
    if (h_row_args_.empty() && d_sample_result_ && sample_slots_ > 0) {
        ...
        if (h_row_args_.empty() || slab.empty()) {
            QUENCH_LOG_WARN("row-batched sampler args alloc failed — falling back to per-row sampling");
            h_row_args_.reset();
            d_row_args_ = nullptr;
        } else {
    ...
    //   2. MoE down projection: top_k * expert_d_ff / 32 blocks (per-expert quantized activations)
    {
        ...
        if (max_blocks > 0) {
            ...
            if (q8_slab.empty() || d8_slab.empty()) {
                QUENCH_LOG_WARN("MMVQ scratch unavailable from the T2 arena, dp4a path disabled");
        ...
        if (qscratch_.q8_1_max_blocks > 0) {
            ...
            if (slab.empty()) {
                QUENCH_LOG_WARN("FFN sparsity mask unavailable from the T2 arena (%zu bytes)", mask_sz);
        ...
        if (max_blocks > 0 && max_tokens_ > 1) {
            ...
            if (has_sub5bit_dense) {
                ...
                auto d8_slab = engine_arena().take_bytes(d8_sz);
                if (q8_slab.empty() || d8_slab.empty()) {
                    QUENCH_LOG_WARN("dp4a prefill scratch unavailable from the T2 arena (%.1f MiB), "
                                 "FP16 cache fallback",
                    ...
                    QUENCH_LOG_INFO("dp4a prefill scratch: %.1f KiB (max_m=%d, max_k=%d)",
    ...
    // Sized for max_batch_size * n_heads * max_splits * (2 + head_dim) floats.
    {
        ...
        auto slab = engine_arena().take_bytes(sz);
        if (slab.empty()) {
            QUENCH_LOG_WARN("Split-K scratch unavailable from the T2 arena (%zu bytes), split-K "
                         "disabled", sz);
            ...
            QUENCH_LOG_INFO("Split-K paged attention scratch: %.2f KiB", sz / 1024.0);
        }
    }
    ...
    // pp1024, +52% pp2048; hd=256 rides the fa2_hd256 port) — the
    ...
    // non-attention layers) take FA2 too.
    ...
    if (fa2_serves_all_prefill()) {
        QUENCH_LOG_INFO("cuBLAS attention S-matrix: skipped (FP16-QK FA2 serves all prefill — "
                     "no S-matrix needed)");
        ...
        if (attn_seq > 0) {
            ...
            if (!attn_scores_buf_) {
                ...
                QUENCH_LOG_INFO("cuBLAS attention S-matrix: %.2f MiB (%d heads x %d x %d)",
                             s_sz / (1024.0 * 1024.0), nh, attn_seq, attn_seq);
            }
        }
    } else {
        QUENCH_LOG_INFO("cuBLAS attention S-matrix: skipped (VRAM-constrained, using tiled WMMA FMHA fallback)");
    }
    ...
    if (runtime_config().attention.fmha_prefill_threshold == -1) {
        ...
        QUENCH_LOG_INFO("auto fmha_prefill_threshold = %d (S-matrix cap + 1)", auto_threshold);
    }
    ...
    if (has_moe_) {
        ...
        // Dequant buffer: 1 expert slot
        {
            ...
            if (!moe_.dequant_buf) {
                QUENCH_LOG_ERROR("Failed to allocate MoE dequant buffer (%zu bytes)", dequant_sz);
                moe_.dequant_buf_size = 0;
            } else {
                moe_.dequant_buf_size = dequant_sz;
                QUENCH_LOG_INFO("MoE dequant buffer: %.2f MiB (1 expert slot)", dequant_sz / (1024.0 * 1024.0));
            }
        }
        ...
        bool any_host_packed_experts = false;
        {
            ...
            if (max_expert_raw > 0 && any_host_packed_experts) {
                ...
                if (!moe_.raw_staging_buf) {
                    QUENCH_LOG_ERROR("Failed to allocate MoE staging buffer (%zu bytes)", max_expert_raw);
                    moe_.raw_staging_size = 0;
                } else {
                    moe_.raw_staging_size = max_expert_raw;
                    QUENCH_LOG_INFO("MoE staging buffer: %.2f MiB (1 expert raw)",
                                 max_expert_raw / (1024.0 * 1024.0));
                }
            } else if (max_expert_raw > 0) {
                QUENCH_LOG_INFO("MoE staging buffer: skipped (all packed experts on device)");
            }
        }
        ...
        if (max_expert_raw > 0) {
            ...
            if (has_host_experts && !runtime_config().moe.no_expert_cache) {
                ...
                if (expert_cache_.init(max_expert_raw, budget, vram_alloc_, mcfg.n_layers,
                    ...
                    QUENCH_LOG_INFO("Expert LRU cache: %d slots (%.2f MiB / %.2f MiB budget)",
                                 expert_cache_.n_slots_,
                                 expert_cache_.n_slots_ * max_expert_raw / (1024.0 * 1024.0),
                                 budget / (1024.0 * 1024.0));
                }
            } else if (has_host_experts) {
                QUENCH_LOG_INFO("Expert LRU cache disabled via QUENCH_NO_EXPERT_CACHE (staging fallback)");
            }
        }
        ...
        if (experts_st_nvfp4) {
            QUENCH_LOG_INFO("MoE batch dequant + fp32_down buffers: skipped "
        ...
        if (!skip_batch_dequant) {
            ...
            for (int ne_try : targets) {
                ...
                if (cap_bytes > 0 && sz > cap_bytes) {
                    QUENCH_LOG_DEBUG("MoE dequant: skipping %d experts (%.0f MiB > cap %.0f MiB)", ne_try,
                ...
                if (!moe_.batch_dequant_buf) {
                    QUENCH_LOG_DEBUG("MoE dequant buf alloc failed for %d experts", ne_try);
                    continue;
                }
                moe_.batch_dequant_buf_size = sz;
                allocated = true;
                QUENCH_LOG_INFO("MoE batch dequant buffer: %.2f MiB (%d experts)", sz / (1024.0 * 1024.0),
                             ne_try);
                break;
            }
            if (!allocated) {
                QUENCH_LOG_INFO("MoE batch dequant buffer: skipped (VRAM insufficient)");
                moe_.batch_dequant_buf = nullptr;
                moe_.batch_dequant_buf_size = 0;
            }
            ...
            // Skipped if VRAM insufficient — forward pass falls back to lazy alloc.
            {
                ...
                if (fp32_sz > 0 && free_bytes > fp32_sz + kReserve) {
                    ...
                    if (moe_.fp32_down_buf) {
                        ...
                        QUENCH_LOG_INFO("MoE fp32_down scratch: %.2f MiB",
            ...
            QUENCH_LOG_INFO("MoE batch dequant buffer: skipped (experts on host)");
            moe_.batch_dequant_buf = nullptr;
            moe_.batch_dequant_buf_size = 0;
        }
        ...
        if (cfg.n_experts > 0) {
            ...
            if (moe_.cutlass3x_packed && moe_.cutlass3x_sf) {
                ...
                if (auto sl = engine_arena().take_bytes(sfa_ptr_bytes); !sl.empty()) {
                    ...
                    QUENCH_LOG_WARN("CUTLASS 3.x SFA pointer array unavailable from the T2 arena");
                    moe_.cutlass3x_sfa_ptrs = nullptr;
                }
                QUENCH_LOG_INFO("CUTLASS 3.x MoE staging: %.2f MiB (packed=%.2f, sf=%.2f) max_expanded=%d",
                             (packed_sz + sf_sz) / (1024.0 * 1024.0), packed_sz / (1024.0 * 1024.0),
                             sf_sz / (1024.0 * 1024.0), max_expanded);
            } else {
                QUENCH_LOG_WARN("CUTLASS 3.x MoE staging: allocation failed, path disabled");
        ...
        if (cfg.n_experts > 0) {
            ...
            if (auto sl = engine_arena().take_bytes(ptr_bytes); !sl.empty()) {
                ...
                QUENCH_LOG_DEBUG("Optional MoE work ptrs unavailable from the T2 arena");
    ...
    if (wcache_.use_fp8) {
        ...
        if (!qscratch_.fp8_act) {
            QUENCH_LOG_WARN("Failed to allocate FP8 activation buffer (%.1f MiB)",
        ...
        }
        {
            ...
            if (!qscratch_.d_act_scale)
                QUENCH_LOG_WARN("FP8 act scale unavailable from the T2 arena");
        }
        ...
        if (qscratch_.fp8_act && qscratch_.d_act_scale) {
            ...
            if (sl_bm.empty() || sl_am.empty()) {
                QUENCH_LOG_WARN("FP8 reduction buffers unavailable from the T2 arena — sync path");
    ...
    if (wcache_.nvfp4_decode_mode > 0 && cutlass_sm120_nvfp4_available()) {
        ...
        if (max_k > 0) {
            ...
            if (!qscratch_.cutlass_act_data || !qscratch_.cutlass_act_sf ||
                ...
                QUENCH_CUDA_CHECK_LOG(cudaMemset(qscratch_.cutlass_act_sf, 0,
                                              qscratch_.cutlass_act_sf_size));
                QUENCH_LOG_INFO("CUTLASS NVFP4 activation scratch: %.2f MiB (data=%.2f, sf=%.2f, ws=%.2f)",
                ...
                if (cutlass_sm120_mxfp4_available() &&
                    ...
                    if (!qscratch_.mxfp4_act_sf ||
                        ...
                        QUENCH_LOG_WARN("Failed to allocate MXFP4 activation buffers, MXFP4 prefill disabled");
                        if (qscratch_.mxfp4_act_sf) {
                        ...
                        QUENCH_LOG_INFO("CUTLASS MXFP4 activation scratch: sf=%.2f MiB, ws=%.2f MiB",
...
bool GraphExecutor::allocate_nvfp4_dequant_workspace() {
    ...
    // what made its verify-chunk capture fail: census crash class).
    ...
    // or short, exactly as the MMVQ tenant does.
    {
        ...
        if (!slab.empty()) {
            ...
            if (nvfp4_dequant_ws_buf_)
                QUENCH_LOG_WARN("nvfp4_dequant: engine arena could not supply %.2f MiB "
    ...
    if (!nvfp4_dequant_ws_buf_) {
        QUENCH_LOG_WARN("gemm_nvfp4 dequant workspace: alloc failed (%zu bytes) — prefill graph "
...
void GraphExecutor::release_moe_batch_buf() {
    if (moe_.batch_dequant_buf) {
        ...
        moe_.batch_dequant_buf = nullptr;
        moe_.batch_dequant_buf_size = 0;
        QUENCH_LOG_INFO("Released MoE batch dequant buffer: %.2f MiB (experts on host)",
                     freed / (1024.0 * 1024.0));
    }
}
...
void GraphExecutor::free_buffers() {
    if (lora_scratch_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(lora_scratch_));
        lora_scratch_ = nullptr;
        lora_scratch_sz_ = 0;
    }
    if (verify_argmax_scratch_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(verify_argmax_scratch_));
        verify_argmax_scratch_ = nullptr;
        verify_argmax_scratch_sz_ = 0;
    }
    if (verify_pen_counts_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(verify_pen_counts_));
    ...
    if (longrope_short_freqs_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(longrope_short_freqs_));
        longrope_short_freqs_ = nullptr;
    }
    if (longrope_long_freqs_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(longrope_long_freqs_));
    ...
    // Free all weight caches (FP16, FP8, NVFP4, CUTLASS, fused KV/gate+up, migrated/overflow)
    {
        ...
        if (wcache_.fp16_bulk_data) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(wcache_.fp16_bulk_data));
        ...
        if (wcache_.cutlass_sf_slab) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(wcache_.cutlass_sf_slab));
        ...
        if (wcache_.fp8_migrated_scales) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(wcache_.fp8_migrated_scales));
        ...
        if (wcache_.fp8_overflow_scales) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(wcache_.fp8_overflow_scales));
        ...
        if (wcache_.fp8_ssm_sidecar_row_scales) {
            QUENCH_CUDA_CHECK_LOG(cudaFree(wcache_.fp8_ssm_sidecar_row_scales));
    ...
    h_row_args_.reset();
    if (d_banned_cache_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(d_banned_cache_));
    ...
    for (int p = 0; p < 2; ++p) {
        if (sample_gather_evt_[p]) {
            QUENCH_CUDA_CHECK_LOG(cudaEventDestroy(sample_gather_evt_[p]));
            sample_gather_evt_[p] = nullptr;
        }
    }
    sample_parity_ = 0;
    ...
    if (chunk_eager_k_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(chunk_eager_k_));
        chunk_eager_k_ = nullptr;
    }
    if (chunk_eager_v_) {
        QUENCH_CUDA_CHECK_LOG(cudaFree(chunk_eager_v_));
...
bool GraphExecutor::chunk_capture_supported() const {
    ...
    // rides the port (d_kv_len is a runtime kernel argument shared by
    ...
    // exists for that layout ( follow-up).
    bool any_moe = false;
    ...
    if (any_moe && !runtime_config().moe.skip) {
        ...
        for (int i = 0; i < cfg.n_layers; ++i) {
            ...
            // graph-capturable (; the fallback also guards itself).
...
bool GraphExecutor::ensure_chunk_capture_scratch(int ctx_capacity) {
    ...
    if (k_slab.empty() || v_slab.empty()) {
        QUENCH_LOG_WARN("chunk-capture scratch unavailable from the T2 arena (2x %.1f MiB, %.1f MiB "
    ...
    QUENCH_LOG_INFO("chunk-capture K/V scratch: 2x %.1f MiB (ctx_capacity=%d, nkv=%d, hd=%d)",
