...
// NVFP4 GEMM tier.
//
// Adapter that wraps the NVFP4 GEMM (M>1 prefill, dequant→cuBLAS
...
// and lives inside the NvFP4QuantResult), m_is_one=false. This handler only
// covers the dequant fallback GEMM path; the CUTLASS sm_120 native FP4
// GEMM (gemm_nvfp4_cutlass_sm120) is a separate tier
// under StorageTier::CUTLASS_NVFP4.
//
...
static GemmDispatchResult nvfp4_gemm_kernel(const GemmKernelArgs& args) {
    QUENCH_CHECK(args.input != nullptr, "nvfp4_gemm_kernel: input is null");
    QUENCH_CHECK(args.output != nullptr, "nvfp4_gemm_kernel: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr, "nvfp4_gemm_kernel: weight_payload is null");
    QUENCH_CHECK(args.input->shape[0] > 1, "nvfp4_gemm_kernel: M must be > 1 (use GEMV strategy for M==1)");
...
// Static registration: only the (M>1) prefill GEMM
// strategy. The (M==1) decode GEMV strategy registers separately.
namespace {
