...
// add it to CMakeLists.txt QUENCH_EXEC_SOURCES, declare the method on
...
void QuantPipeline::build(const Model& model, const DispatchPolicy& rcfg, VRAMAllocator& alloc,
    ...
    // charged it to Phase 3 too — see split_pre_dequant_budget.
    ...
    // budget (a separate UNCONSTRAINED plan feeds the weight-cache
    ...
    if (storage_plan_.failed) {
        QUENCH_LOG_WARN("StoragePlanner: plan failed — %s", storage_plan_.failure_reason.c_str());
    } else {
        QUENCH_LOG_INFO("StoragePlanner: %zu entries, projected VRAM %.2f MiB",
...
void QuantPipeline::apply_arch_rules_(StoragePlan& plan, const ModelConfig& cfg) const {
    ...
    // LM head → NVFP4 per the net rule (nvfp4_lm_head_enabled): the kind
