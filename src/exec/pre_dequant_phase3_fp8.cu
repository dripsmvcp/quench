...
void QuantPipeline::nvfp4_decode_free_fp16_and_migrate_fp8_(size_t& remaining_budget,
    ...
    if (wcache_->use_fp8) {
        ...
        if (!to_migrate.empty()) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_scales_all, to_migrate.size() * sizeof(float)));
            ...
            if (!d_fp8_bulk) {
                ...
                QUENCH_LOG_WARN("FP8 migration cache alloc failed (%.1f MiB): %s",
            ...
            if (migrated > 0) {
                ...
                QUENCH_CUDA_CHECK_LOG(cudaMemcpy(h_scales.data(), d_scales_all, migrated * sizeof(float),
