...
void GemmKernelRegistry::register_kernel(GemmStrategy strategy, GemmKernelFn fn) {
    QUENCH_CHECK(fn != nullptr, "GemmKernelRegistry::register_kernel: fn is null");
    ...
    QUENCH_CHECK(count_ < (sizeof(entries_) / sizeof(entries_[0])),
...
static GemmDispatchResult fp16_gemm_kernel(const GemmKernelArgs& args) {
    QUENCH_CHECK(args.input != nullptr, "fp16_gemm_kernel: input is null");
    QUENCH_CHECK(args.output != nullptr, "fp16_gemm_kernel: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr, "fp16_gemm_kernel: weight_payload is null");

    const Tensor& weight = *static_cast<const Tensor*>(args.weight_payload);
    QUENCH_CHECK(weight.qtype == QType::F16, "fp16_gemm_kernel: weight qtype=%d, expected F16",
...
// runs at library load. This TU registers only FP16 (m_is_one=false; the
