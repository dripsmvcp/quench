...
int exec_max_tokens(const ExecShape& shape, int max_seq_len) {
    ...
    // the intent here would under-reserve by 2x on those models.
...
ExecT2Demand exec_t2_demand(const ExecShape& shape, int max_seq_len) {
    ...
    // 2*expert_d_ff x d_model — which is exactly the 31.64 MiB workspace a
    // tensor-scan sizing would refuse. Deriving it from the config
    ...
    // ~1 MiB and not the ~115 MiB a context-length sizing would suggest.
    ...
    // halves each. Growth might suggest leaving it out of T2 ("a bump arena
    // strands it") — but that is true
    // of the intermediate takes, not of taking the bound once, so it is charged
    // here and the site takes it at this size (A7 step 4b.2).
    ...
    // 152 320 B rounded to 1 MiB, not a defensive 512 MiB
    // reservation; the demand is what makes the smaller number safe, because
