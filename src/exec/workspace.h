...
// behaviour-neutral.
//
// See the Workspace-component design memo.
class Workspace {
public:
    ...
    // holding raw pointers into the arena (captured verify graphs,) use
    ...
    // skips the buffer there, so the estimate must not hold phantom
    // headroom for it. GraphExecutor::workspace_estimate() derives
