...
void GraphExecutor::run_ffn(int layer, cudaStream_t stream) {
    ...
    // the address span of the three weight tensors (gate, up, down).
    {
        ...
        if (!ly.w_gate.dropped_source)
            update(ly.w_gate.data, ly.w_gate.nbytes());
        if (!ly.w_up.dropped_source)
            update(ly.w_up.data, ly.w_up.nbytes());
        if (!ly.w_down.dropped_source)
    ...
    const WeightHandle* hwd = (ly.w_down_id != kInvalidTensorID) ? &registry_.handle(ly.w_down_id) : nullptr;
    ...
    bool will_fuse_down_beta1 = (!has_post_ffn_norm && !will_fuse_down_residual && !will_fuse_down_nvfp4 &&
                                 n > 1 && (wd_tier == StorageTier::FP16 || wd_tier == StorageTier::FP8));
    ...
    //    For decode (n=1): fuse RMSNorm→Q8_1→GEMV to avoid redundant quantization.
    {
        ...
        const WeightHandle* hwg = (ly.w_gate_id != kInvalidTensorID) ? &registry_.handle(ly.w_gate_id)
                                                                     : nullptr;
        const WeightHandle* hwu = (ly.w_up_id != kInvalidTensorID) ? &registry_.handle(ly.w_up_id) : nullptr;
        ...
        if (mxfp4_ffn) {
            ...
            if (hbs > 0 && hadamard_block_size_valid(hbs))
                hadamard_transform_fp16(static_cast<const half*>(no.data), static_cast<half*>(no.data), 1, d,
                                        hbs, stream);
            ...
            gemv_mxfp4_gate_up_fused(mw_g, mw_u, static_cast<const half*>(no.data),
            ...
            rmsnorm_quantize_q8_1(static_cast<const half*>(h.data), static_cast<const half*>(ffn_norm_w.data),
                                  q8, qscratch_.d8_buf, static_cast<half*>(no.data), d, eps, stream,
                                  norm_w_off_);
            ...
            bool fp8_ffn = (n > 1 && !cur_force_fp16_ && hwg && hwg->primary_tier == StorageTier::FP8 &&
                            hwu && hwu->primary_tier == StorageTier::FP8 && qscratch_.fp8_act != nullptr &&
            ...
            if (fp8_ffn) {
                ...
                gemm_cublaslt(fp8_no, fp8_tg, go, 1.0f, 0.0f, qscratch_.d_act_scale, hwg->payload.fp8.d_scale,
                              stream);
                gemm_cublaslt(fp8_no, fp8_tu, uo, 1.0f, 0.0f, qscratch_.d_act_scale, hwu->payload.fp8.d_scale,
                              stream);
            } else {
    ...
    //    eliminating the intermediate FP16 buffer write and one kernel launch.
    {
        ...
        bool fused_down_residual = (!has_post_ffn_norm && n == 1 && q8 != nullptr &&
                                    qscratch_.d8_buf != nullptr && so.qtype == QType::F16 &&
        ...
        if (will_fuse_down_mxfp4) {
            ...
            if (hbs > 0 && hadamard_block_size_valid(hbs)) {
                ...
                hadamard_transform_fp16(static_cast<const half*>(so.data), static_cast<half*>(so.data), 1,
                                        K_d, hbs, stream);
                gemv_mxfp4_residual(wd_mxfp4, static_cast<const half*>(so.data), static_cast<half*>(h.data),
                                    static_cast<const half*>(h.data), M_d, K_d, stream);
            } else if (cfg.ffn_activation != FFNActivation::GEGLU) {
                gemv_mxfp4_swiglu_residual(wd_mxfp4, static_cast<const half*>(go.data),
                                           static_cast<const half*>(uo.data), static_cast<half*>(h.data),
                                           static_cast<const half*>(h.data), M_d, K_d, stream);
            } else {
                gemv_mxfp4_geglu_residual(wd_mxfp4, static_cast<const half*>(go.data),
                                          static_cast<const half*>(uo.data), static_cast<half*>(h.data),
                                          static_cast<const half*>(h.data), M_d, K_d, stream);
            }
            ...
            if (n_mb_d <= 512) {
                ...
                if (cfg.ffn_activation != FFNActivation::GEGLU) {
                    gemv_nvfp4_swiglu_residual(wd_nvfp4, static_cast<const half*>(go.data),
                                               static_cast<const half*>(uo.data), static_cast<half*>(h.data),
                                               static_cast<const half*>(h.data), M_d, K_d, stream);
                } else {
                    gemv_nvfp4_geglu_residual(wd_nvfp4, static_cast<const half*>(go.data),
                                              static_cast<const half*>(uo.data), static_cast<half*>(h.data),
                                              static_cast<const half*>(h.data), M_d, K_d, stream);
                }
            } else {
            ...
            if (cfg.ffn_activation != FFNActivation::GEGLU) {
                ...
                geglu_quantize_q8_1(static_cast<const half*>(go.data), static_cast<const half*>(uo.data), q8,
                                    qscratch_.d8_buf, K_d, stream);
            }
            ...
            if (cfg.ffn_activation != FFNActivation::GEGLU)
                ...
                geglu(go, uo, so, stream);
            gemv_nvfp4_kpar(wd_nvfp4, static_cast<const half*>(so.data), static_cast<half*>(fo.data), M_d,
                            K_d, stream);
            ...
            dispatch_dp4a_gemv(ly.w_down.qtype, ly.w_down.data, q8, qscratch_.d8_buf, fo_ptr, M_d, K_d,
                               stream);
            Tensor fp32_h = view_tokens(fp32_hidden_, n);
            rmsnorm_fp32_accum_to_fp16_kernel<<<n, 256, 0, stream>>>(
                static_cast<const half*>(fo.data), static_cast<const half*>(ly.post_ffn_norm.data),
    ...
    if (lora_) {
        if (const LoraWeights* w = lora_->get(layer, LoraProj::DOWN)) {
            if (!has_post_ffn_norm) {
                ...
                if (!warned) {
                    warned = true;
                    QUENCH_LOG_WARN("LoRA: down_proj adapter on a post-ffn-norm arch is unsupported (v1) — "
                                 "delta skipped");
                }
            }
        }
