...
//
// Phase 3 of the architecture-refactor roadmap
...
namespace quench::pre_dequant_internal {

// net rule for the NVFP4 LM-head decode cache (gemm.nvfp4_lm_head,
// "auto"/"on"/"off", legacy true/false accepted). A parity sweep
...
//    on the Qwen3.6-35B UD-Q4_K_M hero). The dense/MoE
//    net rule's is_dense=false arm would silently void that flag on quantized
//    hybrids (−5% decode on the hero) — pass is_gdn_hybrid so auto defers
...
inline bool create_fused_weight_pair(const Tensor& w_a, const Tensor& w_b,
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(static_cast<char*>(fused_buf) + one_sz, it_b->second.data, one_sz,
