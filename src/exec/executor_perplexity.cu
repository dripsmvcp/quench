...
//    quench_perplexity, whose default config resolves to chunked prefill.
...
void GraphExecutor::for_each_lm_head_batch_(int n_rows, cudaStream_t stream, bool allow_cutlass,
    ...
    for (int c = 0; c < chunk_len; c += mb) {
        ...
        if (lm_cutlass_done) {
            ...
            // whose weight pass is shared across the rows. A
            // per-row loop would re-read the full LM head once per chunk row
            // (0.44 ms/row on Qwen3-8B Q8_0 — the dominant GGUF verify
            // term). Falls back to per-row when the multi-row scratch
...
// prewarm_verify_scratch() removes the last two per-serving allocations a
// --wrap interposer observes (A7 step 5.4). The lazy calls below remain, so a
...
double GraphExecutor::perplexity_nll(const int32_t* tokens, int n, cudaStream_t stream) {
    if (!initialized_ || n < 2) {
        QUENCH_LOG_ERROR("perplexity_nll: not initialized or n < 2 (n=%d)", n);
        return -1.0;
    }
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_tokens, static_cast<size_t>(n) * sizeof(int32_t)));
    QUENCH_CUDA_CHECK_LOG(cudaMalloc(&d_nll, static_cast<size_t>(n) * sizeof(double)));
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(d_tokens, tokens, static_cast<size_t>(n) * sizeof(int32_t),
                                       cudaMemcpyHostToDevice, stream));
    QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(d_nll, 0, static_cast<size_t>(n) * sizeof(double), stream));
    ...
    QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(h_nll_pos.data(), d_nll, static_cast<size_t>(n) * sizeof(double),
    ...
    QUENCH_LOG_INFO("perplexity_nll: n=%d  mean_nll=%.4f  PPL=%.4f", n, h_nll / (n - 1), ppl);
    return ppl;
}
