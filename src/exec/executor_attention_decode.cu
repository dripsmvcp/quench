...
// local variables in scope (including the `after_attention` label).
        ...
        if (force_cublas_decode && n == 1 && attn_scores_buf_) {
            ...
            // so this debug arm stays a faithful reference for gpt-oss.
            {
        ...
        if (cache_dtype == QType::INT4) {
            ...
            // BitDecoding TC path not supported for MXFP4_KV — always scalar.
            paged_attention_set_splitk_scratch(qscratch_.splitk, qscratch_.splitk_size);
        ...
        if (attn_sinks && cache_dtype != QType::F16) {
            ...
            if (!warned_sinks_kv) {
                ...
                QUENCH_LOG_WARN("attention sinks: only the FP16 paged decode kernels apply sinks — "
