...
// FP8 tier.
//
// Adapter that wraps the FP8 prefill path from gemm_dispatch_impl
...
static GemmDispatchResult fp8_gemm_kernel(const GemmKernelArgs& args) {
    QUENCH_CHECK(args.input != nullptr, "fp8_gemm_kernel: input is null");
    QUENCH_CHECK(args.output != nullptr, "fp8_gemm_kernel: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr, "fp8_gemm_kernel: weight_payload is null");
...
// ---------------------------------------------------------------------------
// FP8 cache-miss fallback.
//
// Mirrors the dequant→FP16 cuBLAS fallback that fires when the FP8 path is
...
// (gemm_kernel_registry.cu) — when the dispatch site
// observes a cache-miss + FP16/BF16 weight, it falls through to the FP16
// strategy lookup just below the FP8 block. This handler therefore covers
...
static GemmDispatchResult fp8_cache_miss_dequant_kernel(const GemmKernelArgs& args) {
    QUENCH_CHECK(args.input != nullptr, "fp8_cache_miss_dequant_kernel: input is null");
    QUENCH_CHECK(args.output != nullptr, "fp8_cache_miss_dequant_kernel: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr,
...
// Static registration: the (M>1, cache-hit) prefill
// strategy plus the (M>1, cache-miss dequant fallback) sibling
// under the same FP8 tier. Decode (M==1) has no FP8 strategy because
