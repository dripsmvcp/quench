...
#include "exec/gemm_scratch.h"  // mmvq_scratch_get_or_grow
...
// GGUF small-M tier.
//
// Covers the legacy mmvq + dp4a small-M GGUF branches
// (gemm_dispatch_impl at executor_kernels.cu:2216-2251), and extends
...
//
// Fused-gemv branch: when neither mmvq nor dp4a can run AND the qtype has a fused
...
// backend selection inside each handler. The fused-gemv branch keeps the same
// shape: it extends the existing Q6_K / Q8_0 handlers with a third branch instead
...
// the "input tier" matches the other registry tiers' semantics (the tier name
// often reflects what the engine observes for the weight, not its packed format).
//
// Out of scope (stays on legacy for now):
//   - Q4_1 quant_gemm_int4 (also a small-M path but neither mmvq nor dp4a).
//   - M>1 dequant+cuBLAS fallback (the large-M path).
...
// returns NoMatch / PreconditionFail.
// ---------------------------------------------------------------------------

// `mmvq_scratch_get_or_grow` lives in exec/gemm_scratch.h.
// Engine init MUST call `prewarm_mmvq_scratch` from
...
// Fused-gemv backend: runs the dequant-and-dot kernel
...
// to a real fused kernel only for Q6_K (`gemv_q6k`) and Q8_0
// (`gemv_q8_0`); other qtypes bind to `no_op_fused_gemv_launcher` and the
...
static GemmDispatchResult run_gguf_smallm(const GemmKernelArgs& args, QType qtype, bool mmvq_eligible,
    ...
    QUENCH_CHECK(args.input != nullptr, "gguf_smallm: input is null");
    QUENCH_CHECK(args.output != nullptr, "gguf_smallm: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr, "gguf_smallm: weight_payload is null");
    const Tensor& weight = *static_cast<const Tensor*>(args.weight_payload);
    QUENCH_CHECK(weight.qtype == qtype, "gguf_smallm: weight qtype mismatch");
    ...
    // that legacy evaluates at the call site (these come via GemmKernelArgs
    // from GemmContext::make instead of the
    ...
    // ModelConfig::Overrides::Gemma4::force_mmvq.
    ...
    // Fused-gemv fallback (legacy lines 2267-2276): only Q6_K and
...
// Same idea for the fused-gemv slot: only Q6_K / Q8_0 bind to real
// kernels; the other six qtypes bind to this no-op (the third branch
...
// Q8_0 has a fused-gemv fallback (`gemv_q8_0` in compute/gemm.cu).
...
// Q6_K has a fused-gemv fallback (`gemv_q6k`).
