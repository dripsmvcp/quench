...
bool GraphExecutor::prefill_routes_cutlass_nvfp4_(TensorID id, int M) const {
    ...
    if (h.primary_tier != StorageTier::CUTLASS_NVFP4)
        return false;
    //: small-M verify chunks divert to the batched NVFP4 GEMV overlay
...
void GraphExecutor::gemm_via_handle_(TensorID id, const Tensor& input,
    ...
    if (M == 1 && ctx.beta == 0.0f) {
        ...
        switch (decode) {
            ...
            case StorageTier::MXFP4:
                quench::gemv_dispatch(h, input, output, ctx.stream);
                return;
    ...
    // ---- Prefill (M>1): use prefill_tier for GEMM dispatch ----
    {
        ...
        // Spec-verify chunk: read the NVFP4 decode overlay in one
        ...
        if (ctx.spec_verify_small_m && (ctx.beta == 0.0f || ctx.beta == 1.0f) && M <= 33 &&
            ...
            if (it != ctx.wcache->nvfp4.end()) {
                // beta=1 (o/down residual add,): accumulate variant —
        ...
        //: native ST-NVFP4 verify chunks. The CUTLASS prefill block
    ...
    // return first when one does), i.e. on sm_120 where FP8 prefill is
    // disabled. Decode (M=1, handled earlier) still uses the fast NVFP4 cache.
    ...
    if (M > 1 && h.source_data != nullptr && dequant_gpu_supported(h.source_qtype) &&
        ...
        // → cuBLAS round-trip (the dominant Q8_0 prefill
        // tax). Covers beta=0 and the
        // beta=1 residual-add form; declines (shape / capture-guard) fall
        // through to the dequant fallback below.
        // M >= 2: below 64 the dequant tax dominates even harder —
        // an M=9 spec-decode verify chunk would re-dequantize the ENTIRE model
        // every step (56% of GPU time). The IMMA kernel zero-fills M-tail
        ...
        // on Qwen3-14B-Q6_K: 4.5k vs 6.6k pp512 for the
    ...
    // this, dispatch falls through to quench::gemm_dispatch → the generic gemm(),
    ...
    quench::gemm_dispatch(nullptr, h, input, output, 1.0f, ctx.beta,
