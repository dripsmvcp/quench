...
void QuantPipeline::nvfp4_decode_convert_cutlass_(const ModelConfig& cfg, const VRAMBudget& budget,
    ...
    if (wcache_->nvfp4_decode_mode == 2) {
        ...
        // trust live-free below the guarantee — the floor alone carries it
        // (no physical balloon allocation is held across init).
        if (budget.mandatory_sf_bytes > ct_budget) {
            QUENCH_LOG_INFO("CUTLASS NVFP4 cache: budget floored at guaranteed %.1f MiB "
    ...
    // sub-region) instead of a per-tensor cudaMalloc+cudaMemsetAsync. MoE
    ...
    if (ct_exhausted)
        QUENCH_LOG_INFO("CUTLASS NVFP4 cache: VRAM budget reached (%.1f / %.1f MiB)",
                     ct_total / (1024.0 * 1024.0), ct_budget / (1024.0 * 1024.0));

    int ct_count = 0;
    if (ct_total > 0) {
        void* sf_slab = nullptr;
        QUENCH_CUDA_CHECK_LOG(cudaMalloc(&sf_slab, ct_total));
        if (!sf_slab) {
            // Defensive (floor over-commit or genuine exhaustion): a null
            // slab must not be offset into by the convert passes below.
            QUENCH_LOG_WARN("CUTLASS NVFP4 cache: slab cudaMalloc(%.1f MiB) failed — "
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(sf_slab, 0, ct_total, stream));
    ...
    if (ct_count > 0) {
        ...
        QUENCH_LOG_INFO("CUTLASS sm_120 NVFP4 weight cache: %d tensors, %.2f MiB (skipped %d "
                     "IMMA-prefill weights — dead SF buffer)",
                     ct_count, ct_total / (1024.0 * 1024.0), ct_skipped_dead);
    } else if (ct_skipped_dead > 0) {
        QUENCH_LOG_INFO("CUTLASS sm_120 NVFP4 weight cache: 0 tensors (skipped %d IMMA-prefill "
...
void QuantPipeline::nvfp4_decode_convert_mxfp4_and_native_(const ModelConfig& cfg, cudaStream_t stream) {
    ...
    if (cutlass_sm120_mxfp4_available()) {
        ...
        if (has_mxfp4 && !qscratch_->mxfp4_act_sf) {
            ...
            if (max_k > 0) {
                ...
                QUENCH_LOG_INFO("Native MXFP4: allocated activation scratch (sf=%.2f MiB)",
    ...
    if (wcache_->use_mxfp4 && qscratch_->mxfp4_act_sf != nullptr && cutlass_sm120_mxfp4_available()) {
        ...
        if (mx_count > 0) {
            ...
            QUENCH_LOG_INFO("CUTLASS sm_120 MXFP4 weight cache: %d tensors, %.2f MiB", mx_count,
...
if (mx_native > 0) {
    ...
    // blocks → illegal access,). Engine::init rejects it up front.
    ...
    QUENCH_LOG_INFO("Native MXFP4 GGUF: %d tensors, %.2f MiB (direct → CUTLASS)", mx_native,
    ...
    QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(stream));
    {
        ...
        if (e != cudaSuccess)
            QUENCH_LOG_ERROR("MXFP4 unpack error: %s", cudaGetErrorString(e));
    }
    ...
    // 14) cascade we have not yet root-caused. Until that is resolved,
    // honor the fallback path.
    ...
    if (mxfp4_gemv_available) {
        QUENCH_LOG_INFO("MXFP4 GEMV: all %d weights have linear_scales, skipping FP16 fallback",
                     mx_native);
    }
    ...
    if (fp16_total > 0) {
        ...
        // quench.conf still gets gated by this oversubscribe check (matching the
        // old QUENCH_MXFP4_FP16_FALLBACK=1 semantics); the legacy =force escape
        ...
        if (oversubscribe) {
            ...
            // logits → token-0 ("!") garbage. The earlier "downstream
    ...
    if (fp16_total > 0) {
        ...
        if (ae != cudaSuccess) {
            QUENCH_LOG_ERROR("MXFP4 FP16 bulk alloc failed: %s (%.1f MiB)", cudaGetErrorString(ae),
    ...
    if (fp16_total > 0) {
        QUENCH_CUDA_CHECK_LOG(cudaStreamSynchronize(stream));
        {
            cudaError_t e = cudaGetLastError();
            if (e != cudaSuccess)
                QUENCH_LOG_ERROR("MXFP4 dequant kernel error: %s", cudaGetErrorString(e));
        }
        QUENCH_LOG_INFO("MXFP4 decode fallback: dequant → FP16 cache %.2f MiB",
                     fp16_total / (1024.0 * 1024.0));
        ...
        QUENCH_LOG_INFO("MXFP4 → FP16: replaced %d weight tensor pointers",
