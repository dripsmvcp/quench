...
// NVFP4 GEMV tier.
//
// Adapter that wraps the NVFP4 GEMV (M==1 decode) path from
...
// time and lives inside the NvFP4QuantResult), m_is_one=true. This handler
// only covers decode (M==1); the M>1 path is the NVFP4 GEMM tier
// (gemm_kernel_nvfp4_gemm.cu).
//
...
static GemmDispatchResult nvfp4_gemv_kernel(const GemmKernelArgs& args) {
    QUENCH_CHECK(args.input != nullptr, "nvfp4_gemv_kernel: input is null");
    QUENCH_CHECK(args.output != nullptr, "nvfp4_gemv_kernel: output is null");
    QUENCH_CHECK(args.weight_payload != nullptr, "nvfp4_gemv_kernel: weight_payload is null");
...
// Static registration: only the (M==1) decode GEMV
// strategy. The (M>1) NVFP4 GEMM strategy registers separately.
namespace {
