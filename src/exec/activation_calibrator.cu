...
void ActivationCalibrator::accumulate(int layer, TensorKind kind, const Tensor& input, cudaStream_t stream) {
    ...
    if (it == entries_.end()) {
        ...
        if (!e.d_sum) {
            QUENCH_LOG_WARN("calibration: allocation failed for layer %d kind %s (K=%lld)", layer,
                         tensor_kind_name(kind), static_cast<long long>(K));
            return;
        }
        QUENCH_CUDA_CHECK_LOG(cudaMemsetAsync(e.d_sum, 0, static_cast<size_t>(K) * sizeof(double), stream));
        ...
        QUENCH_LOG_WARN("calibration: layer %d kind %s changed K %lld -> %lld, ignoring", layer,
...
CalibrationStats ActivationCalibrator::snapshot(const std::string& model_id) const {
    ...
    for (const auto& [key, e] : entries_) {
        ...
        if (cudaMemcpy(host.data(), e.d_sum, host.size() * sizeof(double), cudaMemcpyDeviceToHost) !=
            cudaSuccess) {
            QUENCH_LOG_WARN("calibration: D2H copy failed for key %u", key);
            continue;
        }
        CalibrationEntry ce;
