...
bool GraphExecutor::init(const Model& model, QType compute_dtype, bool use_pdl, int max_batch_size,
    ...
    if (model.profile().is_gemma4) {
        ...
        if (cudaMalloc(&v_norm_ones_buf_, buf_bytes) == cudaSuccess) {
            ...
            QUENCH_LOG_INFO("Gemma 4: allocated V-norm ones buffer (%d halfs)", hd_max);
        }
    }
    use_pdl_ = use_pdl;
    ...
    if (has_moe_ && cfg.arch != ModelArch::GEMMA4) {
        ...
        if (max_expert_eff_ != cfg.expert_d_ff) {
            QUENCH_LOG_WARN("expert_d_ff mismatch: config=%d, actual packed tensors=%d — using %d",
    ...
    if (has_ssm_ && (has_moe_ || has_gdn_)) {
        ...
        if (max_tokens_ > capped) {
            QUENCH_LOG_INFO("executor_workspace.cu:%d: Capping max_tokens %d → %d for SSM/GDN hybrid", __LINE__,
                         max_tokens_, capped);
            max_tokens_ = capped;
        }
    }
    ...
    // the model is GDN. Reading has_gdn_ while it is still false would reserve
    // HALF the bytes the pointer carve then hands out, while
    // configure_ssm_workspace() runs later, when the flag IS set — the two
    // would disagree by exactly 2x.
    //
    // On a GDN+MoE model the MoE phase dominates the shared arena and hides
    // this; on a GDN+dense model (Qwen3.5-4B) the SSM phase is the maximum, and
    // the overrun corrupts every token past the halfway point — NaN from the
    // first recurrent layer whose block exceeds it, silently and prefill-only.
    //
    ...
    // replicates on purpose — moving this any earlier would change
    // the cap and under-reserve the arena by 2x instead.
    {
        ...
        if (gdn_idx > 0) {
            has_gdn_ = true;
            QUENCH_LOG_INFO("GDN layers: %d out of %d total", gdn_idx, cfg.n_layers);
        }
    }
    ...
    if (has_ssm_) {
        ...
        QUENCH_LOG_INFO("SSM layers: %d out of %d total", ssm_idx, cfg.n_layers);
    }
    ...
    if (use_pdl_ && pdl::is_available()) {
        ...
        QUENCH_LOG_INFO("PDL enabled on executor + compute + GEMV kernels");
    } else if (use_pdl_) {
        QUENCH_LOG_WARN("PDL requested but not available on this device/CUDA version");
        use_pdl_ = false;
    }
    ...
    if (cfg.yarn_ext_factor > 0.0f) {
        ...
        rope_yarn_corr_dims(n_dims, n_ctx_orig, cfg.rope_theta, cfg.yarn_beta_fast, cfg.yarn_beta_slow,
                            yarn_corr_dims_);
        QUENCH_LOG_INFO("YaRN corr_dims: [%.1f, %.1f] (n_dims=%d, n_ctx_orig=%d)", yarn_corr_dims_[0],
    ...
    if (!cfg.rope_short_factor.empty() && !cfg.rope_long_factor.empty()) {
        ...
        if (e1 != cudaSuccess || e2 != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to allocate LongRoPE frequency buffers: %s",
        ...
        QUENCH_CUDA_CHECK_LOG(cudaMemcpy(longrope_short_freqs_, short_freqs.data(), pairs * sizeof(float),
                                      cudaMemcpyHostToDevice));
        QUENCH_CUDA_CHECK_LOG(cudaMemcpy(longrope_long_freqs_, long_freqs.data(), pairs * sizeof(float),
                                      cudaMemcpyHostToDevice));

        QUENCH_LOG_INFO("LongRoPE: %d freq pairs, orig_max_pos=%d", pairs, longrope_orig_max_pos_);
    }
...
bool GraphExecutor::allocate_workspaces(bool experts_on_host) {
    ...
    if (!ws_.allocate_persistent_workspace(max_tokens_)) {
        QUENCH_LOG_ERROR("Persistent workspace allocation failed — cannot run inference");
        return false;
    }
    if (!ws_.allocate_shared_workspace(max_tokens_)) {
        QUENCH_LOG_ERROR("Shared workspace allocation failed — cannot run inference");
        return false;
    }
    ...
    if (mla_scratch_unservable_) {
        ...
        QUENCH_LOG_ERROR("MLA QKV scratch could not be served from the engine arena — refusing to "
...
size_t Workspace::workspace_estimate(bool include_attn_scores) const {
    ...
    // skip the buffer entirely, so charging it here would hold up to
    // 256 MiB of phantom headroom.
...
bool Workspace::allocate_persistent_workspace(int max_tokens) {
    ...
    if (!persistent_workspace_) {
        QUENCH_LOG_ERROR("Failed to allocate persistent workspace (%.1f MiB)", total / (1024.0 * 1024.0));
        return false;
    }
    ...
    QUENCH_LOG_INFO("Persistent workspace: %.2f MiB (hidden+residual+norm+logits)", total / (1024.0 * 1024.0));
    ...
    if (cfg.norm_placement == NormPlacement::POST_NORM) {
        ...
        if (e2 == cudaSuccess) {
            ...
            QUENCH_LOG_INFO("FP32 residual accumulator: %.2f MiB (post-norm architecture)",
                         fp32_sz / (1024.0 * 1024.0));
        } else {
            QUENCH_LOG_WARN("Failed to allocate FP32 accumulator (%zu bytes): %s — falling back to FP16",
...
bool Workspace::allocate_shared_workspace(int max_tokens) {
    ...
    if (!shared_workspace_) {
        ...
        QUENCH_LOG_WARN("Shared workspace: allocator rejected (%.1f MiB), trying raw cudaMalloc",
                     max_shared / (1024.0 * 1024.0));
        cudaError_t err = cudaMalloc(&shared_workspace_, max_shared);
        if (err != cudaSuccess) {
            QUENCH_LOG_ERROR("Failed to allocate shared workspace (%.1f MiB)", max_shared / (1024.0 * 1024.0));
            return false;
        }
    }
