...
// f32 accumulate) — the short-seq e4m3 quality cliff (/) does not
...
static bool try_fa2_fp16qk_prefill(const DispatchPolicy& rcfg, const Tensor& q, const Tensor& k,
    ...
    if (rcfg.attention.fa2_fp16qk == "never")
        return false;
    // hd=256: stage-1 FA2 port (attention.fa2_hd256, default on.
    ...
    // root-caused (issue) — first chunks (q_offset == 0, the original use case)
    // keep the fast path.
    // Chunk continuations (q_offset > 0) were declined here as a
    // mitigation for catastrophic NLL on the Llama family. Root cause
    // was NOT this kernel: the pinned prefill staging
