...
static bool upload_qtype_mxfp4_(Tensor& weight, QType qtype, QType compute_dtype,
    ...
    if (weight.mxfp4_layout_v2) {
        // GGML type-39 blocks differ from quench's legacy type 31 in TWO ways,
        ...
        // order (element 2i = low / 2i+1 = high of byte i) that every quench
        ...
        // (, Qwen3.5-4B-mxfp4). Normalize to linear order once, here.
    ...
    QUENCH_LOG_DEBUG("  MXFP4 upload: [%lld, %lld] %.2f MiB (data+scales split)", (long long)N, (long long)K,
...
static bool upload_qtype_q4_0_(Tensor& weight, QType qtype, QType compute_dtype,
                                 ...
                                 bool raw_quant, float weight_offset) {
    if (weight.ndim < 2) {
        QUENCH_LOG_WARN("Q4_0 weight has < 2 dims, skipping upload");
        return false;
    }
    ...
    if (!d_scales) {
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_nibbles, stream));
        return false;
    }
...
static bool upload_qtype_q8_0_(Tensor& weight, QType qtype, QType compute_dtype,
                                 ...
                                 bool raw_quant, float weight_offset) {
    if (weight.ndim < 2) {
        QUENCH_LOG_WARN("Q8_0 weight has < 2 dims, skipping upload");
        return false;
    }
...
static bool upload_qtype_q6_k_(Tensor& weight, QType qtype, QType compute_dtype,
                                 ...
                                 bool raw_quant, float weight_offset) {
    if (weight.ndim < 2) {
        QUENCH_LOG_WARN("Q6_K weight has < 2 dims, skipping upload");
        return false;
    }
...
static bool upload_qtype_general_quant_(Tensor& weight, QType qtype, QType compute_dtype,
    ...
    if (raw_quant) {
        ...
        if (cpy_err != cudaSuccess) {
            QUENCH_LOG_ERROR("h2d_copy failed for qtype=%u [%ldx%ld] %zu bytes: %s", (unsigned)qtype,
                          (long)N, (long)K, raw_bytes, cudaGetErrorString(cpy_err));
        }
        gpu_allocs.push_back(d_data);
        QUENCH_LOG_DEBUG("Upload raw qtype=%u [%ldx%ld] %zu bytes -> GPU %p", (unsigned)qtype, (long)N,
        ...
        if (!d_fp16) {
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_raw, stream));
            return false;
        }
...
static bool upload_qtype_raw_fallback_(Tensor& weight, QType qtype, cudaStream_t stream,
    ...
    size_t bytes = weight.nbytes();
    if (bytes == 0) {
        QUENCH_LOG_WARN("Empty raw weight for qtype %u, skipping", std::to_underlying(qtype));
        return false;
    }
    void* d_data = nullptr;
    checked_cuda_malloc(&d_data, bytes, stream);
...
static bool upload_weight(Tensor& weight, QType qtype, QType compute_dtype, cudaStream_t stream,
    ...
    if (!upload_weight_dispatch_(weight, qtype, compute_dtype, stream, gpu_allocs, raw_quant,
                                 weight_offset))
        return false;
...
#define UPLOAD_OR_FAIL(tensor, qtype, msg, layer_idx, ctx)                                          \
    do {                                                                                            \
        if (!upload_weight((tensor), (qtype), (ctx).compute_dtype, (ctx).stream, (ctx).gpu_allocs, \
            ...
            QUENCH_LOG_ERROR("Failed to upload " msg " for layer %d", (layer_idx));                    \
            return false;                                                                           \
        }                                                                                           \
    } while (0)
...
static bool upload_embeddings_and_output(Tensor& tok_emb, Tensor& out_norm, Tensor& out_proj,
    ...
    if (tok_emb.data && !tok_emb.on_device) {
        ...
        if (!upload_unquantized_weight(tok_emb, tok_emb.qtype, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs,
                                       emb_raw, "tok_emb")) {
            QUENCH_LOG_ERROR("Failed to upload token embedding");
            return false;
        }
    }
    ...
    if (out_norm.data && !out_norm.on_device) {
        if (!upload_weight(out_norm, out_norm.qtype, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs,
            ...
            QUENCH_LOG_ERROR("Failed to upload output norm");
            return false;
        }
    }
    ...
    if (out_proj.data && !out_proj.on_device) {
        ...
        if (actually_tied && tok_emb.on_device) {
            out_proj = tok_emb;
            QUENCH_LOG_INFO("Output projection shares GPU data with token embedding (weight tying)");
        } else {
            ...
            if (!upload_unquantized_weight(out_proj, out_proj.qtype, ctx.compute_dtype, ctx.stream,
                ...
                QUENCH_LOG_ERROR("Failed to upload output projection");
                return false;
            }
        }
    }
...
static bool upload_mtp_weights(MtpHead& head, const UploadCtx& ctx) {
    ...
    auto up_norm = [&](Tensor& t, const char* name) -> bool {
        ...
        if (!upload_weight(t, t.qtype, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs,
            ...
            QUENCH_LOG_ERROR("Failed to upload MTP norm: %s", name);
            return false;
        }
        return true;
    };
    auto up = [&](Tensor& t, const char* name) -> bool {
        ...
        if (!upload_unquantized_weight(t, t.qtype, ctx.compute_dtype,
            ...
            QUENCH_LOG_ERROR("Failed to upload MTP tensor: %s", name);
            return false;
        }
        return true;
    };
...
static bool upload_gptq_weight(const TransformerLayer::GPTQWeight& gptq, Tensor& output, cudaStream_t stream,
    ...
    if (!gptq.qweight.data || !gptq.scales.data)
        return false;
    if (gptq.bits != 4) {
        QUENCH_LOG_ERROR("GPTQ: only 4-bit supported (got %d)", gptq.bits);
        return false;
    }
    ...
    if (checked_cuda_malloc(reinterpret_cast<void**>(&d_qweight), qw_bytes, stream) != cudaSuccess || !d_qweight) {
        QUENCH_LOG_ERROR("GPTQ: failed to allocate qweight (%zu bytes)", qw_bytes);
        return false;
    }
    ...
    if (gptq.qzeros.data) {
        ...
        if (checked_cuda_malloc(reinterpret_cast<void**>(&d_qzeros), qz_bytes, stream) != cudaSuccess || !d_qzeros) {
            QUENCH_LOG_ERROR("GPTQ: failed to allocate qzeros");
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_qweight, stream));
            return false;
        }
    ...
    if (checked_cuda_malloc(reinterpret_cast<void**>(&d_scales), sc_bytes, stream) != cudaSuccess || !d_scales) {
        QUENCH_LOG_ERROR("GPTQ: failed to allocate scales");
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_qweight, stream));
        if (d_qzeros)
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_qzeros, stream));
        return false;
    }
    ...
    if (gptq.g_idx.data) {
        ...
        if (checked_cuda_malloc(reinterpret_cast<void**>(&d_g_idx), gi_bytes, stream) != cudaSuccess || !d_g_idx) {
            QUENCH_LOG_WARN("GPTQ: failed to allocate g_idx, falling back to sequential groups");
        } else {
    ...
    if (checked_cuda_malloc(reinterpret_cast<void**>(&d_out), out_bytes, stream) != cudaSuccess || !d_out) {
        QUENCH_LOG_ERROR("GPTQ: failed to allocate output (%zu bytes)", out_bytes);
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_qweight, stream));
        if (d_qzeros)
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_qzeros, stream));
        QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_scales, stream));
        if (d_g_idx)
            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(d_g_idx, stream));
        return false;
    }
...
static bool upload_layer_attention_weights(TransformerLayer& L, int i, const UploadCtx& ctx) {
    ...
    for (auto& [w, gptq, name] : attn_gptq) {
        if (!w.on_device && gptq.qweight.data) {
            if (!upload_gptq_weight(gptq, w, ctx.stream, ctx.gpu_allocs)) {
                QUENCH_LOG_ERROR("Failed to dequant GPTQ %s for layer %d", name, i);
                return false;
            }
            QUENCH_LOG_DEBUG("GPTQ dequant %s layer %d -> [%lld, %lld] FP16", name, i, (long long)w.shape[0],
                          (long long)w.shape[1]);
        }
    }
    ...
    if (L.attn_norm.data && !L.attn_norm.on_device) {
        if (!upload_weight(L.attn_norm, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs, true,
            ...
            QUENCH_LOG_ERROR("Failed to upload attn_norm for layer %d", i);
            return false;
        }
    }
    ...
    if (L.attn_q_norm.data && !L.attn_q_norm.on_device) {
        if (!upload_weight(L.attn_q_norm, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs, true,
            ...
            QUENCH_LOG_ERROR("Failed to upload attn_q_norm for layer %d", i);
            return false;
        }
    }
    if (L.attn_k_norm.data && !L.attn_k_norm.on_device) {
        if (!upload_weight(L.attn_k_norm, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs, true,
            ...
            QUENCH_LOG_ERROR("Failed to upload attn_k_norm for layer %d", i);
            return false;
        }
    }
    ...
    if (L.kv_a_layernorm.data && !L.kv_a_layernorm.on_device) {
        if (!upload_weight(L.kv_a_layernorm, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs,
            ...
            QUENCH_LOG_ERROR("Failed to upload kv_a_layernorm for layer %d", i);
            return false;
        }
    }
    ...
    for (const auto& [bias, name] : biases) {
        if (bias->data && !bias->on_device) {
            if (!upload_unquantized_weight(*bias, QType::NONE, ctx.compute_dtype, ctx.stream,
                ...
                QUENCH_LOG_ERROR("Failed to upload attention bias for layer %d", i);
                return false;
            }
        }
    }
    ...
    for (const auto& [norm, name] : post_norms) {
        if (norm->data && !norm->on_device) {
            if (!upload_unquantized_weight(*norm, QType::NONE, ctx.compute_dtype, ctx.stream,
                                           ctx.gpu_allocs, true, name, i)) {
                QUENCH_LOG_ERROR("Failed to upload post-layer norm for layer %d", i);
                return false;
            }
        }
    }
    ...
    if (L.rope_freqs.data && !L.rope_freqs.on_device && L.rope_freqs.qtype == QType::F32) {
        QUENCH_LOG_INFO("Layer %d: uploading rope_freqs as raw FP32 (%lld elements)", i,
                     (long long)L.rope_freqs.numel());
        size_t bytes = L.rope_freqs.nbytes();
        void* d_data = nullptr;
        QUENCH_CUDA_CHECK_LOG(cudaMallocAsync(&d_data, bytes, ctx.stream));
        if (!d_data) {
            QUENCH_LOG_ERROR("Failed to allocate GPU memory for rope_freqs layer %d", i);
            return false;
        }
...
static bool upload_layer_ffn_weights(TransformerLayer& L, int i, const UploadCtx& ctx) {
    ...
    if (L.ffn_norm.data && !L.ffn_norm.on_device) {
        if (!upload_weight(L.ffn_norm, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs, true,
            ...
            QUENCH_LOG_ERROR("Failed to upload ffn_norm for layer %d", i);
            return false;
        }
    }
...
static bool upload_layer_ssm_weights(TransformerLayer& L, int i, const UploadCtx& ctx) {
    ...
    for (const auto& [t, name] : ssm_fp16) {
        if (t->data && !t->on_device) {
            if (!upload_unquantized_weight(*t, QType::NONE, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs,
                                           true, name, i)) {
                QUENCH_LOG_ERROR("Failed to upload SSM tensor for layer %d", i);
                return false;
            }
        }
    }
    ...
    // real FP32 values.
    // ssm_a needs an extra HF-vs-GGUF transform: quench's GDN scan kernel computes
    ...
    for (Tensor* t : {&L.ssm_a, &L.ssm_d, &L.ssm_dt_b}) {
        ...
        if (!d_data) {
            QUENCH_LOG_ERROR("Failed to allocate GPU memory for SSM F32 tensor in layer %d", i);
            return false;
        }
        ...
        if (t->qtype == QType::F32 || t->qtype == QType::NONE) {
            ...
            QUENCH_LOG_ERROR("SSM scalar tensor has unexpected qtype %u (layer %d)",
        ...
        // model emitted one token forever.
        //
    ...
    if (four_way_ok) {
        ...
        if (cudaMallocAsync(&d_packed, total_bytes, ctx.stream) == cudaSuccess && d_packed) {
            ...
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(base + bytes_a + bytes_b, c.data, bytes_c,
                                                cudaMemcpyDeviceToDevice, ctx.stream));
            QUENCH_CUDA_CHECK_LOG(cudaMemcpyAsync(base + bytes_a + bytes_b + bytes_c, d.data, bytes_c,
                                                cudaMemcpyDeviceToDevice, ctx.stream));
            ...
            QUENCH_LOG_DEBUG("  layer %d: gdn_input_packed [%lld, %lld] %.2f MiB", i, (long long)total_out,
    ...
    if (!L.gdn_input_packed.data && c.data && c.on_device && d.data && d.on_device && c.ndim == 2 &&
        ...
        if (cudaMallocAsync(&d_packed, total_bytes, ctx.stream) == cudaSuccess && d_packed) {
            ctx.gpu_allocs.push_back(d_packed);
            QUENCH_CUDA_CHECK_LOG(cudaMemcpy2DAsync(d_packed, 2 * row_bytes, c.data, row_bytes, row_bytes,
                                                 d_model, cudaMemcpyDeviceToDevice, ctx.stream));
            QUENCH_CUDA_CHECK_LOG(cudaMemcpy2DAsync(static_cast<char*>(d_packed) + row_bytes, 2 * row_bytes,
            ...
            QUENCH_LOG_DEBUG("  layer %d: gdn_alpha_beta_packed [%lld, %lld] %.2f KiB (Step-1 fallback)", i,
...
static void decide_expert_layer_placement_(const std::vector<size_t>& layer_expert_bytes,
    ...
    if (budget >= total_expert_bytes && force_host_n == 0) {
        ...
        QUENCH_LOG_INFO(
            "Expert weights (QUENCH_FORCE_HOST_EXPERTS=%d): uploading %d/%zu MoE layers, %d forced to host",
    ...
    for (int i = 0; i < n_layers; ++i) {
        ...
        auto upload_packed_experts = [&](Tensor& packed, QType qtype, std::vector<Tensor>& expert_vec,
            ...
            if (ctx.is_gpt_oss && qtype == QType::MXFP4) {
                QUENCH_LOG_DEBUG("  %s: gpt-oss MXFP4 experts kept host-resident for NVFP4 convert", name);
                return true;
            }
            ...
            if (dequant_gpu_supported(qtype)) {
                ...
                if (experts_upload_layer[i]) {
                    ...
                    if (key && g_warm &&
                        g_warm->try_restore(key, packed, ctx.stream, ctx.gpu_allocs, kWarmOps,
                                            g_upload_log)) {
                        QUENCH_LOG_DEBUG("  %s: %d experts restored from snapshot (%.2f MiB)", name,
                    ...
                    if (err == cudaSuccess) {
                        ...
                        if (cpy_err != cudaSuccess) {
                            QUENCH_LOG_ERROR("  %s: h2d_copy failed: %s", name, cudaGetErrorString(cpy_err));
                            QUENCH_CUDA_CHECK_LOG(cudaFreeAsync(gpu_ptr, ctx.stream));
                            return false;
                        }
                        ...
                        QUENCH_LOG_DEBUG("  %s: %d experts uploaded to GPU (%.2f MiB)", name, n_experts,
                                      total_raw / (1024.0 * 1024.0));
                        return true;
                    }
                    // cudaMalloc failed — fall through to host path
                    QUENCH_LOG_WARN("  %s: cudaMalloc failed for %.2f MiB, falling back to host", name,
                                 total_raw / (1024.0 * 1024.0));
                }
                ...
                if (is_wsl2()) {
                    ...
                    if (!pinned_buf.empty()) {
                        ...
                        QUENCH_LOG_INFO("  %s: WSL2 pinned copy (%.2f MiB, DMA-ready)", name,
                                     total_raw / (1024.0 * 1024.0));
                    } else {
                    ...
                    if (!reg.empty()) {
                        ...
                        QUENCH_LOG_DEBUG("  %s: %d experts, raw %s pinned on host (%.2f MiB)", name, n_experts,
            ...
            if (!upload_weight(flat, qtype, ctx.compute_dtype, ctx.stream, ctx.gpu_allocs, true, 0.0f,
                               name, i)) {
                QUENCH_LOG_ERROR("Failed to upload packed %s for layer %d", name, i);
                return false;
            }
        ...
        if (!experts_upload_layer[i] && L.expert_gate_packed.data && !L.expert_gate_packed.on_device &&
            ...
            if (gate_pin.empty() || up_pin.empty()) {
                ...
                QUENCH_LOG_ERROR("Gemma 4: host split pinned alloc failed (layer %d, %.1f MiB each)", i,
            ...
            QUENCH_LOG_INFO("Gemma 4: host-split fused gate_up_exps layer %d (n_ff_exp=%ld, %.1f MiB each)", i,
        ...
        if (L.expert_gate_packed.data && L.expert_gate_packed.on_device &&
            ...
            if (e2 != cudaSuccess) {
                QUENCH_LOG_ERROR("Gemma 4: cudaMalloc failed for fused expert split (layer %d, %.1f MiB)", i,
                              half_raw / (1024.0 * 1024.0));
                return false;
            }
            ...
            if (cp != cudaSuccess) {
                QUENCH_LOG_ERROR("Gemma 4: cudaMemcpy2DAsync failed (layer %d): %s", i, cudaGetErrorString(cp));
            ...
            for (int64_t e = 1; e < n_exp; ++e) {  // e=0 already at the right offset
                cudaError_t cp_e = cudaMemcpyAsync(const_cast<char*>(src_base) + e * dst_pitch,
                                                   ...
                                                   cudaMemcpyDeviceToDevice, ctx.stream);
                if (cp_e != cudaSuccess) {
                    QUENCH_LOG_ERROR("Gemma 4: gate compact memcpy failed (layer %d, expert %ld): %s", i,
            ...
            QUENCH_LOG_INFO("Gemma 4: split fused gate_up_exps layer %d (n_ff_exp=%ld, %.1f MiB each)", i,
                         (long)half_rows, half_raw / (1024.0 * 1024.0));
        }
        ...
        if (experts_upload_layer[i]) {
            ...
            for (size_t e = 0; e < L.expert_w_gate.size(); ++e) {
                ...
                if (!upload_weight(L.expert_w_gate[e], L.expert_gate_packed.qtype, ctx.compute_dtype,
                    ...
                    QUENCH_LOG_ERROR("Failed to upload expert_w_gate[%zu] for layer %d", e, i);
                    return false;
                }
            }
            for (size_t e = 0; e < L.expert_w_up.size(); ++e) {
                ...
                if (!upload_weight(L.expert_w_up[e], L.expert_up_packed.qtype, ctx.compute_dtype, ctx.stream,
                    ...
                    QUENCH_LOG_ERROR("Failed to upload expert_w_up[%zu] for layer %d", e, i);
                    return false;
                }
            }
            for (size_t e = 0; e < L.expert_w_down.size(); ++e) {
                ...
                if (!upload_weight(L.expert_w_down[e], L.expert_down_packed.qtype, ctx.compute_dtype,
                                   ctx.stream, ctx.gpu_allocs, true, 0.0f, ename, i)) {
                    QUENCH_LOG_ERROR("Failed to upload expert_w_down[%zu] for layer %d", e, i);
                    return false;
                }
            }
        } else {
...
bool Model::upload_weights_gpu(QType compute_dtype, cudaStream_t stream, size_t expert_reserve_bytes,
    ...
    if (gpu_weights_ready_) {
        QUENCH_LOG_WARN("Weights already uploaded to GPU");
        return true;
    }

    QUENCH_LOG_INFO("Uploading model weights to GPU (%d layers)...", n_layers());
    ...
    if (WeightSnapshot* warm = weight_snapshot_take_armed()) {
        if (warm->matches(*this)) {
            g_warm = warm;
            QUENCH_LOG_INFO("Warm resume: weight snapshot armed (%.2f GiB host)",
                         warm->total_bytes() / (1024.0 * 1024.0 * 1024.0));
        } else {
            QUENCH_LOG_WARN("Armed weight snapshot does not match this model (arch/layers) — ignoring");
        }
    }
    ...
    if (staging_guard.stager.init()) {
        ...
        QUENCH_LOG_INFO("Pinned staging enabled (%dx %.0f MiB ring)", PinnedStager::kRing,
                     PinnedStager::kChunkSize / (1024.0 * 1024.0));
    } else {
        QUENCH_LOG_WARN("Pinned staging alloc failed, using default H2D path");
    }
    ...
    // Cache VRAM state to avoid per-tensor cudaMemGetInfo calls
    {
        ...
        g_cached_free_mem = free_mem;
        g_total_allocated = 0;
        QUENCH_LOG_DEBUG("VRAM at upload start: %.2f GiB free / %.2f GiB total",
    ...
    // --- Encoder embedder extras (, nomic-bert) ---
    for (auto* t : {&tok_emb_norm_, &tok_emb_norm_bias_, &token_types_}) {
        if (t->data && !t->on_device) {
            if (!upload_unquantized_weight(*t, QType::NONE, ctx.compute_dtype, ctx.stream,
                                           ctx.gpu_allocs)) {
                QUENCH_LOG_ERROR("Failed to upload encoder embedding norm/type tensor");
                return false;
            }
        }
    }
    ...
    for (int i = 0; i < n_layers(); ++i) {
        ...
        QUENCH_LOG_DEBUG("Layer %d/%d non-expert weights uploaded", i + 1, n_layers());
    }
    ...
    if (config_.is_nvfp4_prequant) {
        ...
        const bool audit = quench::process_diag_audit_nvfp4_scales();
        ...
        // (pre_dequant_phase3_nvfp4_decode.cu, the copy/free dance). A
        ...
        // SfAtom (Phase-3b) and the fused-projection split are unaffected.
        {
            ...
            if (moe_scale_slabs > 0)
                QUENCH_LOG_INFO("NVFP4 MoE expert micro-scales: %d contiguous (layer,proj) slabs "
        ...
        if (audit && ws2_count > 0) {
            ...
            for (auto& [n, v] : ws2_samples) {
                QUENCH_LOG_INFO("  sample: %s = %.6g", n.c_str(), v);
            }
        }
        if (audit) {
            if (is_count > 0) {
        ...
        if (scale_count > 0)
            QUENCH_LOG_INFO("NVFP4 prequant: uploaded %d scale tensors to GPU", scale_count);
    }
    ...
    if (mtp_.has_value() && mtp_->loaded) {
        ...
        if (upload_mtp_weights(*mtp_, ctx)) {
            QUENCH_LOG_INFO("MTP head: uploaded to GPU (%zu allocations, %.2f GiB BF16→FP16)",
                         gpu_allocations_.size() - allocs_before,
                         static_cast<double>(mtp_->info.file_bytes) /
                             (1024.0 * 1024.0 * 1024.0));
        } else {
            QUENCH_LOG_WARN("MTP head: GPU upload failed — spec-decode disabled");
            mtp_->loaded = false;
        }
    }
    ...
    if (g_warm)
        QUENCH_LOG_INFO("Warm restore: %d uploads served from %s", g_warm->hits(),
    ...
    QUENCH_LOG_INFO("All model weights uploaded to GPU (%zu allocations)", gpu_allocations_.size());
    return true;
}
