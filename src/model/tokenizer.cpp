...
// instead of the symbol run and diverged from canonical segmentation.
...
// NLL on matched text — and every production prompt containing code
...
bool Tokenizer::load(const std::string& path) {
    ...
    if (!parser.ok() || root.type != JType::OBJECT) {
        QUENCH_LOG_WARN("failed to parse tokenizer.json: %s", path.c_str());
        return false;
    }
    ...
    if (!model || model->type != JType::OBJECT) {
        QUENCH_LOG_WARN("tokenizer.json missing 'model' object");
        return false;
    }
    ...
    if (vocab && vocab->type == JType::OBJECT) {
        ...
        QUENCH_LOG_INFO("tokenizer.json: loaded %zu vocab entries (type=%s)", vocab->obj.size(),
                     model_type.c_str());
    }
    ...
    if (merges && merges->type == JType::ARRAY) {
        ...
        QUENCH_LOG_INFO("tokenizer.json: loaded %zu merges", merge_strs.size());
    }
    ...
    if (added && added->type == JType::ARRAY) {
        ...
        for (const auto& tok : added->arr) {
            ...
            // only governs decode-skipping / add_special_tokens). Keying
            // atomic matching on `special` alone would BPE-split Qwen3's
            // non-special
            // markers — `<think>`/`</think>` (151667/151668), `<tool_call>`,
            // `<|fim_*|>` — into "<","think",">" pieces. That breaks
            // the `<think>`-as-stop-token guard and no-think suppression (the
            // closed `<think></think>` prompt block is just text the model
            // re-opens) and mis-tokenises tool-call markers. Promote them to
    ...
    if (model_type == "Unigram") {
        ...
        if (uni_vocab && uni_vocab->type == JType::ARRAY) {
            ...
            QUENCH_LOG_INFO("tokenizer.json: loaded %zu Unigram vocab entries", uni_vocab->arr.size());
        }
    }

    QUENCH_LOG_INFO("tokenizer.json: type=%s vocab_size=%d bos=%d eos=%d add_prefix=%s", type_.c_str(),
...
void Tokenizer::build_special_pieces() {
    ...
    for (size_t id = 0; id < token_types_.size() && id < vocab_.size(); ++id) {
        ...
        // body can never reproduce them ("▁▁" is not in the vocab), so quench
        ...
        //. llama.cpp matches these via its user-defined trie.
    ...
    QUENCH_LOG_INFO("Tokenizer: %zu special pieces cached for pre-split", special_pieces_.size());
}
...
std::vector<int32_t> Tokenizer::encode_gpt2(const std::string& text) const {
    ...
    for (const auto& piece : pieces) {
        ...
        if (pre_tokenizer_ == "llama3" || pre_tokenizer_ == "llama-v3" || pre_tokenizer_ == "llama-bpe") {
            ...
            // merges ("->", "():") impossible. "qwen35" (Qwen3.5/3.6
            // GGUFs) differs from qwen2 only by adding \p{M} to the letter
            // run, which is_letter_at already treats as letters — falling
            // through to gpt2 instead over-split symbol runs (+13% tokens
            // measured on a Qwen3.6-35B corpus).
            ...
            // digit triples, slash-aware symbol runs.
