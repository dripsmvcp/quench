...
private:
    ...
    void expect(TokenType t) {
        if (!check(t)) {
            QUENCH_LOG_WARN("jinja: expected token type %d, got %d ('%s')", std::to_underlying(t),
...
private:
    ...
    Value eval_filter(const FilterExpr& f) {
        ...
        if (f.name == "selectattr" || f.name == "rejectattr" || f.name == "map" || f.name == "select" ||
            ...
            QUENCH_LOG_WARN("jinja: unsupported filter '%s'", f.name.c_str());
            return val;
        }

        QUENCH_LOG_WARN("jinja: unknown filter '%s'", f.name.c_str());
        return val;
    }
    ...
    Value eval_call(const CallExpr& call) {
        ...
        if (auto* var = dynamic_cast<const VariableExpr*>(call.callee.get())) {
            ...
            if (var->name == "raise_exception") {
                ...
                QUENCH_LOG_WARN("jinja: raise_exception: %s", msg.c_str());
                return Value();
            }
            if (var->name == "dict") {
                auto obj = Value::make_object();
    ...
    Value eval_method(const MethodExpr& m) {
        ...
        QUENCH_LOG_WARN("jinja: unknown method '%s'", m.method.c_str());
        return Value();
    }
