...
std::string resolve_model_path(const std::string& model_id, const std::string& revision) {
    ...
    // suggesting `git clone https://huggingface.co//models/...`.
    ...
    if (!looks_like_hf_repo) {
        QUENCH_LOG_ERROR("Model path does not exist: %s", model_id.c_str());
        return "";
    }
    ...
    if (!cache_dir.empty()) {
        ...
        if (fs::is_directory(model_cache)) {
            ...
            if (fs::is_directory(snapshots)) {
                ...
                if (!latest.empty()) {
                    QUENCH_LOG_INFO("Found cached model: %s -> %s", model_id.c_str(), latest.c_str());
                    return latest;
                }
            }
        }
    ...
    // 4. No fetcher: quench's host policy keeps Python tooling off the host and
    ...
    QUENCH_LOG_ERROR("  git clone https://huggingface.co/%s <local-dir>", model_id.c_str());
    QUENCH_LOG_ERROR("  (or use the HF hub CLI on another machine and copy the cache)");
    return "";
}
...
std::string resolve_model_gguf(const std::string& model_id, const std::string& revision) {
    ...
    if (fs::is_directory(path)) {
        ...
        if (gguf.empty()) {
            QUENCH_LOG_ERROR("No .gguf file found in directory: %s", path.c_str());
            return "";
        }
        QUENCH_LOG_INFO("Resolved %s to: %s", model_id.c_str(), gguf.c_str());
        return gguf;
    }

    QUENCH_LOG_ERROR("Path is neither a file nor a directory: %s", path.c_str());
    return "";
}
...
std::string resolve_model_auto(const std::string& model_id, QuenchModelFormat& out_format,
    ...
    if (fs::is_regular_file(path)) {
        if (path.size() >= 5 && path.substr(path.size() - 5) == ".gguf") {
            out_format = QUENCH_FORMAT_GGUF;
            return path;
        }
        QUENCH_LOG_ERROR("Unsupported file extension: %s", path.c_str());
        return "";
    }

    if (fs::is_directory(path)) {
        if (is_safetensors_dir(path)) {
            QUENCH_LOG_INFO("Detected SafeTensors directory: %s", path.c_str());
            out_format = QUENCH_FORMAT_SAFETENSORS;
            return path;
        }

        std::string gguf = find_gguf_in_dir(path);
        if (!gguf.empty()) {
            QUENCH_LOG_INFO("Resolved %s to GGUF: %s", model_id.c_str(), gguf.c_str());
            out_format = QUENCH_FORMAT_GGUF;
            return gguf;
        }

        QUENCH_LOG_ERROR("Directory has neither .safetensors nor .gguf: %s", path.c_str());
        return "";
    }
