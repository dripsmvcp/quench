...
namespace {
// A prefix fprintf, a loop of partial writes and a
// trailing newline would be three calls the logging framework cannot
// express. Build the line, then log it once.
...
std::vector<int32_t> ChatTemplate::apply_chatml(const Tokenizer& tok, const std::vector<ChatMessage>& msgs,
    ...
    if (quench::process_diag_debug_template()) {
        QUENCH_LOG_DEBUG("[DEBUG_TPL] chatml %zu tokens:%s", tokens.size(), join_token_ids(tokens).c_str());
    }
...
std::vector<int32_t> ChatTemplate::apply_gemma(const Tokenizer& tok,
    ...
    if (quench::process_diag_debug_template()) {
        QUENCH_LOG_DEBUG("[DEBUG_TPL] %zu tokens:%s", tokens.size(), join_token_ids(tokens).c_str());
    }
...
std::vector<int32_t> ChatTemplate::apply_phi(const Tokenizer& tok,
    ...
    // Debug: print template token IDs
    if (quench::process_diag_debug_template()) {
        QUENCH_LOG_DEBUG("[DEBUG_TPL] phi %zu tokens:%s", tokens.size(), join_token_ids(tokens).c_str());
        ...
        QUENCH_LOG_DEBUG("[DEBUG_TPL] decoded: %s", decoded.c_str());
    }
...
std::vector<int32_t> ChatTemplate::apply_harmony(const Tokenizer& tok,
    ...
    if (quench::process_diag_debug_template()) {
        QUENCH_LOG_DEBUG("[DEBUG_TPL] harmony %zu tokens:%s", tokens.size(), join_token_ids(tokens).c_str());
    }
...
std::vector<int32_t> ChatTemplate::apply_with_image(const Tokenizer& tok,
    ...
    // so the model answered fluently that it could not see an image.
    //
