...
//   GGUF is supported for compatibility but is not the active dev surface.
//   Priority is NVFP4 + SafeTensors (Qwen3.6-35B-A3B-NVFP4, Qwen3-8B-NVFP4-cortecs,
//   Gemma-4-26B-A4B-it-NVFP4, etc.) — that's where the perf work lives.
//
//   For GGUF bugs, ship the cleanup fix (load errors, missing pointer replaces,
//   resource cleanup cascades) and move on. Don't sink time chasing
//   residual quality issues — especially on community MXFP4 quants, which have
//   a track record of being subtly broken. If the next
...
std::unique_ptr<Model> load_gguf(const std::string& path) {
    ...
    if (fd < 0) {
        QUENCH_LOG_ERROR("Failed to open GGUF file: %s", path.c_str());
        return nullptr;
    }

    struct stat st {};
    if (fstat(fd, &st) != 0) {
        QUENCH_LOG_ERROR("Failed to stat GGUF file: %s", path.c_str());
        close(fd);
        return nullptr;
    }
    ...
    if (mmap_base == MAP_FAILED) {
        QUENCH_LOG_ERROR("Failed to mmap GGUF file: %s (size=%zu)", path.c_str(), file_size);
        return nullptr;
    }
    ...
    if (magic != GGUF_MAGIC) {
        QUENCH_LOG_ERROR("Invalid GGUF magic: 0x%08x", magic);
    ...
    if (version < 2 || version > 3) {
        QUENCH_LOG_ERROR("Unsupported GGUF version: %u (expected 2 or 3)", version);
        munmap(mmap_base, file_size);
        return nullptr;
    }
    ...
    if (reader.failed()) {
        QUENCH_LOG_ERROR("GGUF header truncated: %s", path.c_str());
        munmap(mmap_base, file_size);
        return nullptr;
    }

    QUENCH_LOG_INFO("GGUF v%u: %lu tensors, %lu metadata KVs", version, (unsigned long)tensor_count,
    ...
    if (reader.failed()) {
        QUENCH_LOG_ERROR("GGUF metadata truncated: %s", path.c_str());
        munmap(mmap_base, file_size);
        return nullptr;
    }
    ...
    if (reader.failed()) {
        QUENCH_LOG_ERROR("GGUF tensor info truncated: %s", path.c_str());
        munmap(mmap_base, file_size);
        return nullptr;
    }
    ...
    QUENCH_LOG_DEBUG("Tensor data starts at offset %zu (alignment=%zu)", tensor_data_start, alignment);
    ...
    if (split_count > 1) {
        QUENCH_LOG_INFO("Split GGUF: %d shards", split_count);
        ...
        if (dash_pos == std::string::npos) {
            ...
            QUENCH_LOG_WARN("Split GGUF: cannot derive shard paths from '%s'", path.c_str());
        } else {
            for (int shard = 2; shard <= split_count; shard++) {
                ...
                if (sfd < 0) {
                    QUENCH_LOG_ERROR("Failed to open shard %d: %s", shard, shard_path);
                ...
                if (shard_mmap == MAP_FAILED) {
                    QUENCH_LOG_ERROR("Failed to mmap shard %d: %s", shard, shard_path);
                    for (auto& [p, s] : extra_mmaps)
                        munmap(p, s);
                    munmap(mmap_base, file_size);
                    return nullptr;
                ...
                if (smagic != GGUF_MAGIC || sreader.failed()) {
                    QUENCH_LOG_ERROR("Invalid shard %d header", shard);
                    for (auto& [p, s] : extra_mmaps)
                        munmap(p, s);
                    munmap(mmap_base, file_size);
                    return nullptr;
                ...
                QUENCH_LOG_INFO("  Shard %d: %lu tensors, %.1f MiB", shard, (unsigned long)stensor_count,
    ...
    //: encoder-only models (BERT-family embedders like nomic-bert) would
    ...
    // nomic-bert has a dedicated encoder path: rotary positions,
    ...
    if (is_encoder_only_arch(arch_str) && cfg.arch != ModelArch::NOMIC_BERT) {
        throw std::runtime_error("encoder-only architecture '" + arch_str +
                                 "' is not supported (quench runs causal decoder LMs; "
                                 "embedding encoders need pooling support — only "
                                 "nomic-bert is wired,)");
    }

    QUENCH_LOG_INFO("Architecture: %s -> %s", arch_str.c_str(), model_arch_name(cfg.arch));
    ...
    // YaRN / Dynamic NTK RoPE scaling
    {
        ...
        if (rope_type_str == "longrope") {
            ...
            QUENCH_LOG_INFO("LongRoPE: short_factor[%zu], long_factor[%zu], orig_max_pos=%d",
    ...
    if (arch_str == "gemma4") {
        ...
        if (!cfg.head_dim_per_layer.empty()) {
            ...
            cfg.head_dim = max_hd;
            QUENCH_LOG_INFO("Gemma 4 per-layer head_dim: max=%d", max_hd);
        }

        QUENCH_LOG_INFO("Gemma 4: SWA layers=%zu (of %d), rope_theta_swa=%.0f, key_len=%d, key_len_swa=%d",
    ...
    if (cfg.arch == ModelArch::GPT_OSS) {
        ...
        QUENCH_LOG_INFO("gpt-oss: SWA pattern derived — %zu/%d sliding (even layers, window=%d), rest full",
                     std::count(cfg.swa_layers.begin(), cfg.swa_layers.end(), uint8_t(1)), cfg.n_layers,
                     cfg.sliding_window);
    }
    ...
    if (cfg.mxfp4_hadamard_attn > 0 || cfg.mxfp4_hadamard_ffn > 0)
        QUENCH_LOG_INFO("MXFP4 Hadamard: attn_bs=%d ffn_bs=%d", cfg.mxfp4_hadamard_attn, cfg.mxfp4_hadamard_ffn);
    ...
    // Per-layer arrays (Nemotron hybrid: head_count_kv and feed_forward_length are arrays)
    {
        ...
        if (!cfg.n_kv_heads_per_layer.empty()) {
            ...
            QUENCH_LOG_INFO("Per-layer KV heads: %zu layers, max=%d", cfg.n_kv_heads_per_layer.size(), max_kv);
        }
        if (!cfg.d_ff_per_layer.empty()) {
            ...
            cfg.d_ff = max_ff;
            QUENCH_LOG_INFO("Per-layer d_ff: %zu layers, max=%d", cfg.d_ff_per_layer.size(), max_ff);
        }
    }
    ...
    QUENCH_LOG_INFO("Config: layers=%d d_model=%d d_ff=%d heads=%d kv_heads=%d head_dim=%d vocab=%d ctx=%d",
                 cfg.n_layers, cfg.d_model, cfg.d_ff, cfg.n_heads, cfg.n_kv_heads, cfg.head_dim,
                 cfg.vocab_size, cfg.max_seq_len);
    QUENCH_LOG_INFO("RoPE: theta=%.1f, rope_dim=%d, neox=%d, freq_scale=%.1f, eps=%.2e", cfg.rope_theta,
                 cfg.rope_dim, cfg.rope_neox ? 1 : 0, cfg.rope_freq_scale, cfg.rms_norm_eps);
    if (cfg.yarn_ext_factor > 0.0f)
        QUENCH_LOG_INFO("YaRN: ext_factor=%.1f, attn_factor=%.3f, beta_fast=%.1f, beta_slow=%.1f, n_ctx_orig=%d",
                     cfg.yarn_ext_factor, cfg.yarn_attn_factor, cfg.yarn_beta_fast, cfg.yarn_beta_slow,
                     cfg.rope_n_ctx_orig);
    if (cfg.embed_scale > 0.0f)
        QUENCH_LOG_INFO("Embedding scale: %.2f (sqrt(d_model))", cfg.embed_scale);

    if (cfg.sliding_window > 0) {
        QUENCH_LOG_INFO("Sliding window attention: %d tokens", cfg.sliding_window);
        if (cfg.sliding_window_pattern > 0) {
            QUENCH_LOG_INFO("Sliding window pattern: every %dth layer is global, local_theta=%.1f",
    ...
    if (cfg.ffn_activation != FFNActivation::SWIGLU) {
        ...
        QUENCH_LOG_INFO("FFN activation: %s", act_name);
    }
    if (cfg.norm_placement == NormPlacement::POST_NORM)
        QUENCH_LOG_INFO("Norm placement: post-norm (residual after norm)");
    if (cfg.attn_logit_softcap > 0.0f)
        QUENCH_LOG_INFO("Attention logit softcap: %.1f", cfg.attn_logit_softcap);
    if (cfg.final_logit_softcap > 0.0f)
        QUENCH_LOG_INFO("Final logit softcap: %.1f", cfg.final_logit_softcap);
    ...
    if (cfg.ssm_inner_size > 0) {
        QUENCH_LOG_INFO("SSM: conv_kernel=%d state_size=%d groups=%d inner=%d dt_rank=%d", cfg.ssm_conv_kernel,
                     cfg.ssm_state_size, cfg.ssm_group_count, cfg.ssm_inner_size, cfg.ssm_dt_rank);
    }

    if (cfg.rope_dim > 0) {
        QUENCH_LOG_INFO("Partial RoPE: rope_dim=%d (full head_dim=%d)", cfg.rope_dim, cfg.head_dim);
    }
    ...
    for (const auto& info : tensor_infos) {
        ...
        if (assign_tensor(*model, info.name, t, info.type)) {
            ...
            QUENCH_LOG_DEBUG("Unassigned tensor: %s [%s] shape=[%ld,%ld,%ld,%ld]", info.name.c_str(),
    ...
    if (cfg.vocab_size == 0 && model->tok_emb_.data != nullptr) {
        ...
        QUENCH_LOG_INFO("Inferred vocab_size=%d from token_embd.weight", cfg.vocab_size);
    }
    ...
    if (model->out_proj_.data == nullptr && model->tok_emb_.data != nullptr) {
        ...
        QUENCH_LOG_INFO("Weight tying: output projection shares token embedding");
    }
    ...
    if (cfg.d_ff > 0) {
        ...
        if (fused_count > 0) {
            QUENCH_LOG_INFO("Split fused gate+up FFN in %d layers (d_ff=%d)", fused_count, cfg.d_ff);
        }
    }

    QUENCH_LOG_INFO("Weights: %d assigned, %d skipped", assigned, skipped);
    ...
    //     experts stored as regular FFN tensors, and warn about mismatches.
    {
        ...
        if (n_remapped > 0) {
            QUENCH_LOG_INFO("Remapped %d layers: dense FFN tensors -> shared expert", n_remapped);
        }
        ...
        // quench's RMSNorm expects raw weights (w, not w+1). Subtract 1 back.
        // Qwen3.5: GGUF converter adds +1 to norm weights (same as Gemma).
        // quench's rmsnorm uses weight directly, which is correct since the +1 is
        ...
        if (n_shared_exp > 0 && cfg.n_experts_shared == 0) {
            ...
            QUENCH_LOG_INFO("Inferred shared expert config: n_shared=%d, shared_d_ff=%d", cfg.n_experts_shared,
        ...
        if (cfg.arch == ModelArch::GEMMA4 && !cfg.swa_layers.empty() &&
            ...
            QUENCH_LOG_INFO("Gemma 4: rope_freqs → %d effective freqs, %d global layers", n_pairs, n_global);
        }
        ...
        if (cfg.n_experts > 0 && n_moe == 0) {
            QUENCH_LOG_WARN("Config declares %d experts but no MoE gate tensors found", cfg.n_experts);
        }

        if (n_moe > 0 && n_moe < cfg.n_layers && n_dense_ffn == 0 && n_ssm == 0) {
            QUENCH_LOG_WARN("Only %d/%d layers have MoE, remaining layers have no FFN", n_moe, cfg.n_layers);
        }
    }

    // gpt-oss residual-stream 2^-4 rescale (, GGUF parity with the
    // SafeTensors loader). gpt-oss's huge activations overflow quench's FP16
    ...
    if (cfg.arch == ModelArch::GPT_OSS) {
        ...
        auto scale_q8_0 = [&](Tensor& t) -> bool {
            int64_t n = t.numel();
            if (n % 32 != 0) {
                QUENCH_LOG_ERROR("gpt-oss GGUF rescale: Q8_0 numel %lld not 32-block-aligned", (long long)n);
                return false;
            }
        ...
        auto scale_any = [&](Tensor& t) -> bool {
            ...
            switch (t.qtype) {
                ...
                case QType::Q8_0:
                    return scale_q8_0(t);
                default:
                    QUENCH_LOG_ERROR("gpt-oss GGUF rescale: unsupported qtype %d", std::to_underlying(t.qtype));
                    return false;
            }
        };
        bool ok = true;
        ...
        if (!ok) {
            QUENCH_LOG_ERROR("gpt-oss GGUF: residual-stream rescale failed");
            return nullptr;
        }
        QUENCH_LOG_INFO("gpt-oss GGUF: residual stream rescaled by 2^-4 (Wo + o_bias + expert down bias)");
    }
    ...
    if (it_tok_model != metadata.end()) {
        ...
        // Gemma-4 uses SPM-style BPE: ▁ for spaces + BPE merge ranks.
            ...
            tok_type = "gemma4";
        // BERT WordPiece (, nomic-bert embedder).
        else if (tm == "bert")
            tok_type = "bert";
    }
    ...
    if (it_pre != metadata.end() && !it_pre->second.str_val.empty()) {
        ...
        QUENCH_LOG_INFO("Tokenizer pre-tokenizer: %s", it_pre->second.str_val.c_str());
    }
    ...
    if (it_tokens != metadata.end() && !it_tokens->second.str_array.empty()) {
        ...
        if (tok_type == "gpt2" || tok_type == "gemma4") {
            ...
            if (it_merges != metadata.end() && !it_merges->second.str_array.empty()) {
                ...
                QUENCH_LOG_INFO("Tokenizer: loaded %zu BPE merge rules", it_merges->second.str_array.size());
            }
        }
        ...
        if (it_tpl != metadata.end() && !it_tpl->second.str_val.empty()) {
            ...
            QUENCH_LOG_INFO("Chat template: %zu chars", it_tpl->second.str_val.size());
        }
        ...
        for (const char* key : {"tokenizer.ggml.eot_token_id", "tokenizer.ggml.eog_token_id"}) {
            ...
            if (it_extra != metadata.end()) {
                ...
                if (extra_id >= 0) {
                    ...
                    QUENCH_LOG_INFO("Tokenizer: additional EOS from %s: %d", key, extra_id);
                }
            }
        }

        QUENCH_LOG_INFO("Tokenizer: type=%s, %d tokens, bos=%d, eos=%d (%zu total), add_bos=%d",
                     tok_type.c_str(), tokenizer->vocab_size(), bos_id, eos_id, tokenizer->eos_ids().size(),
                     tokenizer->add_bos() ? 1 : 0);
    } else {
        QUENCH_LOG_WARN("No tokenizer data found in GGUF metadata");
    }

    model->set_tokenizer(std::move(tokenizer));

    QUENCH_LOG_INFO("GGUF model loaded successfully from %s", path.c_str());
    return model;
}
