...
// quench's minimal Jinja2 engine has no {% generation %}/{% endgeneration %} block
...
bool ChatTemplate::init(ChatTemplateFamily family, const Tokenizer& tokenizer, const std::string& jinja_str) {
    ...
    if (!jinja_str.empty()) {
        ...
        if (tpl->parse(cleaned)) {
            ...
            QUENCH_LOG_INFO("Chat template: using Jinja2 engine");
            ...
            QUENCH_LOG_WARN("Jinja2 parse failed (%s), falling back to hardcoded template",
                         tpl->error().c_str());
        }
    }
    ...
    if (!jinja.empty()) {
        ...
        while ((pos = jinja.find(sys_prefix, pos)) != std::string::npos) {
            ...
            if (candidate.find("messages") == std::string::npos &&
                ...
                QUENCH_LOG_INFO("Default system message: %.40s%s", default_system_message_.c_str(),
    ...
    QUENCH_LOG_INFO("Chat template: %s", chat_template_family_name(family_));
    return true;
}
...
std::vector<int32_t> ChatTemplate::apply(const Tokenizer& tok, const std::vector<ChatMessage>& messages,
    ...
    if (use_jinja_ && jinja_tpl_) {
        ...
        if (!tokens.empty())
            return tokens;
        QUENCH_LOG_WARN("Jinja2 render produced empty result, falling back to hardcoded template");
    }
...
void ChatTemplate::auto_detect_stop_tokens(const jinja::Context& ctx) const {
    ...
    for (const auto& [text, id] : control_tokens_) {
        ...
        if (last != std::string::npos && last > rendered_no_gen.size() / 2) {
            ...
            QUENCH_LOG_INFO("Jinja2: auto-detected stop token '%s' (id=%d)", text.c_str(), id);
            break;
        }
    }
}
...
std::vector<int32_t> ChatTemplate::apply_jinja(const Tokenizer& tok, const std::vector<ChatMessage>& msgs,
    ...
    if (rendered.empty()) {
        QUENCH_LOG_WARN("Jinja2 render returned empty string (error: %s)", jinja_tpl_->error().c_str());
        return {};
    }
    QUENCH_LOG_DEBUG("Jinja2 rendered (%zu chars)", rendered.size());
    if (quench::process_diag_debug_template()) {
        std::string escaped;
        for (char c : rendered) {
            if (c == '\n')
                escaped += "\\n";
        ...
        QUENCH_LOG_DEBUG("[DEBUG_TPL_JINJA] rendered: \"%s\"", escaped.c_str());
    }
...
std::vector<int32_t> ChatTemplate::apply_jinja_with_tools(
    ...
    std::string rendered = jinja_tpl_->render(ctx);
    if (rendered.empty()) {
        QUENCH_LOG_WARN("Jinja2 tools render returned empty string (error: %s)", jinja_tpl_->error().c_str());
        return {};
    }
    QUENCH_LOG_DEBUG("Jinja2 tools rendered (%zu chars): %.200s", rendered.size(), rendered.c_str());
...
std::vector<int32_t> ChatTemplate::apply_with_tools(const Tokenizer& tok,
    ...
    if (use_jinja_ && jinja_tpl_ && !tools.empty()) {
        ...
        if (!tokens.empty())
            return tokens;
        QUENCH_LOG_WARN("Jinja2 tools render failed, caller should inject text-based tool prompt");
    }
