...
Model::~Model() {
    ...
    //
    //: these MUST be freed with cudaFreeAsync, not cudaFree. On this
    ...
    // quench_model_free/quench_context_free actually hands the memory back.
    //
    ...
    if (free_fail > 0) {
        QUENCH_LOG_WARN("Model teardown: %d/%zu weight frees failed (first: %s)", free_fail,
    ...
    // (quench_model_free), so direct-C++ model teardown (tests, embedders) leaked
...
void Model::release_gpu_allocation(void* ptr) {
    ...
    if (it != gpu_allocations_.end()) {
        ...
        // instead of reading freed memory and poisoning the CUDA context.
...
// presets.cpp, and quench_api.cpp (RF-003).
// ---------------------------------------------------------------------------
struct ArchEntry {
    ModelArch arch;
    const char* name;
    int c_api_id;  // QUENCH_ARCH_* enum value
...
// QUENCH_ARCH_* values from include/quench/types.h (avoid header dependency)
enum {
    kApiLlama = 0,
    kApiMistral = 1,
    kApiMixtral = 2,
...
static constexpr ArchEntry kArchRegistry[] = {
    ...
    // logits + renormalized top-k softmax is exactly quench's norm_weights path
    ...
    // Encoder-only embedder: rotary (neox pairs), SwiGLU FFN; sampling
...
// --kv-fp8). Measured on a 3.9k-token context, FP8 vs FP16 KV, same
// corpus, RTX 5090:
...
//             (+0.25%), measured on a 3.8k-token context. Vanilla
...
//             = ~0.00% over a 26.5k-token corpus (5 runs each).
//             A single A/B run can read +0.47%..+1.83%, but that is
...
// Measured but NOT added (same gate):
...
// (Qwen3-8B Q8_0: 150→211 tok/s with FP8 KV). Evidence bar is
...
// Measured but NOT added (same evidence bar):
...
ModelArch parse_model_arch(const std::string& s) {
    ...
    // plausible-looking output rather than an error. That fallback is
    // deliberate — the registry above maps 30 strings onto known archs and a
    // Llama-shaped checkpoint usually works through it — but a completely
    // silent fallback makes a genuinely unsupported model look supported.
    // is_encoder_only_arch() guards the one family where the failure is loud
    // (BERT encoders: IMA on the first request); everything else just
    // runs. Say which string was not recognised, once, at parse time.
    if (!s.empty())
        QUENCH_LOG_WARN(
...
bool is_encoder_only_arch(const std::string& s) {
    ...
    // generic decoder → CUDA IMA on the first request. Lowercase first.
    std::string l;
    l.reserve(s.size());
    for (char c : s)
