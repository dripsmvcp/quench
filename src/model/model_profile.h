...
// single source of truth that replaces scattered inline `if (arch==…)`
// branches and repeated "loop the layers, check gdn_gate/ssm_in" detection —
// a bug class where per-site re-derivation drifts out of agreement.
//
...
struct ModelProfile {
    ...
    // Encoder-only embedder (, nomic-bert): bidirectional attention,
