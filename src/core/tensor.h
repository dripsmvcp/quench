...
struct Tensor {
    ...
    //   - quench legacy (GGML type 31): [data (16 bytes) | scale (1 byte)] per block
    ...
    // Set when the original GGUF source bytes have been
    // freed by pre-dequant Phase 4b. `data` is left as a stale hash-key pointer; any
