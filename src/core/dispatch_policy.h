...
//
// config.h is included by 22 files in src/exec/ (85 translation units
// transitively), so its churn is the biggest recompile blast radius in the
// repo. About half that churn lands in sections exec/ never reads (Runtime,
// Vram, Server, Rope); this is what decouples it. ProcessDiag exists
// precisely because leaf utilities cannot carry a RuntimeConfig.
//
// They live in namespace quench::cfg because un-nesting collides otherwise:
// quench::KVCache is the paged cache in memory/kv_cache.h, and quench-bench has its
...
struct KVCache {
    ...
    // dtype resolves to FP16 (pre-hint behavior). quench.conf key only — the
    // old QUENCH_KV_FP8_AUTO env var is no longer read.
    bool fp8_auto_legacy = false;
    // BitDecoding residual FP16 cache for newest N tokens.
    // 0 = disabled. Typical: 4..32.
    ...
    // and gpt-oss-20b (deterministic_gemm A/B).
    // Tri-state: "auto" enables the savings only when prefix caching is
    // off (one-shot quench-cli runs), so serving keeps warm-prefix TTFT;
...
struct Attention {
    ...
    // QK^T mode follows fa2_fp16qk: f16-QK by default (no e4m3 score
    // noise); fp8-QK only when fa2_fp16qk=never AND fp8_fmha=on. Declines
    // (-> FP16 WMMA FMHA) for hd!=128 (Gemma), non-F16, or insufficient
    // smem, so it's safe by default. Legacy env: QUENCH_FMHA_FA2.
    ...
    // f32 accumulate), so the short-seq e4m3 quality cliff
    ...
    // f16-accumulate QK^T in the FP16-QK FA2 kernel. GeForce sm_120
    // runs f16-src/f32-acc HMMA at 1/4 rate; accumulating the score
    // MMA in f16 lifts it to the full-rate class. Measured +4.7-5.0%
    // pp4096 NVFP4 prefill (Qwen3-14B / 30B-A3B, chunk-2048
    // prefill), decode neutral. Quality gate on a 5.8k teacher-forced corpus:
    // 14B-NVFP4 PPL identical, 30B-A3B +0.10%, Q8_0 GGUF +0.013% —
    // scores are softmaxed immediately, so the reduced accumulate
    // precision stays in the noise. Default ON; set
    // false to restore f32 accumulate. Only affects the fa2_fp16qk path,
    // the fp8-QK path keeps f32 accumulate.
    bool fa2_f16acc = true;
    // f16-accumulate the PV MMA as well. With fa2_f16acc on, the PV
    // accumulate is the last 1/4-rate HMMA in the FA2 kernel, dominating its
    // tensor-pipe time ~4:1; packing O as half2 also halves the O-fragment
    // register footprint of the Bq=128 band. Measured (nsys
    ...
    // rounding stays below the f16 output precision). Default ON;
    // set false to restore f32 PV accumulate. Requires
    // fa2_f16acc.
    ...
    // Default ON (measured: kernel 4.3x vs WMMA,
    // e2e +10.6% pp4096 / +24.8% pp8192, PPL 10.44 vs 10.58 on
    // Qwen3.6-35B; a split-D variant measured slower — the 4-warp instance
    // is the keeper). Also gates the FP8-KV deterministic-cuBLAS skip for
    // hd=256 models (engine_init_resolver fa2_serves_attention).
    bool fa2_hd256 = true;
    // amax-scaled e4m3 conversion for the fp8-QK FA2 path. The
    // raw conversion is the quality cliff; scaling Q and K to the
    ...
    // NVFP4-attention spike (SageAttention3 recipe). All three only
    ...
    // mxfp4_paged_kv: KV-append-quant chunked prefill —
    ...
    // or routed through generic-dequant (LM head). Pruning
    ...
    // MLA absorbed-decode latent KV cache (DeepSeek-V2/V3). When
...
struct MoE {
    ...
    // Host-offload debug check: assert device-side mirror
    // == host-side LRU state after every cache mutation. Off by default;
    // turn on via `moe.expert_cache_debug_parity = true` in quench.conf for
    ...
    // Async prefetch: at the start of layer L, issue async
    ...
    // (default — safety first, prefetch perf gains depend on workload
    // and need per-model measurement). Sensible values: 3..16.
    int prefetch_top_k = 0;
    // CUDA Graphs under host-offload: drop the "experts on
    ...
    // TODO: refactor the dispatch kernels to read the device
...
struct GDN {
    ...
    // GDN chunkwise SSD scan. When true,
    ...
    // Bit-near-equivalent output (FP16 1e-3 / FP32 1e-5
    // tolerances); microbench shows +16.7 % on the GDN scan kernel alone
    // at n_tok=4096 (1.567 → 1.343 µs/tok on RTX 5090). A
    // cold-median A/B on Qwen3.6-35B-A3B Q4_K_M showed the end-to-end
    // wall delta is within the cuBLAS variance band (±0.5 % across
    // pp512 / pp2048 / tg128), so the default ON is wall-neutral
    // for the hero MoE model and keeps the kernel-level win for
    // workloads where the GDN scan is a larger share of wall (longer
    // contexts, pure-GDN models like Qwen3.5-4B-GDN /
    // Qwen3.5-9B-GDN). Opt out via
    // `--set gdn.chunkwise_scan=false` if a model regresses.
    // WY-rep and TC-MMA chunkwise variants were benched and stay
    // behind this kernel on sm_120 — it remains the fastest chunkwise
    // path.
...
struct GEMM {
    ...
    // m16n8k16 tile kernel. Experimental scaffold (default off). When enabled,
    ...
    // cuBLAS round-trip for Q8_0 prefill (M ≥ 64). SMEM-staged scales,
    // 128x128x64 tiles, symmetric epilogue. Default on.
    bool q8_imma_enabled = true;
    // Q4_K dense prefill via the IMMA kernel: uses
    // mmq_q4k_imma_reorder's symmetric-s8 + α/β form with the unified
    // β·rowsum epilogue. Experimental: default off. (Distinct from a
    // retired 64x32 q4k_imma kernel that plateaued at 40 TOPS.)
    ...
    // the main lever for the 2.4-2.6x GGUF-MoE prefill gap.
    // Default on.
    ...
    // quality on those).
    // "auto" (net rule) | "on" | "off" (legacy true/false accepted).
    ...
    // MoE GGUF heads where a parity sweep measured the PPL
    ...
    // 1/4 rate (measured: 253 vs 1956 TFLOPS saturated
    ...
    // is the cap. 16F measured +24.9% q8 pp512 model-level (paired
    // restarts), decode neutral. "auto" (default) enables
    ...
    // an older NVFP4 method degraded recurrent-state coherence). Quantified
    // with the quantize-FP16→NVFP4 path + the `quench-cli --perplexity` tool
    ...
    // when max_batch_size > 1). Default ON per a teacher-forced PPL sweep
    // (13.5k-token corpus, quench-cli --perplexity): MoE/hybrid
    // +1.9-2.1% PPL (Coder-30B 10.19→10.38, Modelopt-30B 11.65→11.88,
    // Qwen3.6-35B 13.39→13.66), dense +0.2-0.5% (within the ±0.3-0.5%
    // run-to-run spread), for +8% aggregate concurrent decode
    // @16. Single-stream (n==1) output is
    ...
    // (GGUF-hybrid GDN projections are served by the GGUF branch of
    // fp8_ssm_proj below, which is quality-safer than 4-bit into the
    // recurrent scan. The recurrent
    ...
    // risk. MEASURED: Nemotron-3-Nano-30B **+3.8% decode**,
    ...
    // MEASURED: Qwen3.6-35B-A3B-NVFP4 decode +19% (268.6→320.3
    // tok/s spec-off, 261→308 with default spec), PPL flat (8.021→8.012);
    // Nemotron-3-Nano PPL flat (4.184→4.117).
    // MEASURED (GGUF branch): Qwen3.6-35B UD-Q4_K_M decode +21%
    ...
    // "auto" = ON only for gpt-oss: its BF16 SafeTensors dense
    ...
    // decode-only per-row FP8 is the recipe).
...
struct Speculative {
    ...
    // Context cap for DENSE chunk-verify drafting. With the
    // verify_decode_attn route below, dense n-gram WINS up
    ...
    // The DEPTH-aware gate below (shallow_draft_ctx) is the primary
    // long-context control: the
    // long-context loss is not a context cost — it is draft depth
    // (a verify step costs ~1.4x a decode step @512 rising to ~2.6x
    // @16k, so 1-token drafts stop paying past ~14k while 3-token
    // drafts win +24% at 13k). Hard cap defaults OFF. 0 = no cap.
    int draft_ctx_cap = 0;
    // Depth-aware gate: past this request context, 1-token n-gram/suffix
    ...
    // Run the dense verify chunk's ATTENTION through
    ...
    // Verify-chunk GEMMs (M <= largest capture bucket) read the
    ...
    // Qwen3-Coder-30B-FP4: code-edit +49-81% (93% accept,
    ...
    // this flag. quench-cli --bench pins this false so the canonical
    // perf-baseline decode signal stays raw (verify inherits grouped-GEMM
    // restart variance).
    bool moe = true;
    // At decode batch > 1, ONE request per step may run
    ...
    // turns; fruitless chain breaks alone cost -1.5..-2.7% without it).
    // Default-ON matrix (Coder-30B NVFP4, 2 trials/cell,
    ...
    // nothing (measured 0 drafts on reasoning). Runs as
    ...
    // exactly one batched-GEMV weight sweep (M=4); deeper chains
    // pad into bucket 5+ and pay a second sweep.
    int recycle_depth = 3;
    // Precision gate: only draft hops whose front slot was
    ...
    // Multi-candidate verify: when the
    ...
    // CUTLASS at ~51% effective bandwidth (measured) — the
    ...
    // recapture, ~10-20 ms saved per burst). Rearm and fresh capture
    // share the same first-forward semantics
    // (CudaGraphConditionalRunner::setup).
    ...
    // accepted tokens). quench-cli --bench pins this false (same
    ...
    // match (draft-poor prose — 78-94% depth-1 accept on
    // Qwen3.6-35B-A3B). quench-cli equivalent: --mtp-spec-decode <k>.
    int mtp_k = 0;
    // Serve the MTP chain's full-vocab logits GEMV from the NVFP4 LM-head
    // decode cache when one exists. The chain re-reads the
    ...
    // the guard (raw-economics measurement). Default from the
    ...
    // Graph-captured verify chunk: cache one CUDA graph per
...
struct FFN {
    ...
    // quench::flush_ffn_sparsity_probe_log() (engine destruction or
...
struct Diagnostics {
    ...
    // Trace knobs. The QUENCH_SPEC_TRACE / QUENCH_JUMP_TRACE /
    // QUENCH_PPL_DUMP env names still work — they are
    // debug aids people have in their shell history — but they are seeded
    // into these keys at load, so `--set` and quench.conf reach them too and
    // `quench-cli --help`/quench.conf.example can document them.
    ...
    // tool for isolating the NVFP4 GEMV when diagnosing quality issues
    // such as long-form repetition loops).
    ...
    // graph-captured-verify feasibility probe: stream-capture every
