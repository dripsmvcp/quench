...
Tensor::Tensor(void* data, QType qtype, int ndim, const int64_t* shape, bool on_device)
    ...
    QUENCH_CHECK(ndim >= 0 && ndim <= kMaxDims, "Tensor: ndim=%d out of [0, %d]", ndim, kMaxDims);
...
Tensor Tensor::slice(int64_t start, int64_t end) const {
    QUENCH_CHECK(ndim > 0, "Tensor::slice: ndim=%d, expected > 0", ndim);
    QUENCH_CHECK(start >= 0 && end <= shape[0] && start < end,
