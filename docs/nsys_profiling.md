# quench: Systematic Nsight Systems Profiling & Optimization

## Mission

Conduct a rigorous end-to-end performance audit of `quench` using Nsight Systems (`nsys`) on the RTX 5090 (SM120). Identify the top 5–10 optimization opportunities ranked by expected tok/s impact, and produce a prioritized action list with concrete patches.
...
1. Verify toolchain:
   ...
   - quench built with `-lineinfo` and **NVTX ranges enabled** (`-DQUENCH_ENABLE_NVTX=ON`)
...
2. If NVTX is not yet wired in, **add it first** before profiling. Annotate at minimum:
   - `quench::Engine::generate()` (top-level)
   - Prefill vs. decode phases (separate ranges)
   - Per-layer: attention, FFN, sampling
...
- **W3 — Batched serving:** 16 concurrent requests, mixed lengths. Stresses scheduler and KV layout.
...
- Check for malloc/free in the steady-state hot loop (any `cudaMalloc`/`cudaFree` after warmup is a bug). nsys is the wrong instrument for this one — build with `-DQUENCH_ALLOC_INTERPOSE=ON` and read `[alloc-interpose] steady state`, which attributes every call site. The shipped state is zero (`0 cudaMalloc, 0 cudaMallocAsync, 0 pinned-host allocations while serving`), so any nonzero count is the finding. **Rebuild with the default OFF before measuring throughput** — the shim costs ~3% decode and has already been mistaken for a regression.
...
### 2.6 Quantized-path verification
- Confirm the GEMM/GEMV kernels actually being dispatched on the decode hot path are the NVFP4 variants (and FP8 on prefill), not FP16 fallbacks. Kernel name in `nsys` should make this obvious; an unexpected FP16 GEMM in the hot path on SM120 is running at half rate and is an immediate finding.
...
- Any unexpected BF16/FP16 GEMM on the SM120 decode hot path (running at half rate — should be NVFP4)
