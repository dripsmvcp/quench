...
quench has no memory architecture. It has 380 allocation sites across 81 files.
...
is taken (A1.5). Leaks and OOMs are downstream of that; so are six past
incidents whose fixes are all still visible in
...
Measured on the working tree, including the budget fix (see A1.4).
...
`runtime/cuda_graph.cu` (17), `runtime/engine_graph_decode.cpp` (13),
...
2. `allocate(..., bypass_headroom = true)` skips the check entirely.
...
allocations `>= 16 MiB` — was until the budget fix **unknown to the
...
construction; the caches it starved failed mid-build and cost ~7× decode at
server defaults on the config that exposed it.

### A1.3 Measured footprint — reference config
...
device-used peak sampler. 2 rounds × N concurrent streaming completions,
0 errors, GPU healthy throughout (2857–2932 MHz SM / 13801 MHz mem /
310–444 W under load — no depressed-host artefact).
...
All MiB. Aggregate throughput during the load: 876 tok/s.

**Peak − steady state is +190 MiB.** This is the entire
steady-state per-request allocation surface: there is no transient prefill
spike to cap, because every
...
| checkpoint | dense Δ | what it is |
|---|---:|---|
| `00_pre_init` | 1679.6 | CUDA context + WDDM driver (absolute, not Δ) |
| `01_prewarm_gemm` | +676 | `gemm_init` + `attention_cublas_prewarm` |
| `02_weights+decode_cache` | +4700 | weight upload + pre-dequant caches |
| `03_kv_cache` | +6990 | KV pool + executor workspaces |
| `05_post_warmup` | **+4180** | see A1.5 |
...
|---|---:|
| `WEIGHTS` | 4076.1 |
| `WEIGHT_CACHE_NVFP4` | 2157.7 |
| `KV_BLOCK_POOL` | 4608.0 |
| `EXEC_WORKSPACES` | 396.6 |
| **tracked total** | **11238.3** |
| **untracked residual** | **7177.2 (39 %)** |

Acceptance criterion 6 asks for ≥95 % accounted. The table above is kept
because the shape of the gap is what the fixes are aimed at — it is not the
state today.

**Criterion 6 is met.** The pool ledger is always on rather than gated behind
`--mem-report`, and the library charge is measured over the whole
init instead of the warmup-forward window, so the residual means what it says on every
start. Measured with `--mem-report`: **99.9 % accounted, 16 MiB residual** on
the dense reference config (was 39 % untracked).
...
- **quench's own lazy CUDA module loading.** `CUDA_MODULE_LOADING=EAGER` moved only
...
to leave 14 GiB free; on a config that ends with less headroom the same
constant dominates what remains.
...
- **`Buffer`** (`src/core/buffer.{h,cpp}`): a single-purpose helper with one
  producer and one holder, not a general RAII layer.
...
| `exec/executor_workspace*` | shared workspace | `max(attn, ffn)` shared sizes at `max_tokens` | engine-persistent | ✓ | via `VRAMAllocator` |
...
| **T5b Engine-persistent pinned host** | process | `PinnedBuffer` over `HostPinnedAllocator`; `HostRegistration` for memory quench does not own | per-object leak (move-only owner, no free to forget); asymmetric unregister | n/a |
...
- **T1 and T2 need separate arenas even though both are "stable forever."**
  `server.model_swap` unloads a model and loads another
...
- **T3 is a *group* of pools, not one.** The KV cache already runs two block
  groups (global + SWA), and the residual FP16 ring is another fixed-stride
  slab with per-sequence slots. One `BlockPool<Stride>` template, several
  instantiations.

What does **not** get a tier: the library reservation (F4). It is not quench's
...
configs (A1.3): quench allocates essentially everything once at init and holds it
...
**Superseded: the defect this section describes no longer
exists.** Step 6.4 reordered the build so the caches come first and KV takes the
measured residual, and step 6.9 replaced the wrong free-VRAM reading with a measured
library charge (attribution ~100 %). The pool now grants the full requested
context at 4k, 32k and 128k, and holding a 128k pool at 81 MiB free costs 1 % of
decode. The reasoning below stands as the record of why VMM looked necessary.
...
1. **`cuMemCreate`/`cuMemMap` under WSL2/WDDM.** **Measured — the
   gate is OPEN.** A standalone VMM probe, 24/24 checks, two
...
*Not considered:* `cudaMallocAsync` pools as the backend. quench already pins the
...
```cpp
// src/memory/backend.h — the ONLY place in quench that calls the driver.
class Backend {
public:
...
move-only, and the *only* type in quench that holds a raw device pointer obtained
from the driver.
...
- `Serving` — **debug:** `QUENCH_ASSERT_FAIL` with the tag and a backtrace.
...
mark rather than from `max(attn, ffn, …)` heuristics recomputed in three
places.
...
platform fact rather than a concession.** Measured on this box:
...
2. The pre-dequant phases then re-derive **their own** reserves from live free
   VRAM again, double-counting the KV pool: it is allocated before the
   cache build, so its bytes are already gone from free_vram, and every one of
   them then comes out of the decode cache a second time.
...
4. Six incident-driven clamps stack on the result: `target_blocks`, the
   ...
   affordability cap, the SWA batch-shaped charge, and the allocator-
   headroom floor.
...
struct PlanInput {
    ...
    FeatureSet features; // spec decode, SWA, residual ring
...
struct MemoryPlan {
    ...
    std::vector<PoolPlan> pools; // SWA group, residual ring
...
> **Corrected during step 3 (B3).** This section originally said a
...
`reserved`/`used` attributes. **Measured: it is already zero.**
...
benefit, and per-stream is meaningless when quench runs one compute stream plus a
prefill stream.
...
**Implemented, and the growth path turned out to be
...
`gemm.cu`'s two statics to T2. A growth path that remains is safe for a reason
the `cudaMalloc` version was not: **a bump-arena take is pointer arithmetic,
not a CUDA call, so it is legal under stream capture.**
...
the planner, and all the refcount logic run on CPU-only CI — which is what quench
...
- Injectable failure: fail the *n*-th acquisition, to exercise the rollback
  ...
  (`rollback_partial_allocation`, the workspace-set unwind in
  `executor_workspace_buffers.cu`, and the KV `allocate_blocks` rollback).
...
**The right gate signal is `own_peak`, not the `--mem-report` device total.**
...
absolute — and therefore immune to the 1.6 GB run-to-run swing in free VRAM
(A4.2). Measured byte-identical across repeat runs, which is a better
...
monotonically. Every step leaves the tree green and
keeps decode and prefill within 1 %.

| # | Step | Removes from allowlist | Why here |
|---|---|---:|---|
| 0 | `Backend` + `FakeBackend` + phase guard + the I1 allowlist (allowlist = everything, gate green from day one). No behaviour change. | 0 | The gate must exist before anything moves, or the list can silently grow. |
...
| 2 | **`plan_memory()` alongside `compute_vram_budget()`** — computed alongside, and the KV block count is APPLIED from it. Log both; assert agreement within a tolerance in CI via V8. | 0 | Establishes the plan is right *before* anything depends on it. Highest-information, lowest-risk step. |
...
| 5 | **Per-request allocations** (`engine_graph_decode.cpp`, `engine_scheduler.cpp`, `executor_attention_prefill.cu`, `executor_attention.cu`, per-call pointer arrays) → `ScratchStack`. **This is the step that satisfies I2**; the counter from step 0 must reach zero. **Plan it per BUFFER FAMILY, not per file**: the KV block tables alone are 8 acquisitions against **23 releases in 5 files**, one allocation being freed at 8 sites depending on which path unwinds, so a file-at-a-time pass cannot close them. | ~40 | Depends on the stack existing (4) and on the plan sizing it (2). |
| 6 | **Weight upload + pre-dequant caches** (`model/`, `quant/`, `exec/pre_dequant_*`) → T1 arena. **Deletes the balloon** and the phase-local free-VRAM re-derivation. Switch the allocation order to the plan's tier order. **Re-scoped: both of this step's original justifications are spent.** The acceptance test (`03_kv_cache` ending at 0.0 MiB free) was closed by 6.4's reordering, and the capture-abort the balloon existed to prevent does **not** reproduce on any measured configuration. What remains genuinely open here is I4's other half — the live `cudaMemGetInfo` sizing — not the balloon's rescue. | ~90 | Largest blast radius; must come after the plan is trusted (2) and the KV pool no longer competes for a residual (3). |
| 7 | **VMM backend for the KV pool**, gated on the WSL2 spike (A3.1). If the spike fails, stop here — everything above stands. **The spike passed and the step was still not built: measurement showed its premises spent.** | 0 | Optional by construction. |
...
| | Invariant | Design-time | **Measured** | Target |
|---|---|---|---|---|
| I1 | Single acquisition point | ✗ — 365 sites / 74 files outside `src/memory/` | **✗ — 499 calls / 74 files, of which 224 are acquisitions and 275 releases.** The **pinned-host class is CLOSED: 26 → 0** — every one moved to T5b's `PinnedBuffer`/`HostRegistration`, which is why the total fell 638 → 582 (each migration took its releases with it). What remains is device memory only. Progress came from migrating whole clusters rather than sites; what stalled it before that was migrations keeping their original path as a fallback, which the gate cannot see | ✓ — `Backend`, allowlist empty, CI gate |
...
| I6 | OOM is typed and recoverable | ~ — `RequestStatus::CANCELLED`, plus a warning that fires *after* prefill | **✓ — both halves.** Plan-time: an unservable `--vram-budget` refuses at load with the block arithmetic. Admission-time: `QUENCH_ERROR_CAPACITY` → HTTP 503 `capacity_error`, distinct from a client cancel | ✓ — plan-time failure at load; admission-time 429/503 at runtime |
| I7 | Capacity ≠ occupancy | ✗ — 20–39 % of device memory unattributed | **~ — per-tier reserved *and* live is served** on `/metrics`, plus KV blocks and budget-vs-own. Accounting reaches **98.3 %** once the library reserve is charged at its measured value rather than the 3900 MiB constant; with the constant it is 82.5 % on Qwen3-8B. A measured **102.0 %** — a negative residual — on one config proved a note double-booking, so that config's gap was the counters, not the attribution | ✓ — per-tier reserved *and* live, library reserve named, ≥95 % accounted |
...
2. **`LibraryReserve` calibration** — the ~3.9 GiB constant is measured on this
   ...
   or CUDA bump.
3. **`ScratchStack` under concurrency** — quench runs one compute stream plus a
...
| 4b.2 — the remaining `exec/` workspaces | **done**. The last holdout — the chunk-capture K/V pair — was not unmigratable but mis-diagnosed: "growable, a bump arena strands it" is true of a staircase of takes, not of taking the bound once, and the bound is exactly what `engine_spec_capture.cpp` already computes. What remains outside T2 in `exec/` is the model-resident weight caches (T1, step 6) and one-shot init transients | `fix(exec): the chunk-capture K/V pair to the T2 arena` |
...
| 6.9 — the library charge is measured across the WHOLE init, not the warmup-forward window | **done** — the ledger is always on, so the residual means what it says on every start; attribution 89.7 -> 100 % on the config that was worst, and the worst config's second start gains 4x KV | `fix(memory): measure the library charge across the whole init` |
| **criterion 4 — post-drain live blocks return to baseline** | **RUN and PASSES: live blocks 0 → 0 across 48 requests.** An earlier "one block per request leaks" reading came from a gauge conflating live with cached | |
| 6.7 — the library reserve is measured and reported, not assumed | **done** |
| 6.8 — the measurement is remembered; criteria 5+6 hold from the 2nd start | **done, with one condition made explicit**: "the 2nd start" needs the cache path to outlive the process, and the default lands inside the container quench is run in — `vram.library_reserve_cache` must point at a mounted path or the constant is charged forever (+639 MiB of distributable VRAM lost per start on the config measured) | `feat(memory): remember the measured library reserve` | `feat(memory): measure the library reserve instead of assuming it` |
| 7a — WSL2 VMM spike (the gate) | **done — GO** | `test(memory): WSL2 VMM spike — the step-7 gate is open` |
| 7b — VMM backend implementation | **not built, premises spent** — 6.4 and 6.9 removed the mis-sizing this was to fix, and the elasticity that remains does not reproduce as a defect: the pool grants the full requested context at 4k/32k/128k and holding it at 81 MiB free costs 1 % of decode. Reopen if a config appears where the pool is clamped below the request | |
| 8 — `compute/` statics | **done, by drawing a line rather than by reaching zero.** Migrated: the DRY penalty pair, the cuBLAS/CUTLASS workspace family, **the grow-on-demand statics, which closes that bug class**, and the CUB sort + top-M scratches. What is left in `compute/` belongs to other steps: weight-derived caches keyed by source pointer (T1, step 6), per-call result tensors (I2, step 5 — criterion 3 already reads zero because the serving path does not use them), one-shot init transients that allocate and free inside a prewarm, and test-only overloads. One buffer was migrated and **handed back**: `attention_cublas.cu`'s pointer array has no degradation contract, and a tenant whose caller cannot refuse does not belong in a tier that can run out | `fix(compute): DRY penalty buffers to the T2 arena` / `fix(compute): the cuBLAS and CUTLASS workspaces to the T2 arena` / `fix(compute): the grow-on-demand scratches to T2` / `fix(compute): the CUB sort and top-M scratches to T2` |
| 9a — `--mem-report` with named charges | **done** | `feat(memory): --mem-report — name the charges the pool notes cannot see` |
| **A7 step 9 complete** (9a + 9b.1–9b.5) | | criterion 5 is *not* claimed |

### I1 allowlist baseline

The I1 allowlist opens at **79 files / 717 sites**. That is larger
...
same order of magnitude as the library reserve the pass cannot see; the
incidents were cases of that headroom being mis-divided.
...
does not crash, it silently corrupts cross-request KV.
...
and `test_prefix_cache_equiv.cpp`.
...
2. **A decision on the rollback groups.** The
   `cutlass_act_data`/`_sf`/`_workspace` family and its siblings each
...
`QUENCH_ALLOC_INTERPOSE=ON` links `src/memory/alloc_interpose.cpp` and puts
`-Wl,--wrap=` on the executables, so quench's references to the CUDA allocation
...
2. **The bytes are irrelevant; the count is not.** 1.15 MiB is three orders of
   ...
   growth, not quench's allocations. Step 5's value is removing 414
...
Every number in A1 was measured on this host against `quench:test` built from
the working tree, including the fix to `vram_budget.cpp`/`vram_query.h` that
floors the mode-2 reserve at the allocator's 5 % headroom. Measurements taken
with the GPU otherwise idle (0 containers, no compute processes) and healthy
under load (2857–2932 MHz SM, 13801 MHz mem, 310–444 W). Harness: `MemAccount`
via `diagnostics.vram_audit`, driven by concurrent streaming completions.
