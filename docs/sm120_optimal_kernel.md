...
Canonical design reference for the quench hot-path attention kernel on RTX 5090
(GB202, **sm_120a** — consumer Blackwell). This is the spec future FA2 levers
must measure themselves against. It is grounded in profiling ground-truth
and empirical refutations measured on this chip, not in datacenter
...
Profiling ground-truth: FA2 is **tensor-pipe busiest at
...
- **PV: also f16-accumulate** (`mma.sync.m16n8k16`, default-on; the
...
(e4m3-QK PPL 5722 vs 6.12 — 3 mantissa bits × 36-layer compounding). **FP4
...
|-------|--------|----------|
| f16-acc QK^T + PV | **shipped** | `fa2_f16acc`, +3–4 % pp, +0.37 % PPL |
| Register-resident O | **shipped** | primary FA2 (`attention_fmha_sm120.cu`) |
| cp.async K/V double-buffer | **shipped** | −11.6 % kernel, long ctx |
| Smem row-stride padding | **shipped** | 1.54× kernel |
...
| FP4-QK inside attention | **REFUTED** | PPL 5722, format-intrinsic |

**Punchline:** the optimal sm_120 attention kernel and quench's primary FA2 have
...
> No open re-litigation. One might speculate that the Bq=128
> and deeper-pipeline refutations predate the register-resident-O envelope and
> are worth re-running. Reading the actual dispatcher (`fmha_sm120_fa2_prefill`)
...
> refuted levers were measured against this exact kernel family; they are
> closed, not pending.
...
double-buffer + SFU-overlapped softmax** — and that *is* quench's primary FA2. We
lose to datacenter Blackwell on architecture, not implementation; we win decode
(uncontested NVFP4) in exchange.
